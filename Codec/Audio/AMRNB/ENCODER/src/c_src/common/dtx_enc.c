...
*/ 
#if DISABLE_DTX
int dtx_enc_init (dtx_encState **st, VO_MEM_OPERATOR *pMemOP)
{
	return 0;
}
			...
			)
{
	...
	if ((computeSidFlag != 0))
	{
		...
		voAMRNBEnc_Lsp_lsf(lsp, lsf, M);
		voAMRNBEnc_Reorder_lsf(lsf, LSF_GAP, M);
		voAMRNBEnc_Lsf_lsp(lsf, lsp, M);

		/* Quantize lsp and put on parameter list */
		//voAMRNBEnc_Q_plsf_3(qSt, MRDTX, lsp, lsp_q, st->lsp_index, &st->init_lsf_vq_index);
...
)
{
	...
	voAMRNBEnc_Log2(L_frame_en, &log_en_e, &log_en_m);
					  ...
					  )
{
	...
	{  /* non-speech */
		...
		{ /* in possible analysis hangover */
			...
			if (sub(add(st->decAnaElapsedCount, st->dtxHangoverCount),DTX_ELAPSED_FRAMES_THRESH) < 0)
			{
				*usedMode = MRDTX;  
