...
WMARESULT cxReverbInitHistAndGain(CAudioObjectDecoder *paudec, CxReverbFilter *pcxrf)
{
...
WMARESULT cxReverbInit(CAudioObjectDecoder *paudec, CAudioObject *pau, CxReverbFilter *pcxrf)
{
    WMARESULT wmaResult = WMA_OK;
    ...
    TRACEWMA_EXIT(wmaResult, cxReverbInitHistAndGain(paudec, pcxrf));

exit:
    return wmaResult;
}

Void cxReverbFree(CAudioObjectDecoder *paudec, CxReverbFilter *pcxrf)
{
    if (NULL == pcxrf)
        return;
                      ...
                      Int iSize, Int iCh )
{
    ...
    for( iNumStages = CXREVERB_STAGES; iNumStages > 0; --iNumStages )
    {
        ...
        if( 1 == iStep )
        {
            voWMAMemcpy( pBufRe, pDataReal, sizeof(CoefType)*iNumFreqs );
            pDataReal += iNumFreqs;

#if (CX_DCT_ONLY==0)
            voWMAMemcpy( pBufIm, pDataImag, sizeof(CoefType)*iNumFreqs );
            pDataImag += iNumFreqs;
#endif
        }
    ...
    if( 1 == iStep )
    {
        voWMAMemcpy( pBufRe, pDataReal, sizeof(CoefType)*iSize );
        voWMAMemcpy( pBufIm, pDataImag, sizeof(CoefType)*iSize );
    }
...
//          const CoefType *pOutDelRe);
        ...
        if( iNumFreq - iNumFreqs )
        {
            ...
            voWMAMemcpy( pOutCurRe + iNumFreqs, pInCurRe + iNumFreqs, 
                sizeof(CoefType)*(pcxrf->m_iNumFreq - iNumFreqs));
            voWMAMemcpy( pOutCurIm + iNumFreqs, pInCurIm + iNumFreqs, 
                sizeof(CoefType)*(pcxrf->m_iNumFreq - iNumFreqs));
        }  
...
Void cxReverbFilter_SSE1( CxReverbFilter *pcxrf, Int iCh )
{
    ...
    for( j=0; j < 3; ++j )
    {
        ...
        if( iNumFreq - iNumFreqs )
        {
            ...
            voWMAMemcpy( pOutCurRe + iNumFreqs, pInCurRe + iNumFreqs, sizeof(Float)*(pcxrf->m_iNumFreq - iNumFreqs));
            voWMAMemcpy( pOutCurIm + iNumFreqs, pInCurIm + iNumFreqs, sizeof(Float)*(pcxrf->m_iNumFreq - iNumFreqs));
        }
                       ...
                       ChexFilterOutput eFilterTap)
{
    ...
    for( iNumStages = CXREVERB_STAGES; iNumStages > 0; --iNumStages )
    {
        ...
        if( 1 == iStep )
        {
            voWMAMemcpy( pDataReal, pBufRe, sizeof(CoefType)*iNumFreqs );
            pDataReal += iNumFreqs;

#if (CX_DCT_ONLY==0)
            voWMAMemcpy( pDataImag, pBufIm, sizeof(CoefType)*iNumFreqs );
            pDataImag += iNumFreqs;
#endif
        }
        else for (i=0; i < iNumFreqs; ++i )
    ...
    if( 1 == iStep )
    {
        voWMAMemcpy( pDataReal, pBufRe, sizeof(CoefType)*iSize );
        voWMAMemcpy( pDataImag, pBufIm, sizeof(CoefType)*iSize );
    }
    else for (i=0; i < pcxrf->m_iNumFreq; ++i )
    {
        totalReal += pBufRe[i];
