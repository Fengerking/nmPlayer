    ...
    DWORD dwType, dwVal, cbVal = sizeof(dwVal);
        if (ERROR_SUCCESS == RegQueryValueEx(hKey, TEXT("Count Cycles"), 0, &dwType, (BYTE*)&dwVal, &cbVal)) {
            if ((sizeof(dwVal) == cbVal) && (dwVal)) {
                voWMAMemcpy(&pccs->Format, pFormat, sizeof(pccs->Format));
...
WMARESULT prvDecDelete (CAudioObjectDecoder* paudec)
{
...
#ifdef DEBUG_LINUX
	LOGW("prvDecDelete paudec: 0x%x", (VO_U32)paudec);
#endif
    pau = paudec->pau;
    ...
    if (pau != NULL) 
        prvDeleteChannelGrpInfo(paudec, &(paudec->m_rgChannelGrpInfo), pau->m_cChannel);
    ...
    if (NULL !=pau)
    {
        auDeletePcInfoCommon (paudec, pau, paudec->m_rgpcinfo);
    ...
    if (paudec->m_pLtRtDownmix) {
        ...
        ltrtDownmixFree(paudec, paudec->m_pLtRtDownmix);
#endif
...
#ifdef DEBUG_LINUX
	LOGW("audecDelete paudec: 0x%x", (VO_U32)paudec);
#endif
    auMallocSetState(MAS_DELETE, NULL, 0);
    
//	wmaResult = prvDecDelete (paudec);
    TRACEWMA_EXIT(wmaResult, prvDecDelete (paudec));
//	printf("wmaResult is:  0x%x,\n", wmaResult);

		//Witten added
	DELETE_ARRAY (paudec->pWMAForm);
...
{
    WMARESULT wmaResult = WMA_OK;
    U8  cbWMAV_Version = paudec->m_cbWMAV_Version;
    VO_MEM_OPERATOR *voMemop = paudec->vopMemOP;
	VO_PTR hCheck = paudec->hCheck;
	WMAFormat *pWMAForm = paudec->pWMAForm;
    // Delete any allocated memory inside of paudec
    TRACEWMA_EXIT(wmaResult, prvDecDelete(paudec));
    
    // Wipe clean entire structure
	
	//modified on 2012.6.13, when fixing task#15567: some contents are played 3 times faster than normal, using SmartShare app.
	//Could not call memset here if any global variabes has been set 
	//This code is weird, because framework calls VO_PID_COMMON_HEADDATA twice
	if(!paudec->SupptMtChanl&&!paudec->Support24Bit&&!paudec->SupptHiSmplrt){
		  paudec = (CAudioObjectDecoder *)memset(paudec, 0, sizeof(CAudioObjectDecoder));
	}
  
    
	paudec->m_cbWMAV_Version = cbWMAV_Version;

    paudec->vopMemOP = voMemop;

	paudec->hCheck = hCheck;

	paudec->pWMAForm = pWMAForm;
...
//    assert(paudec->m_rgpcinfoDEC == NULL);
    ...
    for (i = 0; i < pau->m_cChannel; i++)
    {
        ...
        ppcinfo->m_rgsubfrmconfig = (SubFrameConfigInfo *)memset(ppcinfo->m_rgsubfrmconfig, 0, (sizeof(SubFrameConfigInfo)));
#define SIZEBUFD ((pau->m_iMaxSubFrameDiv + 2) * sizeof (I16) + sizeof (I16) + sizeof (U8))
        ...
        ppcinfo->m_rgsubfrmconfig->m_rgiSizeBuf = (I16 *)memset(ppcinfo->m_rgsubfrmconfig->m_rgiSizeBuf, 0, SIZEBUFD);
...
WMARESULT audecGetMoreData(void* dwUser, audecInputBufferInfo* pRet)
{
    ...
    voWMAMemcpy(pRet, pBuf, sizeof(*pRet));
...
#if defined(UNDER_CE) || defined(LINUX) || defined(_XBOX) || defined(WMA_DECPK_BUILD)
    Bool fLH          = WMAB_FALSE;
#else
                ...
                WMAPlayerInfo *pWmaPlayerInfo)
{
    ...
    TRACEWMA_EXIT(wmaResult, prvInitCommonSecondaryFixed(paudec, pau));

    // Allocate memory buffers, then initialize them
    TRACEWMA_EXIT(wmaResult, prvInitCommonAllocate(paudec, pau, WMAB_FALSE));
...
//	TRACEWMA_EXIT(wmaResult, audecReset(paudec));
...
WMARESULT audecChangeOutputFormat(void* pDecHandle, PCMFormat* pPCMFormat, WMA_Bool fTestOnly)
{
    ...
    if (WMAB_TRUE == paudec->pau->m_fUES)
    {
        ...
        {
            voWMAMemcpy(paudec->pau->m_pUES->m_pPCMFormat, pPCMFormat, sizeof(PCMFormat));
        }
    }
#endif
...
{
    struct CAudioObjectDecoder* paudec = (struct CAudioObjectDecoder*)pDecHandle;
...
//    iRVSeed = 1036024048;
    ...
    voWMAMemcpy(&DamagedWMAFormat, pWMAFormat, sizeof(WMAFormat));
    voWMAMemcpy(&DamagedPCMFormat, pPCMFormat, sizeof(PCMFormat));
...
//  pWMAFormat->nSamplesPerSec = 16000;
    ...
    default: //assert(!"Wrong input FormatTag"); 
...
//			audecReset(paudec);
   ...
   if (cSubband <= 0) {
    ...
    if (pPlayerInfo)
        pTemp = voWMAMemcpy(&paudec->m_wmapi, pPlayerInfo, sizeof(paudec->m_wmapi));
    else
        pTemp = memset(&paudec->m_wmapi, 0, sizeof(paudec->m_wmapi));
    ...
    TRACEWMA_EXIT(wmaResult, auInitDec (pau, paudec, iVersion, fUES, cSubband, pWMAFormat->nSamplesPerSec, 
    ...
    paudec->m_rgpcinfo = (PerChannelInfo *)memset (paudec->m_rgpcinfo, 0, sizeof (PerChannelInfo) * pau->m_cChannel);
    TRACEWMA_EXIT(wmaResult, prvAllocatePcInfoCommon(paudec, pau, paudec->m_rgpcinfo));
    ...
    if (paudec->m_fChannelFoldDown == WMAB_TRUE)
    {
        ...
        else // LtRt downmixing
        {
            ...
            WMAFormat2PCMFormat((void *)paudec, pWMAFormat, &inPCMFormat);
            ...
            TRACEWMA_EXIT(wmaResult,
                        ltrtDownmixInitBuf(paudec, paudec->m_pLtRtDownmix,
                                           cLtRtQuality,
...
//        pau->m_rgpcinfo [0].m_rgiHuffDecTbl = g_rgiHuffDecTbl16smOb;
        pau->m_rgpcinfo [0].m_rgiHuffDecTbl = g_rgiHuffDecTblRunLevel16smOb;
...
//        pau->m_rgpcinfo [0].m_rgiHuffDecTbl = g_rgiHuffDecTbl44smQb; 
        pau->m_rgpcinfo [0].m_rgiHuffDecTbl = g_rgihufftable_LevelRun44smQb; 
#if defined (WMA_OPT_VLC_DEC_ARM) && WMA_OPT_VLC_DEC_ARM
...
//        pau->m_rgpcinfo [0].m_rgiHuffDecTbl = g_rgiHuffDecTbl44smOb; 
        pau->m_rgpcinfo [0].m_rgiHuffDecTbl = g_rgihufftable_LevelRun44smOb; 
#if defined (WMA_OPT_VLC_DEC_ARM) && WMA_OPT_VLC_DEC_ARM
...
//        pau->m_rgpcinfo [0].m_rgiLevelEntry = gLevel44smOb;
    ...
    TRACEWMA_EXIT(wmaResult, 
        prvAllocChannelGrpInfo(paudec, (CChannelGroupInfo**)&(paudec->m_rgChannelGrpInfo), pau->m_cChannel));
    ...
    for (k = 0; k < paudec->pau->m_cChannel; k++) {
        ...
        paudec->m_rgChannelGrpInfo[k].m_rgfltMultiXInverse = (ChXFormType *)memset(paudec->m_rgChannelGrpInfo[k].m_rgfltMultiXInverse, 0, sizeof(ChXFormType) * pau->m_cChannel * pau->m_cChannel);
    }
#endif // BUILD_WMAPRO
...
//			paudec->m_fReQuantizeTo16 = WMAB_TRUE;
    ...
    paudec->m_rgpiRecon = (U8 **)memset(paudec->m_rgpiRecon, 0, pau->m_cChannel * sizeof(U8*));
...
Void auInitPcInfoDEC (CAudioObjectDecoder* paudec)
{
    ...
    paudec->m_rgpcinfoDEC = (PerChannelInfoDEC *)memset(paudec->m_rgpcinfoDEC, 0, sizeof(PerChannelInfoDEC) * pau->m_cChannel);
...
WMARESULT audecReset (void* pDecHandle)
{
    ...
    pau->m_rgiPCMInHistory = (I32 *)memset(pau->m_rgiPCMInHistory, 0, sizeof(*pau->m_rgiPCMInHistory) * pau->m_cChannel);
...
#ifdef CALLBACK_TEST
    voWMAMemcpy(&g_audecInputBuffer, &buf, sizeof(g_audecInputBuffer));
#else
...
WMARESULT prvNewInputBuffer(CAudioObjectDecoder* paudec, audecInputBufferInfo* pBuf)
{
    ...
    if (paudec->m_fSPDIF && !paudec->m_ibstrm.m_pfnGetMoreData && !paudec->m_fNeedHeader && fNewPacket)
    {
        ...
        hr = audecReset(paudec); // not sure this is enough
        paudec->m_fNeedHeader = WMAB_TRUE;
    }
...
//			printf("DecodeInfo()!\n");
            ...
            hr = audecReset (paudec);        //codecsts will be begin
			DEBUG_PRINTF(printf("audecReset, frame is error and audio decoder is reset!\n"));
...
//	printf("audecDecode initial is ok!\n");
    ...
    *pcSamplesReady = cSamplesReady;
    ...
    if ((hr == WMA_E_BROKEN_FRAME) ||(hr == WMA_E_FAIL))
    {
        audecLog((paudec, "...BROKEN_FRAME"));
        hr = audecReset (paudec);        //codecsts will be begin
		paudec->cbIn = 0;
		*paudecState = paudec->m_externalState;
		...
		DEBUG_PRINTF(printf("...BROKEN_FRAME!\n"));
    ...
    else if (hr == WMA_S_NO_MORE_FRAME)
    {
        ...
        paudec->m_fNeedHeader = WMAB_TRUE;
        pau->m_codecStatus = CODEC_HEADER;
		DEBUG_PRINTF(printf("...NO_MORE_FRAME!\n"));
...
exit:
    ...
    if (paudec &&
        ...
        paudec->m_externalState == audecStateDecode)
    {
		DEBUG_PRINTF(printf("INPUT_STATE!\n"));
        paudec->m_externalState = INPUT_STATE;
    }
    // UES mode
#ifdef BUILD_UES
...
{   
	...
	DEBUG_PRINTF(printf("pau->m_fAllowSuperFrame: 0x%x, \n", pau->m_fAllowSuperFrame));
	DEBUG_PRINTF(printf("pau->m_iVersion: 0x%x, \n", pau->m_iVersion));
...
NEXT_HDR:
        ...
        if (pau->m_iVersion <= 2)
        {
            ...
            paudec->m_cFrmInPacket = (U16) ((ibstrmGetPacketHeader(&paudec->m_ibstrm) 
                                     << cBitLs) >> cBitRs);
			DEBUG_PRINTF(printf("cBitRs: 0x%x, \n", cBitRs));
			DEBUG_PRINTF(printf("header buffer: 0x%x, \n", paudec->m_ibstrm.m_dwHeaderBuf));
			DEBUG_PRINTF(printf("frame in packet: 0x%x, \n", paudec->m_cFrmInPacket));
...
////            
    ...
    else 
    {
        ...
        if (iCurrPrevDiff != 0)
        {
            TRACEWMA_EXIT (wmaResult, ibstrmFlushOldBits(&paudec->m_ibstrm)); // used to be ibstrmResetPacket, but that caused problems for DRM
        }
...
//	printf("prvRotateTimestampQueue is ok!\n");
    if (paudec->m_fPacketLoss == WMAB_FALSE)
    {
        wmaResult = WMA_OK;
        goto exit;
        ...
        switch (paudec->m_decsts)
        {
            ...
            case DECODE_SUBFRAME :
                ...
                else 
                {
                    ...
                    {
						DEBUG_PRINTF(printf("WMA S NO_MORE_FRAME 1!\n"));
...
//                FFLUSH(stdout);
...
#endif //WMA_COMPRESSION_TOOL
                    {
                        hr = auSubframeRecon(paudec, pau);		// add window and overlap
                    }
                }
            ...
            case END_SUBFRAME2 :
                ...
                if (paudec->m_fLastSubFrame) 
                {
                    ...
                    Temp = prvShiftTimeIndex(pau);
#if 0
                #if !defined(UNDER_CE) && !defined(LINUX) //the statement may not be true for WinCE 
                    ...
                    if (paudec->m_cFrmInPacket <= 0)
                    {
						DEBUG_PRINTF(printf("WMA S NO_MORE_FRAME 2!\n"));
...
#if defined(BUILD_WMAPROLBRV2)
                TRACEWMA_EXIT(hr, reconProcessUnprocessedTiles(paudec, pau));
#endif
...
//                    assert ( pau->m_iPCMReconStart < pau->m_cFrameSampleHalfAdjusted / 2) {
                ...
                break;
            case FRAME_HDR:             
				DEBUG_PRINTF(printf("frame in packet: 0x%x, ", paudec->m_cFrmInPacket));
                if (paudec->m_cFrmInPacket == 0)
                {
					DEBUG_PRINTF(printf("WMA S NO_MORE_FRAME 3!\n"));
                    hr = WMA_S_NO_MORE_FRAME;
                    goto exit;
                }
#ifdef WMA_COMPRESSION_TOOL
                                 ...
                                 U8 *pbBuf, U16 *pcSamples, Int cbDstLength)
{
    ...
    if (shift != 0)
    {
        pbBuf = (U8 *)voWMAMemcpy(pbBuf, (U8*)PAUPRVADVANCEPCMPTR((PCMSAMPLE*)pbBuf, shift*nCh),
...
#if WMA_OPT_FOR_SPEED	//since other channels have no effective data, cannot do post-processing multi-channel transform
#else
                                  ...
                                  U8 *pbDst, U32 cbDstLength, I16 nDRCSetting)
{
    ...
    if (paudec->m_fChannelFoldDown == WMAB_TRUE)
    {
            ...
            WMAB_TRUE == paudec->m_fLtRtDownmixNonnormal)
        {
            ...
            {
				//U32 *temp = (U32 *)pbDst;
//				printf("prvChannelDownMix Source buffer: 0x%x, 0x%x, 0x%x, 0x%x\n", temp[0], temp[1], temp[2], temp[3]);
...
//				printf("prvChannelDownMix Dest buffer: 0x%x, 0x%x, 0x%x, 0x%x\n", temp[0], temp[1], temp[2], temp[3]);
            }
            else
            {
                TRACEWMA_EXIT(wmaResult, prvChannelDownMixFloat (paudec, (U8*) pbDst, cSamplesReturned, (U8*) pbDst));
...
//    prvDisableFPExceptions(&origFPExceptionMask);
    ...
    rgpiRecon = (U8 **)memset(rgpiRecon, 0, sizeof(U8*) * pau->m_cChannel);
    ...
    for (iCh = 0; iCh < pau->m_cChannel; iCh++)
    {
		...
		rgpiRecon [iCh] = (U8*) prvAdvancePCMPointer((PCMSAMPLE *)pbDst,
            paudec->m_nDstBytePerSample, iCh);
#else
        rgpiRecon [iCh] = (U8*) prvAdvancePCMPointer((PCMSAMPLE *)pbDst,
...
//                    TraceResult(wmaResult);
        ...
        if (paudec->m_bBrokenFrame)
        {
            ...
            pbDst = (WMA_U8 *)memset(pbDst, 0, cbDstLength);
        ...
        switch (paudec->m_subfrmdecsts)
        {
            ...
            case SUBFRM_REC_LOSSY:
                ...
                for (iCh = 0; iCh < pau->m_cChannel; iCh++) 
                {
                    ...
                    {
                        ppcinfo->m_rgiCoefRecon = (CBT *)memset (ppcinfo->m_rgiCoefRecon, 0, sizeof (CoefType) * AU_HALF_OR_DOUBLE(pau->m_fWMAProHalfTransform,pau->m_fPad2XTransform,ppcinfo->m_cSubband));
                    }
                }
                ...
                TRACEWMA_EXIT(hr, auInvChannelXForm (pau, WMAB_TRUE));
                // Apply bottom level hierarchical xform
                TRACEWMA_EXIT(hr, auInvChannelXForm (pau, WMAB_FALSE));
            ...
            {   
                for (iCh = 0; iCh < pau->m_cChannel; iCh++)
                {
                    ...
                    pTemp = memset (ppcinfo->m_rgiCoefRecon + pau->m_cHighCutOffAdjusted, 0, iHighToBeZeroed);
            ...
            if (WMAB_FALSE == fSkipAll)
            {
                for (iCh = 0; iCh < pau->m_cChannel; iCh++)
                {
                    ...
                    if (ppcinfo->m_iPower != 0 || pau->m_iVersion > 2) 
                    {
                        ...
                        {
                           pTemp = memset(ppcinfo->m_rgiCoefRecon + ppcinfo->m_cSubband, 0, 
                                    (ppcinfo->m_cSubbandAdjusted - ppcinfo->m_cSubband)*sizeof(CoefType));
                        }
                        hr = (*pau->aupfnDctIV) ((CoefType*) ppcinfo->m_rgiCoefRecon,
                ...
                if (pau->m_bUnifiedLLM == WMAB_FALSE)
                {
                    ...
                    for (iCh = 0; iCh < pau->m_cChInTile; iCh++)
                    {
                        ...
                        if (!ppcinfo->m_bNoDecodeForCx && NULL != ppcinfo->m_rgiCoefRecon )
                            ppcinfo->m_rgiCoefRecon = (CBT *)memset (ppcinfo->m_rgiCoefRecon, 0, sizeof (CoefType) * pau->m_cHighCutOffAdjusted);
#if defined (DEBUG_BIT_DISTRIBUTION)
...
//                        assert (paudec->pau->m_iCurrReconCoef <= (I16) ppcinfo->m_cSubband /2);
                    ...
                    {
                        pTemp = memset (pau->m_fWMAProHalfTransform ? pau->m_rgiCoefReconMLLMOrig : ppcinfo->m_rgiCoefRecon, 
                    ...
                    if (pau->m_fWMAProHalfTransform)
                    {
                        ...
                        ppcinfo->m_rgiCoefRecon = (CBT *)voWMAMemcpy(ppcinfo->m_rgiCoefRecon, pau->m_rgiCoefReconMLLMOrig, sizeof (CoefType) * ppcinfo->m_cSubbandAdjusted);
                    }
            ...
            case SUBFRM_REC_LOSSY:
                ...
                for (iCh = 0; iCh < pau->m_cChInTile; iCh++)
                {
                    ...
                    if (!ppcinfo->m_bNoDecodeForCx)
                    {
                        ...
                        {
                            ppcinfo->m_rgiCoefRecon = (CBT *)memset (ppcinfo->m_rgiCoefRecon, 0, sizeof (CoefType) * ppcinfo->m_cSubbandAdjusted);
                        }
                    }
                }
        #if defined(WMA_SHOW_FRAMES) && defined(_DEBUG)
                ...
                if (pau->m_iVersion > 2 && WMAB_FALSE == fSkipAll) {
                    ...
                    for (iCh = 0; iCh < pau->m_cChInTile; iCh++)
                    {
                        ...
                        if (iChSrc == pau->m_nSubWooferChannel &&
                            ...
                            pTemp = memset((Void*)(ppcinfo->m_rgiCoefRecon + pau->m_cSubWooferCutOffIndex),
                    ...
                    pau->m_bBasePeakPresentTile == WMAB_TRUE)
                {
                    for (iCh = 0; iCh < pau->m_cChInTile; iCh++)
                    {
                        ...
                        if( ppcinfo->m_rgsubfrmconfig[CONFIG_CURRFRM].m_rgfMaskUpdate[0] )
                            ppcinfo->m_rgiMaskQ = (Int *)voWMAMemcpy(ppcinfo->m_rgiMaskQ,
                                   ppcinfo->m_piBasePeakMaskQSave, 
                                   pau->m_cValidBarkBand * sizeof(Int));
                        else
                            ppcinfo->m_rgiMaskQResampled = (Int *)voWMAMemcpy(ppcinfo->m_rgiMaskQResampled,
                                   ppcinfo->m_piBasePeakMaskQSave, 
                                   pau->m_cValidBarkBand * sizeof(Int));
                    }
                }
 ...
 #if WMA_OPT_FOR_SPEED  //skip other channels
						if(iCh>=2){
							break;
						}
#endif
...
void DownmixTest(CAudioObjectDecoder *paudec)
{
    ...
    WMADownmixMatrixToCoeff(pMatrix, pCoeff, &numCoeff,
                                    6, 2, 0x3f, 0x3);
...
// !!!Changes rgfltInp!!!
Void prvDumbBubble(int *rgfltInp, int * rgiInMap, int cNum)
{
    Int i, iTmp;
    Bool fChanged;
//	Float fltTmp;
...
WMARESULT prvInitNonLFEChDnMixThroughProjections (
    ...
    I32UP          cDstChan,
    U32            nDstChannelMask,
    I32UP          rgrgfltChDnMixMtx[MAX_CHANNEL][MAX_CHANNEL])
{
    WMARESULT wmaResult = WMA_OK;
	...
	for(i=0;i<MAX_CHANNEL;i++)
	{
		...
		{
			rgrgfltChDnMixMtx[i][j] = (ChDnMixType)0;
		}
	}
	if(cSrcChan == 5 && nSrcChannelMask == 0x37 && cDstChan == 2 && nDstChannelMask == 3)
		for(i=0; i<2; i++)
			for(j=0; j<6; j++)
				rgrgfltChDnMixMtx[i][j] = rgrgfltChDnMixMtx6_2[i][j];
	else if(cSrcChan == 7 && nSrcChannelMask == 0x637 && cDstChan == 2 && nDstChannelMask == 3)
		for(i=0; i<2; i++)
			for(j=0; j<8; j++)
				rgrgfltChDnMixMtx[i][j] = rgrgfltChDnMixMtx8_2[i][j];
	else if(cSrcChan == 7 && nSrcChannelMask == 0x637 && cDstChan == 5 && nDstChannelMask == 0x37)
		for(i=0; i<6; i++)
			for(j=0; j<8; j++)
				rgrgfltChDnMixMtx[i][j] = rgrgfltChDnMixMtx8_6[i][j];
	else
	{
		// Speaker direction in degrees: all must be +ve, between 0 & 360.
		//    const Float rgfltSpeakerDirection[] =
		...
		//        -110.0F + 360.0F, // SPEAKER_SIDE_LEFT
		//        +110.0F  // SPEAKER_SIDE_RIGHT
		const I32UP rgfltSpeakerDirection[] =
		{
			-30 + 360, // SPEAKER_FRONT_LEFT
			30, // SPEAKER_FRONT_RIGHT
			0,   // SPEAKER_FRONT_CENTER 
			0,   // SPEAKER_LOW_FREQUENCY 
			-135 + 360, // SPEAKER_BACK_LEFT              
			+135, // SPEAKER_BACK_RIGHT
			-15 + 360,  // SPEAKER_FRONT_LEFT_OF_CENTER
			+15,  // SPEAKER_FRONT_RIGHT_OF_CENTER
			+180, // SPEAKER_BACK_CENTER
			-110 + 360, // SPEAKER_SIDE_LEFT
			+110  // SPEAKER_SIDE_RIGHT
			// Any top-speakers should be dealt with by outer systems.
			/*
			#define SPEAKER_TOP_CENTER              0x800
			#define SPEAKER_TOP_FRONT_LEFT          0x1000
		...
		//   Float fltPi;
		I32UP rgiInCMap[MAX_CHANNEL];
		I32UP rgiOutCMap[MAX_CHANNEL];
		I32UP iTmp0;
		U32   nMask;
	
		//    Float rgfltInPhi[MAX_CHANNEL];
		I32UP rgfltInPhi[MAX_CHANNEL];	//////////////////////////////
		...
		I32UP rgfltOutPhi[MAX_CHANNEL];	//////////////////////////////
		I32UP rgfltOutDel[MAX_CHANNEL];	//////////////////////////
		...
		{ 
			for(i = 0; i < cDstChan; i++)
			{
				for(j = 0; j < cSrcChan; j++)
				{
					//            rgrgfltChDnMixMtx[i][j] = 
					//                ((Float) (   (Int)     (1000.0F* rgrgfltChDnMixMtx[i][j]/fltBTmp + 0.5F) ) )/1000.0F;
					rgrgfltChDnMixMtx[i][j] = (I32UP)(((int64)rgrgfltChDnMixMtx[i][j] << 23)/ fltBTmp);
				}
			}
...
{
    WMARESULT wmaResult = WMA_OK;
    I32UP i, cSrcChannelTmp, cDstChannelTmp;
    I32UP iChSrc, iChDst;
    U32   nSrcChannelMaskTmp, nDstChannelMaskTmp;
    U32   nMask = 0x1;
//	Float rgrgfltChDnMixMtxTmp[MAX_CHANNEL][MAX_CHANNEL];
	I32UP rgrgfltChDnMixMtxTmp[MAX_CHANNEL][MAX_CHANNEL];
    I16   nSrcSubWooferChan = -1, nDstSubWooferChan = -1;
        ...
        (nDstSubWooferChan != -1))
    {
        ...
        for (iChDst = 0; iChDst < nDstSubWooferChan; iChDst++)
        {
            ...
            {
//                rgrgfltChDnMixMtx[iChDst][iChSrc] = CHDN_FROM_FLOAT(rgrgfltChDnMixMtxTmp[iChDst][iChSrc]);
                rgrgfltChDnMixMtx[iChDst][iChSrc] = rgrgfltChDnMixMtxTmp[iChDst][iChSrc];
            }
            for (iChSrc = nSrcSubWooferChan+1; iChSrc < cSrcChannel; iChSrc++)
            {
//                rgrgfltChDnMixMtx[iChDst][iChSrc] = CHDN_FROM_FLOAT(rgrgfltChDnMixMtxTmp[iChDst][iChSrc-1]);
                rgrgfltChDnMixMtx[iChDst][iChSrc] = rgrgfltChDnMixMtxTmp[iChDst][iChSrc-1];
            }
        }
        for (iChDst = nDstSubWooferChan+1; iChDst < cDstChannel; iChDst++)
        {
            ...
            {
//                rgrgfltChDnMixMtx[iChDst][iChSrc] = CHDN_FROM_FLOAT(rgrgfltChDnMixMtxTmp[iChDst-1][iChSrc]);
                rgrgfltChDnMixMtx[iChDst][iChSrc] = rgrgfltChDnMixMtxTmp[iChDst-1][iChSrc];
            }
            for (iChSrc = nSrcSubWooferChan+1; iChSrc < cSrcChannel; iChSrc++)
            {
//                rgrgfltChDnMixMtx[iChDst][iChSrc] = CHDN_FROM_FLOAT(rgrgfltChDnMixMtxTmp[iChDst-1][iChSrc-1]);
                rgrgfltChDnMixMtx[iChDst][iChSrc] = rgrgfltChDnMixMtxTmp[iChDst-1][iChSrc-1];
            }
        }
    }
    else if ((nSrcSubWooferChan == -1) &&
...
//                rgrgfltChDnMixMtx[iChDst][iChSrc] = CHDN_FROM_FLOAT(rgrgfltChDnMixMtxTmp[iChDst-1][iChSrc]);
        ...
        (nDstSubWooferChan == -1))
    {
        ...
        for (iChDst = 0; iChDst < cDstChannel; iChDst++)
        {
            ...
            {
//                rgrgfltChDnMixMtx[iChDst][iChSrc] = CHDN_FROM_FLOAT(rgrgfltChDnMixMtxTmp[iChDst][iChSrc] * cDstChannel/(cDstChannel + 0.2F));
                rgrgfltChDnMixMtx[iChDst][iChSrc] = (I32UP)(((long long)rgrgfltChDnMixMtxTmp[iChDst][iChSrc]) * cDstChannel*10/(cDstChannel*10 + 2));
            }
            iChSrc = nSrcSubWooferChan;
            rgrgfltChDnMixMtx[iChDst][iChSrc] = CHDN_FROM_FLOAT(0.2F/(cDstChannel + 0.2F));
            for (iChSrc = nSrcSubWooferChan+1; iChSrc < cSrcChannel; iChSrc++)
            {
//                rgrgfltChDnMixMtx[iChDst][iChSrc] = CHDN_FROM_FLOAT(rgrgfltChDnMixMtxTmp[iChDst][iChSrc-1] * cDstChannel/(cDstChannel + 0.2F));
                rgrgfltChDnMixMtx[iChDst][iChSrc] = (I32UP)(((long long)rgrgfltChDnMixMtxTmp[iChDst][iChSrc-1]) * cDstChannel*10/(cDstChannel*10 + 2));
            }
        }
    }
    else
                         ...
                         Int cDstChannel, U32 nDstChannelMask)
{
    ...
    paudec->m_rgrgfltChDnMixMtx = (ChDnMixType **)memset(paudec->m_rgrgfltChDnMixMtx, 0, sizeof(ChDnMixType*) * cDstChannel);
    ...
    for (iCh = 0; iCh < cDstChannel; iCh++)
    {
        ...
        paudec->m_rgrgfltChDnMixMtx[iCh] = (ChDnMixType *)memset(paudec->m_rgrgfltChDnMixMtx[iCh], 0, sizeof(ChDnMixType) * cSrcChannel);
    }
#if 0
                ...
                for (iChSrc = 0; iChSrc < cSrcChannel; iChSrc++) 
                {
                    ...
                    else
                    {
                        paudec->m_rgrgfltChDnMixMtx[iChDst][iChSrc] = 
                          CHDN_FROM_FLOAT(
                        (Float)voWMAPow2((int)(((double)iMixDownCoeff*3.321928)/(256.0 * 20.0))));
...
WMARESULT prvPrepareResampledMaskV3_Channel(CAudioObject* pau, PerChannelInfo* ppcinfo)
{
    ...
    if (ppcinfo->m_fAnchorMaskAvailable) {
        ...
        wmaResult = auResampleMaskV3 (pau, 
...
//INLINE Void *prvSetSampleCoefType(const CoefType cValue, 
//								  CoefType *pCurrentPos,
//                                  const Int iOffset)
//{
...
#undef SHIFT_BITS
        ...
        else
        {
            ...
            paudec->m_rgfltPostProcXformBlend = (ChPPXFormType *)voWMAMemcpy(paudec->m_rgfltPostProcXformBlend, 
                           ...
                           Int nDRCSetting)
{
    ...
    drcZero(pau->m_pDrc);
    TRACEWMA_EXIT(wmaResult, drcInit(paudec, pau->m_pDrc, NULL, &drcData, &drcOpts));
...
WMARESULT prvWriteMetaFileGetCommonFrmHdr (CAudioObjectDecoder * paudec)
{
    ...
    pau->m_cFrmUsedBitsFromParser = prvBitsMove (paudec, pau->m_rgbFrameBits, pau->m_cFrameBitsBufLen / 8 + 1, 
