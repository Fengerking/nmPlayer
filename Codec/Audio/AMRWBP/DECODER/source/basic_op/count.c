...
#define FRAME_LEN  (1.0f/(1000*FRAME_SIZE))   // OUTPUT in WMOPS
 ...
 * -- AlignedMalloc() memory area big enough to hold the string s
 * -- copy string into new area
 * -- return pointer to new area
 *
 * returns NULL if either s==NULL or AlignedMalloc() fails
 */
{
    char *dup;
    ...
    if ((dup = (char *) AlignedMalloc(strlen(s)+1)) == NULL)
        return NULL;
...
#if WMOPS
static Word32 WMOPS_frameStat(int on)
...
void Reset_WMOPS_counter (void)
{
    ...
    total_wmops[currCounter] += ((float) tot) * FRAME_LEN;
...
void WMOPS_output (Word16 dtx_mode, float fscale)
{
#if WMOPS
    Word16 i;
     ...
     */
    tot = WMOPS_frameStat(1);
    tot_wm = (Word32)(total_wmops[currCounter]* fscale + ((float) tot) * FRAME_LEN* fscale);

    AMRWBPLUS_FPRINTF_NEED (stdout, "%10s:WMOPS=%.3f",
	     objectName[currCounter]?objectName[currCounter]:"",
	     ((float) tot) * FRAME_LEN* fscale);

    if (nbframe[currCounter] != 0)
        AMRWBPLUS_FPRINTF_NEED (stdout, "  Average=%.3f",
                 tot_wm / (float) nbframe[currCounter]);
    
    AMRWBPLUS_FPRINTF_NEED (stdout, "  WorstCase=%.3f",
             ((float) glob_wc[currCounter]) * FRAME_LEN* fscale);
    ...
    if (dtx_mode == 0)
    {
        ...
        AMRWBPLUS_FPRINTF_NEED (stdout, "  WorstWC=%.3f", ((float) tot_wc) * FRAME_LEN * fscale);
    }
    AMRWBPLUS_FPRINTF_NEED (stdout, " (%d frames)\n", nbframe[currCounter]);
