...
CBasePlusObject* bpNew(CAudioObjectDecoder *paudec)
{
	WMARESULT wmaResult;
    ...
    pbp = (CBasePlusObject *)memset(pbp, 0, sizeof(CBasePlusObject));

    return pbp;
} // bpNew

Void bpDelete(CAudioObjectDecoder *paudec, 
			  CAudioObject*    pau,
              CBasePlusObject* pbp)
{
    ...
    if (pau != NULL) 
    {
        prvDeleteChannelGrpInfo(paudec, &(pbp->m_rgBPChannelGrpInfo), pau->m_cChannel);
    }
...
WMARESULT bpInit (CAudioObjectDecoder *paudec, 
				  CAudioObject*    pau,
                  CBasePlusObject* pbp)
{
    ...
    pbp->m_rgrgcValidSFBand = (Int**)memset(pbp->m_rgrgcValidSFBand, 0, sizeof(Int*) * NUM_SFBAND_TABLES);
    ...
    for (k = 0; k < NUM_SFBAND_TABLES; k++)
    {
        ...
        pbp->m_rgrgcValidSFBand[k] = (Int*)memset(pbp->m_rgrgcValidSFBand[k], 0, sizeof(Int*) * pau->m_cPossibleWinSize);
    }
    ...
    pbp->m_rgrgiSFBandIndexOrig = (I16**)memset(pbp->m_rgrgiSFBandIndexOrig, 0, sizeof(I16*) * NUM_SFBAND_TABLES);
    ...
    for (k = 0; k < NUM_SFBAND_TABLES; k++)
    {
        ...
        pbp->m_rgrgiSFBandIndexOrig[k] = (I16*)memset(pbp->m_rgrgiSFBandIndexOrig[k], 0, sizeof(I16) * (MAX_SF_BANDS_TBL[k] + 1));
    }
    ...
    pbp->m_rgiSFBandIndexPrev = (I16*)memset(pbp->m_rgiSFBandIndexPrev, 0, sizeof(I16) * (MAX_SF_BANDS + 1));
    ...
    pbp->m_rgiSFBandIndexCurr = (I16*)memset(pbp->m_rgiSFBandIndexCurr, 0, sizeof(I16) * (MAX_SF_BANDS + 1));
    ...
    pbp->m_rgiSFQ = (I32*)memset (pbp->m_rgiSFQ, 0, sizeof (I32) * MAX_SF_BANDS * pau->m_cChannel);
    ...
    pbp->m_rgiSFBandMarker = (I8*)memset (pbp->m_rgiSFBandMarker, 0, sizeof (I8) * MAX_SF_BANDS);
    ...
    pbp->m_rgiSFQResampled = (I32*)memset (pbp->m_rgiSFQResampled, 0, sizeof (I32) * MAX_SF_BANDS * pau->m_cChannel);
    ...
    pbp->m_rgiBPBadCoef = (I32*)memset (pbp->m_rgiBPBadCoef, 0, cShrBufLen);
    ...
    pbp->m_rgrgiBPBadCoef = (I32**)memset (pbp->m_rgrgiBPBadCoef, 0, sizeof(I32*) * pau->m_cChannel);
    ...
    pbp->m_rgiBadCoefSegPos = (I16*)memset (pbp->m_rgiBadCoefSegPos, 0, sizeof(I16) * BPPLUS_MAXBADCOEFSEG * 2 * pau->m_cChannel);
    ...
    pbp->m_rgiBPWeightFactor = (I32*)memset (pbp->m_rgiBPWeightFactor, 0, sizeof(I32) * BPPLUS_MAXBADCOEFSEG * pau->m_cChannel);
    ...
    TRACEWMA_EXIT(wmaResult, 
        prvAllocChannelGrpInfo(paudec, (CChannelGroupInfo**)&(pbp->m_rgBPChannelGrpInfo), pau->m_cChannel));
    ...
    for (k = 0; k < pau->m_cChannel; k++) 
    {
        ...
        pbp->m_rgBPChannelGrpInfo[k].m_rgfltMultiXInverse = (ChXFormType*)memset(pbp->m_rgBPChannelGrpInfo[k].m_rgfltMultiXInverse, 0, sizeof(ChXFormType) * pau->m_cChannel * pau->m_cChannel);
    }
    ...
    pbp->m_rgfBPVisited = (Bool*)memset(pbp->m_rgfBPVisited, 0, sizeof(Bool) * pau->m_cChannel);
...
WMARESULT prvBPAddBasePlustoBase(CAudioObject*      pau)
{
    ...
    for (iCh = 0; iCh < pau->m_cChInTile; iCh++) 
    {
        ...
        {
			pTemp = voWMAMemcpy(ppcinfo->m_rgCoefRecon + pbp->m_iExplicitStartPos, ppcinfo->m_rgBPCoef + pbp->m_iExplicitStartPos, 
...
Void prvBasePlusGetSFIndex(const CAudioObject *pau, I16 *piSFBandIndex, Int iWin)
{
    CBasePlusObject *pbp = pau->m_pbp;
    ...
    if (0 == iWin)
    {
        piSFBandIndex = (I16 *)voWMAMemcpy(piSFBandIndex, piSFBandIndexWin0, sizeof(I16)*(cValidSFBandWin0+1));
    }
    else
    { 
...
Int prvBasePlusIdentifyBadlyCodedCoef(CAudioObject* pau)
{
    ...
    for (iCh = 0; iCh < pau->m_cChInTile; iCh++)
    {
        ...
        ppcinfo->m_rgiBPBadCoef = (I32 *)memset(ppcinfo->m_rgiBPBadCoef, 0XFFFFFFFF, sizeof(I32) * ppcinfo->m_cSubFrameSampleHalf/32);
    }
    ...
    for (iChGrp = 0; iChGrp < cChannelGrp; iChGrp++)
    {
        ...
        for (iChSrc = 0; iChSrc < pau->m_cChannel; iChSrc++)
        {
            ...
            rgiBPBadCoef = (I32 *)voWMAMemcpy(rgiBPBadCoef, pau->m_pucNonzeroMask, ppcinfo->m_cSubFrameSampleHalf/8);
...
earlyexit:
    ...
    if (fTooManyHoles)
    {
        ...
        for (iCh = 0; iCh < pau->m_cChInTile; iCh++)
        {
            ...
            ppcinfo->m_rgiBPBadCoef = (I32 *)memset(ppcinfo->m_rgiBPBadCoef, 0, sizeof(I32) * ppcinfo->m_cSubFrameSampleHalf/32);
        }
        iTotalBadCoef = 0;
    }
    ...
    if (pau->m_bCx)
    {
        ...
        for (iCh = 1; iCh < pau->m_cChInTile; iCh++)
        {
            ...
            ppcinfo->m_rgiBPBadCoef = (I32 *)voWMAMemcpy(ppcinfo->m_rgiBPBadCoef, ppcinfoCh0->m_rgiBPBadCoef, sizeof(I32) * ppcinfo->m_cSubFrameSampleHalf/32);
            ppcinfo->m_rgiBadCoefSegPos = (I16 *)voWMAMemcpy(ppcinfo->m_rgiBadCoefSegPos, ppcinfoCh0->m_rgiBadCoefSegPos, sizeof(I16) * BPPLUS_MAXBADCOEFSEG * 2);
        }
    }
...
WMARESULT prvBasePlusCopyChannelXFormFromBaseCommon (CAudioObject *pau)
{
    ...
    for (iChGrp = 0; iChGrp <  pbp->m_cBPChannelGroup; iChGrp++)
    {
        ...
        pcgiDst->m_rgfChannelMask = (Bool *)voWMAMemcpy(pcgiDst->m_rgfChannelMask, pcgiSrc->m_rgfChannelMask, sizeof(Bool) * cChannel);
        pcgiDst->m_rgbRotationAngle = (I8 *)voWMAMemcpy(pcgiDst->m_rgbRotationAngle, pcgiSrc->m_rgbRotationAngle,
            sizeof(I8) * (cChannel * (cChannel -1)/2));
        pcgiDst->m_rgbRotationSign = (I8 *)voWMAMemcpy(pcgiDst->m_rgbRotationSign, pcgiSrc->m_rgbRotationSign,
            sizeof(I8) * cChannel);
        // Memory not allocated in CBasePlusObject
      //  voWMAMemcpy(pcgiDst->m_rgfltMultiXForward, pcgiSrc->m_rgfltMultiXForward, 
      //      sizeof(Float) *
      //              cChannel * cChannel);
        pcgiDst->m_rgfltMultiXInverse = (ChXFormType *)voWMAMemcpy(pcgiDst->m_rgfltMultiXInverse, pcgiSrc->m_rgfltMultiXInverse, sizeof(ChXFormType) * cChannel * cChannel);
    }
    return wmaResult;
