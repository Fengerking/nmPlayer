...
HX_RESULT rv_depack_get_codec_init_info(rv_depack* pDepack, rv_format_info** ppInfo)
{
    ...
    if (pDepack && ppInfo)
    {
            ...
            pInt->ulActiveSubStream < pInt->multiStreamHdr.ulNumSubStreams)
        {
            ...
            *ppInfo = (rv_format_info*)rv_depacki_malloc(pInt, sizeof(rv_format_info));
            if (*ppInfo)
            {
                /* NULL out the memory */
    ...
    if (ppDepack)
    {
        ...
        if (pInt && pInt->fpFree)
        {
            ...
            if (pInt->rule2Flag.pulMap)
            {
                ...
                pInt->rule2Flag.pulMap     = (long unsigned int*)HXNULL;
            ...
            if (pInt->multiStreamHdr.rule2SubStream.pulMap)
            {
                ...
                pInt->multiStreamHdr.rule2SubStream.pulMap     = (long unsigned int*)HXNULL;
            ...
            if (pInt->bIgnoreSubStream)
            {
                ...
                pInt->bIgnoreSubStream = (HXBOOL*)HXNULL;
            }
