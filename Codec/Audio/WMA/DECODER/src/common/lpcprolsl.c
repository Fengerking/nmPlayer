...
*   Output: Nothing.
************************************************/
WMARESULT prvInitLMSPredictor(CAudioObjectDecoder *paudec, LMSPredictor * pLMSPrdtr, U16 iMaxOrder, U8 * pMem, I32 * pcbMemUsed, I32 cbMemTtl)
{
    ...
    pLMSPrdtr->m_rgiFilter = (I16 *)memset(pLMSPrdtr->m_rgiFilter, 0, sizeof(pLMSPrdtr->m_rgiFilter[0]) * iMaxOrder);
    ...
    pLMSPrdtr->m_rgiFilterBackup = (I16 *)memset(pLMSPrdtr->m_rgiFilterBackup, 0, sizeof(pLMSPrdtr->m_rgiFilterBackup[0]) * iMaxOrder);
    ...
    pLMSPrdtr->m_rgiPrevVal = (I32 *)memset (pLMSPrdtr->m_rgiPrevVal, 0, sizeof(pLMSPrdtr->m_rgiPrevVal[0]) * iMaxOrder * 2);
    ...
    pLMSPrdtr->m_rgiUpdate = (I16 *)memset (pLMSPrdtr->m_rgiUpdate, 0, sizeof(pLMSPrdtr->m_rgiUpdate[0]) * iMaxOrder * 2);
    ...
    pLMSPrdtr->m_rgiUpdateBackup = (I16 *)memset (pLMSPrdtr->m_rgiUpdateBackup, 0, sizeof(pLMSPrdtr->m_rgiUpdateBackup[0]) * iMaxOrder * 2);
exit:
    return wmaResult;
}
...
Void prvFreeLMSPredictor(CAudioObjectDecoder *paudec, LMSPredictor * pLMSPrdtr)
{
/*
...
Void prvLMSPredictorUpdate_I16_C(CAudioObject * pau, LMSPredictor * pLMSPredictor, I32 iInput, I32 iPred)
{
    ...
    if (pLMSPredictor->m_iRecent == 0) {
		Void *pTemp;
        pTemp = voWMAMemcpy(rgiPrevVal + pLMSPredictor->m_iOrder, rgiPrevVal, sizeof(rgiPrevVal[0]) * pLMSPredictor->m_iOrder);
        pTemp = voWMAMemcpy(rgiUpdate + pLMSPredictor->m_iOrder, rgiUpdate, sizeof(rgiUpdate[0]) * pLMSPredictor->m_iOrder);
...
//  pLMSPrdtr->m_iOrder_Div_2 = (pLMSPrdtr->m_iOrder >> 1);
    ...
    pLMSPrdtr->m_rgiFilter = (I16 *)memset(pLMSPrdtr->m_rgiFilter, 0, sizeof(pLMSPrdtr->m_rgiFilter[0]) * pLMSPrdtr->m_iOrder);
    ...
    pLMSPrdtr->m_rgiPrevVal = (I32 *)memset(pLMSPrdtr->m_rgiPrevVal, 0, sizeof(pLMSPrdtr->m_rgiPrevVal[0]) * LLMB_CLMSFLT_MAX_ORDER * 2);
    pLMSPrdtr->m_rgiUpdate = (I16 *)memset(pLMSPrdtr->m_rgiUpdate, 0, sizeof(pLMSPrdtr->m_rgiUpdate[0]) * LLMB_CLMSFLT_MAX_ORDER * 2);
