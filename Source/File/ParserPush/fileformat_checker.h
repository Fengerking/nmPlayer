...
VO_BOOL SkipID3v2Data( interface_io * ptr_io )
{
	ID3v2Header header;
	if( -1 == ptr_io->read( (VO_PBYTE)&header , ID3V2_HEADER_LEN) )
		return VO_FALSE;
	...
	{
		ptr_io->set_pos( -1 * ID3V2_HEADER_LEN , VO_FILE_CURRENT );
		return VO_FALSE;
	}

	//calculate the size and skip it!!
	VO_U32 dwSize = CALCULATE_SIZE_SYNCHSAFE(header.btSize);
	ptr_io->set_pos( dwSize , VO_FILE_CURRENT );

	return VO_TRUE;
}

VO_S32 SkipID3v2Data(VO_PBYTE pBuf, VO_S32 iLen)
{
	//judge if it is ID3v2 header!!
	static const VO_CHAR byteID3[3] = {0x49, 0x44, 0x33};
	if(memcmp(pBuf, (VO_PTR)byteID3, 3))
		return 0;

	//calculate the size and skip it!!
	return CALCULATE_SIZE_SYNCHSAFE((pBuf + 6));
}
...
VO_BOOL get_rightlibrary( VO_PBYTE ptr_buffer , VO_S32 buffer_size , VO_TCHAR* ptr_modulename , VO_TCHAR* ptr_apiname )
{
	...
	else if ( IsTS(ptr_buffer, buffer_size) )
	{
		vostrcpy(ptr_modulename, _T("voTsParser"));
		vostrcpy(ptr_apiname, _T("voGetParserAPI"));
		VOLOGI("It is TS" );
	}
	else
	{
...
VO_BOOL IsTS (VO_PBYTE pHeadData, VO_S32 nHeadSize)
{
	...
	if (nHeadSize >= 195 && FindTSPacketHeader2(pHeadData, nHeadSize, 194))
		return VO_TRUE;
