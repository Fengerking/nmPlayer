	...
	************************************************************************/
/*******************************************************************************
	File:		CSourcePlayer.cpp

	Contains:	CSourcePlayer class file
...
#define LOG_TAG "CSourcePlayer"

#include "CSourcePlayer.h"
...
#define VOINFO(format, ...) do { \
	LOGI("[david] %s()->%d: " format, __FUNCTION__, __LINE__, __VA_ARGS__); } while(0);

typedef OMX_ERRORTYPE (* VOOMXGetEngineAPI) (VOOMX_ENGINEAPI * pEngine, OMX_U32 uFlag);

int CSourcePlayer::g_voAndroidStreamSource = 0;

CSourcePlayer::CSourcePlayer(void)
	: m_nStatus (-1)
	, m_nLoop (0)
	, m_bUninit (false)
	, m_nVideoWidth (0)
...
int CSourcePlayer::SetCallBack (VOACallBack pCallBack, void * pUserData)
{
	  JNILOGE("BEFORE SET CALLBACK");
	m_pCallBack = pCallBack;
	m_pUserData = pUserData;
	return 0;
}

int CSourcePlayer::SetFilePipe (void * pFilePipe)
{
	m_pFilePipe = (OMX_VO_FILE_OPERATOR *)pFilePipe;
	return 0;
}

int CSourcePlayer::Init(const char* path)
{
	 JNILOGI("init");
...
/*
	...
	if (m_hModule == NULL) 
		{
			 JNILOGE("m_hModule is NULL");
		return -1;
	}
	*/

     JNILOGI("BEFORE INIT");
	//tmp m_fAPI.Init (&m_hPlay, 0, vomeCallBack, this);
	JNILOGE("before vomeCallBack");
	   OMX_ERRORTYPE ret=m_fAPI.Init (vomeCallBack, this);
	     	JNILOGE("after vomeCallBack");
	     JNILOGI("after INIT");
	     /*
	if (m_hPlay == NULL)
		return -1;
*/
	JNILOGI2("working path: %s", path);
	/*
...
m_fAPI.SetParam ( VOME_PID_WorkingPath, (void*)path);
	m_fAPI.SetParam ( OMX_VO_IndexSourceDrm, &m_drmCB);
	m_fAPI.SetParam ( OMX_VO_IndexLibraryOperator, &m_libOP);
...
JNILOGI("BEFORE audio render");
	...
	m_fAPI.SetParam ( VOME_PID_AudioRenderBuffer, &audioRender);

JNILOGI("BEFORE video render");
	OMX_VO_CHECKRENDERBUFFERTYPE	videoRender;
	videoRender.pUserData = this;
	videoRender.pCallBack = vomeVideoRenderBufferProc;
	JNILOGI2("vomeVideoRenderBufferProc: %p", vomeVideoRenderBufferProc);
//	m_fAPI.SetParam (m_hPlay, VOME_PID_VideoRenderBuffer, &videoRender);
	m_fAPI.SetParam ( VOME_PID_VideoRenderBuffer, &videoRender);
	
	JNILOGE("finished Init");

	return 0;
}

int CSourcePlayer::SetSource (const char* pSource)
{
...
int CSourcePlayer::SetSource (int fd, int64_t offset, int64_t length)
{
...
OMX_U32 CSourcePlayer::vomeLoadSourceProc (OMX_PTR pParam)
{
	CSourcePlayer * pPlayer = (CSourcePlayer *)pParam;
 	
 	JNILOGI("vomeLoadSourceProc ");
	return pPlayer->vomeLoadSource ();
}

OMX_U32 CSourcePlayer::vomeLoadSource (void)
{
  JNILOGI("vomeLoadSource v2");
      
	int nRC = BuildGraph ();

  JNILOGD2("after build graph %d", nRC);
...
OMX_U32 CSourcePlayer::vomeWaitLoadSource (int nWaitTime)
{
	if (nWaitTime <= 0)
		nWaitTime = 10000;
...
int CSourcePlayer::SetLoop (int nLoop)
{
	m_nLoop = nLoop;
...
int CSourcePlayer::PrepareAsync (void)
{
	JNILOGI("before WaitLoadSource");
	vomeWaitLoadSource (30000);
...
void CSourcePlayer::NotifyOpenSource (void)
{
...
int CSourcePlayer::BuildGraph (void)
{
   JNILOGE2 ("CSourcePlayer BuildGraph  %d %d %d %p", m_nStatus, m_hPlay, m_nFile, m_hFile); 
	if (m_nStatus >= 0)
		return 0;
...
/*
	...
	if (m_nFile == -1 &&  m_hFile == NULL && strlen (m_szFile) <= 0)
		return -1;
 */
 JNILOGI("After buildgraph check");
...
nRC = m_fAPI.GetParam ( VOME_PID_AudioFormat, &audioFormat);
	if (nRC == OMX_ErrorNone)
	{
...
int CSourcePlayer::Run (void)
{
       JNILOGE ("CSourcePlayer Run v2"); 
       /*
   	if (m_hPlay == NULL)
       	return -1;
*/
	if (m_pCallBack != NULL)
        ...
        JNILOGE ("CSourcePlayer Run after callback"); 
...
JNILOGE("before vome run");
   if (m_fAPI.Run ()== OMX_ErrorNone)
	{
		JNILOGE(" vome run no error");
		if (m_nStartPos > 0)
		{
			if (m_nSourceType > 0)
			{
...
bool CSourcePlayer::IsPlaying (void)
{
    if (m_nStatus == 1)
    	return true;
    else
...
int CSourcePlayer::SetPos (int nPos)
{
	if (m_nDuration == 0)
		return -1;
...
int CSourcePlayer::GetPos (int * pPos)
{
//	VOLOGI ("******   GetPos");
...
int CSourcePlayer::GetDuration (int * pPos)
{
	/*
    if (m_hPlay == NULL)
    	return -1;
...
int CSourcePlayer::CallBackFunc (int nID, void * pParam1, void * pParam2)
{
...
int CSourcePlayer::RenderVideo (OMX_VO_VIDEOBUFFERTYPE * pVideoBuffer)
{
...
OMX_S32 CSourcePlayer::vomeCallBack (OMX_PTR pUserData, OMX_S32 nID, OMX_PTR pParam1, OMX_PTR pParam2)
{
	return ((CSourcePlayer*) pUserData)->CallBackFunc (nID, pParam1, pParam2);
}

OMX_ERRORTYPE CSourcePlayer::vomeVideoRenderBufferProc (OMX_HANDLETYPE hComponent,
										   OMX_PTR pAppData,
										   ...
										   OMX_IN OMX_PTR pFormat)
{
	  ...
	  JNILOGD("vomeVideoRenderBufferProc");
//	JNILOGW("@@@@@@  CSourcePlayer vomeVideoRenderBufferProc 000");

	if (nType != (OMX_INDEXTYPE)OMX_VO_IndexVideoBufferType)
		return OMX_ErrorNotImplemented;
     
	return (OMX_ERRORTYPE) ((CSourcePlayer *)pAppData)->RenderVideo ((OMX_VO_VIDEOBUFFERTYPE *)pFormat);
	//return OMX_ErrorNone;
}

OMX_ERRORTYPE CSourcePlayer::vomeAudioRenderBufferProc (OMX_HANDLETYPE hComponent,
													   OMX_PTR pAppData,
													   OMX_BUFFERHEADERTYPE* pBuffer,
													   OMX_IN OMX_INDEXTYPE nType,
													   OMX_IN OMX_PTR pFormat)
{
//	JNILOGW("@@@@@@  CSourcePlayer vomeAudioRenderBufferProc \n");
  JNILOGD("vomeAudioRenderBufferProc");

	CSourcePlayer *					pVOME = (CSourcePlayer *)pAppData;
...
int CSourcePlayer::LoadModule (void)
{
	/*
#ifdef LINUX
	char libfile[256];
...
OMX_PTR CSourcePlayer::OMX_voLoadLib (OMX_PTR	pUserData, OMX_STRING pLibName, OMX_S32 nFlag)
{
	OMX_PTR	hModule = NULL;
	char	szDll[256];

#ifdef LINUX
	CSourcePlayer *pm = (CSourcePlayer*)pUserData;
	if (pm) {
...
OMX_PTR CSourcePlayer::OMX_voGetAddress (OMX_PTR	pUserData, OMX_PTR hLib, OMX_STRING pFuncName, OMX_S32 nFlag)
{
	if (hLib == NULL)
		return NULL;
...
OMX_S32 CSourcePlayer::OMX_voFreeLib (OMX_PTR	pUserData, OMX_PTR hLib, OMX_S32 nFlag)
{
	if (hLib == NULL)
		return -1;
...
OMX_U32 CSourcePlayer::OMX_voSourceDrmCallBack (OMX_PTR pUserData, OMX_U32 nFlag, OMX_PTR pParam, OMX_U32 nReserved)
{
