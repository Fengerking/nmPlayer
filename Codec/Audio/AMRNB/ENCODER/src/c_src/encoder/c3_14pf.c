...
Word16 voAMRNBEnc_code_3i40_14bits(
						...
						)
{
	...
	Word32 i, index, sharp;
	sharp = shl2(pitch_sharp, 1);
	if (sub3(T0, L_CODE) < 0)
	{
		for (i = T0; i < L_CODE; i++) {
			h[i] = add3(h[i], mult3(h[i - T0], sharp));
		}
	}
	voAMRNBEnccor_h_x(h, x, dn, 1);
	voAMRNBEnc_set_sign(dn, dn_sign, dn2, 6);
	cor_h(h, dn_sign, rr);
#if !SAFE_CODE_search_3i40
	search_3i40(dn, dn2, rr, codvec);  /* function result */
	index = build_code_14pf(codvec, dn_sign, code, h, y, sign);
#else
	vo_search_3i40(dn, dn2, rr, codvec);
	move16 (); /* function result */
	index = vo_build_code_14pf(codvec, dn_sign, code, h, y, sign);
#endif
	...
	*-----------------------------------------------------------------*/
	...
	{
		for (i = T0; i < L_CODE; i++) { 
			code[i] = add3(code[i], mult3(code[i - T0], sharp));
		}
	}
	return index;
}
...
)
{
	...
	for (k = 0; k < NB_PULSE; k++)
	{
		...
		/* track = pos%5 */
		track = sub(i, extract_l(L_shr2(L_mult(index, 5), 1)));
		...
		{
			track = 1; 
			index = add(shl2(index, 4), 8);
		}
		else if ((track == 4))
		{
			track = 2; 
			index = add(shl2(index, 8), 128);
		}
