...
)
{
	...
	if(T0 < L_SUBFR)
		for (i = T0; i < L_SUBFR; i++)    /* code[i] += pitch_sharp*code[i-T0] */
			code[i] = (code[i] + ((code[i-T0] * sharp)>>15));
...
)
{
  ...
  Word32 i, cor;
  Word16 k, ldec, l_fin_sup, l_fin_inf;
  Word16 h[L_SUBFR];
	...
	for (track=3, trk=0; track<5; track++, trk++)
	{
		...
		for (i=0; i<2; i++)
		{
			...
			for (i1=track; i1<L_SUBFR; i1+=STEP)
			{
				...
				sq2 = (ps2 * ps2)>>15;
				alp_16 = (Word16)((alp2 + 0x8000)>>16);

				//sq2 = mult(ps2, ps2);
				//alp_16 = round(alp2);

				s = ((alp * sq2)<<1) - ((sq * alp_16)<<1);
		...
		p0 = rri1i2 + ((i0 * 6554)>>15);
		p1 = ptr_rri1i3_i4 + ((i1 * 6554)>>15);
		...
		for (i3=1; i3<L_SUBFR; i3+=STEP)
		{
			...
			*p3++ = (Word16)((s + 0x8000)>>16);
		}

		/* i2 loop: 8 positions in track 0 */
		p0 = rri0i2 + ((i0 * 6554)>>15);
		p1 = ptr_rri0i3_i4 + ((i1 * 6554)>>15);
