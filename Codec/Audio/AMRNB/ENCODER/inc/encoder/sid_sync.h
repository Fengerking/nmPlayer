...
*****************************************************************************
*/
int voAMRNBEnc_sid_sync_init (sid_syncState **st, VO_MEM_OPERATOR *pMemOP);
...
/* reset of sid_sync module (i.e. set state memory to zero)
   returns 0 on success
 */
void voAMRNBEnc_sid_sync_exit (sid_syncState **st, VO_MEM_OPERATOR *pMemOP);
/* de-initialize sid_sync module (i.e. free status struct)
   stores NULL in *st
 */

int voAMRNBEnc_sid_sync_set_handover_debt (sid_syncState *st, /* i/o: sid_sync state  */
                                Word16 debtFrames);
...
void voAMRNBEnc_sid_sync(sid_syncState *st , /* i/o: sid_sync state      */
              enum Mode mode,
