...
bool CWaveOutAudio::SetInputFormat(int nFormat)
{
	if(nFormat == INPUT_AAC)
		m_nInputFormat = kAudioFormatMPEG4AAC;
	else if(nFormat == INPUT_MP3)
	{
...
//		return m_nPlayingTime;
	...
	if (m_dwSysTime == 0)
		m_dwSysTime = voOS_GetSysTime ();

	unsigned long dwPlayTime = m_nPlayingTime + (voOS_GetSysTime () - m_dwSysTime);
	return dwPlayTime;
}
