			   ...
			   Word16 *cor_hp_max) /* o   : max high-pass filtered norm. correlation */
{
	...
	{
		/* high-pass filtering */
		temp = L_shl2(corr[-i], 1);
		temp1 = L_sub(temp, corr[-i-1]);
		t0 = L_sub (temp1, corr[-i+1]);   
		t0 = L_abs (t0);
	...
	/* high-pass filtering */
	t0 = L_sub(L_shl2(t0, 1), L_shl2(t1, 1));
	t0 = L_abs (t0);
	...
	{
		*cor_hp_max = shl(cor_max, negate(shift)); 
	}
	return 0;
}
