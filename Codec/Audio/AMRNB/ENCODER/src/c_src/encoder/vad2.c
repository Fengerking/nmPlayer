...
static Word16 fn10Log10 (Word32 L_Input, Word16 fbits)
{
	...
	Word32 Ltmp;
	Word16 tmp;
	voAMRNBEnc_Log2(L_Input, &integer, &fraction);
...
static Word16 block_norm (Word16 * in, Word16 * out, Word16 length, Word16 headroom)
{
...
Word16 vad2 (Word16 * farray_ptr, vadState2 * st)
{
	...
	*/
	...
	//r_fft(data_buffer); //compiler for G1, so delete
	...
	else
	{
		...
		for (i=LO_CHAN; i<=HI_CHAN; i++) {
			...
			Ltmp1 = L_add(Ltmp1, voAMRNBEnc_Pow2(hi1, lo1));
		}
		...
		if (sub(xt, st->tsnr) > 0)
		{
			...
			st->tsnr = vo_round(L_add(L_mult(29491, st->tsnr), L_mult(3277, xt)));
		}
		/* else if (xt > 0.625*tsnr) */	
		else if (sub(xt, mult(20480, st->tsnr)) > 0)
		{
			/* tsnr = 0.998*tsnr + 0.002*xt; */
			st->tsnr = vo_round(L_add(L_mult(32702, st->tsnr), L_mult(66, xt)));
		}
	}
	...
	if (xt < 0)
	{
		...
		tmp = vo_round(L_shl(L_mult(xt, xt), 7));
		st->negSNRvar = vo_round(L_add(L_mult(32440, st->negSNRvar), L_mult(328, tmp)));
	...
	*/
	...
	for (i = LO_CHAN; i <= HI_CHAN; i++)
	{
		...
		st->ch_enrg_long_db[i] = vo_round(L_add(Ltmp1, Ltmp2));
	}
...
**************************************************************************
*/
int vad2_init (vadState2 **state, VO_MEM_OPERATOR *pMemOP)
{
	vadState2* s;
	...
	*state = NULL;
	/* allocate memory */
	if ((s = (vadState2 *)voAMRNBEnc_mem_malloc(pMemOP, sizeof(vadState2), 32)) == NULL){
		return -1;
	}
	vad2_reset(s);
	*state = s;
...
**************************************************************************
*/
void vad2_exit (vadState2 **state, VO_MEM_OPERATOR *pMemOP)
{
