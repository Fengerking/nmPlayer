    ...
    CWMVMBMode* pmbmd, I32_WMV imbX, I32_WMV imbY)
{
    ...
    //I32_WMV iPredCBP = 0;
    I32_WMV iBlk;
    CWMVMBMode* pmbmd_lastrow =  pmbmd-pWMVDec->m_uintNumMBX;
...
I32_WMV DecodeIntraBlockInterlaceV2 (tWMVDecInternalMember * pWMVDec,
                                                    I32_WMV imbY, I32_WMV imbX, 
                                                    I32_WMV iBlk, CWMVMBMode *pmbmd, 
                                                    ...
                                                    Bool_WMV bAcPredOn)
{
    ...
    // AC/DC Prediction 
    decodeDCACPredInterlaceV2 (pWMVDec, imbY, imbX, iBlk, pmbmd, &iPredDir, rgiPredCoef, bAcPredOn);
    ...
    if (pmbmd->m_rgbCodedBlockPattern2 [iBlk]) {  //if has DCT coeff
        result = DecodeInverseIntraBlockQuantizeInterlaceV2 (pWMVDec, ppDCTTableInfo_Dec, piZigzagInv, rgiCoefRecon, 64);
    }
    if (ICERR_OK != result)
        return result;
    ...
    if (bAcPredOn) { // row
        PerformACPredictionInterlaceV2 (pWMVDec,
                                                   iBlk, pmbmd, 
                                                   rgiCoefRecon, rgiPredCoef, iPredDir);
    } 

    StoreDCACPredCoefInterlaceV2 (pWMVDec, imbY, imbX, iBlk, rgiCoefRecon); 

     if(!pmbmd->m_rgbCodedBlockPattern2 [iBlk] && !bAcPredOn )
        rgiCoefRecon[0] = (I16_WMV)((rgiCoefRecon[0] * pDQ->iDCStepSize));
    else
                                            ...
                                            I16_WMV *ppxliErrorU, I16_WMV *ppxliErrorV)
{
    ...
    for (iBlk = 0; iBlk < 4; iBlk++) {
        result = DecodeIntraBlockInterlaceV2 (pWMVDec,
                                                            imbY, imbX, iBlk, pmbmd,
                                                            pWMVDec->m_ppIntraDCTTableInfo_Dec, 
                                                            pDQ, bAcPredOn);
        if (ICERR_OK != result)
            return ICERR_ERROR;
    }
    ...
    for (iBlk = 4; iBlk < 6; iBlk++) {
        DecodeIntraBlockInterlaceV2 (pWMVDec,
                                                imbY, imbX, iBlk, pmbmd, 
    ...
    if ((pWMVDec->m_iOverlap & 1) && pmbmd->m_bOverlapIMB)  //  m_bOverlapIMB ×´Ì¬³ö´í  321
	{
        InvOverlapInterlaceV2 (pWMVDec,
                                        imbY, imbX, pmbmd, 
                                        ppxliTextureQMBY, ppxliTextureQMBU, ppxliTextureQMBV,
                                        ppxliErrorY, ppxliErrorU, ppxliErrorV, 
                                        pWMVDec->m_ppxliFieldMB, pWMVDec->m_ppxliIntraRowY,
                                        pWMVDec->m_ppxliIntraRowU, pWMVDec->m_ppxliIntraRowV);
    } 
    else
    {
        if (pmbmd->m_chFieldDctMode == FALSE) {
            CopyIntraFieldMBtoFrameMBInterlaceV2 ( pWMVDec->m_ppxliFieldMB,
                                                                ppxliTextureQMBY, ppxliTextureQMBU, ppxliTextureQMBV,
                                                                pWMVDec->m_iWidthPrevY, pWMVDec->m_iWidthPrevUV);
        } else {
            CopyIntraFieldMBtoFieldMBInterlaceV2 (pWMVDec->m_ppxliFieldMB, 
                                                                ppxliTextureQMBY, ppxliTextureQMBU, ppxliTextureQMBV, 
                                                                pWMVDec->m_iWidthPrevY, pWMVDec->m_iWidthPrevUV);
        }
    }
...
Void_WMV ScalePredForDQuantInterlaceV2 (tWMVDecInternalMember * pWMVDec,
                                                        I32_WMV noScaleFlag, I16_WMV *pPred, 
                                                        I16_WMV *pPredScaled, CWMVMBMode* pmbmd,U32_WMV stepsize)
{
    int i;
    if(noScaleFlag)
    {
    ...
    {
        I32_WMV iTmp;
        iTmp = pPred [0] * pWMVDec->m_prgDQuantParam[(pmbmd - stepsize)->m_iQP].iDCStepSize;
        pPredScaled [0] = divroundnearest (iTmp, pWMVDec->m_prgDQuantParam[pmbmd->m_iQP].iDCStepSize);
        for (i = 1; i < 8; i++) {
            iTmp = pPred [i] * (pmbmd - stepsize)->m_iQP;
    ...
    I16_WMV*pPredIntraL, I16_WMV* pPredIntraT)
{
    ...
    if (!bIsTopBlkBoundary && !bIsLeftBlkBoundary) {
     ...
     if (bIsLeftBlockI && bIsTopBlockI) {              
        ...
        PredDir = ScaleDCPredForDQuantInterlaceV2 (pWMVDec, iBlk, pmbmd, iTopDC, iLeftDC, iTopLeftDC);         
    } 
...
{
    I32_WMV iWidth, iIndex,stepsize, noScaleFlag =0;
    I16_WMV *pIntra, *pIntra_lastrow,*pPredIntraL, *pPredIntraT,*pPredIntra;
    ...
    *iPredDir = NONE;
    ...
    if (iBlk < 4)
    {
        ...
        iIndex = (1 * 2 + (iBlk >> 1)) * iWidth + (imbX * 2 + (iBlk & 1));
        pIntra = pWMVDec->m_pX9dct + iIndex * 16;

        if((imbY&1) == 0)
        {
            pIntra = pWMVDec->m_pX9dct + iIndex * 16;
            pIntra_lastrow = pIntra - iWidth * 16;
        }
        else
        {
             if(iBlk == 0 || iBlk ==1)
                pIntra_lastrow = pIntra + 16*iWidth ;
            else
                pIntra_lastrow = pIntra - 3*16*iWidth;

             pIntra = pIntra - 2*16*iWidth;
        }
    } 
    else
    {
        ...
        if (iBlk == 4)
        {
           /* pIntra = pWMVDec->m_pX9dctU + iIndex * 16;*/
            ...
            {
                pIntra_lastrow = pWMVDec->m_pX9dctU + iIndex * 16;
                pIntra =  pIntra_lastrow-16*iWidth;
            }
        }
        else
        {
            ...
            {
                pIntra_lastrow = pWMVDec->m_pX9dctV + iIndex * 16;
                pIntra =  pIntra_lastrow-16*iWidth;
            }
        }
    }
    ...
    pPredIntraT = pIntra_lastrow;//pIntra - iWidth * 16;
    *iPredDir = SetIBlockBoundaryForDCPred (pWMVDec, imbY, imbX, iBlk,
                                                        pmbmd, pPredIntraL, pPredIntraT);   
    if(*iPredDir == TOP)
    {
        pPredIntra =  pPredIntraT;
        stepsize =  pWMVDec->m_uintNumMBX;
        if(iBlk == 2 || iBlk == 3)
            noScaleFlag =  1;
    }
    else //Left
    ...
    {
        if(bAcPredOn)
            ScalePredForDQuantInterlaceV2 (pWMVDec, noScaleFlag, pPredIntra, rgiPredCoef, pmbmd,stepsize);
        else
        {
            if(noScaleFlag)
                rgiPredCoef [0] = pPredIntra [0];
            else
            {
                I32_WMV iTmp = pPredIntra [0] * pWMVDec->m_prgDQuantParam[(pmbmd - stepsize)->m_iQP].iDCStepSize;
                rgiPredCoef [0] = divroundnearest (iTmp, pWMVDec->m_prgDQuantParam[pmbmd->m_iQP].iDCStepSize);
            }
        }
    }
    else
    ...
    I32_WMV imbY, I32_WMV imbX, I32_WMV iBlk, I16_WMV *piLevelBlk)
{
    ...
    if (iBlk < 4) {
        ...
        I32_WMV iIndex = (1 * 2 + (iBlk >> 1)) * iWidth + (imbX * 2 + (iBlk & 1));
        pIntra = pWMVDec->m_pX9dct + iIndex * 16;
        if((imbY&1) == 0)
        {
            pIntra = pWMVDec->m_pX9dct + iIndex * 16;
        ...
        {
            pIntra = pWMVDec->m_pX9dctU + iIndex * 16-16*iWidth;
        }
    } else {
        I32_WMV iWidth = pWMVDec->m_uintNumMBX;
        I32_WMV iIndex = 1 * iWidth + imbX;
        ...
        {
            pIntra = pWMVDec->m_pX9dctV + iIndex * 16-16*iWidth;
        }
    }
...
I32_WMV decodeVOPHeadInterlaceV2 (tWMVDecInternalMember * pWMVDec)    
{
    ...
    {//I32_WMV iTime =
         BS_getBits( pWMVDec->m_pbitstrmIn,  8);  //zou --->Value = vc1DECBIT_GetBits(pBitstream, VC1_BITS_TFCNTR);
    }
   ...
   // g_iStepsize = pWMVDec->m_iQPIndex * 2 + pWMVDec->m_bHalfStep;
    ...
    {   
       ...
       // memset (pWMVDec->m_rgchSkipPrevFrame, 0, (size_t) pWMVDec->m_uintNumMB);
        ...
        huffindex = BS_getBits(pWMVDec->m_pbitstrmIn, 2);
        if(huffindex >7)
            return ICERR_ERROR;
        pWMVDec->m_pHufMVTable_V9 = pWMVDec->m_pHufMVTables[huffindex];
        huffindex = BS_getBits(pWMVDec->m_pbitstrmIn,3);
        if(huffindex >7)
            return ICERR_ERROR;
        pWMVDec->m_pHufPCBPCYDec = pWMVDec->m_pHufInterlaceCBPCYTables[huffindex];
        pWMVDec->m_pHufICBPCYDec = pWMVDec->m_pHufPCBPCYDec;
   
        huffindex = BS_getBits(pWMVDec->m_pbitstrmIn, 2);
        if(huffindex > 3)
            return ICERR_ERROR;
        pWMVDec->m_p2MVBP = pWMVDec->m_p2MVBPTables[huffindex];
        ...
        if (pWMVDec->m_iX9MVMode == MIXED_MV || pWMVDec->m_tFrmType == BVOP)
        {
            ...
            if(huffindex > 3)
                return ICERR_ERROR;
            pWMVDec->m_p4MVBP = pWMVDec->m_p4MVBPTables[huffindex];
        }
        ...
        if (pWMVDec->m_bXformSwitch) {
            if (BS_getBits(pWMVDec->m_pbitstrmIn, 1) == 1)
            {
                ...
                if(huffindex > 3)
                    return ICERR_ERROR;
                pWMVDec->m_iFrameXformMode = s_pXformLUT[huffindex];
            }
            else
