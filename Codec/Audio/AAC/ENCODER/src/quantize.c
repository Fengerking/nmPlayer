...
static Word16 quantizeSingleLine(const Word16 gain, const Word32 absSpectrum)
{
  ...
  if (absSpectrum) {
    ...
    if (finalShift < INT_BITS) {
      x = L_mpy_ls(x, pow2tominusNover16[minusFinalExp & 15]) >> 1;

      x += k >> (INT_BITS - finalShift);
	  ...
	  qua = extract_l(x);
      //qua = extract_l(L_shr(x, (finalShift - 1)));
    }
  }
                           ...
                           Word32 *mdctSpectrum)
{
  ...
  for (line=0; line<noOfLines; line++) {
    ...
    if( quantSpectrum[line] != 0 ) {
      ...
      //mdctSpectrum[line] = L_shl(accu, (iquantizershift + specExp + 1));
