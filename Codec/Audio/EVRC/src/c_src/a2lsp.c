...
void a2lsp(short *freq, short *a)
{
	...
	static long LSTEPS[4] = {13636520, 6818260, 3409130, 1704565};
	long Lq[11], t[7], s[6];
	long Lprev[2];
	int lspnumber;
	int root, notlast;
	...
	for (j = 1; j < 6; j++)
	{
		Lq[j] = (a[j - 1] + a[10 - j]) << 11;
		Lq[j] -= Lq[j - 1];
	}

	Lq[5] = Lq[5] >> 1;
	offset = 5;
	Lq[1 + offset] = ((a[0] - a[9]) << 11) + Lq[0];

	for (j = 2; j < 6; j++)
	{
		Lq[j + offset] = (a[j - 1] - a[10 - j]) << 11;
		Lq[j + offset] += Lq[j - 1 + offset];
	}

	Lq[5 + offset] = Lq[5 + offset] >> 1;
	Lprev[0] = LW_MAX;
	Lprev[1] = LW_MAX;
	lspnumber = 0;
	notlast = TRUE;
	...
	while (notlast)
	{
		...
		while (root)
		{
			Ltemp = interpolation_cos129((short)((Lfrequency + 0x8000) >> 16)) >> 1;
			for (j = 1; j < 5; j++)
			{
				s[j] = Ltemp;
			}
			...
			if ((num == 1) || (Lfrequency >= 0x40000000L))
			{
				...
				{
					if (L_abs(t[6]) < (Lprev[iswitch]>0?Lprev[iswitch]:-Lprev[iswitch]))
					{
						freq[lspnumber] = (Lfrequency < 0x7fff8000L) ? (Shortword)((Lfrequency + 0x00008000) >> 16) : (Shortword)0x7fff;;
					}
					else
						freq[lspnumber] = (short)(((Lfrequency - LSTEP) + 0x8000) >> 16);
