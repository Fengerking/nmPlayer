...
voSource2DataBufferItemList::voSource2DataBufferItemList(VO_S32 nMaxSampleSize, VO_S32 nMaxBufferSize)
...
VO_S32 voSource2DataBufferItemList::AddBuffer (VO_SOURCE2_SAMPLE * pBuffer)
{
	...
	VO_U32 nSize = pBuffer->Size & 0x7fffffff;
	...
	if (pData != NULL)
	{
		if (pData->GetAvailableSize () < nSize)
			pData = NULL;
	}
	...
	pData->WriteData ((VO_PBYTE)pBuffer->pBuffer, nSize);
...
VO_S32 voSource2DataBufferItemList::GetBuffTime (void)
{
	...
	VO_S64 nTime = m_pFirstItemFull->m_bufHead.Time;
	voSource2DataBufferItem * pItem = m_pFirstItemFull;
	while (pItem != NULL)
	{
		if (pItem->m_pNext == NULL)
		{
			nBufferTime = nBufferTime + (pItem->m_bufHead.Time - nTime);
			break;
		}

		if (pItem->m_pNext->m_bufHead.Time < pItem->m_bufHead.Time)
		{
			nBufferTime = nBufferTime + (pItem->m_bufHead.Time - nTime);
			nTime = pItem->m_pNext->m_bufHead.Time;
		}
