...
void search_ixiy_asm(Word16 *dn,
		Word16 *dn2,
		Word16 *cor_x,
		Word16 *,
		Word16 *ix,
...
static void cor_h_vec_012(
		...
		Word16 *rrixix[NB_TRACK],/* (i) correlation of h[x] with h[x]      */
		Word16 cor[]             /* (o) result of correlation (NB_POS elements) */
);

static void cor_h_vec_30(
		Word16 h[],           /* (i) scaled impulse response                 */
		Word16 vec[],         /* (i) scaled vector (/8) to correlate with h[] */
		Word16 track,         /* (i) track to use                            */
		Word16 sign[],        /* (i) sign vector                             */
...
static void search_ixiy(
		...
		Word16 * alp,             /* (i/o) wbamrc_energy of all fixed pulses       */
		...
		Word16 *rrixiy[NB_TRACK]  /* (i) corr. of pulse 1 with pulse 2   */
);
#endif


#define	dn2			(cn + L_SUBFR)
#define	sign		(dn2 + L_SUBFR)
#define	vec			(sign + L_SUBFR)
#define	ind			(vec + L_SUBFR)
#define	codvec		(ind + (NPMAXPT * NB_TRACK))
#define nbpos		(codvec + NB_PULSE_MAX)
#define	cor_x		(nbpos + 10)
#define	cor_y		(cor_x + NB_POS)
#define	pos_max		(cor_y + NB_POS)
#define	h_buf1		(pos_max + NB_TRACK)
#define	ipos		(h_buf1 + (4 * L_SUBFR))
#define	rrixix_1	(ipos + NB_PULSE_MAX)
#define rrixiy_1	(rrixix_1 + (NB_TRACK*NB_POS))
...
)
{
	...
	Word16 *rrixix[NB_TRACK],*rrixiy[NB_TRACK];
	Word32 h_buf2[2*L_SUBFR];
	Word16 *h_buf = (Word16 *)h_buf2;

	for(i=0;i<NB_TRACK;i++)
		rrixix[i] = ((Word16 *)rrixix_1 + (i*NB_POS));

	for(i=0;i<NB_TRACK;i++)
		rrixiy[i] = ((Word16 *)rrixiy_1 + (i*MSIZE));
...
#ifdef ASM_OPT
	s = voAMRWBEncDot_product12_asm(cn, cn, L_SUBFR, &exp);
#else
	s = Dot_product12(cn, cn, L_SUBFR, &exp);
#endif

	Isqrt_n(&s, &exp);
	s = L_shl(s, (exp + 5));             /* saturation can occur here */
	k_cn = extract_h(L_add(s, 0x8000));

		/* set k_dn = 32..512 (ener_dn = 2^30..2^22) */
#ifdef ASM_OPT
		s = voAMRWBEncDot_product12_asm(dn, dn, L_SUBFR, &exp);
#else
		...
		for (k = 0; k < nbiter; k++)
		{
			...
			for (j = pos, st = 0; j < nb_pulse; j += 2, st++)
			{
				...
				*--------------------------------------------------*/
				...
				{
				case 0:
					cor_h_vec_0_asm(h, vec, ipos[j],sign, rrixix, cor_x);
					break;
				case 1:
					cor_h_vec_1_asm(h, vec, ipos[j],sign, rrixix, cor_x);
					break;
				case 2:
					cor_h_vec_2_asm(h, vec, ipos[j],sign, rrixix, cor_x);
					break;
				case 3:
					cor_h_vec_3_asm(h, vec, ipos[j], sign, rrixix, cor_x);
					break;
				}
#else
				if(ipos[j]==3)
				{
					cor_h_vec_30(h, vec, ipos[j], sign, rrixix, cor_x);
				}
				else
					cor_h_vec_012(h, vec, ipos[j], sign, rrixix, cor_x);
#endif
				/*--------------------------------------------------*
				* Find best positions of 2 pulses.                 *
				*--------------------------------------------------*/
#ifdef ASM_OPT
                search_ixiy_asm(dn,dn2,cor_x,rrixiy[0],&ix,&iy,nbpos[st],ipos[j],&ps,&alp);
#else
				...
				{
					vec[i]   += add1((*p0++), (*p1++));        /* can saturate here. */
					vec[i+1] += add1((*p0++), (*p1++));        /* can saturate here. */
					vec[i+2] += add1((*p0++), (*p1++));        /* can saturate here. */
					vec[i+3] += add1((*p0++), (*p1++));        /* can saturate here. */
				}
			}
...
void cor_h_vec_30(
		Word16 h[],                           /* (i) scaled impulse response                 */
		Word16 vec30[],                         /* (i) scaled vector (/8) to correlate with h[] */
		Word16 track,                         /* (i) track to use                            */
		Word16 sign30[],                        /* (i) sign vector                             */
		Word16 *rrixix[NB_TRACK],              /* (i) correlation of h[x] with h[x]      */
		Word16 cor_1[]                       /* (o) result of correlation (NB_POS elements) */
)
{
		Word32 i, j, pos, corr;
		Word16 *p0, *p1, *p2,*p3,*cor_x30,*cor_y30;
		Word32 L_sum1,L_sum2;
		cor_x30 = cor_1;
		cor_y30 = cor_1 + NB_POS;
		p0 = rrixix[track];
		p3 = rrixix[0];
		pos = track;
		...
		for (i = 0; i < NB_POS; i+=2)
		{
			...
			*cor_x30++ = vo_mult(corr, sign30[pos]) + (*p0++);
			corr = (L_sum2 + 0x8000) >> 16;
			*cor_y30++ = vo_mult(corr, sign30[pos-3]) + (*p3++);
			pos += STEP;
...
void cor_h_vec_012(
		Word16 h[],                           /* (i) scaled impulse response                 */
		Word16 vec12[],                         /* (i) scaled vector (/8) to correlate with h[] */
		Word16 track,                         /* (i) track to use                            */
		Word16 sign12[],                        /* (i) sign vector                             */
		Word16 *rrixix[NB_TRACK],              /* (i) correlation of h[x] with h[x]      */
		Word16 cor_1[]                       /* (o) result of correlation (NB_POS elements) */
)
	{
		Word32 i, j, pos, corr;
		Word16 *p0, *p1, *p2,*p3,*cor_x12,*cor_y12;
		Word32 L_sum1,L_sum2;
		cor_x12 = cor_1;
		cor_y12 = cor_1 + NB_POS;
		p0 = rrixix[track];
		p3 = rrixix[track+1];
		pos = track;
		...
		for (i = 0; i < NB_POS; i+=2)
		{
			...
			corr = (L_sum1 + 0x8000) >> 16;
			cor_x12[i] = vo_mult(corr, sign12[pos]) + (*p0++);
			corr = (L_sum2 + 0x8000) >> 16;
			cor_y12[i] = vo_mult(corr, sign12[pos + 1]) + (*p3++);
            pos += STEP;
			...
			corr = (L_sum1 + 0x8000) >> 16;
			cor_x12[i+1] = vo_mult(corr, sign12[pos]) + (*p0++);
			corr = (L_sum2 + 0x8000) >> 16;
			cor_y12[i+1] = vo_mult(corr, sign12[pos + 1]) + (*p3++);
			pos += STEP;
		}
		return;
}
...
void search_ixiy(
		...
		Word16 * iy,                          /* (o) position of pulse 2                */
		Word16 dn[],                          /* (i) corr. between target and h[]       */
		Word16 dn21[],                         /* (i) vector of selected positions       */
		Word16 cor_x1[],                       /* (i) corr. of pulse 1 with fixed pulses */
		Word16 cor_y1[],                       /* (i) corr. of pulse 2 with fixed pulses */
		Word16 *rrixiy[NB_TRACK]                /* (i) corr. of pulse 1 with pulse 2   */
)
{
