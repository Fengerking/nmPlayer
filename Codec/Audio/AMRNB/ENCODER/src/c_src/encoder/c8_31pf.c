...
)
{
	...
	for (k = 0; k < NB_PULSE; k++)
	{
		...
		pos_index = shr2(i, 2);                      /* index = pos/4 */
						  ...
						  Word16 pos_indxC) /* i : position and sign of 8 pulses (compressed) */
{
	...
	ia = shr2(pos_indxA, 1);
	ib = extract_l(L_shr2(L_mult(shr2(pos_indxB, 1), 5), 1));
	ic = extract_l(L_shr2(L_mult(shr2(pos_indxC, 1), 25), 1));            
	indx = shl(add3(ia, add3(ib, ic)), 3);
	ia = pos_indxA & 1; 
	ib = shl2((pos_indxB & 1), 1);
	ic = shl2((pos_indxC & 1), 2);
	indx = add3(indx , add3(ia, add3(ib, ic)));  
	return indx;
						   ...
						   Word16 indx[])      /* o : position and sign of 8 pulses (compressed) */
{
	...
	*/
	...
	ib = shr2(pos_indx[7], 1) & 1; 

	if (sub3(ib, 1) == 0)
		ia = sub3(4, shr2(pos_indx[3], 1));
	else
		ia = shr2(pos_indx[3], 1);

	ib = extract_l(L_shr2(L_mult(shr2(pos_indx[7], 1), 5), 1));       
	ib = add3(shl3(add3(ia, ib), 5), 12);
	ic = shl2(mult(ib, 1311), 2);
	ia = pos_indx[3] & 1; 
	ib = shl2((pos_indx[7] & 1), 1);
	indx[NB_TRACK_MR102+2] = add3(ia, add3(ib, ic));
}
...
void voAMRNBEnc_code_8i40_31bits (
...
)
{
	...
	set_sign12k2_MR102 (dn, cn, sign, pos_max, NB_TRACK_MR102, ipos, STEP_MR102);
	/* same setsign alg as GSM-EFR new constants though*/
	cor_h (h, sign, rr);
#if OPT_CBSEARCH
	voAMRNBEnc_search_10and8i40 (NB_PULSE, STEP_MR102, NB_TRACK_MR102,dn, rr, ipos, pos_max, codvec);
#else//OPT_CBSEARCH^
	voAMRNBEnc_search_10and8i40 (NB_PULSE, STEP_MR102, NB_TRACK_MR102,dn, rr, ipos, pos_max, codvec);
#endif//OPT_CBSEARCHws
#if !SAFE_CODE_build_code_8i40   
	build_code (codvec, sign, cod, h, y, linear_signs, linear_codewords);
#else
	vo_build_code_8i40(codvec, sign, cod, h, y, linear_signs, linear_codewords);
#endif
