...
CBasePort::CBasePort(CBaseComp * pComp, OMX_U32 nIndex)
	...
	, m_nLastBufferTime (0)
{
	memset((void*)&m_diminfo, 0, sizeof(VO_VIDEO_DECDIMENSION));
	FillPortType();
...
OMX_ERRORTYPE CBasePort::AllocBuffer(OMX_PARAM_PORTDEFINITIONTYPE* pPortDef)
{
	OMX_ERRORTYPE errType = OMX_ErrorNone;
...
OMX_ERRORTYPE CBasePort::GetDefinition(OMX_PTR pParam) {

	OMX_PARAM_PORTDEFINITIONTYPE *pdef = NULL;
	if (pParam == NULL) 
		pdef = &m_portType;
	else
		pdef = (OMX_PARAM_PORTDEFINITIONTYPE*)pParam;

	InitOMXParams(pdef);
	pdef->nPortIndex = m_nIndex;
	OMX_ERRORTYPE errType = m_pComp->GetParameter(OMX_IndexParamPortDefinition, pdef, sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
	if (errType != OMX_ErrorNone) 
...
OMX_ERRORTYPE CBasePort::SetDefinition(OMX_PTR pParam) {
	...
	InitOMXParams(pdef);
	pdef->nPortIndex = m_nIndex;
	OMX_ERRORTYPE errType = m_pComp->SetParameter(OMX_IndexParamPortDefinition, pdef, sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
	if (errType != OMX_ErrorNone) 
	  return OMX_ErrorUndefined;
...
OMX_ERRORTYPE CBasePort::FillPortType (OMX_PARAM_PORTDEFINITIONTYPE* pdef)
{
	OMX_ERRORTYPE errType = OMX_ErrorNone;

	size_t size = sizeof(OMX_PARAM_PORTDEFINITIONTYPE);
	if (pdef) 
		memcpy(&m_portType, pdef, size);
	else
		errType = GetDefinition(&m_portType);
	...
	if (m_portType.eDir == OMX_DirOutput)
	{
		...
		{
			m_nPadWidth = m_portType.format.video.nFrameWidth;
			m_nPadHeight = m_portType.format.video.nFrameHeight;
		}
	}
	...
	OMX_PARAM_BUFFERSUPPLIERTYPE bs;
	size = sizeof(OMX_PARAM_BUFFERSUPPLIERTYPE);
	InitOMXParams(&bs);
	bs.nPortIndex = m_nIndex;

	errType = m_pComp->GetParameter(OMX_IndexParamCompBufferSupplier, &bs, size);
	if (errType == OMX_ErrorNone)
	{
		if (bs.eBufferSupplier == OMX_BufferSupplyInput && m_portType.eDir == OMX_DirInput)
			m_bSupplier = OMX_TRUE;
		else if (bs.eBufferSupplier == OMX_BufferSupplyOutput && m_portType.eDir == OMX_DirOutput)
			m_bSupplier = OMX_TRUE;
		else
			m_bSupplier = OMX_FALSE;
	}
