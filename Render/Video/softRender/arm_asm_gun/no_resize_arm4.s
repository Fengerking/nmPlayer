	...
	.section .rdata
	.global	 cc_mb_16x16_r90_arm 
	.extern	 ccClip63 	
	@AREA	 .text , CODE, READONLY
	.section .text

  .macro cc_mb_16x16_r90_arm_num_count Num
@	a0 = psrc_u[num] - 128@
@	a1 = psrc_v[num] - 128@
...
@	a3 = (psrc_y[(2*num + 1)] - 16)*ConstY@-------------->@a7 = (psrc_y[in_stride+2*num] - 16)*ConstY@
...
@	*((int32_t*)(out_buf+(4*num))) = (int32_t)a4@@-------->*((int32_t*)(out_buf)) = (int32_t)a4@
		...
		mov	r12, r12, lsr #1		@r12 = a5  = (ccClip63[((a3 + a1)>>22)])>>1@			
        	orr     r10, r10, r8           @********r10 = a4  = (a5 << 16)@
        	orr     r9, r9, r12, lsl #6           	@********r9 = a3  = (a5<<6)@
        	orr     r8, r10, r9, lsl #5           @********r10 = a4  = (a3 << 21)@

@	a7 = (psrc_y[in_stride+2*num] - 16)*ConstY@-------------->@a3 = (psrc_y[(2*num + 1)] - 16)*ConstY@
...
@	a7  = (a5<<6)@
@@@@@@@@@@@@@@		a4  = (a7<<5)@//dif form left a4  = (a7 << 21)@
@	*((int32_t*)(out_buf+(out_stride+4*num))) = (int32_t)a4@-------->out_buf+=-out_stride@ *(int32_t*)(out_buf) = (int32_t)a4@	

			
		ldrb	r10, [r0, #((\Num*2)+1)]			@r7 = psrc_y[(2*num + 1)]	    	
@		add	r10, r0, r3			@r8 = psrc_y+in_stride
@		ldrb	r10, [r10, #0]			@r10 = psrc_y[in_stride+2*num]

		ldr	r12, =0x129fbe			@r12 = ConstY		
	.if \Num>0
        	str	r8, [r4, r5]!			@*((int32_t*)(out_buf+(4*num))) = (int32_t)a4@	
	.else
        	str	r8, [r4, #\Num]			@*((int32_t*)(out_buf+(4*num))) = (int32_t)a4@	        	
	.endif
        	sub	r10, r10, #16	
		...
		mov	r12, r12, lsr #1		@r12 = a5  = (ccClip63[((a3 + a1)>>22)])>>1@			
        	orr     r10, r10, r8           @********r10 = a4  = (a5 << 16)@
        	orr     r9, r9, r12, lsl #6           	@********r9 = a3  = (a5<<6)@
        	orr     r10, r10, r9, lsl #5           @********r10 = a4  = (a3 << 21)@
        	str	r10, [r4, r5]!			@*((int32_t*)(out_buf+(out_stride+4*num))) = (int32_t)a4@						
  .endm  

 cc_mb_16x16_r90_arm:  @PROC

@void cc_mb_16x16_r90_c(uint8_t *psrc_y, uint8_t *psrc_u, uint8_t *psrc_v, const int32_t in_stride,
...
@
@	*((int32_t*)(out_buf+(out_stride))) = (int32_t)a4@
@	out_buf+=out_stride@
@
@	a4  = (ccClip63[((a3 + a1)>>22)])>>1@
@	a5  = (ccClip63[((a3 - a0)>>22)])@
...
@		int32_t a0, a1, a2, a3, a4, a5, a7@
	...
	cc_mb_16x16_r90_arm_num_count 0
	cc_mb_16x16_r90_arm_num_count 1
	cc_mb_16x16_r90_arm_num_count 2
	cc_mb_16x16_r90_arm_num_count 3
	cc_mb_16x16_r90_arm_num_count 4
	cc_mb_16x16_r90_arm_num_count 5
	cc_mb_16x16_r90_arm_num_count 6
	cc_mb_16x16_r90_arm_num_count 7	
