...
VO_U32 vosrcInit(MODULENAME) (VO_HANDLE * phHandle, VO_ADAPTIVESTREAM_PLAYLISTDATA * pData , VO_SOURCE2_EVENTCALLBACK * pCallback,VO_ADAPTIVESTREAMPARSER_INITPARAM * pInitParam)
{
    VO_U32   ulRet = 0;
	...
	ulRet = pSrc->Init_DASH(pData,pCallback,pInitParam);
...
/*Interface is not used now because controller dont' use it anymore*/
...
VO_U32 vosrcSeek(MODULENAME) (VO_HANDLE hHandle, VO_U64* pTimeStamp,VO_ADAPTIVESTREAMPARSER_SEEKMODE sSeekMode)
{
    VO_U32   ulRet = 0;
	SOURCE_CLASS * pSrc = (SOURCE_CLASS *)hHandle;
	if (pSrc == NULL)
	...
	ulRet = pSrc->Seek_DASH(pTimeStamp,sSeekMode);
	return ulRet;
}
...
VO_U32 vosrcGetProgramInfo(MODULENAME) (VO_HANDLE hHandle, VO_U32 nProgramId, _PROGRAM_INFO **ppProgramInfo)
{
    VO_U32	 ulRet = 0;
	SOURCE_CLASS * pSrc = (SOURCE_CLASS *)hHandle;
	if (pSrc == NULL)
...
VO_U32 vosrcGetCurTrackInfo(MODULENAME) (VO_HANDLE hHandle, VO_SOURCE2_TRACK_TYPE nOutPutType , _TRACK_INFO **ppTrackInfo )
{
    VO_U32	 ulRet = 0;
	SOURCE_CLASS * pSrc = (SOURCE_CLASS *)hHandle;
	if (pSrc == NULL)
...
VO_U32 vosrcSelectStream(MODULENAME) (VO_HANDLE hHandle, VO_U32 nStreamID,VO_SOURCE2_ADAPTIVESTREAMING_CHUNKPOS sPrepareChunkPos)
{
    VO_U32	 ulRet = 0;
	SOURCE_CLASS * pSrc = (SOURCE_CLASS *)hHandle;
	if (pSrc == NULL)
	...
	ulRet = pSrc->SelectStream_DASH(nStreamID,sPrepareChunkPos);	
	//ulRet = pSrc->SelectStream_DASH(nStreamID);
	return ulRet;
}
#ifdef _new_programinfo
VO_U32 vosrcSelectTrack(MODULENAME) (VO_HANDLE hHandle, VO_U32 nTrackID,VO_SOURCE2_TRACK_TYPE sTrackType)
#else
...
#ifdef _new_programinfo
		ulRet = pSrc->SelectTrack_DASH (nTrackID,  sTrackType);
#else
