...
**
*/
void  Find_Fcbk( Word16 *Dpnt, Word16 *ImpResp, LINEDEF *Line, Word16 Sfc, G723EncState *st)
{
	int      i;
	Word16   T0_acelp, gain_T0;
	Word16   Srate ;
    Word16   WrkRate = st->Ratemode;
...
**
*/
void  Find_Acbk( Word16 *Tv, Word16 *ImpResp, G723EncState *st, LINEDEF
				*Line, Word16 Sfc )
{
	int      i,j,k,l;
	CODSTATDEF *CodStat = (CODSTATDEF *)st->cod_state;
    Word16   *PrevExc = CodStat->PrevExc;
	Word32   Acc0,Acc1 ;
	...
	Decod_Acbk( RezBuf, PrevExc,Olp, Lid, Gid ,st) ;
...
void  Decod_Acbk( Word16 *Tv, Word16 *PrevExc, Word16 Olp, Word16 Lid,
				 Word16 Gid, G723EncState *st)
{
	int   i,j   ;
    //Word16   *PrevExc = st->cod_state->PrevExc;
	Word16   WrkRate  = st->Ratemode;
	Word32   Acc0  ;
	Word16   RezBuf[SubFrLen+ClPitchOrd-1] ;
	Word16  *sPnt ;
