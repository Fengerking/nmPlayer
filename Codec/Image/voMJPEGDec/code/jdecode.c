...
#ifdef STABILITY  //zou 426
	if(jpara->MCUBlock.blockNum == 5 || jpara->MCUBlock.blockNum == 2)
		return VO_ERR_DEC_MJPEG_INDATAERROE;
#endif
...
INT32	JPEG_interlmcu_dec(Jpeg_DecOBJ *decoder, VO_VIDEO_BUFFER *pOutData)
{
	...
	_IDCT_Block8x8  idct_block8x8_V6;
	...
	idct_block8x8_V6 = decoder->idct_block_V6;
				...
				for(k = 0; k < jpara->Ns; k++)	{
					...
					for(v_index = 0; v_index < cinfo->v_sample; v_index++)	{
						...
						for(h_index = 0; h_index < cinfo->h_sample; h_index++)	{
							...
							if(decoder->enableNormalChroma ||(0x00000011 == jpara->image_type)
								||(0x00111122 == jpara->image_type)|| (0x11111111== jpara->image_type))/* enableNormalChroma or YUV420 format */
							{
#ifdef VOARMV7
                                if(mcublock->last_nonzero[blockindex]<= 1) //when mcublock->last_nonzero[blockindex]<= 1,we use v6 is faster
                                    (*idct_block8x8_V6)(mcublock->WorkBlock[blockindex], dst, pOutData->Stride[k], NULL, 0);
                                else
#endif
...
#ifdef VOARMV7
                                if(mcublock->last_nonzero[blockindex]<= 1)//when mcublock->last_nonzero[blockindex]<= 1,we use v6 is faster
                                    (*idct_block8x8_V6)(mcublock->WorkBlock[blockindex], decoder->chromaBuffer, decoder->addstep, NULL, 0);
                                else
#endif
									(*idct_block8x8)(mcublock->WorkBlock[blockindex], decoder->chromaBuffer, decoder->addstep, NULL, 0);

									switch(jpara->image_type)
									{			
									case 0x00111111:/*VO_COLOR_YUV_PLANAR444*/
										JPEG_PLANAR444_to_PLANAR420(decoder->chromaBuffer, decoder->addstep, dst, pOutData->Stride[k]);
										//dst += 4;
										break;
									case 0x00111121:/*VO_COLOR_YUV_PLANAR422_12*/
										JPEG_PLANAR422_12_to_PLANAR420(decoder->chromaBuffer, decoder->addstep, dst, pOutData->Stride[k]);
										//dst += 8;
										break;
									case 0x00111112:/*VO_COLOR_YUV_PLANAR422_21*/
										JPEG_PLANAR422_21_to_PLANAR420(decoder->chromaBuffer, decoder->addstep, dst, pOutData->Stride[k]);
										//dst += 4;
										break;
									case 0x00111141:/*VO_COLOR_YUV_PLANAR411*/
										JPEG_PLANAR411_to_PLANAR420(decoder->chromaBuffer, decoder->addstep, dst, pOutData->Stride[k]);
										//dst += 16;								
										break;
									case 0x00111114:/*VO_COLOR_YUV_PLANAR411V*/
										JPEG_PLANAR411V_to_PLANAR420(decoder->chromaBuffer, decoder->addstep, dst, pOutData->Stride[k]);
										//dst += 4;
										break;
									default:
										return VO_ERR_INVALID_ARG;	
		...
		for(i = 0; i < v_blocks; i++)
		{
			...
			{					
				...
				*block16 += 1024;	
				...
				if(decoder->enableNormalChroma ||(0x00000011 == jpara->image_type)
					||(0x00111122 == jpara->image_type)|| (0x11111111== jpara->image_type))
				{
