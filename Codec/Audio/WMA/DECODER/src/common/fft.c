...
#if 0
	{
		FILE *hdata = fopen("imdct.dat", "wb");
		fwrite(rgiCoef, 1, cSB*sizeof(Int), hdata);
		fclose(hdata);
	}
#endif
    piCoefTop    = rgiCoef;
...
//		printf("after fft rgiCoef: 0x%x, 0x%x, 0x%x, 0x%x\n", rgiCoef[0], rgiCoef[1], rgiCoef[2], rgiCoef[3]); 
//        pfnFFT(rgiCoef, (fPowerOfTwo ? nLog2SB - 1 : iFFTSize), FFT_FORWARD); 
    ...
    piCoefTop      = rgiCoef;           //post processing, 
    piCoefBottom   = rgiCoef + cSB - 2; 
...
//WMARESULT prvInvFftComplexConjugate(CoefType *rgfltData, Int nOrder, 
                                    CoefType *rgfltTmp,  U32 cSizeTmp)
{
    ...
    rgfltTmp = (CoefType *)memset (rgfltTmp, 0, cSizeTmp * sizeof(Float)); 
    rgfltTmp = (CoefType *)voWMAMemcpy (rgfltTmp, rgfltData, (N + 2) * sizeof(Float));
 ...
 //   prvFFT4DCT(rgfltTmp, nOrder, FFT_INVERSE);
