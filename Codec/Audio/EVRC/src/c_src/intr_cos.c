...
/* cos(x)=cos(a)+(x-a)sin(a)-pow((a-x),2)*cos(a) */
	...
	count=(freq>0?freq:-freq) >>7;
	 
	temp=extract_l((count << 7) - freq );
/* (a-x)sin a  */
/* Scale factor for (a-x): 3217=pi2/64 */
	sin_data = sin_cosT[count<<1];
	cos_data = sin_cosT[(count<<1) + 1];
  	//sin_data=sin129_table[count];
	//cos_data=cos129_table[count];
 	
	Ltemp=L_mpy_ls((3217 * temp)<<1,sin_data);

/* (a-x) sin(a) - [(a-x)*(a-x)*cos(a)] /2 */
/* Scale factor for (a-x)*(a-x):  20213=pi2*pi2/64 */

	Ltemp=Ltemp - L_mpy_ls(((((10106 * temp) + 0x4000)>>15) * temp)<<1,cos_data);
 
/* Scaled up by 64/2 times */ 
	Ltemp= Ltemp  << 6;
 	Lresult= (L_deposit_h(cos_data) + (Ltemp)) ;
