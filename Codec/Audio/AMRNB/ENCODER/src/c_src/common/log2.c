				...
				)
{
	...
	*exponent =  (30- exp); 
	...
	i = (i - 32);
	L_y = L_deposit_h (Log2_table[i]);           /* table[i] << 16        */
	tmp = (Log2_table[i]- Log2_table[i + 1]);    /* table[i] - table[i+1] */
		   ...
		   )
{
	...
	exp = norm_l (L_x);
	voAMRNBEnc_Log2_norm ((L_x << exp), exp, exponent, fraction);
}
