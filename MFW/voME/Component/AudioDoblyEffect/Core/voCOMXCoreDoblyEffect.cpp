...
#include "voCOMXCoreDoblyEffect.h"
#include "voComponentEntry.h"
#include "voCOMXAudioDoblyEffect.h"

#include "voLog.h"

voCOMXCoreDoblyEffect::voCOMXCoreDoblyEffect(void)
{
	VOLOGF ();
	...
	strcpy (m_szCompName, "OMX.VisualOn.Audio.DoblyEffect.XXX");
	...
	// Audio Dobly effect
	strcpy (m_szCompRoles[0], "audio_effect.dobly");
}
...
OMX_ERRORTYPE voCOMXCoreDoblyEffect::Init (OMX_PTR hInst)
{
	VOLOGF ();
...
OMX_STRING voCOMXCoreDoblyEffect::GetName (OMX_U32 nIndex)
{
	VOLOGF ();
...
OMX_ERRORTYPE voCOMXCoreDoblyEffect::LoadComponent (OMX_COMPONENTTYPE * pHandle, OMX_STRING pName)
{
	VOLOGF ();
	...
	if (!strcmp (pName, m_szCompName))
		pComponent = new voCOMXAudioDobyEffect (pHandle);
	else
...
OMX_ERRORTYPE voCOMXCoreDoblyEffect::FreeComponent (OMX_COMPONENTTYPE * pHandle)
{
	VOLOGF ();
...
OMX_ERRORTYPE voCOMXCoreDoblyEffect::GetComponentsOfRole (OMX_STRING role, OMX_U32 *pNumComps, OMX_U8  **compNames)
{
	VOLOGF ();
...
OMX_ERRORTYPE voCOMXCoreDoblyEffect::GetRolesOfComponent (OMX_STRING compName, OMX_U32 *pNumRoles, OMX_U8 **roles)
{
	VOLOGF ();
