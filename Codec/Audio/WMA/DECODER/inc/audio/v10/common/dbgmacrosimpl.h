...
INLINE WMARESULT DbgRefrInit(CAudioObjectDecoder *paudec, CAudioObject *pau)
{
...
INLINE WMARESULT DbgRefrOpenRef(CAudioObjectDecoder *paudec, CAudioObject *pau)
{
    WMARESULT wmaResult = WMA_OK;
    ...
    {         
        ...
        TRACEWMA_EXIT(wmaResult, DbgRefrInit(paudec, pau));
    }            
                       ...
                       CoefType *pHist)
{
    ...
    if (g_bEnableStageWiseConf)                                 
    {
        ...
        if (bEnable) {                                           
            ...
            if (!bPCM)
            {
                voWMAMemcpy(buf2, pau->m_rgCoefXFormMCLT,                    
            ...
            {
                voWMAMemcpy(buf, pau->m_rgCoefXFormMCLT+prti->m_iSizeCurr/2,
...
INLINE WMARESULT DbgRefrOpenCan(CAudioObjectDecoder *paudec, CAudioObject *pau)
{
    WMARESULT wmaResult = WMA_OK;
