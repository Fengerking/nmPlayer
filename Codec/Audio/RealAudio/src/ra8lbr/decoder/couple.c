...
void JointDecodeMLT(Gecko2Info *gi, int *mltleft, int *mltrght)
{
	...
	for (r = gi->cplStart; r < gi->nRegions; r++) {
		...
		for (i = NBINS; i; i--) {
			*pmltrght = MULSHIFT32(scalerght, *pmltleft) << 1;
			pmltrght++;
			*pmltleft = MULSHIFT32(scaleleft, *pmltleft) << 1;
			pmltleft++;
		}
	}
