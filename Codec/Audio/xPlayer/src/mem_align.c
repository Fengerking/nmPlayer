...
************************************************************************/
#include	"mem_align.h"
void *as_mem_malloc(VO_MEM_OPERATOR *pMemop, unsigned int size, unsigned char alignment)
{
	int ret;
	...
	if (!alignment) {
		...
		MemInfo.Flag = 0;
		MemInfo.Size = size + 1;
		ret = pMemop->Alloc(VO_INDEX_DEC_AMRNB, &MemInfo);
		if(ret != 0)
			return 0;
		...
		*mem_ptr = (unsigned char)1;
		...
		pMemop->Set(VO_INDEX_DEC_AMRNB, tmp, 0, size + alignment);
...
void as_mem_free(VO_MEM_OPERATOR *pMemop, void *mem_ptr)
{
	unsigned char *ptr;
	if (mem_ptr == 0)
		return;
	...
	ptr -= *(ptr - 1);
	/* Free the memory */
	pMemop->Free(VO_INDEX_DEC_AMRNB, ptr);
}
