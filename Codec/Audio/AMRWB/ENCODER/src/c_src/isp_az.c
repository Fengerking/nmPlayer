	   ...
	   )
{
	...
	a[nc] = (Word16)(vo_L_shr_r(t0, q_sug));    /* from Q23 to Q12 and * 0.5 */
	/* a[m] = isp[m-1]; */
	...
	for (i = 2; i <= n; i++)
	{
		*f = f[-2];                        
		for (j = 1; j < i; j++, f--)
		{
			...
			t0 = voMpy_32_16(hi, lo, *isp);  /* t0 = f[-1] * isp    */
			t0 = t0 << 1;
...
static void Get_isp_pol_16kHz(Word16 * isp, Word32 * f, Word16 n)
{
	...
	/* All computation in Q23 */
	f[0] = vo_L_mult(4096, 256);                /* f[0] = 1.0;        in Q23  */
	f[1] = vo_L_mult(isp[0], -64);              /* f[1] = -2.0*isp[0] in Q23  */
