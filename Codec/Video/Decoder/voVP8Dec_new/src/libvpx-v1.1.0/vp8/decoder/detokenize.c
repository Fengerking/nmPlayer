...
int vp8_decode_mb_tokens(VP8D_COMP *dx, MACROBLOCKD *x)
{
    ...
    const FRAME_CONTEXT * const fc = dx->common.cfc;
    char *eobs = x->eobs;

    int i;
    int nonzeros;
    int eobtotal = 0,eobtotal_uv=0;
    ...
    for (i = 16; i < 24; ++i)
    {
        ...
        *a = *l = (nonzeros > 0);
        ...
        eobs[i] = nonzeros;
        eobtotal_uv += nonzeros;		
        qcoeff_ptr += 16;
    }

	if(eobtotal_uv == 0)
		x->mode_info_context->mbmi.mb_skip_coeff_uv = 1;
	else
		x->mode_info_context->mbmi.mb_skip_coeff_uv = 0;

	if(eobtotal == 0)
		x->mode_info_context->mbmi.mb_skip_coeff_y = 1;
	else
		x->mode_info_context->mbmi.mb_skip_coeff_y = 0;

    return eobtotal+eobtotal_uv;
}
