...
int CVOWPlayer::CaptureFrame (int nPos, int nWidth, int nHeight, VO_IV_COLORTYPE nColor,
							 VO_VIDEO_BUFFER ** ppVideoBuffer, VO_VIDEO_FORMAT ** pVideoFormat, uint32_t* out_npDuration)
{
	if (m_hVOWPlay == NULL)
		return -1;
...
int CVOWPlayer::RenderVideo (VO_VIDEO_BUFFER * pVideoBuffer, VO_VIDEO_FORMAT * pVideoSize)
{
	if (m_pCallBack == NULL)
	{
		...
		if (m_bBlackFrame && (m_nCapturePos + 2000 < m_nDuration)) {		
			JNILOGE("Black frame and Pos < Duration");
	...
	{
		JNILOGI2 ("Video Color %d W %d, H %d", pVideoBuffer->ColorType, pVideoSize->Width, pVideoSize->Height);
		...
		JNILOGI ("SET_VIDEO_SIZE in render");
...
{
#ifdef _LINUX
	m_hVOWModule = dlopen ("/data/local/voOMXPlayer/lib/libvoMMPlay.so", RTLD_NOW);
	if (m_hVOWModule == NULL)
		m_hVOWModule = dlopen ("/data/local/voOMXPlayer/lib/libvoMMPlay.so", RTLD_NOW);
		
	if (m_hVOWModule == NULL)
		m_hVOWModule = dlopen ("/data/data/com.visualon.vome/lib/libvoMMPlay.so", RTLD_NOW);
		
	
	if (m_hVOWModule == NULL)
		m_hVOWModule = dlopen ("libvoMMPlay.so", RTLD_NOW);
	if (m_hVOWModule == NULL)
		m_hVOWModule = dlopen ("voMMPlay.so", RTLD_NOW);
		
 if (m_hVOWModule == NULL) {
 	  JNILOGI ("begin to load libvoedit.so");
	...
	VOWGETAPI pAPI = (VOWGETAPI) dlsym(m_hVOWModule, "vommGetPlayAPI");
	if (pAPI == NULL)
	{
