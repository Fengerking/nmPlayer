...
Void prvSubFrameRestoreChannelDC(CAudioObject *pau)
{
    I16 iCh;
    I32 i;
...
Void prvLPCFilteringInv(CAudioObject *pau, CBT * rgiCoefRecon, I32 * rgiLPCPrevSamples, CBT * rgiLPCResidue, I32 * rgiLPCFilter, I32 iLen, I16 iOrder)
{
    ...
    rgiLPCPrevSamples = (I32 *)memset(rgiLPCPrevSamples, 0, sizeof(rgiLPCPrevSamples[0]) * LLM_LPC_ORDER);
...
Void prvDecLMSPredictor_I16_C(CAudioObject * pau, LMSPredictor * pLMSPredictor, CBT *rgiResidue, I32 iLen)
{
    ...
    for (i = 0; i < iLen; i++) {
        ...
        if (iRecent == 0) {
            pTemp = voWMAMemcpy(rgiPrevValRdBuf + iOrder, rgiPrevValRdBuf, iOrder_X_2);
            pTemp = voWMAMemcpy(rgiUpdateRdBuf + iOrder, rgiUpdateRdBuf, iOrder_X_2);
            iRecent = iOrder - 1;
        }
        else
            iRecent = iRecent - 1;
...
Void prvDecLMSPredictor_I32_C(CAudioObject * pau, LMSPredictor * pLMSPredictor, CBT *rgiResidue, I32 iLen)
{
    ...
    for (i = 0; i < iLen; i++) {
        ...
        //BUGBUG in BETA. We cannot take advantge of long buffer (reduce voWMAMemcpy times) in BETA. It is a BETA bug
        //please check updateSpeed function. In RTM we fix the bug and we can use
        //  voWMAMemcpy(rgiUpdate + iMaxOrder * 2 - iOrder, rgiUpdate, ...);

        if (iRecent == 0) {
            pTemp = voWMAMemcpy(rgiPrevValRdBuf + iOrder, rgiPrevValRdBuf, iOrder_X_4);
            pTemp = voWMAMemcpy(rgiUpdateRdBuf + iOrder, rgiUpdateRdBuf, iOrder_X_2);
            iRecent = iOrder - 1;
        }
        else
