...
//  temp = sub( blockType, SHORT_WINDOW );                                                            
//  if (temp != 0)
...
//      }
//    }
//    win[i] = m_pow2_cordic(accugsStart,scale);                                                      
//  }
//}
                                 ...
                                 Word32       *pWork32)
{
    ...
    //for (i=lpcStartLine; i<lpcStopLine; i++){
    //  weightedSpectrum[i] = extract_h(pWork32[i] << maxShift);
    //}
...
static Word16 AutoToParcor(Word32 workBuffer[], Word32 reflCoeff[], Word16 numOfCoeff) {
  ...
  for(i=0; i<numOfCoeff; i++) {
	...
	workBuffer0 = workBuffer[0] << shift;
    /* calculate refc = -workBuffer[numOfCoeff+i] / workBuffer[0]; -1 <= refc < 1 */
    refc = L_negate(Div_32(workBuffer[numOfCoeff + i] << shift, workBuffer0));
  ...
  denom = MULHIGH(workBuffer[0], 0x0147ae14*2);
