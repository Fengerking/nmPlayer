/*-------------------------------------------------------------------*
*                         DECIM54.C								 *
*-------------------------------------------------------------------*
* Decim_12k8   : decimation of 16kHz signal to 12.8kHz.             *
* Oversamp_16k : oversampling from 12.8kHz to 16kHz.                *
*-------------------------------------------------------------------*/
#include "typedef.h"
#include "basic_op.h"
#include "acelp.h"
#include "cnst.h"

#define FAC5   5
#define DOWN_FAC  26215                    /* 4/5 in Q15 */

#define NB_COEF_DOWN  15

/* Local functions */
static void Down_samp(
					  Word16 * sig,                         /* input:  signal to downsampling  */
					  Word16 * sig_d,                       /* output: downsampled signal      */
					  Word16 L_frame_d                      /* input:  length of output        */
					  );
#if 0
static Word16 fir_down[120] =
{            /* table x4/5 */
	-1,		-3,		-6,		-5,
	0,		9,		19,		24,
	18,		0,		-26,	-50,
	-58,	-41,	0,		54,
	99,		111,	77,		0,
	-95,	-170,	-188,	-128,
	0,		153,	270,	294,
	198,	0,		-233,	-408,
	-441,	-295,	0,		344,
	601,	649,	434,	0,
	-507,	-888,	-964,	-647,
	0,		770,	1366,	1505,
	1030,	0,		-1293,	-2379,
	-2746,	-1997,	0,		3034,
	6575,	9894,	12254,	13107,
	12254,	9894,	6575,	3034,
	0,		-1997,	-2746,	-2379,
	-1293,	0,		1030,	1505,
	1366,	770,	0,		-647,
	-964,	-888,	-507,	0,
	434,	649,	601,	344,
	0,		-295,	-441,	-408,
	-233,	0,		198,	294,
	270,	153,	0,		-128,
	-188,	-170,	-95,	0,
	77,		111,	99,		54,
	0,		-41,	-58,	-50,
	-26,	0,		18,		24,
	19,		9,		0,		-5,
	-6,		-3,		-1,		0
};
#endif 

Word16 amrwb_fir_down[4][30] =
{
	{-5, 24, -50, 54, 0, -128, 294, -408, 344, 0, -647, 1505, -2379, 3034, 13107, 3034, -2379, 1505, -647, 0, 344, -408,
	294, -128, 0, 54, -50, 24, -5, 0},

	{-6, 19, -26, 0, 77, -188, 270, -233, 0, 434, -964, 1366, -1293, 0, 12254, 6575, -2746, 1030, 0, -507, 601, -441,
	198, 0, -95, 99, -58, 18, 0, -1},

	{-3, 9, 0, -41, 111, -170, 153, 0, -295, 649, -888, 770, 0, -1997, 9894, 9894, -1997, 0, 770, -888, 649, -295, 0,
	153, -170, 111, -41, 0, 9, -3},

	{-1, 0, 18, -58, 99, -95, 0, 198, -441, 601, -507, 0, 1030, -2746, 6575, 12254, 0, -1293, 1366, -964, 434, 0,
	-233, 270, -188, 77, 0, -26, 19, -6}
};

void Init_Decim_12k8(
					 Word16 mem[]                          /* output: memory (2*NB_COEF_DOWN) set to zeros */
)
{
	Set_zero(mem, 2 * NB_COEF_DOWN);
	return;
}

void Decim_12k8(
				Word16 sig16k[],                      /* input:  signal to downsampling  */
				Word16 lg,                            /* input:  length of input         */
				Word16 sig12k8[],                     /* output: decimated signal        */
				Word16 mem[]                          /* in/out: memory (2*NB_COEF_DOWN) */
)
{
	Word16 lg_down;
	Word16 signal[L_FRAME16k + (2 * NB_COEF_DOWN)];

	voAMRWBEnc_Copy(mem, signal, 2 * NB_COEF_DOWN);

	voAMRWBEnc_Copy(sig16k, signal + (2 * NB_COEF_DOWN), lg);

	lg_down = (lg * DOWN_FAC)>>15;

	Down_samp(signal + NB_COEF_DOWN, sig12k8, lg_down);

	voAMRWBEnc_Copy(signal + lg, mem, 2 * NB_COEF_DOWN);

	return;
}

#if 1
static void Down_samp(
					  Word16 * sig,                         /* input:  signal to downsampling  */
					  Word16 * sig_d,                       /* output: downsampled signal      */
					  Word16 L_frame_d                      /* input:  length of output        */
					  )
{
	Word32 i, j, frac, pos;
	Word16 *x;
	static Word16 *y;
	Word32 L_sum;

	pos = 0;                                 /* position is in Q2 -> 1/4 resolution  */
	for (j = 0; j < L_frame_d; j++)
	{
		i = (pos >> 2);                   /* integer part     */
		frac = pos & 3;                   /* fractional part */
        x = sig + i - NB_COEF_DOWN + 1;
        y = amrwb_fir_down[frac];

		L_sum = *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x++ * *y++;
		L_sum += *x * *y;

	    L_sum = L_shl2(L_sum, 2);               /* saturation can occur here */
	    sig_d[j] = extract_h(L_add(L_sum, 0x8000));                //can't be modified
		pos += FAC5;              /* pos + 5/4 */
	}
	return;
}
#else
static void Down_samp(
					  Word16 * sig,                         /* input:  signal to downsampling  */
					  Word16 * sig_d,                       /* output: downsampled signal      */
					  Word16 L_frame_d                      /* input:  length of output        */
					  )
{
	Word16 i, j, frac, pos;

	pos = 0;                                 /* position is in Q2 -> 1/4 resolution  */
	for (j = 0; j < L_frame_d; j++)
	{
		i = (pos >> 2);                   /* integer part     */
		frac = (Word16) (pos & 3);           /* fractional part */

		sig_d[j] = Interpol(&sig[i], fir_down, frac, FAC4, NB_COEF_DOWN);       

		pos += FAC5;              /* pos + 5/4 */
	}
	return;
}
#endif

