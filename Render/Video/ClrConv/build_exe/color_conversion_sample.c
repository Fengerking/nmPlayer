...
//		show_usage();
		...
		printf("VOANDROID  ");
#else
		if ((in_file=fopen("E:/MyResource/Video/clips/VC1/win32_smoothstream640x480.yuv", "rb")) == NULL) {
			goto LExit;
		}

		if ((out_file=fopen("E:/MyResource/Video/clips/VC1/win32_smoothstream640x480.rgb32", "w+b")) == NULL) {
			goto LExit;
		}
#endif

	in_width = (argv[3][0] -0x30)*1000+(argv[3][1] -0x30)*100 + (argv[3][2] -0x30)*10 + (argv[3][3] -0x30);
	in_height = (argv[4][0] -0x30)*1000+(argv[4][1] -0x30)*100 + (argv[4][2] -0x30)*10 + (argv[4][3] -0x30);
	loopcount = (argv[5][0] -0x30)*1000+(argv[5][1] -0x30)*100 + (argv[5][2] -0x30)*10 + (argv[5][3] -0x30);
	
	printf("in_width = %d in_height=%d loopcount=%d\n",in_width,in_height,loopcount);
	in_stride = in_width;
	...
	pOutput.ColorType = VO_COLOR_RGB32_PACKED;//VO_COLOR_RGB565_PACKED;//VO_COLOR_YUYV422_PACKED;//VO_COLOR_RGB565_PACKED;
	...
	voCCRRR.SetColorType(hnd, VO_COLOR_YUV_PLANAR420, VO_COLOR_RGB32_PACKED);
	...
	printf("\n cc %d frame, time: %d ms  %2.3f \n", k, (int)totalenctime,(k*1000.0)/totalenctime);
