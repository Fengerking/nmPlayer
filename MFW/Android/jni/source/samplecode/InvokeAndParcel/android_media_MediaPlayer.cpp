...
JNIMediaPlayerListener::JNIMediaPlayerListener(JNIEnv* env, jobject thiz, jobject weak_thiz)
{
    ...
    if (clazz == NULL) {
        LOGE("Can't find com/visualon/vome/voMediaPlayer");
...
{
     LOGE("VOJNI invoke called"); 
      if (java_request == NULL && java_reply == NULL) 
	return 0;
/*
...
*/
    ...
    Parcel *reply = parcelForJavaObject(env, java_reply);
     ...
     int          nTest      = 0;
     const char16_t* pTest16 = NULL; 
     long         lTest      = 0;   
     size_t len;
  
     nTest  = request->readInt32();
     pTest16 = request->readString16Inplace(&len);
     String8 str8(pTest16);
     lTest   = (long) request->readInt64();
      
     LOGE("VALUE FROM INVOKE param int: %d \n", nTest); 
     LOGE("string value %s len %d\n", str8.string(), len);
     LOGE("long value %ld\n", lTest);

     String16 str16("Yu Wei at Visual On");

     reply->writeInt32(813);
     reply->writeString16(str16);
     reply->writeInt64(3181979);
...
android_media_MediaPlayer_native_init(JNIEnv *env)
{
    ...
    clazz = env->FindClass("com/visualon/vome/voMediaPlayer");
    if (clazz == NULL) {
        jniThrowException(env, "java/lang/RuntimeException", "Can't find com/visualon/vome/voMediaPlayer");
        return;
    }
...
static const char* const kClassPathName = "com/visualon/vome/voMediaPlayer";

// This function only registers the native methods
static int register_android_media_MediaPlayer(JNIEnv *env)
{
    return AndroidRuntime::registerNativeMethods(env,
                "com/visualon/vome/voMediaPlayer", gMethods, NELEM(gMethods));
}
