    ...
    {        
        ...
        case FHDR_PREV:
            if (pau->m_fAllowSubFrame && pau->m_iVersion <= 2)
            {
                ...
                DEBUG_PRINTF(printf("prvDecodeFrameHeader_FHDR_PREV!\n"));
            ...
            {                
                ...
                for (iCh = 0; iCh < pau->m_cChInTile; iCh++) 
                {
                    ...
                    if (ppcinfo->m_iCurrSubFrame == 0) 
                    {
                        ...
                        ppcinfo->m_rgiMaskQ = (Int *)memset(ppcinfo->m_rgiMaskQ, 0, sizeof(Int) * NUM_BARK_BAND);
                        ppcinfo->m_rgiMaskQResampled = (Int *)memset(ppcinfo->m_rgiMaskQResampled, 0, sizeof(Int) * NUM_BARK_BAND);
...
//                fSkipAll &= (ppcinfo->m_iPower == 0);
				if(!(ppcinfo->m_iPower == 0))fSkipAll = WMAB_FALSE;
...
//                fSkipAll = (pau->m_rgpcinfo [0].m_iPower == 0) & (pau->m_rgpcinfo [1].m_iPower == 0);
				if((pau->m_rgpcinfo [0].m_iPower == 0) && (pau->m_rgpcinfo [1].m_iPower == 0))fSkipAll = WMAB_TRUE;
				else fSkipAll = WMAB_FALSE;
...
*/
                ...
                if (!ppcinfo->m_bNoDecodeForCx)
                    ppcinfo->m_rgiCoefQ = (I32 *)memset (ppcinfo->m_rgiCoefQ, 0, sizeof (I32) *
						AU_HALF_OR_DOUBLE(pau->m_fWMAProHalfTransform, pau->m_fPad2XTransform, ppcinfo->m_cSubbandActual));
...
//								printf("CWMAInputBitStream_m_pBuffer             EQU	%d\n", offsetof(CWMAInputBitStream, m_pBuffer));
//								printf("CWMAInputBitStream_m_cbBuflen            EQU	%d\n", offsetof(CWMAInputBitStream, m_cbBuflen));
//								printf("CWMAInputBitStream_m_dwDot               EQU	%d\n", offsetof(CWMAInputBitStream, m_dwDot));
//								printf("CWMAInputBitStream_m_dwBitsLeft          EQU	%d\n", offsetof(CWMAInputBitStream, m_dwBitsLeft));
//								printf("CWMAInputBitStream_m_dwDotT              EQU	%d\n", offsetof(CWMAInputBitStream, m_dwDotT));
//								printf("CWMAInputBitStream_m_cBitDotT            EQU	%d\n", offsetof(CWMAInputBitStream, m_cBitDotT));
//								printf("CWMAInputBitStream_m_cFrmBitCnt          EQU	%d\n", offsetof(CWMAInputBitStream, m_cFrmBitCnt));
                                for (; paudec->m_iBand < pau->m_cValidBarkBand; paudec->m_iBand++)  
                                {
                                    Int iDiff;   
//									printf("huffDecGet_i()\n");
//									printf("pDecodeTable: 0x%x, 0x%x, 0x%x, 0x%x\n", pDecodeTable[0], pDecodeTable[1], pDecodeTable[2], pDecodeTable[3]);
                                    TRACEWMA_EXIT(wmaResult, huffDecGet_i (pDecodeTable,&paudec->m_ibstrm, (unsigned long *)&iDiff, (unsigned long *)0));
                                    iDiff -= MAX_MASKQUANT_DIFF;
//									printf("huffDecGet_i() is success!\n");
                    ...
                    {   //else artifically set to constants since nothing got sent; see comments in msaudioenc.c
                        ...
                        ppcinfo->m_rgiMaskQ = (Int *)memset( ppcinfo->m_rgiMaskQ, 0, pau->m_cValidBarkBand*sizeof(Int) );
                        ppcinfo->m_iMaxMaskQ = 0;
                    }
                    if (pau->m_iVersion > 2) 
                    {
