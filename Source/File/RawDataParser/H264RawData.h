...
#include "voType.h"
#include "H264RawDataFileDataStruct.h"
#include "voList.h"
#include <stdio.h>
#include <string.h>
	//Get file format
static  VO_S32 RawDataGetFileFormat(VO_PTCHAR pFilePath)
{
...
{
public:
	CH264RawData(VO_FILE_OPERATOR* pFileOp, VO_MEM_OPERATOR* pMemOp, VO_LIB_OPERATOR* pLibOP, VO_SOURCEDRM_CALLBACK* pDrmCB);
	virtual ~CH264RawData(void);
...
protected:
	...
	VO_U32 ParseH264RawDataFile();

	//Get the H264 head data
	VO_S32 Get264HeadData(char* currPos,int size);

	//Get the frame data
	VO_S32 GetNextFrame(char* currPos,int size);

	//virtual	VO_U32	GetHeadData(VO_CODECBUFFER* pHeadData);

protected:
	VO_BYTE				aStreamBlock[STREAM_BUFFER_SIZE];	// a block of stream data.
