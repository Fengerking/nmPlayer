...
static char g_szpackagepath[128]; 
...
static void vomp_setPackagePath(JNIEnv *env, jobject thiz, jstring path) {
    ...
    if (pathStr == NULL) {  // Out of memory
		...
		strcpy(g_szpackagepath, pathStr);
		status_t opStatus = mp->setWorkingPath(pathStr);

    env->ReleaseStringUTFChars(path, pathStr);
    process_media_player_call( env, thiz, opStatus, "java/io/IOException", "setPackageName failed." );
}
...
vomp_native_setup(JNIEnv *env, jobject thiz, jobject weak_this)
{
    ...
    mp->setListener(listener);
		mp->setWorkingPath(g_szpackagepath);
...
vomp_release(JNIEnv *env, jobject thiz)
{
    ...
    if (mp != NULL) {
        // this prevents native callbacks after the object is released
				mp->stop();
        //mp->disconnect();
				VOINFO("set NULL listener...");
        mp->setListener(0);
				VOINFO("NULL listener already...");
    }
}
...
static void vomp_native_init(JNIEnv *env,  jobject thiz, jstring path) {
	...
	if (strlen(pathStr) < 128)
		strcpy(g_szpackagepath, pathStr);
	else
		strcpy(g_szpackagepath, "/data/data/com.visualon.vome");
