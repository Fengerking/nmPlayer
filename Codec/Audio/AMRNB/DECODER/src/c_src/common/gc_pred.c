...
**************************************************************************
*/
int gc_pred_init (gc_predState **state, VO_MEM_OPERATOR *pMemOP)
{
	gc_predState* s;
	...
	*state = NULL;
	/* allocate memory */
	if ((s= (gc_predState *) voAMRNBDecmem_malloc(pMemOP, sizeof(gc_predState), 32)) == NULL){
		return -1;
	}
	gc_pred_reset(s);
	*state = s;
...
**************************************************************************
*/
void gc_pred_exit (gc_predState **state, VO_MEM_OPERATOR *pMemOP)
{
...
{
	voAMRNBDecCopy (st_src->past_qua_en, st_dest->past_qua_en, NPRED);
	voAMRNBDecCopy (st_src->past_qua_en_MR122, st_dest->past_qua_en_MR122, NPRED);
}
		...
		)
{
	...
	*-------------------------------------------------------------------*/
...
#ifdef  ARMv7_opt
    ener_code = voAMRNBDecsum_armv7_xx(ener_code, code, code, L_SUBFR);
#else
	ener_code = code[0]*code[0]<<1;
	/* MR122:  Q12*Q12 -> Q25 */
	/* others: Q13*Q13 -> Q27 */
	for (i = 1; i < L_SUBFR; i++)
	{
		ener_code = (ener_code +  ((code[i]*code[i])<<1));
		i++;
		ener_code = (ener_code +  ((code[i]*code[i])<<1));
		i++;
		ener_code = (ener_code +  ((code[i]*code[i])<<1));
	...
	if ((mode - MR122) == 0)
	{
		...
		ener_code = ((ener_code+0x00008000)>>16)*26214<<1;;//L_mult (round (ener_code), 26214);   /* Q9  * Q20 -> Q30 */

		/*-------------------------------------------------------------------*
		*  energy of code:                                                  *
		*  ~~~~~~~~~~~~~~~                                                  *
		*  ener_code(Q17) = 10 * Log10(energy) / constant                   *
		*                 = 1/2 * voAMRNBDecLog2(energy)                              *
		*                                           constant = 20*Log10(2)  *
		*-------------------------------------------------------------------*/
		/* ener_code = 1/2 * voAMRNBDecLog2(ener_code); Note: voAMRNBDecLog2=log2+30 */
		voAMRNBDecLog2(ener_code, &exp, &frac);
		ener_code = ((exp - 30)<<16) +( frac<<1);// voAMRNBDecL_Comp ( (exp - 30), frac);     /* Q16 for log()    */
		/* ->Q17 for 1/2 log()*/
		...
		*-------------------------------------------------------------------*/
		...
		//for (i = 0; i < NPRED; i++)
		{
			ener = (ener+((st->past_qua_en_MR122[0]* pred_MR122[0])<<1));
			ener = (ener+((st->past_qua_en_MR122[1]* pred_MR122[1])<<1));
			ener = (ener+((st->past_qua_en_MR122[2]* pred_MR122[2])<<1));
			ener = (ener+((st->past_qua_en_MR122[3]* pred_MR122[3])<<1));
		}
		...
		*-------------------------------------------------------------------*/
		...
		voAMRNBDecL_Extract(ener, exp_gcode0, frac_gcode0);
	}
	else /* all modes except 12.2 */
	{
		...
		*-----------------------------------------------------------------*/
		...
		/* voAMRNBDecLog2 = log2 + 27 */
		voAMRNBDecLog2_norm (ener_code, exp_code, &exp, &frac);
		...
		*         = K - fact * voAMRNBDecLog2(ener_code)
		...
		*-----------------------------------------------------------------*/
		...
		L_tmp = (L_tmp>> 8);                   /*          -> Q16 */
		voAMRNBDecL_Extract(L_tmp, exp_gcode0, frac_gcode0); /*       -> Q0.Q15 */
	}
}
