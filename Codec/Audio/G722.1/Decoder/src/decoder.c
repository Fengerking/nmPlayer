...
//			 Word16 *old_decoder_mlt_coefs,
//			 Word16 frame_error_flag)
void  decoder(G722DecState *st, Word16 *decoder_mlt_coefs,Word16 *p_mag_shift)
{
    Bit_Obj  *bitobj = (Bit_Obj *)st->bitobj;
	Rand_Obj *randobj = (Rand_Obj *)st->randobj;
	Word16   number_of_regions = st->number_of_regions;
	Word16   *p_old_mag_shift  = &(st->old_mag_shift);
	Word16   *old_decoder_mlt_coefs = st->old_decoder_mlt_coefs;
	Word16   frame_error_flag = st->frame_error_flag;
	...
	if (frame_error_flag == 0) 
	{
		...
		bitobj->number_of_bits_left = bitobj->number_of_bits_left - num_categorization_control_bits;
							...
							Word16 *decoder_category_balances)
{
	...
	while (categorization_control > 0) 
	{
		...
		decoder_power_categories[region] += 1;
...
Word16 index_to_array(Word16 index,Word16 *array,Word16 category)
{
	...
	for (j=temp; j>=0; j--) 
	{
		...
		temp = (Word16)(L_mult0(q,max_bin_plus_one));
		array[j] = sub(p,temp);
		p = q;
		temp = array[j];
		if (temp != 0) 
