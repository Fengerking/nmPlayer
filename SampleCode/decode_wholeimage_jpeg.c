...
#endif//_WIN32_WCE
{
	...
	#else
	TCHAR *path = _T("E:/MyResource/Video/clips/jpeg/148_JPEG_JPEG_600x419.jpg");
	#endif
#endif
	long total = 0;
	long total1 = 0;
...
#ifdef LINUX
#else
	if(!(out_file = fopen("E:/MyResource/Video/clips/jpeg/LENA.yuv", "w+b"))){
		printf("Open output file error!");
	}
#endif
#endif
	...
	total += end - begin;
	{
		FILE *fp =fopen("/data/local/performance.txt","a");
		fprintf(fp,"%d ms \n",total);
		fclose(fp);
	}
#else
	...
	if(ret == VO_ERR_NONE)
		{
			...
			/*unsigned char* Rp = outdata.Buffer[0];
			unsigned char* Gp = outdata.Buffer[1];
			unsigned char* Bp = outdata.Buffer[2];
   ...
   #else
			writebmpfile("E:/MyResource/Video/clips/jpeg/LENA.bmp",image,width,height);
   #endif
#endif
			free(image);
		}
