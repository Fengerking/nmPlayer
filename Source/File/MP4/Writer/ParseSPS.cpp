...
static unsigned char* GetNextFrame(unsigned char* currPos,int size)
{
	unsigned char* p = currPos+3;  
...
//static unsigned char testAVCCfg[] = {0x01,0x42 ,0xc0 ,0x33 ,0xff ,0xe1 ,0x00 ,0x17 ,0x67 ,0x42 ,0xc0 ,0x33 ,0xab ,0x40 ,0x28 ,0x02
...
int H264Stream2AVCConfig(unsigned char * inBuf, int inSize,unsigned char* outBuf,int* outSize)
{
	//refer 14496-15 5.2.4.1.1
	int prefixLen = inBuf[2]==1?3:4;
	unsigned char* pData = inBuf + prefixLen;
	int spsSize,ppsSize,size=0;
	unsigned char* buffer;
	unsigned char* pps = GetNextFrame(inBuf,inSize);
	if ((pData[0]&0x0f)!=7||!pps)
	{
		printf("this is not a valid sps or pps\n");
		return -1;
	}
