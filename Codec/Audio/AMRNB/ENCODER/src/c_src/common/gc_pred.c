...
**************************************************************************
*/
int voAMRNBEnc_gc_pred_init (gc_predState **state, VO_MEM_OPERATOR *pMemOP)
{
	gc_predState* s;
	...
	*state = NULL;
	/* allocate memory */
	if ((s= (gc_predState *)voAMRNBEnc_mem_malloc(pMemOP, sizeof(gc_predState), 32)) == NULL){
		return -1;
	}
	voAMRNBEnc_gc_pred_reset(s);
	*state = s;
	return 0;
}
/*************************************************************************
*
*  Function:   voAMRNBEnc_gc_pred_reset
*  Purpose:    Initializes state memory to zero
*
**************************************************************************
*/
int voAMRNBEnc_gc_pred_reset (gc_predState *state)
{
	Word16 i;
	if (state == (gc_predState *) NULL){
		return -1;
...
**************************************************************************
*/
void voAMRNBEnc_gc_pred_exit (gc_predState **state, VO_MEM_OPERATOR *pMemOP)
{
...
*
*************************************************************************/
void voAMRNBEnc_gc_pred(
			 gc_predState *st,   /* i/o: State struct                           */
			 enum Mode mode,     /* i  : AMR mode                               */
			 Word16 *code,       /* i  : innovative codebook vector (L_SUBFR) MR122: Q12, other modes: Q13 */
			 Word16 *exp_gcode0, /* o  : exponent of predicted gain factor, Q0  */
			 Word16 *frac_gcode0,/* o  : fraction of predicted gain factor  Q15 */
			 Word16 *exp_en,     /* o  : exponent of innovation energy, Q0 (only calculated for MR795) */
			 Word16 *frac_en     /* o  : fraction of innovation energy, Q15 (only calculated for MR795) */
			 )
{
	Word32  i;
	Word32  ener_code = 0;
	...
	*-------------------------------------------------------------------*/
...
#else
	ener_code = L_mac((Word32) 0, code[0], code[0]);/* MR122:  Q12*Q12 -> Q25 */ /* others: Q13*Q13 -> Q27 */
	for (i = 1; i < L_SUBFR; i++)
	...
	if (mode == MR122)
	{
		...
		ener_code = vo_round(ener_code)*26214<<1;;//L_mult (round (ener_code), 26214);   /* Q9  * Q20 -> Q30 */
		/*-------------------------------------------------------------------*
		*  energy of code:                                                  *
		*  ~~~~~~~~~~~~~~~                                                  *
		...
		*-------------------------------------------------------------------*/
		...
		voAMRNBEnc_Log2(ener_code, &exp, &frac);
		ener_code = ((exp - 30)<<16) + (frac<<1);// L_Comp ( (exp - 30), frac);     /* Q16 for log()    */
		/* ->Q17 for 1/2 log()*/
		/*-------------------------------------------------------------------*
		*  predicted energy:                                                *
		*  ~~~~~~~~~~~~~~~~~                                                *
		...
		*-------------------------------------------------------------------*/
		...
		ener += ((st->past_qua_en_MR122[0]* pred_MR122[0])<<1);
		ener += ((st->past_qua_en_MR122[1]* pred_MR122[1])<<1);
		ener += ((st->past_qua_en_MR122[2]* pred_MR122[2])<<1);
		ener += ((st->past_qua_en_MR122[3]* pred_MR122[3])<<1);
		/*-------------------------------------------------------------------*
		...
		*exp_gcode0 = ener >> 16;
		*frac_gcode0 = (ener - ((*exp_gcode0) << 16))>>1;	
	...
	else /* all modes except 12.2 */
	{
		...
		*-----------------------------------------------------------------*/
		...
		/* Log2 = log2 + 27 */
		voAMRNBEnc_Log2_norm (ener_code, exp_code, &exp, &frac);
...
*
*************************************************************************/
void voAMRNBEnc_gc_pred_update(
					gc_predState *st,      /* i/o: State struct                     */
					Word16 qua_ener_MR122, /* i  : quantized energy for update, Q10 (log2(qua_err)) */
					Word16 qua_ener        /* i  : quantized energy for update, Q10 (20*log10(qua_err)) */
					)
{
	Word16 i;
	for (i = 3; i > 0; i--)
...
*
*************************************************************************/
void voAMRNBEnc_gc_pred_average_limited(
							 gc_predState *st,       /* i: State struct                    */
							 Word16 *ener_avg_MR122, /* o: everaged quantized energy,  Q10 (log2(qua_err))  */
							 Word16 *ener_avg        /* o: averaged quantized energy,  Q10 (20*log10(qua_err)) */
							 )
{
	Word16 av_pred_en;
	/* do average in MR122 mode (log2() domain) */
	av_pred_en =  st->past_qua_en_MR122[0];
	av_pred_en += st->past_qua_en_MR122[1];
	av_pred_en += st->past_qua_en_MR122[2];
	av_pred_en += st->past_qua_en_MR122[3];
	/* av_pred_en = 0.25*av_pred_en */
	av_pred_en =(av_pred_en << 13)>>15; //mult (av_pred_en, 8192);
	...
	*ener_avg_MR122 = av_pred_en;  
	...
	av_pred_en  =  st->past_qua_en[0];
	av_pred_en +=  st->past_qua_en[1];
	av_pred_en +=  st->past_qua_en[2];
	av_pred_en +=  st->past_qua_en[3];
	/* av_pred_en = 0.25*av_pred_en */
	av_pred_en =(av_pred_en<<13)>>15;// mult (av_pred_en, 8192);
