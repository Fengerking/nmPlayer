				...
				Word16 *category_balances)
{
	...
	if (temp > 0)
	{
		number_of_available_bits = number_of_available_bits - frame_size;
	...
	offset = calc_offset(rms_index,
		                 number_of_regions,
						 number_of_available_bits);

	/* compute the power categories based on the uniform offset */
	compute_raw_pow_categories(power_categories,
		                       rms_index,
							   number_of_regions,
							   offset);
	...
	comp_powercat_and_catbalance(power_categories,
		                         category_balances,
								 rms_index,
								 number_of_available_bits,
								 number_of_regions,
								  ...
								  Word16 offset)
{
	...
	for (j=0; j<num_categorization_control_possibilities-1; j++) 
	{
		...
		two_x_number_of_available_bits = number_of_available_bits << 1;
...
Word16 calc_offset(Word16 *rms_index,Word16 number_of_regions,Word16 available_bits)    
{
	...
	do 
	{
		...
		delta = delta >> 1;   /* for the while loop */
	} while (delta > 0);
	return(answer);
}
/***************************************************************************
