...
)
{
    ...
    AMRWBDecSetZero(code2, 2 * L_SUBFR);
    ...
    *prev_state = state;
    ...
    if (state == 0)
    {
        for (i = 0; i < L_SUBFR; i++)
        {
            if (code[i] != 0)
            {
                for (j = 0; j < L_SUBFR; j++)
                {
                    code2[i + j] += ((code[i] * ph_imp_low[j]) + 0x4000) >> 15;
                }
            }
        }
    } else if (state == 1)
    {
        for (i = 0; i < L_SUBFR; i++)
        {
            if (code[i] != 0)
            {
                for (j = 0; j < L_SUBFR; j++)
                {
                    code2[i + j] += ((code[i] * ph_imp_mid[j]) + 0x4000) >> 15;
                }
            }
        }
    }
    if (state < 2)
    {
        for (i = 0; i < L_SUBFR; i++)
        {
            code[i] = code2[i] + code2[i + L_SUBFR];
        }
    }   
#else
    Word16 i, j, state;
