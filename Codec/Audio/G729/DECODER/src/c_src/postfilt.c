				 ...
				 )
{
	...
	*-------------------------------------------------------------------*/
	...
	Word16 *Az;                 /* pointer to Az_4: LPC parameters in each subframe */
...
#ifdef ASM_OPT
	    Weight_Az_asm(Az, GAMMA2_PST, M, Ap3);
        Weight_Az_asm(Az, GAMMA1_PST, M, Ap4);
#else
...
#ifdef  ASM_OPT
        Residu_asm(Ap3, &syn[i_subfr], res2, L_SUBFR);
#else
...
#if 0 // ASM_OPT 
        Copy_asm(&res2[L_SUBFR-PIT_MAX], &res2[-PIT_MAX], PIT_MAX);
	    Copy_asm(&scal_res2[L_SUBFR-PIT_MAX], &scal_res2[-PIT_MAX], PIT_MAX);
#else
				  ...
				  )
{
	...
	*---------------------------------------------------------------------------*/
...
#ifdef ASM_OPT
		corr = sum_mac_v4_xy(corr, p, p1, L_subfr);
#else
		for (j=0; j<L_subfr; j++)
...
#ifdef  ASM_OPT
	ener0 = sum_mac_v4(ener0, p, L_subfr);
#else
	for ( i=0; i<L_subfr; i++, p++)
	...
	if(cmax > en)      /* if pitch gain > 1 */
	{
		...
		cmax = (mult(cmax, GAMMAP) >> 1);  /* cmax(Q14) = cmax(Q15) * GAMMAP */
		en = (en >> 1);          /* Q14 */
		i = add(cmax, en);
		if(i > 0)
		{
