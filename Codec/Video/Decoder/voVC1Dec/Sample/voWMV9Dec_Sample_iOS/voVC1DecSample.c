...
int WINAPI WinMain(HINSTANCE hInstance,
                   ...
                   int        CmdShow)
#elif defined (_IOS)
int beginTest(char* inSrcFile, char* outYUVFile, char* outSpeedFile,int cpu,int times)
#else               
int		 main( )
#endif
{
...
#define MAX_HEADER_SIZE	1024
	...
	do 
	{
         ...
         else*/
		{
			...
			strcpy(outFileName, outYUVFile);
            strcpy(outSpeedName,outSpeedFile);
            printf("%s \n",outSpeedFile);
		...
		if (outSpeedName[0] != '\0') {
			speedFile = fopen(outSpeedName, "ab");
		}
		else {
			speedFile = NULL;
		}
...
#		if defined(PPC2003) || defined(PPC2005)
            ...
            End:
                ...
                if (speedFile) {
                    fprintf(speedFile, " total frame = %d, total time = %2.3f fps. (%dx%d)\n", frameNumber, frameNumber/duration,outFormat.Format.Width, outFormat.Format.Height);
                }
