...
)
{
    ...
    *gain_cod = extract_h(L_tmp);
     ...
     *------------------------------------------------------------------*/
    voAMRNBEnc_Log2 (L_deposit_l(g_code), &exp, &frac); /* Log2(x Q12) = log2(x) + 12 */
    exp = (exp - 12);
    ...
    qua_ener = vo_round (L_shl(L_tmp, 13)); /* Q12 * Q0 = Q13 -> Q10 */

    voAMRNBEnc_gc_pred_update(pred_st, qua_ener_MR122, qua_ener);
}
 ...
 *
 *************************************************************************/
void voAMRNBEnc_MR475_update_unq_pred(
    gc_predState *pred_st, /* i/o: gain predictor state struct            */
...
)
{
    ...
    else
    {
        ...
        frac_gcode0 = extract_l (voAMRNBEnc_Pow2(14, frac_gcode0));
        ...
        */
        ...
        voAMRNBEnc_Log2 (L_deposit_l (frac), &exp, &frac);
        exp = (exp + tmp);
        ...
        else
        {
            ...
            qua_ener = vo_round (L_shl(L_tmp, 13));
            /* Q12 * Q0 = Q13 -> Q26 -> Q10     */
        }
    }
 ...
 *
 *************************************************************************/
Word16 voAMRNBEnc_MR475_gain_quant(              /* o  : index of quantization.                 */
    gc_predState *pred_st,     /* i/o: gain predictor state struct            */
...
)
{
     ...
     *-------------------------------------------------------------------*/
    sf0_gcode0 = extract_l(voAMRNBEnc_Pow2(14, sf0_frac_gcode0));
    sf1_gcode0 = extract_l(voAMRNBEnc_Pow2(14, sf1_frac_gcode0));
    /*
    ...
    voAMRNBEnc_gc_pred(pred_st, MR475, sf1_code_nosharp,
