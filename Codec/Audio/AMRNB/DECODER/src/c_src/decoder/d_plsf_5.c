...
#define ALPHA     31128   /* ALPHA    ->  0.95  */
#define ONE_ALPHA 1639    /* ONE_ALPHA-> (1.0-ALPHA) */
			  ...
			  )
{
	...
	if (bfi != 0)                               /* if bad frame */
	{
		...
		for (i = 0; i < M; i++)
		{
			/* lsfi_q[i] = ALPHA*st->past_lsf_q[i] + ONE_ALPHA*voAMRNBDecmean_lsf[i]; */
			lsf1_q[i] = add(mult (st->past_lsf_q[i], ALPHA), mult (voAMRNBDecmean_lsf[i], ONE_ALPHA));
			lsf2_q[i] = lsf1_q[i]; 
		}
		/* estimate past quantized residual to be used in next frame */
		for (i = 0; i < M; i++)
		{
			/* temp  = voAMRNBDecmean_lsf[i] +  st->past_r_q[i] * LSP_PRED_FAC_MR122; */
			temp = add (voAMRNBDecmean_lsf[i], mult (st->past_r_q[i], LSP_PRED_FAC_MR122));
		...
		/* if good LSFs received */
	{
		...
		p_dico = &voAMRNBDecdico1_lsf[(indice[0] << 2)];  
		lsf1_r[0] = *p_dico++;                  
		lsf1_r[1] = *p_dico++;                  
		lsf2_r[0] = *p_dico++;                  
		lsf2_r[1] = *p_dico++;                  

		p_dico = &voAMRNBDecdico2_lsf[(indice[1] << 2)];  
		lsf1_r[2] = *p_dico++;                  
		lsf1_r[3] = *p_dico++;                  
		lsf2_r[2] = *p_dico++;                  
		lsf2_r[3] = *p_dico++;                  
		...
		sign = indice[2] & 1;                   
		i = (indice[2]>> 1);
		p_dico = &voAMRNBDecdico3_lsf[(i << 2)];          
		...
		p_dico = &voAMRNBDecdico4_lsf[(indice[3] << 2)];  
		lsf1_r[6] = *p_dico++;                  
		lsf1_r[7] = *p_dico++;                  
		lsf2_r[6] = *p_dico++;                  
		lsf2_r[7] = *p_dico++;                  

		p_dico = &voAMRNBDecdico5_lsf[(indice[4] << 2)];  
		lsf1_r[8] = *p_dico++;                  
		lsf1_r[9] = *p_dico++;                  
		lsf2_r[8] = *p_dico++;                  
		lsf2_r[9] = *p_dico++;                  
		...
		for (i = 0; i < M; i++)
		{
			temp = voAMRNBDecmean_lsf[i] + ((st->past_r_q[i] * LSP_PRED_FAC_MR122)>>15);
	...
	voAMRNBDecCopy (lsf2_q, st->past_lsf_q, M);
