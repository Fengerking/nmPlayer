...
//    CoefType* pctDst      = (CoefType*) piDst;
...
#if WMA_OPT_FOR_SPEED   //no need to read other channels
#else
...
#if WMA_OPT_FOR_SPEED  //only use ctCh0
			sTemp = DNMIXROUND(ctCh0);
#else
...
#if WMA_OPT_FOR_SPEED  //only use ctCh1
			sTemp = DNMIXROUND(ctCh1);
#else
    ...
    else if (cSrcChannel >= cDstChannel)
    {
        ...
        {
            // Initialize
            pctTemp = (CoefType*)memset(pctTemp, 0, sizeof(CoefType) * cDstChannel);
            for (iChDst = 0; iChDst < cDstChannel; iChDst++)
            {
				int optForSpeedflag = WMA_OPT_FOR_SPEED;
				if(optForSpeedflag&&cSrcChannel>2){
					pctTemp[iChDst] = GET_COEFF(iChDst);
#ifdef BOOST_CHANFLDDN_POWER
					if (bComputeFoldDownGain)
					{
						CoefType ctTemp;
	...
	//					printf("c: 0x%x\n", c);
            ...
            {
				U16 *temp;
//				printf("pctTemp[%d]: 0x%x\n", iChDst, pctTemp[iChDst]);
                sTemp = DNMIXROUND(pctTemp[iChDst]);
                DNMIXCLIP(sTemp);
                SET_COEFF(sTemp, iChDst, iBlock);
//				printf("paudec->m_ppcbtTemp[0] address: 0x%x and piDst address: 0x%x\n", paudec->m_ppcbtTemp[0], (int)piDst);
//				printf("paudec->m_ppcbtTemp[%d][%d]: 0x%x\n", iChDst, iBlock, paudec->m_ppcbtTemp[iChDst][iBlock]);
				temp = (U16 *)piDst;
//				printf("piDst address and value: 0x%x, 0x%x\n", (int)piDst, temp[0]);
            }
                                ...
                                U8 *piDst)
{
    ...
    for (iBlock = 0; iBlock < nBlocks; iBlock++)
    {
        ...
        {
            Int nOut;
            wmaResult = ltrtDownmix(paudec, paudec->m_pLtRtDownmix, iSamples, &nOut);
