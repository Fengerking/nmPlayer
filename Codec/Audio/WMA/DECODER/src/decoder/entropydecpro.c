...
{      
    ...
    while (paudec->m_ppxformsts != PPXFORM_DONE) {
        ...
        case CHXFORM_BEGIN:
            ...
            {
                paudec->m_rgfltPostProcXformPrev = (ChPPXFormType *)voWMAMemcpy(paudec->m_rgfltPostProcXformPrev,
            ...
            if (paudec->m_rgfltPostProcXform)
                paudec->m_rgfltPostProcXform = (ChPPXFormType *)memset(paudec->m_rgfltPostProcXform, 
...
WMARESULT prvDecodeMaskHeaderV3_Channel (CAudioObjectDecoder* paudec, Int iCh)
{
        ...
        ppcinfo->m_fAnchorMaskAvailable == WMAB_TRUE) 
    {
        ...
        paudec->pau->m_rgpcinfo[iCh].m_rgiMaskQ = (Int *)voWMAMemcpy (paudec->pau->m_rgpcinfo[iCh].m_rgiMaskQ, paudec->pau->m_rgpcinfo[iCh].m_rgiMaskQResampled, sizeof(Int) * paudec->pau->m_cValidBarkBand);
    }
...
//          ppcinfo->m_rgiHuffDecTbl = g_wmaHuffDecTbls_3_SS;
          ppcinfo->m_rgiHuffDecTbl = g_wmaHuffDecTbls_runlevel_3_SS;
...
//          ppcinfo->m_rgiHuffDecTbl = g_wmaHuffDecTbls_3_SM;
          ppcinfo->m_rgiHuffDecTbl = g_wmaHuffDecTbls_runlevel_3_SM;
...
//	Int bitCnt;
	...
	while (1) {
		...
		case GET_VEC4:
			...
			TRACEWMA_EXIT(wmaResult, huffDecGetMIXVec(g_wmaHuffTbls_0_0_Sym2VEC4, pibstrm, &HVResult));
			if (HVResult.state) 
			{
				pState->getState = GET_VEC2;
				TRACEWMA_EXIT(wmaResult, huffDecGetMIXVec(g_wmaHuffTbls_1_0_Sym2VEC2, pibstrm, &HVResult));
				if (HVResult.state) 
				{
					pState->getState = GET_VEC1;
					TRACEWMA_EXIT(wmaResult, huffDecGet_i(g_wmaHuffDecTbls_2_0, pibstrm,(U32*)&iResult, NULL));
					if (iResult == (Int)g_wmaVec1NSyms) 
					{
						pState->getState = GET_ESC_VEC1;
						TRACEWMA_EXIT(wmaResult,prvDecodeEscapeLargeVal(&iResult, g_wmaVec1NSyms,
																pibstrm,
																paudec, pState));
						pState->vec[0] = iResult;
					}
					else {
						pState->vec[0] = iResult;
					}
					...
					pState->vec[0]	= (HVResult.iResult >> 4) & 0xf;
					pState->vec[1]	= (HVResult.iResult     ) & 0xf;
					pState->vecIdx += 2;
				}
				pState->getState = GET_VEC2;
				TRACEWMA_EXIT(wmaResult, huffDecGetMIXVec(g_wmaHuffTbls_1_0_Sym2VEC2, pibstrm, &HVResult));
				if (HVResult.state) 
				{
					...
					pState->vec[2]	= (HVResult.iResult >> 4) & 0xf;
					pState->vec[3]	= (HVResult.iResult     ) & 0xf;
				}
			}

			else {
				iResult = HVResult.iResult;
				pState->vec[0] = (iResult >> 12) & 0xf;
				pState->vec[1] = (iResult >>  8) & 0xf;
				pState->vec[2] = (iResult >>  4) & 0xf;
				pState->vec[3] = (iResult      ) & 0xf;
			}
			pState->getState= GET_SIGN4;
			TRACEWMA_EXIT(wmaResult, ibstrmPeekBits(pibstrm, 4, (U32 *)&iResult));
			{
		...
		case GET_VEC2:
			...
			if (HVResult.state) 
				...
				pState->vec[pState->vecIdx]   = (HVResult.iResult >> 4) & 0xf;
				pState->vec[pState->vecIdx+1] = (HVResult.iResult     ) & 0xf;
				pState->vecIdx += 2;
				if (pState->vecIdx == 4) pState->getState = GET_SIGN4;
			}
			break;
...
//	Int bitCnt;
	...
	while (1) {
		switch (pState->getState) {
		case DECODE_RL:
			TRACEWMA_EXIT(wmaResult, huffDecGetMix(ppcinfo->m_rgiHuffDecTbl, pibstrm, &huffResult, (U32*)&uSign));
			if (huffResult.state == 0) {
				pState->getState = DECODE_ESC_RLLEVEL;
				pau->m_iSign = 0;
				break;
			}
			else if (huffResult.state == 1) {
				pau->m_iLevel = 0;
				pau->m_cRunOfZeros = (I16) (ppcinfo->m_cSubbandActual - pau->m_iCurrReconCoef - 1);
				pau->m_iSign = 0;
				goto exit;
				...
				TRACEWMA_EXIT(wmaResult, ibstrmFlushBits(pibstrm, 1));
				pState->curRun += huffResult.run;
				pau->m_cRunOfZeros = pState->curRun;
				pState->curRun = 0;
				pau->m_iLevel = huffResult.level;
				pau->m_iSign = (I32)(uSign >> 31) - 1;
				goto exit;
			}
		case DECODE_ESC_RLLEVEL:
                               ...
                               PerChannelInfo* ppcinfo)
{
    ...
    switch (paudec->m_maskrlsts)  {
        case VLC:
            TRACEWMA_EXIT(wmaResult, huffDecGetMix (g_rgunHuffDecTbl_LevelRunMaskRLCV3, &paudec->m_ibstrm, &huffResult, &uSign));
            if (huffResult.state == 0)
                goto escape;
			if (huffResult.state == 1)
            {
                /*
