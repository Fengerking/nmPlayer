...
return (extract_h(L_shl2(Ltemp,15))); 
}

short ran_g(long *iset, long *gset, short *seed0)
{
#if (FUNC_RAN_G_OPT)
//	static int iset = 0;
//	static long gset;
...
/* ======================================================================== */
	if (!(*iset))
	{
		sv1 = (ran0(seed0) - 16384) << 1;
		sv2 = (ran0(seed0) - 16384) << 1;
 ...
 */
		...
		rsq_s = (short)(((rsq << shft_fctr) + 0x8000) >> 16);
		stmp2 = divide_s((short)((ltmp1 + 0x8000) >> 16), rsq_s);
		...
		ltmp2 = (long)stmp2 << (shft_fctr1 + 16);
		stemp1 = sqroot(ltmp2);
		...
		if (shft_fctr & 0x0001)
		{
			stemp1 = (short)(((long)stemp1 * 23170) >> 15);
			shft_fctr++;
		}
		 ...
		 */
		shft_fctr1 = norm_s((Word16)stemp1);
		stemp1 <<= shft_fctr1;
		shft_fctr = shft_fctr - shft_fctr1;

		*gset = ((long)sv1 * stemp1) << 1;
		...
		*gset = L_shl(*gset, (short)(shft_fctr - 2));

		*iset = 1;
		return round32(L_shl(((long)sv2 * stemp1 << 1), (short)(shft_fctr - 2)));
	}
	else
	{
		*iset = 0;
...
short e_ran_g(long *iset, long *gset, short *seed0)
{
#if (FUNC_E_RAN_G_OPT)
//	static int iset = 0;
//	static long gset;
 ...
 */
		...
		iset = 1;
		return round32(L_shl(L_mult(sv2, stemp1), (short)(shft_fctr - 2)));
	}
	else
	{
		iset = 0;
		...
		/* 7 */ short n)
{
	...
	if (!flag)
		evrc_ecom->ext_Seed = 1234;

	Sum = evrc_ecom->ext_Sum;
 ...
 */
	...
	for (k = 0; k < length; k++)
	{
		sum += (short)abs_s(input[k]);
	}
	sum <<= 1;	/* correct for scaling */

	if (sum < SubFrameSize)
	{
		sum = fnLog10((long)scale << 16);
		sum = -(((sum << 3) + 484942745));	/* add (log8)/4 = 484842745 (VALUE ADJUSTED TO BETTER MATCH FLOAT MODEL) */
	}
	else
	{
		lscale = ((long)SubFrameSize * scale) << 1;
		shft_scale = (short)(norm_l(lscale)); 
		sscale = round32(lscale << shft_scale); 
		shft_sum = (short)norm_l(sum) - 1; 
		ssum = (short)(((sum << shft_sum) + 0x8000) >> 16);
		...
		sum = (long)shl(divide_s(ssum, sscale), (short)((shft_scale - shft_sum))) << 16;
		/* sum = fnLog10(sum); */
		sum = fnLog10(sum) + 141412467;   /* add log (2^7) scaled down by 32 */
		sum = (sum << 3) + 969485490;	/* add (log8)/2 = 969685490 (VALUE ADJUSTED TO BETTER MATCH FLOAT MODEL) */
	}
	...
	if (n == NoOfSubFrames - 1)
	{
		...
		for (k = 0, sum = 2147483647, ptr = Logqtbl; k < 256; k++)
		{
			for (j = 0, tmp = 0; j < 3; j++)
			{
				 ...
				 */
				D = Sum[j] - (*ptr++);
				chen_temp_l = ((long)D * D) << 1;
				if (((chen_temp_l ^ tmp) & MIN_32) == 0)
			    {
			        tmp += chen_temp_l;
			        if ((chen_temp_l ^ tmp) & MIN_32)
		            {
		                tmp = MAX_32;
		            }
			    }
		...
		for (k = 0; k < FrameSize - 1; k++)
		{
			if (k >= j)
			{
				stmp = e_ran_g(&evrc_ecom->e_ran_g_iset, &evrc_ecom->e_ran_g_gset, &evrc_ecom->ext_Seed);
				stemp1 = k / (length - 1);
				output[k - j] = (short)((((long)stmp * Sum[stemp1] >> 4) + 0x8000) >> 16);
			}
		}
		stmp = e_ran_g(&evrc_ecom->e_ran_g_iset, &evrc_ecom->e_ran_g_gset, &evrc_ecom->ext_Seed);
		output[k - j] = (short)((((long)stmp * Sum[2] >> 4) + 0x8000) >> 16);	/* last excitation */
	}
 ...
 */
	...
	else
	{
		 ...
		 */
		/* sum = L_shl(L_divide(sum, lscale), (shft_scale + 7 - shft_sum)); */
		sum = L_deposit_h(shl(divide_s(ssum, sscale), (short)((shft_scale - shft_sum))));
		/* sum = fnLog10(sum); */
...
void GetExc800bps_dec(ERVC_COMPONENT *evrc_ecom, 
					  short *output, 
					  short length, 
					  short best, 
					  short flag, 
...
#define P333	10923			/* (1/3) */
	...
	if (n == 0)
	{
		...
		else
		{
			...
			for (j = 0; j < 3; j++)
			{
				Ltemp += (long)Powqtbl[evrc_ecom->ext_PrevBest * 3 + j] * P333;
			}
			Ltemp <<= 1;
				
			for (j = 0; j < 3; j++)
			{
				Sum[j] = (short)((Ltemp + 0x8000) >> 16);
			}
		}
	}
...
/* NOTE: Logqtbl[] and pow() function has been replaced with Powqtbl[] */
	...
	for (i = 0; i < length; i++)
	{
		temp = ran_g(&evrc_ecom->ran_g_iset, &evrc_ecom->ran_g_gset, &evrc_ecom->ext_Seed); 
		Ltemp = (long)sum * temp;
		output[i] = (short)((Ltemp + 0x80000) >> 20);
	}
