...
{
  Word16 temp0, temp1, temp2, temp3, i,idx;
  Word16 data0, data1;
  ...
  {
     idx = i<<2;
     temp0 = ((in[idx]>> 2) -  (COEFF5_1* data0>>15));
     temp1 = add(data0, (COEFF5_1* temp0>>15));

     temp3 = ((in[idx+1]>> 2) - (COEFF5_2* data1>>15));
     temp2 = add(data1, (COEFF5_2*temp3>>15));
     
     out[idx] = add(temp1, temp2);                       
     out[idx+1] = sub(temp1,temp2);                        
     
     data0 = ((in[idx+2]>> 2) - (COEFF5_1 * temp0>>15));
     temp1 = add(temp0, (COEFF5_1 * data0>>15));
     
     data1 = ((in[idx+3]>>2) - (COEFF5_2* temp3>>15));
     temp2 = add(temp3, (COEFF5_2*data1>>15));
     
     out[idx+2] = add(temp1, temp2);                       
     out[idx+3] = sub(temp1, temp2);                      
  } 
 ...
 *
 ***************************************************************************/
__voinline void filter5(Word16 *in0,    /* i/o : input values; output low-pass part  */
                    ...
                    )
{
  ...
  temp0 = (*in0 - (COEFF5_1*data[0]>>15));
  temp1 = add(data[0], (COEFF5_1* temp0>>15));
  data[0] = temp0;                               

  temp0 = (*in1 - (COEFF5_2 * data[1]>>15));
  temp2 = add(data[1], (COEFF5_2 * temp0>>15));
  data[1] = temp0;                               

  *in0 = (add(temp1, temp2)>> 1);              
  *in1 = (sub(temp1, temp2)>> 1);                  
}
 ...
 *
 ***************************************************************************/
__voinline void filter3(Word16 *in0,   /* i/o : input values; output low-pass part  */ 
                    Word16 *in1,   /* i/o : input values; output high-pass part */
                    Word16 *data   /* i/o : updated filter memory               */
                    )
{
  ...
  temp1 = (*in1 - (COEFF3 *(*data)>>15));
  temp2 = add(*data, (COEFF3 * temp1>>15));
  *data = temp1;                              
    ...
    )
{
  ...
  l_temp2 = L_add(l_temp1, L_shl2(*sub_level, (16 - scale)));
  *sub_level = extract_h(L_shl2(l_temp1, scale));
         ...
         )
{
   ...
   st->corr_hp_fast = vo_round(L_tmp);           /* Q15 */    
...
/*************************************************************************
*
*  Function:   voAMRNBEnc_vad1_init
*  Purpose:    Allocates state memory and initializes state memory
*
**************************************************************************
*/
int voAMRNBEnc_vad1_init (vadState1 **state, VO_MEM_OPERATOR *pMemOP)
{
    vadState1* s;
    ...
    *state = NULL;
    ...
    /* allocate memory */
    if ((s = (vadState1 *)voAMRNBEnc_mem_malloc(pMemOP, sizeof(vadState1), 32)) == NULL){
        return -1;
    }
    
    voAMRNBEnc_vad1_reset(s);
    *state = s;
...
/*************************************************************************
*
*  Function:   voAMRNBEnc_vad1_reset
*  Purpose:    Initializes state memory to zero
*
**************************************************************************
*/
int voAMRNBEnc_vad1_reset (vadState1 *state)
{
   Word16 i, j;
...
/*************************************************************************
*
*  Function:   voAMRNBEnc_vad1_exit
*  Purpose:    The memory used for state memory is freed
*
**************************************************************************
*/
void voAMRNBEnc_vad1_exit (vadState1 **state, VO_MEM_OPERATOR *pMemOP)
{
...
/****************************************************************************
 *
 *     Function     : voAMRNBEnc_vad_complex_detection_update
 *     Purpose      : update vad->bestCorr_hp  complex signal feature state 
 *
 ***************************************************************************/
void voAMRNBEnc_vad_complex_detection_update (vadState1 *st,       /* i/o : State struct */
...
/****************************************************************************
 *
 *     Function     : voAMRNBEnc_vad_tone_detection
 ...
 *
 ***************************************************************************/
void voAMRNBEnc_vad_tone_detection (vadState1 *st,  /* i/o : State struct            */
...
/****************************************************************************
 *
 *     Function     : voAMRNBEnc_vad_tone_detection_update
 ...
 *                    encoder before call to voAMRNBEnc_vad_tone_detection() function.
 *
 ***************************************************************************/
void voAMRNBEnc_vad_tone_detection_update (
                vadState1 *st,              /* i/o : State struct              */
...
/****************************************************************************
 *
 *     Function     : voAMRNBEnc_vad_pitch_detection
 ...
 *
 ***************************************************************************/
void voAMRNBEnc_vad_pitch_detection (vadState1 *st,   /* i/o : State struct                  */
 ...
 *
 ***************************************************************************/
Word16 voAMRNBEnc_vad1(vadState1 *st,      /* i/o : State struct                 */
           ...
           )
{
   ...
   {  
      pow_sum = L_mac(pow_sum, in_buf[i-LOOKAHEAD], in_buf[i-LOOKAHEAD]);i++;
      pow_sum = L_mac(pow_sum, in_buf[i-LOOKAHEAD], in_buf[i-LOOKAHEAD]);i++;
      pow_sum = L_mac(pow_sum, in_buf[i-LOOKAHEAD], in_buf[i-LOOKAHEAD]);i++;
      pow_sum = L_mac(pow_sum, in_buf[i-LOOKAHEAD], in_buf[i-LOOKAHEAD]);i++;
      pow_sum = L_mac(pow_sum, in_buf[i-LOOKAHEAD], in_buf[i-LOOKAHEAD]);i++;
