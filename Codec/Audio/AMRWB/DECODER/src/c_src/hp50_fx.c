...
)
{
    ...
    do
    {
        ...
        L_tmp += y2_lo * a[2];
        L_tmp = L_tmp >> 14;
        L_tmp += (y1_hi * a[1] + y2_hi * a[2] + x0 * b[0] + x1 * b[1]+ x2 * b[2]) << 1;
        L_tmp <<= 2;           /* coeff Q12 --> Q13 */
        y2_hi = y1_hi;
        y2_lo = y1_lo;
		y1_hi = (Word16)(L_tmp>>16);
		y1_lo = (Word16)((L_tmp & 0xffff)>>1);
        L_tmp = L_shl2(L_tmp, 1);           /* coeff Q14 --> Q15 with saturation */
        *signal++ = vo_round(L_tmp);
    }while(--num !=0);
