...
************************************************************************/
/*******************************************************************************
File:		CAudioEncoder.cpp

Contains:	CAudioEncoder class file
...
VO_S32  CAudioEncoder::SetEncParam(VOEDT_OUTPUT_AUDIO_PARAM *param)
{
	memcpy(&m_outputParam,param,sizeof(VOEDT_OUTPUT_AUDIO_PARAM));
	return VO_ERR_NONE;
}
VO_U32 CAudioEncoder::Init ()
{
	VO_S32 nCodecType = m_outputParam.nCodecType;
	VO_S32 nSampleRate = m_outputParam.nSampleRate;
	VO_S32 nChannels = m_outputParam.nChannels;
	VO_S32 nSampleBits = m_outputParam.nBitRate;
	if (nCodecType > VO_AUDIO_CodingMIDI || nCodecType < VO_AUDIO_CodingPCM)
	{
		if (m_pError != NULL)
	...
	m_OutputInfo.Format.SampleRate = nSampleRate;
	m_OutputInfo.Format.Channels = nChannels;
	m_OutputInfo.Format.SampleBits = nSampleBits;
...
*/
	...
	while (nRC != VO_ERR_NONE)
	{
		...
		nRC = m_funEnc.Init (&m_hEnc, (VO_AUDIO_CODINGTYPE)nCodecType, &initInfo);
		if (m_hEnc == NULL)
		{
...
VO_U32 CAudioEncoder::UpdateParam (void)
{
	...
	case VO_AUDIO_CodingAAC:
		...
		aacpara.adtsUsed = m_outputParam.nAdtsUsed;
		aacpara.bitRate = m_outputParam.nBitRate;
		aacpara.nChannels = m_outputParam.nChannels;
		aacpara.sampleRate = m_outputParam.nSampleRate;
		SetParam(VO_PID_AAC_ENCPARAM, &aacpara);	
		break;
	case VO_AUDIO_CodingMP3:
		nRC |= SetParam(VO_PID_AUDIO_CHANNELS, &m_outputParam.nChannels);
		nRC |= SetParam(VO_PID_AUDIO_SAMPLEREATE ,&m_outputParam.nSampleRate);
		nRC |= SetParam(VO_PID_AUDIO_BITRATE, &m_outputParam.nBitRate);
		break;
	}
