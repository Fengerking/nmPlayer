...
*************************************************************************/
void
voAMRNBEnc_calc_unfilt_energies(
					 ...
					 )
{
	...
	for (i = 0; i < L_subfr; i++)
	{
		...
		tmp = sub(res[i], vo_round(L_temp));           /* LTP residual, Q0 */
		s = L_mac (s, tmp, tmp);
	}

	exp = norm_l(s);
	ltp_res_en = extract_h (s << exp);
	exp = 15 - exp;
...
*
*************************************************************************/
void voAMRNBEnc_calc_filt_energies(
				   ...
				   )
{
	...
	for (i = 1; i < L_SUBFR; i++)
		s = L_mac(s, xn[i], y2[i]);
	exp = norm_l(s);
	frac_coeff[3] = negate(extract_h(s << exp)); 
	exp_coeff[3] = (7 - exp);         
...
*
*************************************************************************/
void voAMRNBEnc_calc_target_energy(
						Word16 xn[],     /* i: LTP target vector,                       Q0  */
