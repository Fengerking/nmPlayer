...
@ .endm	
	...
	.align 4	
W_table:
		.long 0x0B190235			@w7 = D0.S16[0]	w1 = D0.S16[1]		
		.long 0x06490968			@w3 = D0.S16[2] w5 = D0.S16[3]			
		.long 0x0A740454			@w6 = D1.S16[0] w2 = D1.S16[1]		
		.long 0x000000b5			@181= D1.S32[1]	
	.align 4
_Armv7IdctA: @PROC
...
@endp
	...
	.align 4	
Armv7IdctCC: @PROC	
@ r0 = v, r1 = dst, r2 = dst_stride, r3 = Src, r12 = [sp] = src_stride
...
@ r0 = Block, r1 = dst, r2 = dst_stride, r3 = Src, r12 = [sp] = src_stride
	...
	cmp			r3, #0
	beq			IDCT_CPY_NOSRC_4x4_2
	LOAD_IDCT_SRC_ADD 0				
IDCT_CPY_NOSRC_4x4_2:	
	STR_IDCT 0	
...
@q8  x0 x1 d16-d17
@q11 x4 x5 d22-d23		d18
@q10 x3 x2 d20-d21	
@q9  x7 x6 d18-d19		d22				
@row one

@	int x0,x4,x3,x7,tmp@
@	x0 = (Blk[0] << 11) + 65536@ // +final rounding
@	x4 = Blk[1]@
@  	x3 = (Blk[2] << 11)@
@  	x7 = Blk[3]@	
	vmov.s32	q3, #65536	
	vshll.s16	q12, d20, #11
	vshll.s16	q13, d16, #11
	vadd.s32	q13, q13, q3
...
@    	x4 = tmp + (W2 - W6) * x4@
@    	x7 = tmp - (W2 + W6) * x7@	
	vmull.s16	q8, d22, D1[0]			@W6*x7
	vmlal.s16	q8, d18, D1[1]			@x4 = W6*x7 + W2*x4@
	vmull.s16	q3, d18, D1[0]			@W6*x4
	vmlsl.s16	q3, d22, D1[1]			@x7 = W6*x4 - W2*x7@
	
@	tmp = x0 + x3@
@	x0 -= x3@
	vadd.s32	q10, q13, q12		@tmp = x0 + x3@
	vsub.s32	q13, q13, q12		@x0 -= x3@
...
@	Blk[2] = (x0 - x7) >> 17@ //2
	...
	vadd.s32	q15, q10, q8		@Blk[ 0]
	vsub.s32	q14, q10, q8		@Blk[ 3]	
	vadd.s32	q12, q13, q3		@Blk[ 1]
	vsub.s32	q3, q13, q3		@Blk[ 2]
	...
	vshrn.s32	d16, q15, #15
	vshrn.s32	d18, q12, #15	
	vshrn.s32	d20, q3, #15
	vshrn.s32	d22, q14, #15			
@row tow
@q4 x0 x1 d8-d9
@q5 x4 x5 d10-d11
@q6 x3 x2 d12-d13
...
@    	x4 = tmp + (W2 - W6) * x4@
@    	x7 = tmp - (W2 + W6) * x7@	
	vmull.s16	q4, d14, D1[0]			@W6*x7
	vmlal.s16	q4, d10, D1[1]			@x4 = W6*x7 + W2*x4@
	vmull.s16	q3, d10, D1[0]			@W6*x4
	vmlsl.s16	q3, d14, D1[1]			@x7 = W6*x4 - W2*x7@
	
@	tmp = x0 + x3@
@	x0 -= x3@
	vadd.s32	q5, q13, q12		@tmp = x0 + x3@
	vsub.s32	q13, q13, q12		@x0 -= x3@
...
@	Blk[2] = (x0 - x7) >> 17@ //2
	...
	vadd.s32	q15, q5, q4		@Blk[ 0]
	vsub.s32	q14, q5, q4		@Blk[ 3]	
	vadd.s32	q12, q13, q3		@Blk[ 1]
	vsub.s32	q3, q13, q3		@Blk[ 2]
	...
	vshrn.s32	d8, q15, #15
	vshrn.s32	d10, q12, #15	
	vshrn.s32	d12, q3, #15
	vshrn.s32	d14, q14, #15
@ 	mov		pc,lr	
 .endm 
   .macro	Col8_4x8_D
