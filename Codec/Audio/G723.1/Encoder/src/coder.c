...
{
	int     i,j;
	Word16  Info, out_len=0;
	CODCNGDEF *CodCng = (CODCNGDEF*)st->cod_cng;
	CODSTATDEF *CodStat = (CODSTATDEF *)st->cod_state;
	VADSTATDEF *VadStat = (VADSTATDEF *)st->vad_state;
	...
	*/
	...
	/* Convert to Lsp */
	AtoLsp( LspVect, &UnqLpc[LpcOrder*(SubFrames-1)], st->cod_state->PrevLsp) ;

	/* Compute the Vad */
	Ftyp = (Word16) Comp_Vad(st) ;

	/* VQ Lsp vector */
	Line.LspId = Lsp_Qnt( LspVect, st->cod_state->PrevLsp);
	...
	*/
	...
	for ( i = 0 ; i < Frame ; i ++ )
		Dpnt[PitchMax+i] = st->input_buffer[i] ;
	...
	if(Ftyp != 1) {
		...
		*/
		...
		/* Save PrevWgt */
		for ( i = 0 ; i < PitchMax ; i ++ )
			CodStat->PrevWgt[i] = st->input_buffer[i+Frame-PitchMax] ;



		/* CodCng => Ftyp = 0 (untransmitted) or 2 (SID) */
		Cod_Cng(st, &Ftyp, &Line, QntLpc);

		/* Update the ringing delays */
		Dpnt = st->input_buffer;
		for( i = 0 ; i < SubFrames; i++ ) {

			/* Update exc_err */
			Update_Err(Line.Olp[i>>1], Line.Sfs[i].AcLg, Line.Sfs[i].AcGn, st);

			Upd_Ring( Dpnt, &QntLpc[i*LpcOrder], &PerLpc[i*2*LpcOrder],CodStat) ;
			Dpnt += SubFrLen;
		}
	}
		...
		*/
		...
		j = 0 ;
		for ( i = 0 ; i < SubFrames ; i ++ ) {
			Filt_Pw(st->input_buffer, Dpnt, (Word16) j , Pw[i] ) ;
			j += SubFrLen ;
		}
		free ( (char *) Dpnt ) ;
		...
		*/
		...
		for ( i = 0 ; i < SubFrames ; i ++ ) {
			...
			Sub_Ring( Dpnt, &QntLpc[i*LpcOrder], &PerLpc[i*2*LpcOrder], CodStat, Pw[i]) ;

			/* Compute adaptive code book contribution */
			Find_Acbk( Dpnt, ImpResp, st, &Line, (Word16) i ) ;

			/* Compute fixed code book contribution */
			Find_Fcbk( Dpnt, ImpResp, &Line, (Word16) i ,st) ;

			/* Reconstruct the excitation */
			Decod_Acbk( ImpResp, CodStat->PrevExc, Line.Olp[i>>1],Line.Sfs[i].AcLg, Line.Sfs[i].AcGn, st) ;
	...
	Line_Pack( &Line, st, Ftyp );

	Info = st->output_buffer[0] & (Word16)0x0003;
