	...
	*startoff += 2;
	...
	for(index = 0; index < numidf; index++)
	{
		...
		case EXIFTAG_DATETIMEORIGINAL:
			...
			strncpy((INT8*)Exifdir->DataTimeOriginal, (INT8*)ValuePtr, ByteCount < 19 ? ByteCount : 19);
			break;			
		case EXIFTAG_DATETIMEDIGITIZED:
			exiflib->enEXIFDir |= 0x00000010;
			strncpy((INT8*)Exifdir->DataTimeDigitized , (INT8*)ValuePtr, ByteCount < 19 ? ByteCount : 19);
			break;			
	...
	*startoff += 2;
	...
	for(index = 0; index < numidf; index++)
	{
		...
		case GPSTAG_MAPDATUM:
			...
			strncpy((INT8*)GPSdir->GPSMapDatum , (INT8*)ValuePtr, strlength);
			break;
		case GPSTAG_DATESTAMP:
			exiflib->enGPSDir |= 0x00080000;
			strncpy((INT8*)GPSdir->GPSDatastemp, (INT8*)ValuePtr, 11);
			break;
		}		
	}
	...
	*startoff += 2;
	...
	for(index = 0; index < numidf; index++)
	{
		...
		case TIFFTAG_MAKE:
			...
			strncpy((INT8*)Tiffdir->CameraMake, (INT8*)ValuePtr, ByteCount < 39 ? ByteCount : 39);
			break;			
		case TIFFTAG_MODEL:
			exiflib->enTIFFD |= 0x00000002;
			strncpy((INT8*)Tiffdir->CameraModel, (INT8*)ValuePtr, ByteCount < 39 ? ByteCount : 39);
			break;			
		case TIFFTAG_DATETIME:
			exiflib->enTIFFD |= 0x00000004;
			strncpy((INT8*)Tiffdir->DateTime, (INT8*)ValuePtr, 19);
			break;
