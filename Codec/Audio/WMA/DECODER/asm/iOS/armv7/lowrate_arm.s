...
@// there are 71 instructions in the unrolled code vs 25 in the loop.  
  ...
  .globl  _voWMAMemset	
  .globl  _g_InverseFraction
  .globl  _qstCalcQuantStep_ARM
  .globl  _prvInverseQuantizeLowRate
...
.set OFFiRecon                   , 0x00
.set OFFrgiCoefRecon             , 0x04
.set OFFcQSIMWFracBits           , 0x08
.set OFFcMaxWeightFracBits       , 0x0C
.set OFFuiQuantStepXInvMaxWeight , 0x10
.set OFFiShiftCoefQ              , 0x14
.set OFFiBark                    , 0x18
.set OFFrgbBandNotCoded_iBark    , 0x18
.set OFFiCoefQ                   , 0x1C
.set OffrgiNoisePower_iNoiseBand , 0x1C
.set OFFuiInvMaxWeight           , 0x20
.set OFFiBarkX4                  , 0x24
.set OFFrgiCoefQ                 , 0x28
.set OFFpffltSqrtBWRatio         , 0x28
.set OFFpWF_pCoefRecon           , 0x2C
.set OFFpiCoefQ                  , 0x2C
.set OFFrgbBandNotCoded          , 0x30
.set OFFrgffltSqrtBWRatio        , 0x34
.set OFFrgiNoisePower            , 0x38
.set OFFcFracBits                , 0x38
.set OFFptRandState              , 0x3C 
.set OFFffltNoisePower           , 0x40
.set OFFffltNoisePower_iFracBits , OFFffltNoisePower+FastFloat_iFracBits
.set OFFffltNoisePower_iFraction , OFFffltNoisePower+FastFloat_iFraction
.set OFFiUBLimit                 , 0x48
.set OFFcSubFrameSampleHalf      , 0x4C

.set OFFLocalSize                , 0x5c	@0x54
.set OFFLocalSaveReg             , 10*4
.set OFFpau                      , OFFLocalSize-4	@ 0+OFFLocalSaveReg+OFFLocalSize
.set OFFpWF                      , OFFLocalSize-8	@ 0x54	;4+OFFLocalSaveReg+OFFLocalSize
@OFFpWF2						equ OFFLocalSize-4
.set INV_MAX_WEIGHT_FRAC_BITS    , 30
...
pg_InverseFraction:  .word       _g_InverseFraction
...
r4cWFFracBits   .req  r4
r5QuantStepXMaxWeightXWeightFactor  .req  r5
r6iShiftCoefQ   .req  r6
r6iCoefScaled   .req  r6
r8piCoefRecon   .req  r8
r9qrand         .req  r9
r10prgiCoefQ    .req  r10
...
pciRandA:        .word       0x19660d      @ const U32 a = 1664525;
pciRandC:        .word       0x3c6ef35f    @ const U32 c = 1013904223;
pciDitherFactor: .word       0x51eb851f    @ const Int iDitherFactor = 0x51EB851F;     // LPC  0.04 * 2^35 
pCAudioObject_m_tRandState:  .word CAudioObject_m_tRandState
pWMA_E_BROKEN_FRAME:         .word 0x80040002
