...
offset1 		RN	9
	...
	add	$srcStride,$srcStride,#4
	add	$destStride,$destStride,#4	
	cmp		offset0,#0
	bne		FULL_OUT_
FULL_OUT
	ldr    src1,[$src],$srcStride
	pld	[$src, $srcStride, lsl #1] 	
	subs   OutLoopCount,OutLoopCount,#1	
	str    src1,[$dest],$destStride	
	bne    FULL_OUT 
	b_EXIT_FUNC
...
v7_FULL_OUT
	bgt	v7_FULL_OUT_16
	add	$srcStride,$srcStride,#8
	add	$destStride,$destStride,#8	
FULL_OUT_v7_8
	vld1.8 {d0}, [$src],$srcStride
	pld	[$src, $srcStride, lsl #1] 		
	subs   OutLoopCount,OutLoopCount,#1	
	vst1.64 {d0}, [$dest],$destStride
	bne    FULL_OUT_v7_8 
	b_EXIT_FUNC
