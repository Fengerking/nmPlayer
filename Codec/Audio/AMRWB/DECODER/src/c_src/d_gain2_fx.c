...
extern const Word16 voAMRWBDecQuaGain7b[128*2];
extern const Word16 voAMRWBDecQuaGain6b[64*2]; 
#define MEAN_ENER    30
#define PRED_ORDER   4
#define L_LTPHIST    5
...
{
    Word16 *past_gain_pit, *past_gain_code, *past_qua_en, *gbuf, *pbuf, *prev_gc;
	const Word16 *p;
    Word16 *pbuf2, *seed;
     ...
     *-----------------------------------------------------------------*/
#ifdef  ASM_OPT
    L_tmp = Dot_product12_asm(code, code, L_subfr, &exp);
#else
    L_tmp = Dot_product12(code, code, L_subfr, &exp);
#endif

    exp = exp - (18 + 6);                /* exp: -18 (code in Q9), -6 (/L_subfr) */

    Isqrt_n(&L_tmp, &exp);

    gcode_inov = extract_h(L_shl(L_tmp, (exp - 3)));  /* g_code_inov in Q12 */
    ...
    if (bfi != 0)
    {
        ...
        *past_gain_pit = tmp;     
        ...
        L_tmp = (past_qua_en[0] << 14);           /* x 0.25 */
        L_tmp += (past_qua_en[1] << 14);     /* x 0.25 */
        L_tmp += (past_qua_en[2] << 14);     /* x 0.25 */
        L_tmp += (past_qua_en[3] << 14);     /* x 0.25 */
    ...
    L_tmp = (gcode0 * 5443)<<1;          /* *0.166096 in Q15 -> Q24     */
    L_tmp >>=8;               /* From Q24 to Q16             */
    ...
    if(nbits == 6)
    {
        p = &voAMRWBDecQuaGain6b[index + index];
    } else
    {
        p = &voAMRWBDecQuaGain7b[index + index];
    }
    *gain_pit = *p++;                                 /* selected pitch gain in Q14 */
    g_code = *p++;                                    /* selected code gain in Q11  */

    L_tmp = (g_code * gcode0)<<1;                   /* Q11*Q0 -> Q12 */
    ...
    *past_gain_code = vo_round(L_shl2(*gain_cod, 3)); 
    ...
    *prev_gc = *past_gain_code;            
    ...
    /* adjust gain according to energy of code */
	exp = (*gain_cod)>>16;
	frac = (*gain_cod & 0xffff)>>1;

    //L_Extract(*gain_cod, &exp, &frac);
    L_tmp = Mpy_32_16(exp, frac, gcode_inov);
    *gain_cod = L_shl2(L_tmp, 3);                       /* gcode_inov in Q12 */
