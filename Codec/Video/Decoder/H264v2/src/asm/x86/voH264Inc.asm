    ...
    %ifnum %1
        %if %1 != 0
            ...
            %if mmsize <= 16 && HAVE_ALIGNED_STACK
				;下面这样计算 stack_size_padded 的目的是要保证 SUB 后的esp 值16位对齐，原因是 gcc 保证了运行时函数进入前的 esp 值16位对齐
				;gprsize:代表 call 语句后面的语句的地址压栈所占用栈的空间大小，32位系统为 4 ， 64位系统为 8
				;stack_offset:代表进入函数后，在函数开头被压栈的寄存器所占用的栈空间大小，一般为 ：4 × 被压栈寄存器个数
...
%if mmsize == 32
	;advanced vector extension , set the upper half of all YMM registers to zero
    vzeroupper
%endif
    ret
%endmacro
...
%macro INIT_MMX 0-1+
    ...
    %rep 8
    CAT_XDEFINE m, %%i, mm %+ %%offset
    CAT_XDEFINE nmm, %%i, %%offset
    %assign %%i %%i+1
...
%macro INIT_XMM 0-1+
    ...
    %assign %%i 0
    %assign %%offset num_mmregs-1
    %rep num_mmregs
    CAT_XDEFINE m, %%i, xmm %+ %%offset
    CAT_XDEFINE nxmm, %%i, %%offset
    %assign %%i %%i+1
    %assign %%offset %%offset-1
    %endrep
...
%macro vo_global 1
vo_global_internal %1
%endmacro

%macro vo_global_internal 1
%xdefine %%function_name vo_prefix %+ _ %+ %1
