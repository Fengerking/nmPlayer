	...
	EXPORT	|cc_mb_16x16_r90_arm|
	IMPORT	|ccClip63|	
	AREA	|.text|, CODE, READONLY

  macro 
  cc_mb_16x16_r90_arm_num_count $Num
;	a0 = psrc_u[num] - 128;
;	a1 = psrc_v[num] - 128;
...
;	*((VO_S32*)(out_buf+(4*num))) = (VO_S32)a4;;-------->*((VO_S32*)(out_buf)) = (VO_S32)a4;
...
;	a7 |= (a5<<6);
;;;;;;;;;;;;;;		a4 |= (a7<<5);//dif form left a4 |= (a7 << 21);
;	*((VO_S32*)(out_buf+(out_stride+4*num))) = (VO_S32)a4;-------->out_buf+=-out_stride; *(VO_S32*)(out_buf) = (VO_S32)a4;	
...
;		ldrb	r10, [r10, #0]			;r10 = psrc_y[in_stride+2*num]
	...
	if $Num>0
        	...
        	str	r8, [r4, #$Num]			;*((VO_S32*)(out_buf+(4*num))) = (VO_S32)a4;	        	
	endif
        	sub	r10, r10, #16	
