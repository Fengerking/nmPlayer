				   ...
				   )
{
	...
	for (i_subfr = 0;  i_subfr < L_FRAME; i_subfr += L_SUBFR) {
		...
		/*****************************************************************/
		temp1 = Random(seed);
		frac = (temp1 & (Word16)0x0003) - 1;
		if(frac == 2)
			frac = 0;
		temp1 = (temp1 >> 2);
		t0 = ((temp1 & (Word16)0x003F) + 40);
		temp1 = (temp1 >> 6);
		temp2 = temp1 & (Word16)0x0007;
		pos[0] = ((temp2 << 2) + temp2); /* 5 * temp2 */
		temp1 = (temp1 >> 3);
		...
		pos[3] = ((temp2 & (Word16)1) + 3); /* j+3*/
		temp2 = (shr(temp2, 1)) & (Word16)7;
		temp2 = ((temp2 << 2) + temp2); /* 5i */
		...
		*/
		...
		sh = norm_l(L_acc);
		temp1 = extract_h((L_acc << sh));  /* fact << (sh+1) */
		...
		/****************************************/
#ifdef  ASM_OPT
        Pred_lt_3_asm(cur_exc, t0, frac, L_SUBFR);
#else
		...
		L_ener = 0L;
#ifdef ASM_OPT
        L_ener = sum_mac_v4(L_ener, excs,L_SUBFR);
#else
		for(i=0; i<L_SUBFR; i++) {
