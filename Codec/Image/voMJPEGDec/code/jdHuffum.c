...
static int Build_Vlc_Tab(Jpeg_DecOBJ* decoder,
						  VLC_TAB* tab, 
						 ...
						 INT32 prefixbits)
{
	...
	if(tabbits==0)
	{
		return VO_ERR_DEC_MJPEG_INDATAERROE;
	}
#endif
	...
	for (i=0; i<size; ++i)
	{
		if ((tab_val[i] >> 12) >= 12)
		{
			...
			ret = Build_Vlc_Tab(decoder, tab, huf_spec, count, bits, (prefix << tabbits)|i, prefixbits+tabbits);
			if(ret != 0)
				return ret;
...
{
	INT32 ret,i;//VO_ERR_DEC_MJPEG_INDATAERROE
	...
	if(huff_tbl)
	{
		for(i = 0; i < NUM_HUFF_TBLS; i++)
		{
			if(huff_tbl->dc_huff_tbl_ptrs[i])
			{
				...
				ret =InitVLCTab(decoder, &huff_vlc->dc_vlc_tab[i], &entropy->dc_huff_tbls[i],
			...
			if(huff_tbl->ac_huff_tbl_ptrs[i])
			{
				...
				ret=InitVLCTab(decoder, &huff_vlc->ac_vlc_tab[i], &entropy->ac_huff_tbls[i],
	...
	{
		ret=InitVLCTab(decoder, &huff_vlc->dc_vlc_tab[0], &entropy->dc_huff_tbls[0],
			bits_dc_luminance,	 val_dc_luminance);
		if(ret != 0)
			return ret;
		ret=InitVLCTab(decoder, &huff_vlc->dc_vlc_tab[1], &entropy->dc_huff_tbls[1],
			bits_dc_chrominance, val_dc_chrominance);
		if(ret!= 0)
			return ret;
		ret=InitVLCTab(decoder, &huff_vlc->ac_vlc_tab[0], &entropy->ac_huff_tbls[0],
			bits_ac_luminance,	 val_ac_luminance);
		if(ret!= 0)
			return ret;
		ret=InitVLCTab(decoder, &huff_vlc->ac_vlc_tab[1], &entropy->ac_huff_tbls[1],
		...
		if (is_DC_band) 
		{
			...
			{	
				...
				if(huff_tbl->dc_huff_tbl_ptrs[tbl])
				{
					...
					ret = InitVLCTab(decoder, &huff_vlc->dc_vlc_tab[tbl], &entropy->dc_huff_tbls[tbl],
			...
			if(huff_tbl->ac_huff_tbl_ptrs[tbl])
			{
				...
				ret = InitVLCTab(decoder, &huff_vlc->ac_vlc_tab[tbl], &entropy->ac_huff_tbls[tbl],
				 ...
				 const UINT8 *val)
{
	...
	INT32 ret;
#ifdef STABILITY
	if(bits==NULL||val==NULL)  //error test
	{
		return VO_ERR_DEC_MJPEG_INDATAERROE;
	}
#endif
	...
	ret = Build_Vlc_Tab(decoder, vtab, huf_spec, k, VLC_BITS, 0, 0);
...
INT32	decode_MCU(Jpeg_DecOBJ *decoder)
{
	...
	for(i = 0; i < mcublock->blockNum; i++)
	{
...
#ifdef STABILITY
		if(qtbl==NULL)   //zou 418
			return VO_ERR_DEC_MJPEG_INDATAERROE;
		if(qtbl->quantrawval==NULL)
			return VO_ERR_DEC_MJPEG_INDATAERROE;
#endif
		...
		if(tval)
		{
			//BITGETCAVALUE(bitstream, tval, tmp_v);	//define BITGETCAVALUE(bitstream, val, tmp_v)												
			tmp_v = cache << used_bits;								
			//UPDATACACHE(bitstream, tval);	
			{
				used_bits += tval;			
				while(used_bits >= 8 && (!unread_marker))		
				{							    	
					if(free_in_buffer <= 0)	
						FILLSBUFFER(bitstream);			
					ch = *next_output_byte++;			
					free_in_buffer--;					
					if(ch == 0xFF)						
					{									
