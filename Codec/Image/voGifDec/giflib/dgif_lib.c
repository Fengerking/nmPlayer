...
static int DGifBufferedCheck(GifFileType *GifFile, GifByteType *Buf,
                             GifByteType *NextByte);
...
#ifdef VO_ARMV7
	Private->vomask = (0x0fff>>(12-Private->RunningBits));
	reset_prefix_64_armv7(Prefix,LZ_MAX_CODE+1);
#else
    ...
    while (i < LineLen ) {    /* Decode LineLen items. */
...
#ifdef VO_ARMV7
			if(Private->RunningBits>=6)
				reset_prefix_64_armv7(Prefix,1<<Private->RunningBits ); //LZ_MAX_CODE
			else
				for (j = 0; j < (1<<Private->RunningBits); j++) //4096
		...
		else 
		{
			...
			else 
			{
				...
				{
					unsigned char* outptr = Line+i;
					i+=StackPtr;

					if(StackPtr==1)	{
						*(outptr++) = Stack[0];
					}else if(StackPtr==2){
						*(outptr++) = Stack[1];	*(outptr++) = Stack[0];
					}else if(StackPtr==3){
						*(outptr++) = Stack[2];	*(outptr++) = Stack[1];	*(outptr++) = Stack[0];
					}else{
						while (StackPtr != 0 )
						{
							*(outptr++) = Stack[--StackPtr];
						}
					}
					StackPtr =0;
				}				
...
#ifdef VO_ARMV7
	*Code = (Private->CrntShiftDWord & Private->vomask);
#else
...
DGifBufferedInput(GifFileType * GifFile,
    ...
    if (Buf[0] == 0) {
        ...
        /******************/
		/*if (READ(GifFile, &temp, 1) == 1)
		{
            
			if (temp > 0) 
			{
				if (READ(GifFile, Buf+Buf[0]+1, temp) != temp) {
					_GifError = D_GIF_ERR_READ_FAILED;
					return GIF_ERROR;
				}
				Buf[0] +=temp;
...
static int
DGifBufferedCheck(GifFileType * GifFile,
                  GifByteType * Buf,
                  GifByteType * NextByte) {
