...
void vp8_build_inter16x16_predictors_mb(VP8D_COMP *pbi, MACROBLOCKD *x,
                                        unsigned char *dst_y,
                                        unsigned char *dst_u,
                                        unsigned char *dst_v,
                                        int dst_ystride,
...
#if USE_FRAME_THREAD
	if (pbi->nThdNum > 1)		//wait for ref progress
	{
		//1. calc max y
		int max_y =  ((_16x16mv.as_mv.row - x->mb_to_top_edge)>> 3) + 20;
		//2. wait
		WaitForProgress(pbi->pThdInfo, x->pRef,  max_y);
	}
#endif
...
static void build_inter4x4_predictors_mb(VP8D_COMP *pbi, MACROBLOCKD *x)
{
    int i;
    ...
    if (x->mode_info_context->mbmi.partitioning < 3)
    {
		...
		{
			//1. calc max y
			int max_y = x->block[ 0].bmi.mv.as_mv.row;
			if (x->block[ 2].bmi.mv.as_mv.row > max_y)
			{
				max_y = x->block[ 2].bmi.mv.as_mv.row;
			}
			if (x->block[ 8].bmi.mv.as_mv.row > max_y)
			{
				max_y = x->block[ 8].bmi.mv.as_mv.row;
			}
			if (x->block[ 10].bmi.mv.as_mv.row > max_y)
			{
				max_y = x->block[ 10].bmi.mv.as_mv.row;
			}
			max_y = ((max_y - x->mb_to_top_edge)>>3) + 20;
    ...
    else
    {
        for (i = 0; i < 16; i += 2)
        {
			...
			{
				//1. calc max y
				int max_y = d0->bmi.mv.as_mv.row > d1->bmi.mv.as_mv.row ? d0->bmi.mv.as_mv.row : d1->bmi.mv.as_mv.row;
				max_y = ((max_y - x->mb_to_top_edge)>>3) + 20;
...
void vp8_build_inter_predictors_mb(VP8D_COMP *pbi, MACROBLOCKD *xd)
{
// #if USE_FRAME_THREAD
// 	if (pbi->nThdNum > 1)
// 	{
// 		WaitForProgress(pbi->pThdInfo, xd->pRef, 2000);
// 	}
// #endif
	
    if (xd->mode_info_context->mbmi.mode != SPLITMV)
    {
        vp8_build_inter16x16_predictors_mb(pbi, xd, xd->dst.y_buffer,
    ...
    {
        build_4x4uvmvs(xd);
        build_inter4x4_predictors_mb(pbi, xd);
    }
}
