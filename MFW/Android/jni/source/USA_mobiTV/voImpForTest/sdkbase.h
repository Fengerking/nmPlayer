...
#	define VOAPI __cdecl
	...
	/**
	* Call convention between modules for callback
	*/
...
enum
{
	...
	_MAKEBASE(0x00170000, ADPCM)
	_MAKEBASE(0x00180000, G711)
	_MAKEBASE(0x00190000, G722)
	_MAKEBASE(0x001A0000, G723)
	_MAKEBASE(0x001B0000, iLBC)
	_MAKEBASE(0x001C0000, AC3)
	_MAKEBASE(0x001D0000, S263)
	_MAKEBASE(0x001E0000, DRA)
};
 ...
 */
#define VORC_SUCCEEDED(rc) (!VORC_FAILED(rc))

/**
 * Check the return code if failed or not.
 */
#define VORC_FAILED(rc) ((rc)& _VOBASE_RC)
...
typedef enum
{
	...
	VOID_COM_VIDEO_ERR_NUM			= VOID_COM_BASE | 0x0012,   /*!< Get error number in decoded Video frame*/
	VOID_COM_VIDEO_RENDERLAYER		= VOID_COM_BASE | 0x0013,	/*!< Primary/overlay layer for video rendering*/
	VOID_COM_VIDEO_DISABLEOVERLAY	= VOID_COM_BASE | 0x0014,	/*!< Disable overlay layer*/
	vOID_COM_VIDEO_DEINTERLACE		= VOID_COM_BASE | 0x0015,	/*!< Enable deinterlace*/
}
VOCOMMONPARAMETERID;
...
typedef struct 
{
	uint32 decPictureWidth;				/*!< picture width offered outside*/
	uint32 decPictureHeight;			/*!< picture height offered outside*/
	uint32 decNumRefFrames;				/*!< number of reference and reconstruction  frames */
	uint32 decReferenceFrameWidth;			/*!< including padding */
	uint32 decReferenceFrameHeight;			/*!< including padding */
	uint32 decPrivateDataSize;			/*!< size in byte */
	uint8 *decReferenceFrames[DEC_MAX_NUM_REF_FRAME][3];
	uint8 *pDecPrivateData;
}VOVIDEODECQUERYMEM;
...
typedef enum
{
	...
	VOYUV_PLANAR420_NV12	= 36		/*!< YUV planar mode:420  vertical sample is 2, horizontal is 2  */
}
VOVIDEOTYPE;
...
typedef struct
{
	BYTE *data_buf[3];			/*!< Buffer pointer */
...
typedef enum
{
		...
		VC_S263					= 0X00002000,   /*!< S263 video codec*/
		...
		AC_DRA					= 0X80010000,	/*!< DRA audio codec*/
		AC_FLAC					= 0x80020000,	/*!< FLAC audio codec*/
		AC_G711                 = 0x80040000,   /*!< G711 speech codec*/
		AC_G722                 = 0x80080000,   /*!< G722 speech codec*/
		AC_G723                 = 0x80100000,   /*!< G723 speech codec*/
		AC_iLBC                 = 0x80200000,    /*!< iLBC speech codec*/
}VOAVCODEC;
...
typedef struct
{
	void* (VOAPI *Alloc)(int size, uint32* pPhysAddr);
