		...
		if(index < 1)
		{
			for (sets_left = set_count;sets_left > 0;sets_left--) 
			{
				...
				do 
				{
					...
					out_val_low = (Word16)(acca >> 1);

					dummy = add(in_val_low,dither_ptr[i++]);
					acca = L_add(dummy,-in_val_high);
					out_val_high = (Word16)(acca >> 1);
		...
		else
		{
			for (sets_left = set_count;sets_left > 0;sets_left--) 
			{
				...
				do 
				{
					in_val_low      = *in_ptr++;
					in_val_high     = *in_ptr++;
					out_val_low     = in_val_low + in_val_high;
					out_val_high    = in_val_low + (-in_val_high);
	...
	for (set_count_log = dct_length_log - 2 ;set_count_log >= 0;set_count_log--) 
	{
		...
		for (sets_left = set_count;sets_left > 0;sets_left--) 
		{
			...
			do 
			{
				...
				sum = (cos_even * in_low_even)<<1;
				sum += ((-msin_even) * in_high_even) << 1;
				out_low_even = voRound(L_shl(sum,1));

				sum = (msin_even * in_low_even)<<1;
				sum += (cos_even * in_high_even)<<1;
				out_high_even = voRound(L_shl(sum,1));

				sum = (cos_odd * in_low_odd) << 1;
				sum += (msin_odd * in_high_odd) <<1;
				out_low_odd = voRound(L_shl(sum,1));

				sum = (msin_odd * in_low_odd)<<1;
				sum +=((-cos_odd) * in_high_odd)<<1;
				out_high_odd = voRound(L_shl(sum,1));
