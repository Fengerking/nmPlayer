...
LINEDEF  Line_Unpk(G723DncState *st, Word16 *Ftyp)
{
	int   i  ;
	Word16  BitStream[192] ;
	Word16 *Bsp = BitStream ;
	char  *Vinp = st->input_buffer;
	Word16  Crc = st->g_Crc;
	LINEDEF Line ;
	Word32  Temp ;
	Word16  Info;
	...
	*Ftyp = 1;
	/* Decode the bit-rate */
	st->bitRate = (Info == 0) ? Rate63 : Rate53;
	...
	for ( i = 0 ; i < SubFrames ; i ++ ) {
		...
		if ( (st->bitRate == Rate63) && (Line.Olp[i>>1] < (SubFrLen-2) ) ) {
...
**
*/
void  Scale( Word16 *Tv, Word32 Sen ,G723DncState *st)
{
	int   i ;
