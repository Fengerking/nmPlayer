...
static const VO_U32 g_bit_increment = 0;
             ...
             VO_S16 * tmp_block_array)
{
  ...
  VO_U8  *p_target_block = p_perdiction_buff;
  ...
  for (i = 0; i < 4; i++) {
	  ...
	  p_target_block[prediction_stride * i + 0] = (VO_U8) Clip3(0, 255, ((29 * c[0] + 
                                                    55 * c[1] + c[3] + rnd_factor) >> shift_2nd) +
                                                    p_perdiction_buff[prediction_stride * i + 0]);

	  p_target_block[prediction_stride * i + 1] = (VO_U8) Clip3(0, 255, ((55 * c[2] -
                                                    29 * c[1] + c[3] + rnd_factor) >> shift_2nd) +
                                                    p_perdiction_buff[prediction_stride * i + 1]);

	  p_target_block[prediction_stride * i + 2] = (VO_U8) Clip3(0, 255, ((74 * (tmp_block[i] -
                                                    tmp_block[8 + i] + tmp_block[12 + i]) + rnd_factor) >> 
                                                    shift_2nd) + p_perdiction_buff[prediction_stride * i + 2]);

	  p_target_block[prediction_stride * i + 3] = (VO_U8) Clip3(0, 255, ((55 * c[0] +
             ...
             VO_S16 * tmp_block_array)
{
  ...
  VO_U8  *p_target_block = p_prediction_buff;
  ...
  for (j = 0; j < 4; j++) {
	  ...
	  p_tmp_block++;
	  p_target_block += prediction_stride;
...
/**
 * cos invtransform at 8x8 when the non zero is in the left-top 4X4 block
 * \param p_invtransform_buff   [IN]       dequant coef data which need be transformed.
 * \param p_prediction_buff     [IN]       other base information.
 * \param p_reconstruction_buff [OUT]      the reconstruction pixel data.
 * \param reconstruction_stride [IN]       stride of the reconstruction pixel buffer.
             ...
             VO_S16 * block_array)
{
  ...
  for (j = 0; j < 4; j++) {
	// Utilizing symmetry properties to the maximum to minimize the number of multiplications
	  ...
	  EO[0] = coef_t8[2 * 8 + 0] * p_invtransform[2 * 8];
	  EO[1] = coef_t8[2 * 8 + 1] * p_invtransform[2 * 8];
	  EE[0] = coef_t8[0 * 8 + 0] * p_invtransform[0];
	  EE[1] = coef_t8[0 * 8 + 1] * p_invtransform[0];
  ...
  for (j = 0; j < 8; j++) {
	  ...
	  EO[0] = coef_t8[2 * 8 + 0] * p_tmp_block[2 * 8];
	  EO[1] = coef_t8[2 * 8 + 1] * p_tmp_block[2 * 8];
	  EE[0] = coef_t8[0 * 8 + 0] * p_tmp_block[0];
	  EE[1] = coef_t8[0 * 8 + 1] * p_tmp_block[0];
               ...
               VO_S16 * block_array)
{
  ...
  const VO_S16 *p_invtransform = p_invtransform_buff;
  const VO_U8 *p_prediction = p_prediction_buff;
  VO_U8  *p_reconstruction = p_prediction_buff;
  ...
  for (j = 0; j < 16; j++) {
	...
	p_tmp_block++;
	p_reconstruction += prediction_stride;
	p_prediction += prediction_stride;
  }
               ...
               VO_S16 * block_array)
{
  ...
  for (j = 0; j < 8; j++) {
	  ...
	  EEO[0] = coef_t16[4 * 16 + 0] * p_invtransform[4 * 16];
	  EEE[0] = coef_t16[0 * 16 + 0] * p_invtransform[0];
	  EEO[1] = coef_t16[4 * 16 + 1] * p_invtransform[4 * 16];
	  EEE[1] = coef_t16[0 * 16 + 1] * p_invtransform[0];
  ...
  for (j = 0; j < 16; j++) {
	...
	EEO[0] = coef_t16[4 * 16 + 0] * p_tmp_block[4 * 16];
	EEE[0] = coef_t16[0 * 16 + 0] * p_tmp_block[0];
	EEO[1] = coef_t16[4 * 16 + 1] * p_tmp_block[4 * 16];
	EEE[1] = coef_t16[0 * 16 + 1] * p_tmp_block[0];
...
VO_VOID IDCT32X32_8x8(const VO_S16 * p_invtransform_buff,
               VO_U8 * p_prediction_buff,
               const VO_U32 prediction_stride,
               VO_S16 * block_array)
{
  ...
  for (j = 0; j < 8; j++) {
	...
	for (k = 0; k < 4; k++) {
	  EEO[k] = coef_t32[4 * 32 + k] * p_invtransform[4 * 32];
	}
	//EEEO[0] = coef_t32[8 * 32 + 0] * p_invtransform[8 * 32] + coef_t32[24 * 32 + 0] * p_invtransform[24 * 32];
	//EEEO[1] = coef_t32[8 * 32 + 1] * p_invtransform[8 * 32] + coef_t32[24 * 32 + 1] * p_invtransform[24 * 32];
	EEEE[0] = coef_t32[0 * 32 + 0] * p_invtransform[0];
	EEEE[1] = coef_t32[0 * 32 + 1] * p_invtransform[0];
  ...
  for (j = 0; j < 32; j++) {
	  ...
	  for (k = 0; k < 4; k++) {
	    EEO[k] = coef_t32[4 * 32 + k] * p_tmp_block[4 * 32];
	  }
	  //EEEO[0] = coef_t32[8 * 32 + 0] * p_tmp_block[8 * 32] + coef_t32[24 * 32 + 0] * p_tmp_block[24 * 32];
	  //EEEO[1] = coef_t32[8 * 32 + 1] * p_tmp_block[8 * 32] + coef_t32[24 * 32 + 1] * p_tmp_block[24 * 32];
	  EEEE[0] = coef_t32[0 * 32 + 0] * p_tmp_block[0];
	  EEEE[1] = coef_t32[0 * 32 + 1] * p_tmp_block[0];
...
VO_VOID IDCT32X32_16x16(const VO_S16 * p_invtransform_buff,
               VO_U8 * p_prediction_buff,
               const VO_U32 prediction_stride,
               VO_S16 * block_array)
{
  ...
  for (j = 0; j < 16; j++) {
	...
	EEEO[0] = coef_t32[8 * 32 + 0] * p_invtransform[8 * 32];
	EEEO[1] = coef_t32[8 * 32 + 1] * p_invtransform[8 * 32];
	EEEE[0] = coef_t32[0 * 32 + 0] * p_invtransform[0];
	EEEE[1] = coef_t32[0 * 32 + 1] * p_invtransform[0];
  ...
  for (j = 0; j < 32; j++) {
	  ...
	  EEEO[0] = coef_t32[8 * 32 + 0] * p_tmp_block[8 * 32];
	  EEEO[1] = coef_t32[8 * 32 + 1] * p_tmp_block[8 * 32];
	  EEEE[0] = coef_t32[0 * 32 + 0] * p_tmp_block[0];
	  EEEE[1] = coef_t32[0 * 32 + 1] * p_tmp_block[0];
             ...
             VO_S16 * tmp_block_array)
{
  ...
  for (i = 0; i < 4; i++) {
	  // Intermediate Variables
	  p_target_block[prediction_stride * i + 0] = (VO_U8) Clip3(0, 255, ((29 * tmp_block[i] + rnd_factor) >> shift_2nd) +
                                                    p_prediction_buff[prediction_stride * i + 0]);
	  p_target_block[prediction_stride * i + 1] = (VO_U8) Clip3(0, 255, ((55 * tmp_block[i] + rnd_factor) >> shift_2nd) + 
                                                    p_prediction_buff[prediction_stride * i + 1]);
	  p_target_block[prediction_stride * i + 2] = (VO_U8) Clip3(0, 255, ((74 * tmp_block[i] + rnd_factor) >> 
                                                    shift_2nd) + p_prediction_buff[prediction_stride * i + 2]);
	  p_target_block[prediction_stride * i + 3] = (VO_U8) Clip3(0, 255, ((84 * tmp_block[i] + rnd_factor) >> shift_2nd) +
...
VO_VOID IDCT_NONE_1X1(const VO_S16 *p_invtransform_buff,
                   VO_U8 *p_prediction_buff,
                   const VO_U32 prediction_stride,
                   VO_S16 * typeU32_width)	
{    
    p_prediction_buff[0] = (VO_U8) Clip3(0, 255, p_invtransform_buff[0] + p_prediction_buff[0]); 
    return;
}
                   ...
                   VO_S16 * typeU32_width)	
{
    ...
    const VO_S16 *p_coeff = p_invtransform_buff;
    VO_U8  *p_reconstruction = p_prediction_buff;

    for (j = 0; j < tuWidth; j++) {
        for (k = 0; k < tuWidth; k++) {
            p_reconstruction[k] = (VO_U8) Clip3(0, 255, p_coeff[k] + p_reconstruction[k]); 
        }
        p_reconstruction += prediction_stride;
		p_coeff += tuWidth;
