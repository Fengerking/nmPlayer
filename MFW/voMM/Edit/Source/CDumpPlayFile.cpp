...
VO_U32 CDumpPlayFile::DecoderVideoData (VO_VIDEO_BUFFER * pOutput, VO_VIDEO_FORMAT * pVideoFormat)
{
	VO_U32	nThreadTime = voOS_GetThreadTime (NULL);
	VO_U32	nSystemTime = voOS_GetSysTime ();

	VO_U32 nRC =  CPlayFile::DecoderVideoData (pOutput, pVideoFormat);
...
VO_U32 CDumpPlayFile::EndOfSource (void)
{
	...
	if (m_pConfig != NULL)
		pDumpFile = m_pConfig->GetItemText ("vowPlay", "DumpLogFile");
	...
	int nDumpLevel = 0;
	if (m_pConfig != NULL)
		nDumpLevel = m_pConfig->GetItemValue ("vowPlay", "DumpLogLevel", 0);
	if (nDumpLevel < 2)
	{
		m_nVideoDecIndex = 0;
		m_nVideoRenIndex = 0;
	...
	*pExt = 0;
	...
	for (i = 0; i < m_nVideoDecIndex; i++)
	{
		if (i == 0)
		{
			sprintf (szLine, "%d	%d	%d	%d	%d	%d	%d	%d	%d	%d\r\n",
							i, m_pVideoDecThreadNum[i], m_pVideoDecSystemNum[i], m_pVideoSystemTime[i] - m_pVideoSystemTime[0], m_pVideoPlayTime[i],
							m_pVideoSystemTime[i] - m_pVideoSystemTime[0] - m_pVideoPlayTime[i], m_pVideoSampleTime[i], 0,
							m_pVideoPlayTime[i] - m_pVideoSampleTime[i], m_pVideoKeyFrame[i]);
		}
		else
		{
			sprintf (szLine, "%d	%d	%d	%d	%d	%d	%d	%d	%d	%d\r\n",
							i, m_pVideoDecThreadNum[i], m_pVideoDecSystemNum[i], m_pVideoSystemTime[i] - m_pVideoSystemTime[0], m_pVideoPlayTime[i],
							m_pVideoSystemTime[i] - m_pVideoSystemTime[0] - m_pVideoPlayTime[i],
							m_pVideoSampleTime[i], m_pVideoSampleTime[i] - m_pVideoSampleTime[i-1],
							m_pVideoPlayTime[i] - m_pVideoSampleTime[i], m_pVideoKeyFrame[i]);
		}
		m_pFileOP->Write (hFile, szLine, strlen (szLine));
	}
	...
	if (m_nVideoRenIndex > 0)
	{
		...
		{
			if (i == 0)
				sprintf (szLine, "%d	%d		%d		%d		%d\r\n", i, m_pVideoRenThreadNum[i], m_pVideoRenSystemNum[i], 0, 0);
			else
				sprintf (szLine, "%d	%d		%d		%d		%d\r\n", i, m_pVideoRenThreadNum[i],  m_pVideoRenSystemNum[i],
								m_pVideoRenSysTime[i] - m_pVideoRenSysTime[0], m_pVideoRenSysTime[i] - m_pVideoRenSysTime[i-1]);
			m_pFileOP->Write (hFile, szLine, strlen (szLine));
		}
	}
	...
	if (m_nAudioDecIndex > 0)
	{
		...
		for (i = 0; i < m_nAudioDecIndex; i++)
		{
			sprintf (szLine, "%d	%d		%d\r\n", i, m_pAudioDecThreadNum[i], m_pAudioDecSystemNum[i]);
			m_pFileOP->Write (hFile, szLine, strlen (szLine));
		}
	}
	...
	if (m_nAudioRenIndex > 0)
	{
		...
		for (i = 0; i < m_nAudioRenIndex; i++)
		{
			...
			sprintf (szLine, "%d	%d	%d	%d	%d	%d	%d	%d\r\n", i, m_pAudioRendThreadNum[i], m_pAudioRendSampleTime[i],
								nSysTime, m_pAudioRendPlayingTime[i], nSysTime - m_pAudioRendPlayingTime[i],
								m_pAudioRendSampleTime[i] - m_pAudioRendPlayingTime[i], m_pAudioRendBufferTime[i]);
			m_pFileOP->Write (hFile, szLine, strlen (szLine));
		}
	}
	...
	if (m_nVideoReadIndex > 0)
	{
		...
		for (i = 0; i < m_nVideoReadIndex; i++)
		{
			...
			if (i == 0)
				sprintf (szLine, "%d	%d	%d	%d	%s	%d\r\n", i, m_pVideoReadThreadNum[i], m_pVideoReadSampleTime[i], 0, szKey, m_pVideoReadKeyFrame[i] & 0X7FFFFFFF);
			else
				sprintf (szLine, "%d	%d	%d	%d	%s	%d\r\n", i, m_pVideoReadThreadNum[i], m_pVideoReadSampleTime[i],
																m_pVideoReadSampleTime[i] - m_pVideoReadSampleTime[i-1], szKey, m_pVideoReadKeyFrame[i] & 0X7FFFFFFF);
			m_pFileOP->Write (hFile, szLine, strlen (szLine));
		}
	}
	...
	if (m_nAudioReadIndex > 0)
	{
		...
		{
			if (i == 0)
				sprintf (szLine, "%d	%d	%d	%d	%d\r\n", i, m_pAudioReadThreadNum[i], m_pAudioReadSampleTime[i], 0, m_pAudioReadSampleSize[i] & 0X7FFFFFFF);
			else
				sprintf (szLine, "%d	%d	%d	%d	%d\r\n", i, m_pAudioReadThreadNum[i], m_pAudioReadSampleTime[i],
																m_pAudioReadSampleTime[i] - m_pAudioReadSampleTime[i-1], m_pAudioReadSampleSize[i] & 0X7FFFFFFF);
			m_pFileOP->Write (hFile, szLine, strlen (szLine));
		}
	}
