...
void rv_depacki_cleanup_format_info_array(rv_depack_internal* pInt)
{
    if (pInt && pInt->pSubStreamHdr)
    {
        ...
        pInt->pSubStreamHdr = (rv_format_info*)HXNULL;
    }
}
...
HX_RESULT rv_depacki_check_rule_book(rv_depack_internal* pInt, rm_stream_header* hdr)
{
    ...
    {
        UINT32      ulSize          = 0;
        HXBOOL*     pTmp            = (HXBOOL*)HXNULL;
        char*       pszRuleBook     = (char*)HXNULL;
        char*       pStr            = (char*)HXNULL;
        char*       pStrLimit       = (char*)HXNULL;
        UINT32      ulRule          = 0;
                        ...
                        while(pStr < pStrLimit && *pStr)
                        {
                            while(pStr < pStrLimit && *pStr != ';')
                            {
                                ...
                                if (pStr + ulPNMStrLen < pStrLimit && *pStr == '$')
                                {
									...
									if (!_strnicmp(pszPNMStr, pStr, ulPNMStrLen))
								#else
									if (!strncasecmp(pszPNMStr, pStr, ulPNMStrLen))
								#endif	//UNDER_CE || defined WIN32
									 {
                                        pStr += ulPNMStrLen;
                                        pTmp[ulRule] = TRUE;
                                    }
...
HX_RESULT rv_depacki_send_current_frame(rv_depack_internal* pInt)
{
    ...
    if (pInt && pInt->fpAvail)
    {
        ...
        if (pInt->pCurFrame)
        {
            ...
            pInt->pCurFrame = (rv_frame*)HXNULL;
        }
    }
                              ...
                              rv_frame**          ppFrame)
{
    if (pInt && ppFrame && *ppFrame)
    {
        ...
        if ((*ppFrame)->pData)
        {
            ...
            (*ppFrame)->pData = (BYTE*)HXNULL;
        }
        /* Free the segment array */
        if ((*ppFrame)->pSegment)
        {
            rv_depacki_free(pInt, (*ppFrame)->pSegment);
            (*ppFrame)->pSegment = (rv_segment*)HXNULL;
        }
        /* NULL out the frame */
        ...
        pFrameHdr->ulFrameSize && pFrameHdr->ulNumPackets)
    {
        ...
        if (pFrame)
        {
            ...
            if (pFrame->pData)
            {
                ...
                pFrame->pSegment = (rv_segment*)rv_depacki_malloc(pInt, ulSize);
                if (pFrame->pSegment)
                {
                    /*
