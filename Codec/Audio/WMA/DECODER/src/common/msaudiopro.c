...
WMARESULT prvAllocateBarkResampleMatrix(CAudioObjectDecoder *paudec, CAudioObject* pau)
{
...
#endif
    {
        ...
        if (pau->m_rgrgrgnBarkResampleMatrix == NULL) {
            ...
            Int iWin0, iWin1;
            pau->m_rgrgrgnBarkResampleMatrix = (U8 ***)memset(pau->m_rgrgrgnBarkResampleMatrix, 0, sizeof (U8**) * pau->m_cPossibleWinSize);
            ...
            for (iWin0 = 0; iWin0 < pau->m_cPossibleWinSize; iWin0++) {
                ...
                pTemp = memset(pau->m_rgrgrgnBarkResampleMatrix[iWin0], 0, sizeof (U8*) * pau->m_cPossibleWinSize);
                ...
                for (iWin1 = 0; iWin1 < pau->m_cPossibleWinSize; iWin1++) {
                    ...
                    pTemp = memset(pau->m_rgrgrgnBarkResampleMatrix[iWin0][iWin1], 0, sizeof (U8) * NUM_BARK_BAND);
                }
            }
        }
    }
...
Void prvSetSubWooferCutOffs (CAudioObject* pau)
{
    ...
    for (iWin = 0; iWin < pau->m_cPossibleWinSize; iWin++)    {
        ...
        iIndex = (Int) voWMACeil((fltSubWooferCutOffHz * cFrameSample * fltSamplingPeriod) + 0.5F); // Should they be multiple of 2, or 4 or such?
...
{
    Int iIndex = 0;
    static QuantFloat qfltRMS;  // clean up warning build, need double check the update.
...
WMARESULT auPostScaleCoeffsV3(CAudioObject *pau)
{
    ...
    for (iCh = 0; iCh < pau->m_cChInTile; iCh++)
    {
        ...
        if (ppcinfo->m_bNoDecodeForCx)		//aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
            continue;

        ppcinfo->m_cLeftShiftBitsTotal -= ppcinfo->m_cLeftShiftBitsQuant;
        ppcinfo->m_cLeftShiftBitsTotal -= pau->m_cLeftShiftBitsFixedPost;//aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
                                 ...
                                 U8 fMaskUpdate)
{
    ...
    I32 iHighCutOff = 0;
    I32 iLo, iHi;
    static QuantFloat *qfltQuantizer;   // eithe a FastFloat or a Float, as appropriate to the build.
...
WMARESULT prvMultiXIDCTAllocate(CAudioObjectDecoder *paudec, CAudioObject *pau)
{
    ...
    pTemp = memset(pau->m_rgrgrgfltMultiXIDCT, 0, sizeof(ChXFormType**) * (cChannel + 1));
    ...
    for (iGrpSize = 1; iGrpSize <= cChannel; iGrpSize++)
    {
        ...
        pTemp = memset(pau->m_rgrgrgfltMultiXIDCT[iGrpSize], 0, sizeof(ChXFormType*) * iGrpSize);
        ...
        if (iGrpSize > MAX_PREDEF_MATRIX_CHANNELS) {
            ...
            for (iCh = 0; iCh < iGrpSize; iCh++) {
                ...
                pTemp = memset(pau->m_rgrgrgfltMultiXIDCT[iGrpSize][iCh], 0, sizeof(ChXFormType) * iGrpSize);
            }
        }
    }
...
{
    WMARESULT wmaResult = WMA_OK;
    Int iGrpSize, predefmatrixch = MAX_PREDEF_MATRIX_CHANNELS;
...
Void auInitBarkResampleMatrix (CAudioObject* pau)
{
    ...
    for (iWin0 = 0; iWin0 < pau->m_cPossibleWinSize; iWin0++)
    {
        for (iWin1 = 0; iWin1 < pau->m_cPossibleWinSize; iWin1++)
        {
            pTemp = memset(pau->m_rgrgrgnBarkResampleMatrix[iWin0][iWin1], 0,
                                 ...
                                 Int cChannel)
{
    ...
    *prgChannelGrpInfo = (CChannelGroupInfo *)memset(*prgChannelGrpInfo, 0, sizeof(CChannelGroupInfo) * cChannel);
    for (iCh = 0; iCh < cChannel; iCh++) {
        ...
        if (cChannel > 1) {
            ...
            pcgi->m_rgbRotationAngle = (I8 *)memset(pcgi->m_rgbRotationAngle, 0, sizeof(I8) * (cChannel * (cChannel - 1)) / 2);
        }
        ...
        pcgi->m_rgbRotationSign = (I8 *)memset(pcgi->m_rgbRotationSign, 0, sizeof(I8) * cChannel);
    }
exit:
    return wmaResult;
} // prvAllocChannelGrpInfo
                                   ...
                                   ChXFormMType* rgfltTmpFinal)
{
    ...
    for (i = 0; i < cSize-1; i++)
    {
        ...
        rgfltTmpCurr = (ChXFormMType *)voWMAMemcpy(rgfltTmpCurr, rgfltMultiXInverse + cMatrixSize * i, sizeof(ChXFormMType) * cSize);
        rgfltTmpFinal = (ChXFormMType *)voWMAMemcpy(rgfltTmpFinal, rgfltMultiXInverse + cMatrixSize * (cSize-1), sizeof(ChXFormMType) * cSize);
                                          ...
                                          ChXFormType* rgfltTmpFinal)
{
    ...
    rgfltMultiXInverseM = (ChXFormMType *)memset(rgfltMultiXInverseM, 0, sizeof(ChXFormMType) * cSize * cSize);
    for (i = 0; i < cSize; i++)
...
WMARESULT prvLLMVerBResetAllCom_MLLM(CAudioObject * pau, PerChannelInfo * ppcInfoComms)
{
    ...
    for (iCh = 0; iCh < pau->m_cChInTile; iCh++) {
        ...
        pTemp = memset(ppcInfoComm->m_rgiLPCPrevSamples, 0, LLM_LPC_ORDER * sizeof(I32));
        // Step 1
        pTemp = memset(ppcInfoComm->m_iLastSamples, 0, LLM_VERB_ACFILTER_ORDER_MAX * sizeof(I32));
        // Step 2
        {
                              ...
                              Bool fBase)
{
    ...
    // flat part of 2nd half of prev
    {
        ...
        {
            voWMAMemcpy(pctCoefOut, pctCoefHist, (-iOverlap - (-iSizePrev/2)) * sizeof(CoefType));
        }
    }
...
#define COS2  (pSinCosTable->cos_2PIbycSB)
    ...
    // flat part of first half of cur
    {
        ...
        {
            voWMAMemcpy(pctOut, pctNew, (iSizeCurr/2 - iOverlap) * sizeof(CoefType));
        }
    }
                      ...
                      Bool bFlip)
{
    ...
    if (bFlip)
        pctCoefHist = voWMAMemcpy(pctCoefHist, pctCoefNew+iSizeCurr/2, iSizeCurr/2*sizeof(CoefType));
    else
        pctCoefHist = voWMAMemcpy(pctCoefHist, pctCoefNew, iSizeCurr/2*sizeof(CoefType));
}
...
#if 0
Void prvReplaceSamplesFromFileCh(CAudioObjectDecoder *paudec, CAudioObject *pau, const TCHAR *szFile,
                                 ...
                                 Int iStart, Int iEnd)
{
    ...
    if (iEnd-iStart > 0)
        voWMAMemcpy(ppcinfo->m_rgCoefRecon+iStart, g_pReplBuf[iCh]+iStart,
...
Void prvReplaceSamplesFromFile(CAudioObjectDecoder *paudec, CAudioObject *pau, Float fStart, Float fEnd)
{
    ...
    for (iCh = 0; iCh < pau->m_cChInTile; iCh++)
    {
        ...
        ppcinfo = pau->m_rgpcinfo + iChSrc;
        prvReplaceSamplesFromFileCh(paudec, pau, g_szReplaceFile, ppcinfo, iChSrc,
                                    iStart, iEnd);
    }
}
#endif
                                        ...
                                        Float fStart, Float fEnd)
{
    ...
    {
        // DCT
        voWMAMemcpy(pDest+iStart,
               pBufMono+iStart,
               (iEnd-iStart)*sizeof(CoefType));
        // DST
        voWMAMemcpy(pDest+prti->m_iSizeCurr+iStart,
...
Void prvReplaceSamplesFromFilePostXForm(CAudioObjectDecoder *paudec, CAudioObject *pau)
{
    ...
    for (iCh = 0; iCh < pau->m_cChInTile; iCh++)
    {
        ...
        voWMAMemcpy(ppcinfo->m_rgCoefRecon, pBuf,
