...
//   
//  while (tmp >= 0) {
//    sideInfoBits = sideInfoBits + seg_len_bits;
//    tmp = tmp - escape_val;
//  }
//  return extract_l(sideInfoBits);
//}
            ...
            if (j - ((psectionInfo->sfbStart + psectionInfo->sfbCnt) - 1) == 0) {
              ...
              for (k = j + 1; k < psectionInfo->sfbStart + psectionInfo->sfbCnt; k++) {
                ...
                if (maxValueInSfb[k] != 0) {
                  int tmp = ABS(scalefacGain[k] - lastValScf);
					found = 1;                                             
            ...
            SECTION_DATA  *sectionData)
{
  ...
  sectionData->noOfGroups     = sfbCnt/sfbPerGroup; //ffr_Short_Div(sfbCnt, sfbPerGroup);
                         ...
                         const Word16   blockType)
{
  ...
  for (i=0; i<sectionData->noOfSections; i++) {
    Word16 siBits;
    sectionData->huffmanBits += psectionInfo->sectionBits;
       
    if (blockType != SHORT_WINDOW)
      siBits = sideInfoTabLong[psectionInfo->sfbCnt];
    else
      siBits = sideInfoTabShort[psectionInfo->sfbCnt];

    psectionInfo->sectionBits += siBits;
    sectionData->sideInfoBits += siBits;
