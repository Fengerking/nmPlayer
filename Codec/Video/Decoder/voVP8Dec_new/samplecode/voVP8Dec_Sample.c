...
#define DEFAULT_DIS_LOOPFILTER	0
#define ERROR_VP8 0
...
#endif
{
	...
	int TIMES = DEFAULT_TIMES,nthreads =DEFAULT_THREAD,disabledeblock=DEFAULT_DIS_LOOPFILTER;

	sprintf(mfilename,"vp80-05-sharpness-1439.ivf");
	sprintf(mfilenameout,"");
	...
	printf("****** vp8 test start******\r\ninput %s\r\noutput %s\r\nthreads %d\r\n", mfilename, mfilenameout, nthreads);
#ifdef VOWINXP
	srand((int)time(0));
#endif
	for(j=0;j<1;j++)
...
// 		if(decode_frames >=20) //zou97
// 			break;

		ret =voVP8Dec.GetOutputData(phCodec, &outdata, &outInfo);

		if (ret == 0 && outInfo.Format.Type != VO_VIDEO_FRAME_NULL)
		{
#if Enable_OutPut
			if (outfile)
			{
	...
	//flush last frames
	printf("flush last frames\n");
	voVP8Dec.SetParam(phCodec,VO_ID_VP8_FLUSH_PICS, &outInfo);
	ret=voVP8Dec.GetOutputData(phCodec,&outdata,&outInfo);

	while(ret == 0 && (outInfo.Format.Type != VO_VIDEO_FRAME_NULL || outInfo.Flag)) //in the end of file, we must get all next output
	{
		if (outInfo.Format.Type != VO_VIDEO_FRAME_NULL)
		{
#if Enable_OutPut
...
#endif
	printf("time %d\n", dx_time);
    return ((float)decode_frames) / (dx_time/1000.0);
}
