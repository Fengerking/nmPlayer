...
float beginTest_vp8(char *infilename ,char *outfilename,int times)
{	
	VO_HANDLE phCodec;
	...
	ret =voVP8Dec.SetParam(phCodec,VO_PID_DEC_VP8_THREADS,&nthreads);
	...
	while (!read_one_frame_data(infile, &(indata.Buffer), &(indata.Length), &buf_alloc_sz,is_ivf)) // parse data
    {
		...
		//if(outInfo.Format.Width==212)
		output_one_frame_data(outfile,outdata,outInfo,0);
		
		//if(decode_frames >200)
		//	goto STOP;
		...
		if(0)
		{
			char yuvname[255];
			FILE* fp ;
			sprintf(yuvname,"D:/svn/Numen/engineer/zou_zhihuang/Codec/Video/Vp8/vp8-test-vectors-r1/error/0t/%d.yuv",decode_frames);
			fp =fopen(yuvname,"wb");
			output_one_frame_data(fp,outdata,outInfo,0);
			fclose(fp);
		}
#endif		
		decode_frames++;
...
STOP:
	...
	fclose(infile);
	fclose(outfile);
	printf("----%d %d %f fps \n",dx_time,decode_frames,((float)decode_frames) / (float)((dx_time)/1000.0));
