...
WMARESULT prvInitMCLMSPredictor(CAudioObjectDecoder *paudec, CAudioObject * pau, MCLMSPredictor * pMCLMSPrdtr, I16 iMaxOrder)
{
...
//    pMCLMSPrdtr->m_rgiFilter = (I16 *)auMalloc(sizeof(pMCLMSPrdtr->m_rgiFilter[0]) * iMaxOrder * pau->m_cChannel * pau->m_cChannel);
    if(pMCLMSPrdtr->m_rgiFilter == NULL)
    {
        wmaResult = TraceResult(WMA_E_OUTOFMEMORY);
        goto exit;
    }
    pMCLMSPrdtr->m_rgiFilter = (I16 *)memset(pMCLMSPrdtr->m_rgiFilter, 0, sizeof(pMCLMSPrdtr->m_rgiFilter[0]) * iMaxOrder * pau->m_cChannel * pau->m_cChannel);
    ...
    pMCLMSPrdtr->m_rgiFilterBackup = (I16 *)memset(pMCLMSPrdtr->m_rgiFilterBackup, 0, sizeof(pMCLMSPrdtr->m_rgiFilterBackup[0]) * iMaxOrder * pau->m_cChannel * pau->m_cChannel);
    ...
    pMCLMSPrdtr->m_rgiFilterCurrTime = (I16 *)memset(pMCLMSPrdtr->m_rgiFilterCurrTime, 0, sizeof(pMCLMSPrdtr->m_rgiFilterCurrTime[0]) * pau->m_cChannel * pau->m_cChannel);
    ...
    pMCLMSPrdtr->m_rgiFilterCurrTimeBackup = (I16 *)memset(pMCLMSPrdtr->m_rgiFilterCurrTimeBackup, 0, sizeof(pMCLMSPrdtr->m_rgiFilterCurrTimeBackup[0]) * pau->m_cChannel * pau->m_cChannel);
    ...
    pMCLMSPrdtr->m_rgiPrevVal = (I32 *)memset (pMCLMSPrdtr->m_rgiPrevVal, 0, sizeof(pMCLMSPrdtr->m_rgiPrevVal[0]) * iMaxOrder * pau->m_cChannel * 2);
    ...
    pMCLMSPrdtr->m_rgiUpdate = (I16 *)memset (pMCLMSPrdtr->m_rgiUpdate, 0, sizeof(pMCLMSPrdtr->m_rgiUpdate[0]) * iMaxOrder * pau->m_cChannel * 2);
...
Void prvMCLMSPredictorReset(CAudioObject * pau, MCLMSPredictor * pMCLMSPrdtr)
{
    ...
    pMCLMSPrdtr->m_rgiPrevVal = (I32 *)memset (pMCLMSPrdtr->m_rgiPrevVal, 0, sizeof(pMCLMSPrdtr->m_rgiPrevVal[0]) * iOrder * pau->m_cChannel * 2);
    pMCLMSPrdtr->m_rgiUpdate = (I16 *)memset (pMCLMSPrdtr->m_rgiUpdate, 0, sizeof(pMCLMSPrdtr->m_rgiUpdate[0]) * iOrder * pau->m_cChannel * 2);
...
Void prvMCLMSPredictorUpdate_I16_C(CAudioObject * pau, MCLMSPredictor * pMCLMSPredictor, CBT * rgiInput, CBT *rgiPred)
{
    ...
    if (pMCLMSPredictor->m_iRecent == 0) {
		Void *pTemp;
        pTemp = voWMAMemcpy(rgiPrevVal + pMCLMSPredictor->m_iOrder * pau->m_cChannel, rgiPrevVal, sizeof(rgiPrevVal[0]) * pMCLMSPredictor->m_iOrder_X_CH);
        pTemp = voWMAMemcpy(pMCLMSPredictor->m_rgiUpdate + pMCLMSPredictor->m_iOrder * pau->m_cChannel, pMCLMSPredictor->m_rgiUpdate, sizeof(pMCLMSPredictor->m_rgiUpdate[0]) * pMCLMSPredictor->m_iOrder_X_CH);
...
Void prvMCLMSPredictorUpdate_I32_C(CAudioObject * pau, MCLMSPredictor * pMCLMSPredictor, CBT * rgiInput, CBT * rgiPred)
{
    ...
    if (pMCLMSPredictor->m_iRecent == 0) {
		Void *pTemp;
        pTemp = voWMAMemcpy(pMCLMSPredictor->m_rgiPrevVal + pMCLMSPredictor->m_iOrder * pau->m_cChannel, pMCLMSPredictor->m_rgiPrevVal, sizeof(pMCLMSPredictor->m_rgiPrevVal[0]) * pMCLMSPredictor->m_iOrder_X_CH);
        pTemp = voWMAMemcpy(pMCLMSPredictor->m_rgiUpdate + pMCLMSPredictor->m_iOrder * pau->m_cChannel, pMCLMSPredictor->m_rgiUpdate, sizeof(pMCLMSPredictor->m_rgiUpdate[0]) * pMCLMSPredictor->m_iOrder_X_CH);
        pMCLMSPredictor->m_iRecent = pMCLMSPredictor->m_iOrder_X_CH;
    }
}
...
Void prvDecMCLMSPredictorPredAndUpdate_I16(CAudioObject * pau, MCLMSPredictor * pMCLMSPredictor, CBT *rgiInput, CBT *rgiPred)
{
    ...
    if (pMCLMSPredictor->m_iRecent == 0) {
		Void *pTemp;
        pTemp = voWMAMemcpy(piPrevVal + pMCLMSPredictor->m_iOrder_X_CH, piPrevVal, sizeof(piPrevVal[0]) * pMCLMSPredictor->m_iOrder_X_CH);
        pTemp = voWMAMemcpy(pMCLMSPredictor->m_rgiUpdate + pMCLMSPredictor->m_iOrder_X_CH, pMCLMSPredictor->m_rgiUpdate, sizeof(pMCLMSPredictor->m_rgiUpdate[0]) * pMCLMSPredictor->m_iOrder_X_CH);
        pMCLMSPredictor->m_iRecent = pMCLMSPredictor->m_iOrder_X_CH;
    }
}
...
Void prvFreeMCLMSPredictor(CAudioObjectDecoder *paudec, MCLMSPredictor * pMCLMSPrdtr)
{
/*    
