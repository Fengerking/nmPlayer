	     ...
	     )
{
    ...
    t0 = voMpy_32(Kh, Kl, Kh, Kl);           /* K*K      in Q31 */
    ...
    t0 = voMpy_32(Rh[0], Rl[0], hi, lo);     /* Alpha in Q31    */
    ...
    for (i = 2; i <= M; i++)
    {
        ...
        t0 = 0;                           
        for (j = 1; j < i; j++)
            t0 = vo_L_add(t0, voMpy_32(Rh[j], Rl[j], Ah[i - j], Al[i - j]));
        ...
        for (j = 1; j < i; j++)
        {
            t0 = voMpy_32(Kh, Kl, Ah[i - j], Al[i - j]);
			t0 = vo_L_add(t0, ((Ah[j] << 16) + (Al[j] << 1)));

            //t0 = vo_L_add(t0, L_Comp(Ah[j], Al[j]));
		...
		hi = t0 >> 16;
		lo = (t0 & 0xffff)>>1;
        t0 = voMpy_32(alp_h, alp_l, hi, lo); /* Alpha in Q31    */
