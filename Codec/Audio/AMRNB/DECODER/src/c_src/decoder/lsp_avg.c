...
**************************************************************************
*/
int lsp_avg_init (lsp_avgState **state, VO_MEM_OPERATOR *pMemOP)
{
	lsp_avgState* s;
	...
	*state = NULL;
	/* allocate memory */
	if ((s = (lsp_avgState *) voAMRNBDecmem_malloc(pMemOP, sizeof(lsp_avgState), 32)) == NULL){
		return -1;
	}
	lsp_avg_reset(s);
	*state = s;
...
{ 
	...
	voAMRNBDecCopy(voAMRNBDecmean_lsf, &st->lsp_meanSave[0], M);
	return 0;
}
...
**************************************************************************
*/
void lsp_avg_exit (lsp_avgState **state, VO_MEM_OPERATOR *pMemOP)
{
			  ...
			  )
{
	...
	for (i = 0; i < M; i++) {
		...
		L_tmp -= (EXPCONST* st->lsp_meanSave[i])<<1;//L_msu(L_tmp, EXPCONST, st->lsp_meanSave[i]);

		/* Add 0.16 of newest LSPs to mean */
		L_tmp += (EXPCONST* lsp[i])<<1; //L_mac(L_tmp, EXPCONST, lsp[i]);
		/* Save means */
		st->lsp_meanSave[i] = vo_round(L_tmp);             /* Q15 */
