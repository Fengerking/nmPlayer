...
{
    pBufFilt = (BufFilt *)memset(pBufFilt, 0, sizeof(BufFilt));
}

void bufFiltFree(CAudioObjectDecoder *paudec, struct BufFilt *pBufFilt)
{
...
WMARESULT bufFiltInit(CAudioObjectDecoder *paudec, BufFilt *pBufFilt, Double *coeffs, __in_bound Int startPos,
                      __in_bound Int endPos, Int cInputs)
{
    ...
    if ( pBufFilt->m_iStartPos > 0 ) { hr = WMA_E_INVALIDARG;  return hr; }
    if (pBufFilt->m_iLength - pBufFilt->m_iStartPos <= 0)
    { hr = WMA_E_INVALIDARG;  return hr; }
    if ( pBufFilt->m_iLength <= 0 ) { hr = WMA_E_INVALIDARG;  return hr; }
...
//    pBufFilt->m_filtCoeffs = (CompType*)mallocAligned(iLengthCoefPad * sizeof(CompType), 16);
    pBufFilt->m_filtCoeffs = (float *)mallocAligned(iLengthCoefPad * sizeof(CompType), 16);
    CHECKWMA_ALLOC(hr, pBufFilt->m_filtCoeffs);
    pBufFilt->m_filtCoeffs = (float *)memset(pBufFilt->m_filtCoeffs, 0, iLengthCoefPad * sizeof(CompType));
    ...
    pBufFilt->m_dataBuf = (float *)memset(pBufFilt->m_dataBuf, 0, iLengthDataPad * sizeof(DataType));
...
{
    float *pOut = pInOutBuf, *pData0 = NULL, *pData1 = NULL, *pCoef = NULL, total;
    Int cCoef, i, j;
	
    assert(pBufFilt && pInOutBuf);

    pData0 = (float *)voWMAMemcpy(pBufFilt->m_dataBuf + pBufFilt->m_curBufLen, pInOutBuf, cLen * sizeof(float));
    ...
    pBufFilt->m_dataBuf = (float *)auMemmove(pBufFilt->m_dataBuf, pBufFilt->m_dataBuf + cLen,
...
WMARESULT bufFilt_SSE(CAudioObjectDecoder *paudec, BufFilt *pBufFilt, float *pInOutBuf, Int cLen)
{
    ...
    voWMAMemcpy(pBufFilt->m_dataBuf + pBufFilt->m_curBufLen, pInOutBuf, cLen * sizeof(float));
    ...
    // copy data history to the beginning of data buffer
    auMemmove(pBufFilt->m_dataBuf, pBufFilt->m_dataBuf + cLen,
            pBufFilt->m_curBufLen * sizeof(float));
...
{
    pBufDelay = (BufDelay *)memset(pBufDelay, 0, sizeof(BufDelay));
}

void bufDelayFree(CAudioObjectDecoder *paudec, BufDelay *pBufDelay)
{
...
WMARESULT bufDelayInit(CAudioObjectDecoder *paudec, BufDelay *pBufDelay, Int delay)
{
    WMARESULT hr = WMA_OK;
    ...
    pBufDelay->m_dataBuf = (float *)memset(pBufDelay->m_dataBuf, 0, pBufDelay->m_iDelay * sizeof(DataType));

    pBufDelay->m_endDataBuf = (DataType*)auMalloc(pBufDelay->m_iDelay * sizeof(DataType));
    CHECKWMA_ALLOC(hr, pBufDelay->m_endDataBuf);
    pBufDelay->m_endDataBuf = (float *)memset(pBufDelay->m_endDataBuf, 0, pBufDelay->m_iDelay * sizeof(DataType));
...
WMARESULT bufDelay(CAudioObjectDecoder *paudec, BufDelay *pBufDelay, float *in, Int inLen, Int *pOutLen)
{
    ...
    assert(ptsToSave <= pBufDelay->m_iDelay);
    pBufDelay->m_endDataBuf = (float *)voWMAMemcpy(pBufDelay->m_endDataBuf, in + inLen - ptsToSave, ptsToSave * sizeof(DataType));
    ...
    if (ptsOut >= pBufDelay->m_curBufLen)
    {
        pBufDelay->m_dataBuf = (float *)voWMAMemcpy(pBufDelay->m_dataBuf, pBufDelay->m_endDataBuf, ptsToSave * sizeof(DataType));
    ...
    else
    {
        ...
        pBufDelay->m_dataBuf = (float *)auMemmove(pBufDelay->m_dataBuf, pBufDelay->m_dataBuf + ptsOut,
            (pBufDelay->m_curBufLen - ptsOut) * sizeof(DataType));
        pTemp = (float *)voWMAMemcpy(pBufDelay->m_dataBuf + pBufDelay->m_curBufLen - ptsOut, pBufDelay->m_endDataBuf,
