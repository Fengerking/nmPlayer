            ...
            case SUBFRM_REC_PLLM:
                ...
                for (paudec->m_iChannel = 0; paudec->m_iChannel < pau->m_cChInTile; paudec->m_iChannel++) 
                {
                    ...
                    if (ppcinfo->m_iPower == 0) {
                        ...
                        // in decoder side.
                        pTemp = memset (ppcinfo->m_rgiCoefRecon + ppcinfo->m_rgsubfrmconfig[CONFIG_CURRFRM].m_rgiSubFrameStart[0], 0, sizeof (CoefType) * ppcinfo->m_cSubFrameSampleHalf);
...
{      
    ...
    while (paudec->m_hdrdecsts != HDR_DONE) {
        ...
        case HDR_ACFILTER:
            if (pau->m_bDoACFilter == WMAB_TRUE) {
                for (; pau->m_iACFilterCurr < pau->m_iACFilterOrder; pau->m_iACFilterCurr++) {
                    if (pau->m_bSeekable == WMAB_TRUE) {
        ...
        case HDR_MCLMS_COEFF_A:
            if (pau->m_bDoMCLMS == WMAB_TRUE) {
                if (pau->m_bSendMCLMS == WMAB_TRUE) {
                    ...
                    for (; paudec->m_iLMSCurrCoeff < pau->m_MCLMSPredictor.m_iOrder * pau->m_cChannel * pau->m_cChannel;
            ...
            if (pau->m_bDoMCLMS == WMAB_TRUE) {
                if (pau->m_bSendMCLMS == WMAB_TRUE) {
                    ...
                    for (; paudec->m_iChannel < pau->m_cChannel; paudec->m_iChannel++) {
                        for (; paudec->m_iLMSCurrCoeff < paudec->m_iChannel; paudec->m_iLMSCurrCoeff++) {
                            cBits = pau->m_MCLMSPredictor.m_cBitsSentPerCoeff;
                            TRACEWMA_EXIT(wmaResult, ibstrmGetBits (&paudec->m_ibstrm, cBits, &iResult));
                            iResult = (iResult << iShiftL);
                            iResult = ((I32)iResult >> iShiftR);
            ...
            for (; paudec->m_iChannel < pau->m_cChInTile; paudec->m_iChannel++) {                       
                ...
                switch(paudec->m_hdrdecFilterParasts)
                {
                    ...
                    case HDRFLT_CLMS_ORDERS:
                        ...
                        for (; ppcInfoComm->m_iLMSPredictorCurr < ppcInfoComm->m_cLMSPredictors; ppcInfoComm->m_iLMSPredictorCurr++) {
                            ...
                            for (; ppcInfoComm->m_iLMSPredictorCurr < ppcInfoComm->m_cLMSPredictors; ppcInfoComm->m_iLMSPredictorCurr++) {
                                switch(paudec->m_hdrdecFilterParaCLMSsts) 
                                {
                                    ...
                                    case HDRFLT_CLMS_COEFFS_VALUE:
                                        ...
                                        for (;paudec->m_iLMSCurrCoeff < ppcInfoComm->m_rgLMSPredictors[ppcInfoComm->m_iLMSPredictorCurr].m_cCoeffsSent;
                                                paudec->m_iLMSCurrCoeff++) 
                                        {
                                            TRACEWMA_EXIT(wmaResult, ibstrmGetBits (&paudec->m_ibstrm, cBits, &iResult));
                                            iResult = (iResult << iShiftL);
...
Void prvDecoderCDLMSPredictorHelper(CAudioObjectDecoder *paudec, PerChannelInfo* ppcinfoComm, PerChannelInfo* ppcinfo, CBT * rgiResidue, I32 iDataLen)
{
    ...
    for (i = ppcinfoComm->m_cLMSPredictors - 1; i >= 0; i--) {
        struct LMSPredictor * pLMSPredictor = ppcinfoComm->m_rgLMSPredictors + i;
        if (Q1 > 0) {
...
//		printf("rgiCoefRecon: 0x%.8x, 0x%.8x, 0x%.8x, 0x%.8x, 0x%.8x, 0x%.8x, 0x%.8x, 0x%.8x,\n", rgiResidue[0], rgiResidue[1], 
//		rgiResidue[2], rgiResidue[3], rgiResidue[4], rgiResidue[5], rgiResidue[6], rgiResidue[7]);
