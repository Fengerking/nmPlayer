...
)
{
  ...
  for(i=0; i<MA_NP; i++)
	  memcpy(&freq_prev[i][0],&freq_prev_reset[0],2*M);
...
#ifdef ASM_OPT  
	Lsp_pre_select_asm(rbuf, lspcb1, &cand_cur );
#else
...
)
{
  ...
  *cand = 0;
  ...
  for ( i = 0 ; i < NC0 ; i++ ) {
    ...
    //for ( j = 0 ; j < M ; j++ )
	{
      tmp = rbuf[0] - lspcb1[i][0];
      L_tmp += ((tmp * tmp));

	  tmp = rbuf[1] - lspcb1[i][1];
      L_tmp += ((tmp * tmp));

	  tmp = rbuf[2] - lspcb1[i][2];
      L_tmp += ((tmp * tmp));

	  tmp = rbuf[3] - lspcb1[i][3];
      L_tmp += ((tmp * tmp));

	  tmp = rbuf[4] - lspcb1[i][4];
      L_tmp += ((tmp * tmp));

	  tmp = rbuf[5] - lspcb1[i][5];
      L_tmp += ((tmp * tmp));

	  tmp = rbuf[6] - lspcb1[i][6];
      L_tmp += ((tmp * tmp));

	  tmp = rbuf[7] - lspcb1[i][7];
      L_tmp += ((tmp * tmp));

	  tmp = rbuf[8] - lspcb1[i][8];
      L_tmp += ((tmp * tmp));

	  tmp = rbuf[9] - lspcb1[i][9];
      L_tmp += ((tmp * tmp));

    }

    L_temp = (L_tmp<<1) - L_dmin;
    if (  L_temp< 0L) {
      L_dmin = L_tmp<<1;
      *cand = i;
    }
...
)
{
  ...
  //for ( j = NC ; j < M ; j++ )
  buf[5] = sub(rbuf[5], lspcb1[5]);
  buf[6] = sub(rbuf[6], lspcb1[6]);
  buf[7] = sub(rbuf[7], lspcb1[7]);
  buf[8] = sub(rbuf[8], lspcb1[8]);
  buf[9] = sub(rbuf[9], lspcb1[9]);
  ...
  *index = 0;
  ...
  for ( k1 = 0 ; k1 < NC1 ; k1++ ) {
   ...
   // for ( j = NC ; j < M ; j++ ) 
	{
		      tmp = sub(buf[5], lspcb2[k1][5]);
      tmp2 = ( wegt[5] * tmp )>>15;
      L_dist += ((tmp2 * tmp )<<1);
	        tmp = sub(buf[6], lspcb2[k1][6]);
      tmp2 = ( wegt[6] * tmp )>>15;
      L_dist += ((tmp2 * tmp )<<1);
	        tmp = sub(buf[7], lspcb2[k1][7]);
      tmp2 = ( wegt[7] * tmp )>>15;
      L_dist += ((tmp2 * tmp )<<1);
	        tmp = sub(buf[8], lspcb2[k1][8]);
      tmp2 = ( wegt[8] * tmp )>>15;
      L_dist += ((tmp2 * tmp )<<1);
      tmp = sub(buf[9], lspcb2[k1][9]);
      tmp2 = ( wegt[9] * tmp )>>15;
      L_dist += ((tmp2 * tmp )<<1);
    }
...
)
{
  ...
  for ( i = 0 ; i < M ; i++ ) {
    if ( buf[i] > 0 ){
      ...
      L_acc = ( buf[i] * buf[i] )<<1;           /* L_acc in Q27 */
      tmp = extract_h( ( L_acc << 2 ) );       /* tmp in Q13 */

      L_acc = ( tmp * CONST10 )<<1;             /* L_acc in Q25 */
      tmp = extract_h( ( L_acc << 2 ) );       /* tmp in Q11 */
  ...
  /* */
  L_acc = ( wegt[4] * CONST12 )<<1;             /* L_acc in Q26 */
  wegt[4] = extract_h( ( L_acc << 1 ) );       /* wegt in Q11 */

  L_acc = ( wegt[5] * CONST12 )<<1;             /* L_acc in Q26 */
  wegt[5] = extract_h( ( L_acc <<1 ) );       /* wegt in Q11 */
