...
#ifndef __MPEG_READBITS_H__
#define __MPEG_READBITS_H__
#include "voMpeg4DecGlobal.h"
#include "voMpeg4Dec.h"

#define BYTESWAP(a) ((a<<24) | ((a<<8)&0x00ff0000) | ((a>>8)&0x0000ff00) | (a>>24))

typedef VO_MPEG4_DEC BitStream;

static INLINE VO_VOID InitBitStream(BitStream* bs_ptr, VO_U8 *stream, VO_S32 len)
{
	VO_U32 tmp;
...
#define UPDATE_CACHE_LARGE(bs_ptr)	\
 ...
 tmp = BYTESWAP(tmp);\
 if (-32 == bs_ptr->nBitPos){\
 bs_ptr->cache_a = tmp;\
 bs_ptr->cache_b = 0;\
 }else{\
 bs_ptr->cache_a |= (tmp >> (32 + bs_ptr->nBitPos));\
 bs_ptr->cache_b = (tmp << (-bs_ptr->nBitPos));\
 }\
 bs_ptr->pBitPtr += 4;\
 bs_ptr->nBitPos += 32;\
