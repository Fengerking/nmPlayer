...
)
{
	...
	for (j = 0; j < FREQBANDS; j++)
	{
		...
		e_mem->snr[j] = L_shr(L_divide(L_shl(e_mem->signal_energy[j], (Word16)n_shiftcnt), (e_mem->band_noise_sm[j] << d_shiftcnt)), (Word16)(23 - (d_shiftcnt - n_shiftcnt)));
		e_mem->snr_map[j] = 0;
	}
}
...
)
{
	...
	for (j = 0; j < FREQBANDS; j++)
	{
		...
		energy[j] = (L_mpy_ls(R[0], r_filt_x2[j][0]) >> 1);	/* E. Chen: zeroth entry is not normed */
...
)
{
	...
	if (e_mem->first == 0)
	{
		...
		for (i = 0; i < FREQBANDS; i++)
		{
			e_mem->frame_energy_sm[i] = L_shl2(e_mem->band_power[i], 3);
		}
	}
	else
	{
		for (i = 0; i < FREQBANDS; i++)
		{
			...
			e_mem->frame_energy_sm[i] = L_add(L_mpy_ls(L_sub(e_mem->frame_energy_sm[i],
														 L_shl2(e_mem->band_power[i], 3)), SMSNR), L_shl2(e_mem->band_power[i], 3));
		}
	}
...
/*************************************************************************/
	...
	if (e_mem->adaptcount > ADP)
	{
		...
		for (i = 0; i < FREQBANDS; i++)
		{
			...
			if ((e_mem->band_noise_sm[i] + 1) < L_shl2(L_mpy_ls(e_mem->band_noise_sm[i], INC_FACTOR), 1))
			{
				/* E. Chen: e_mem->band_noise_sm[i] *= INC_FACTOR; */
				e_mem->band_noise_sm[i] = L_shl2(L_mpy_ls(e_mem->band_noise_sm[i], INC_FACTOR), 1);
			}
			else
			{
			...
			for (i = 0; i < FREQBANDS; i++)
			{
					 ...
					 e_mem->frame_energy_sm[i]))
				{
					...
					if ((e_mem->band_noise_sm[i] + 1) <
						L_shl2(L_mpy_ls(e_mem->band_noise_sm[i], IS96_INC), 1))
					{
...
/************************************************************************/
	...
	for (i = 0; i < FREQBANDS; i++)
	{
		...
		if (e_mem->band_noise_sm[i] > 0)
		{
			...
			snr_temp = L_divide(L_shl(e_mem->signal_energy[i], (Word16)n_shiftcnt), (e_mem->band_noise_sm[i] << d_shiftcnt));
			e_mem->snr[i] = L_shr(snr_temp, (Word16)(23 - (d_shiftcnt - n_shiftcnt)));
		}
		else
		{
	...
	if (e_mem->snr_stat_once != 0)
	{
		...
		for (i = 0; i < FREQBANDS; i++)
		{
			...
			if (e_mem->snr[i] > 0)
			{
				...
				e_mem->snr_map[i] = extract_h(L_add(L_sub((snr_temp >> 9), 4 * 65536), 32768));
			}
			else
			{
				e_mem->snr_map[i] = 0;
				  ...
				  )
{
	...
	for (k = 0; k < FREQBANDS; k++)
	{
		...
		for (i = 0; i < 2; i++)
		{
			...
			if (L_shl2(e_mem->band_power[k], 3) > L_shl2(L_mpy_ls(e_mem->band_noise_sm[k], THRESH_SNR[k][e_mem->snr_map[k]][i]), 7))
			{
