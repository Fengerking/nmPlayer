...
#if USE_FRAME_THREAD
		if (ctx->pbi->nThdNum > 1)
		{
			if (ctx->cfg.flush) //handle flush cmd
			{
				DecodeFrameMain(ctx->pbi, &ctx->img, 0, NULL, 0, 2);
				ctx->cfg.flush = 0;
				return res;
			}
			DecodeFrameMain(ctx->pbi, &ctx->img, data_sz, data, deadline, data != NULL);
// 			if (ctx->img.planes[VPX_PLANE_Y])
// 			{
				ctx->img_avail = 1;
// 			}
		}
