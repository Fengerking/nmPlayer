...
vomsmrender::vomsmrender(VO_PTR hInst, VO_MEM_OPERATOR * pMemOP) 
...
vomsmrender::~vomsmrender() {
	...
	if (m_pBuffer != NULL)
	{
		m_pMemOP->Free (VO_INDEX_SNK_AUDIO, m_pBuffer);
		m_pBuffer = NULL;
	}
}
...
bool vomsmrender::open()
{
	...
	if (status < 0) {
		VOINFO("Cannot read config");
		return false;	
	}

	/*VOINFON(m_pvoaparam->wchannels);
	VOINFON(m_pvoaparam->dwsamplerate);
	VOINFON(m_pvoaparam->dwAvgBytesPerSec);*/
...
VO_U32 vomsmrender::Render(VO_PBYTE buffer, VO_U32 size, VO_U64 nStart, VO_BOOL bWait) {
	...
	if (m_nRenderNum == 0 && m_nBuffSize + size < m_nminbufsize * 2)
	{
		...
		{
			VO_MEM_INFO	mi;
			mi.Size = m_nminbufsize * 2;
			m_pMemOP->Alloc(VO_INDEX_SNK_AUDIO, &mi);
			m_pBuffer = (VO_PBYTE)mi.VBuffer;
		}

		m_pMemOP->Copy (VO_INDEX_SNK_AUDIO, m_pBuffer + m_nBuffSize, buffer, size);
	...
	if (m_nBuffSize > 0 && m_nBuffSize < m_nminbufsize * 2)
	{
		...
		m_pMemOP->Copy (VO_INDEX_SNK_AUDIO, m_pBuffer + m_nBuffSize, buffer, nRestSize);
