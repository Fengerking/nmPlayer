...
OMX_ERRORTYPE COMXEngine::Init (VOMECallBack pCallBack, OMX_PTR pUserData)
{
	JNILOGI("engine init");
	Release ();
	JNILOGI("after release");
	if (LoadDll () == 0)
	{
		JNILOGE("load dll failed");
		return OMX_ErrorUndefined;
	}
		JNILOGI("before call init");
	m_OMXEng.Init (&m_hEngine, 0, pCallBack, pUserData);

	if (m_hEngine != NULL)
    {

       JNILOGI2(" pCallBack = %x", pCallBack);
#if !defined(_WIN32_XP)
       //m_OMXEng.SetParam (m_hEngine,VOME_PID_WorkingPath, (void*)(VO_APPLICATION_LOCATION));
       m_OMXEng.SetParam (m_hEngine,VOME_PID_WorkingPath, (void*)("/data/data/com.visualon.vome/"));
#endif//_WIN32_XP
...
OMX_U32 COMXEngine::LoadDll (void)
{
	...
	*(pPos + 1) = 0;
	...
	if (m_hDll == NULL)
	{
		m_hDll = dlopen ("libvoOMXME.so", RTLD_NOW);
		pathIdx = 1;
	}
	if (m_hDll == NULL)
	{
		...
		strcpy(tmpPath,"/data/data/com.visualon.com/lib/");strcat(tmpPath,"libvoOMXME.so");
		m_hDll = dlopen (tmpPath, RTLD_NOW);
		pathIdx = 2;
	}
	if (m_hDll == NULL) {
	  //m_hDll = dlopen ("/data/data/com.mobitv.mkplayer/lib/libvoOMXME.so", RTLD_NOW);
	  m_hDll = dlopen ("/data/data/com.visualon.vome/lib/libvoOMXME.so", RTLD_NOW);
		pathIdx = 3;
	}
	
	if (m_hDll == NULL) {
	  //m_hDll = dlopen ("/data/data/com.mobitv.mkplayer/lib/libvoOMXME.so", RTLD_NOW);
	  m_hDll = dlopen ("/data/data/com.qualcomm.mflo.flotestplayer/lib/libvoOMXME.so", RTLD_NOW);
		pathIdx = 3;
	}
	...
	if (m_hDll == NULL)
	{
		JNILOGE2 ("@@@@@@ Load Module Error %s ", dlerror ());
		return -1;
	}
	else
	{
		JNILOGI2 ("@@@@@@ Load FrameWork done %d ", pathIdx);
	}
