...
tWMVDecodeStatus CQ_InitCQueue(tWMVDecInternalMember * pWMVDec, CQueue * pCQ, I32_WMV iNumElements)
{
    ...
    {
#ifdef XDM
		pQueueElement = (CQueueElement *)pWMVDec->m_MemXDM.pPrivateBuff;
		pWMVDec->m_MemXDM.pPrivateBuff += sizeof( CQueueElement);
#else
...
Bool_WMV CQ_GetElement(CQueue * pCQ, Void_WMV **pElement, I32_WMV iElementNum)
{
    CQueueElement* pCurr;
    I32_WMV i;

    if (pElement == NULL_WMV || iElementNum >= pCQ->m_iCurrQueueElements)
        return FALSE_WMV;
...
#ifdef XDM
	pRL->m_cFrameQueue = (CQueue *)pWMVDec->m_MemXDM.pPrivateBuff;
	pWMVDec->m_MemXDM.pPrivateBuff += sizeof( CQueue);
#else
...
#endif	//VIDEO_MEM_SHARE

	{
		for (i = 0; i < iNumRefFrames; i++)
		{
	#ifdef XDM
			pFrame = (tYUV420Frame_WMV *)pWMVDec->m_MemXDM.pPrivateBuff;
			pWMVDec->m_MemXDM.pPrivateBuff += sizeof( tYUV420Frame_WMV);
	#else
			pFrame = (tYUV420Frame_WMV *)wmvMalloc (pWMVDec, sizeof (tYUV420Frame_WMV), DHEAP_STRUCT);
			if(pFrame==NULL_WMV)
			{
			...
			status = Plane_init (pFrame,  iSizeYplane,  iSizeUVplane, iPad, pWMVDec, iPlaneWidth, iPlaneHeight);
			pFrame->m_timeStamp = 0;
			pFrame->m_frameType = 0;
...
Void_WMV RL_AddFrame (CReferenceLibrary * pRL,Void_WMV *pFrame)
{
    CQ_AddElement(pRL->m_cFrameQueue, pFrame, 0);
    pRL->m_iCurrRefFrames ++;
}

I32_WMV RL_GetNextFrame (CReferenceLibrary * pRL, Void_WMV **ppFrame)
{
    I32_WMV iFrameNum;
    *ppFrame = NULL_WMV;

    if (pRL->m_iCurrRefFrames == 0)
        iFrameNum = -1;
    
    else
    {
        iFrameNum = (pRL->m_iFrameNum++) % pRL->m_iCurrRefFrames;

        if (pRL->m_iFramesChecked++ ==  pRL->m_iCurrRefFrames)
            iFrameNum = -1;

        else
            CQ_GetElement(pRL->m_cFrameQueue, ppFrame, iFrameNum);    
    }
