...
voStreamDigestAuth::voStreamDigestAuth(void)
{
    ...
    memset(m_strClientNonce, 0, 32);
	m_ulstrClientNonceLen = 0;
    memset(m_strClientNonceCount, 0, 32);
    m_ulstrClientNonceCountLen = 0;
...
VO_VOID voStreamDigestAuth::SetClientNonce(VO_CHAR *pClientNonce,VO_U16 len)
{
    memset(m_strClientNonce, 0, 32);
    m_ulstrClientNonceLen = 0;
	...
	memcpy(m_strClientNonce, pClientNonce, len);
    m_ulstrClientNonceLen = len;
}

VO_VOID voStreamDigestAuth::SetClientNonceCount(VO_CHAR *pClientNonceCount,VO_U16 len)
{
    memset(m_strClientNonceCount, 0, 32);
    m_ulstrClientNonceCountLen = 0;

	if(pClientNonceCount == NULL)
	{
	    return;
	}
	
	memcpy(m_strClientNonceCount, pClientNonceCount, len);
    m_ulstrClientNonceCountLen = len;    
}
...
VO_VOID voStreamDigestAuth::SetQOPString(VO_CHAR *pQop,VO_U16 len)
{ 
	if(!pQop)
	{
	    return;
	...
	m_strQopLen = len ;
	m_strQop = new char[m_strQopLen+1];
	memset(m_strQop, 0, m_strQopLen+1);
	strcpy(m_strQop,pQop);
...
VO_BOOL	voStreamDigestAuth::GetRespAuth(VO_CHAR *buf, unsigned int len)
{
	...
	//For the   {response = MD5{HA1:nonceCount:clientNonce:qop:HA2} } 
	VO_U16 tempLen = 32*2+64+m_strNonceLen+2;
	char *temp = new char[tempLen+1];
	...
	strcat(temp,m_strClientNonceCount);	
	iLen += m_ulstrClientNonceCountLen;
	temp[iLen] = ':';
	iLen++;

	strcat(temp,m_strClientNonce);
	iLen += m_ulstrClientNonceLen;
	temp[iLen] = ':';
	iLen++;

	strcat(temp,m_strQop);
	iLen += m_strQopLen;
	temp[iLen] = ':';
	iLen++;
	...
	VOLOGI("%s, +++++++++++len:%d",temp, iLen);
	MD5Data(buf,temp,iLen);
