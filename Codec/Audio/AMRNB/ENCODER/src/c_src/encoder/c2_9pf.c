...
*****************************************************************************
*/
static void search_2i40_9pf(
					 Word16 subNr,       /* i : subframe number                               */
					 Word16 dn[],        /* i : correlation between target and h[]            */
					 Word16 rr[][L_CODE],/* i : matrix of autocorrelation                     */
					 Word16 codvec[]     /* o : algebraic codebook vector                     */
);
static Word16 build_code_9pf(
					  Word16 subNr,       /* i : subframe number                               */
...
Word16 voAMRNBEnc_code_2i40_9bits(
					   Word16 subNr,       /* i : subframe number                               */
					   ...
					   )
{
	...
	sharp = shl3(pitch_sharp, 1);
	if (sub3(T0, L_CODE) < 0)
		for (i = T0; i < L_CODE; i++) {
			h[i] = add3(h[i], mult3(h[i - T0], sharp)); 
		}
		voAMRNBEnccor_h_x(h, x, dn, 1);
		voAMRNBEnc_set_sign(dn, dn_sign, dn2, 8); /* dn2[] not used in this codebook search */
		cor_h(h, dn_sign, rr);
		search_2i40_9pf(subNr, dn, rr, codvec); /* function result */
		index = build_code_9pf(subNr, codvec, dn_sign, code, h, y, sign);
...
#endif//AFE_CODE_search_2i40_9pf
