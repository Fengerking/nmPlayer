...
void Usage(void)
{
	printf("\nvoi_aac_Encoderr Usage:\n");
	printf(">> voi_aac_Encoderr infile.pcm outfile.aac\n");
...
LPCSTR g_parfile = "/AACEncapp.par";
FILE* fconfig;
...
#define  OUTPUT	  1
	...
	if(!(fconfig = fopen(g_parfile, "rb")))
	{
		...
		MessageBox(NULL, msg, TEXT("aac Encoder error"), MB_OK);
		return 0;
	}
...
#ifdef LINUX
	handle = dlopen("/data/local/tmp/voAACEnc.so", RTLD_NOW);
	if(handle == 0)
	{
...
#define  MAX_REMAINED 2048
	...
	MessageBox(NULL, msg, TEXT("aac Encoder Finished"), MB_OK);
#else
	printf("Encoder Time: %d clocks, Encoder Frames: %d", total, EncoderdFrame);
#endif	
