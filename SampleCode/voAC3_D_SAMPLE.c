...
int	outlfeon = 1;                      /* output subwoofer present flag */ 
int	outputmode = 7;					   /* output channel configuration */
int	numchans = 6;
...
int decoder(const char* srcfile, const char* dstfile)
{
	...
	returnCode = voGetAC3DecAPI(&AudioAPI);
...
#ifdef _WIN32_WCE
	wsprintf(msg, TEXT("Decode Time: %d clocks, decode frames: %d"), t2, framecount);
...
#ifdef _WIN32_WCE
	inFileName  = "/Storage Card/AC3/FRMSZ37.AC3";
	outFileName = "/Storage Card/AC3/Mabel_51.pcm";
#else
	inFileName  = ac3fname;
