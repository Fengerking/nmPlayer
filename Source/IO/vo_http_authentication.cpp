...
VO_VOID vo_http_authentication::Process_Property( VO_CHAR* ptr_info , const VO_CHAR* propname )
{
	...
	if( *ptr_end == '"' )
	{
		...
		{
			if( ptr_end - ptr_temp < sizeof( m_pszRealm ) )
			{
		...
		{
			if( ptr_end - ptr_temp < sizeof( m_pszQop ) )
			{
		...
		{
			if( ptr_end - ptr_temp < sizeof( m_pszNonce ) )
			{
		...
		{
			if( ptr_end - ptr_temp < sizeof( m_pszOpaque ) )
			{
		...
		{
			if( ptr_end - ptr_temp < sizeof( m_pszURI ) )
			{
		...
		{
			if( ptr_end - ptr_temp < sizeof( m_pszStale ) )
			{
		...
		{
			if( ptr_end - ptr_temp < sizeof( m_pszAlg ) )
			{
...
{
	if( strlen(ptr_challenge) < sizeof(m_challengeinfo) )
	{
	...
	if( ptr_autheninfo_head )
	{
		...
		if( ptr_linend - ptr_autheninfo_head < sizeof(authentype) )
		{
    ...
    )
{
	...
	if (voiostrnicmp(pszAlg, "md5-sess", strlen("md5-sess")) == 0)
	{
		md5.reset();
    ...
    )
{
	...
	if (voiostrnicmp(pszQop, "auth-int", strlen ("auth-int")) == 0) 
	{
		md5.update( (VO_BYTE*)":", 1);
