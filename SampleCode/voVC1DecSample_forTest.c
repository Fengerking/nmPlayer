...
int   OutputOneFrame( VO_VIDEO_BUFFER *par, VO_VIDEO_OUTPUTINFO* outFormat,FILE*fp);
...
#endif
{
	...
	unsigned int  outputmode = 1;
	unsigned int  aspectratio = 1;
...
#define MAX_HEADER_SIZE	1024
	...
	do 
	{
		...
		else*/
		{
			...
			CPUNUM = *argv[3] -0x30;
			loopCount =  (*argv[4][0] -0x30)*100 + (*argv[4][1] -0x30)*10 + (*argv[4][2] -0x30);
#endif
		}
		...
		do 
		{
            ...
            if(1){
...

                {
					...
					unsigned char asf = 0x25;
					int len =inDataHead.Length+1;
            ...
            if(1)
            {
                ...
                printf("frametype =%d, %d %d  returnCode = %d  frameNumber = %d  \n",outFormat.Format.Type,outFormat.Format.Width,outFormat.Format.Height,returnCode,frameNumber);
                ...
                { 
					...
					if(1){
						...
						static int z=0;
						char name[255];
						sprintf(name,"E:/MyResource/Video/clips/VC1/notbitmatch/yuv/smoothstream%dx%d_%d.yuv",
							outFormat.Format.Width,outFormat.Format.Height,z++);
						fp =fopen(name,"ab");
						OutputOneFrame(&outData, &outFormat,fp);
						fclose(fp);
...
End2: 
        ...
        printf(" \n\n decode %d times, %d ,Average speed : %2.3f fps  \n ",decoderCalledCount, frameNumber,mfps/decoderCalledCount);
