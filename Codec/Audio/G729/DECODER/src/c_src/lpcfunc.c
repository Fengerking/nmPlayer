	...
	*f = - L_mult(*lsp, 512);    /* f[1] =  -2.0 * lsp[0];  in Q24  */
	...
	for(i=2; i<=5; i++)
	{
		*f = f[-2];
		...
		{
	        hi = extract_h(f[-1]);
	        lo = extract_l(L_msu((f[-1] >> 1),hi,16384));
            t0 = L_mult(hi, *lsp);
			 ...
			 )
{
	...
	for(i= m-(Word16)1; i >= 0; i--)
	{
		...
		tmp = vo_round(L_shl(L_tmp, 3));     /*(lsp[i]-table[ind])*slope[ind])>>12*/
			  ...
			  )
{
	...
	for(i=0; i<m; i++)
	{
		...
		lsp[i] = add(table2[ind], extract_l((L_tmp >> 13)));
			  ...
			  )
{
	...
	for(i= m-(Word16)1; i >= 0; i--)
	{
		...
		freq = add(shl(ind, 9), extract_l((L_tmp >> 12)));
