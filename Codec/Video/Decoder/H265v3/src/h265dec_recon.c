...
DECLARE_ALIGNED(16, const VO_S32, intra_pred_angle[33])= 
{
	32, 26, 21, 17, 13, 9, 5, 2, 0, -2, -5, -9, -13, -17, -21, -26, -32,
	-26, -21, -17, -13, -9, -5, -2, 0, 2, 5, 9, 13, 17, 21, 26, 32
};

DECLARE_ALIGNED(16, const VO_S32, inv_angle[15])= 
{
...
#define INTRA_ASM_FUNC_NAME(func_name)		\
	extern VO_VOID  func_name( VO_U8 *p_cur, \
	VO_U8 *top,								\
	VO_U8 *left,							\
	VO_S32 dst_stride,						\
	VO_S32 cIdx,							\
	...
	INTRA_ASM_FUNC_NAME(H265_IntraPred_Angular_4_8_GE18_X86_sse2)
	INTRA_ASM_FUNC_NAME(H265_IntraPred_Angular_8_8_GE18_X86_sse2)
	INTRA_ASM_FUNC_NAME(H265_IntraPred_Angular_16_8_GE18_X86_sse2)
	INTRA_ASM_FUNC_NAME(H265_IntraPred_Angular_32_8_GE18_X86_sse2)

	INTRA_ASM_FUNC_NAME(H265_IntraPred_Angular_4_8_LT18_X86_sse2)
	INTRA_ASM_FUNC_NAME(H265_IntraPred_Angular_8_8_LT18_X86_sse2)
	INTRA_ASM_FUNC_NAME(H265_IntraPred_Angular_16_8_LT18_X86_sse2)
	INTRA_ASM_FUNC_NAME(H265_IntraPred_Angular_32_8_LT18_X86_sse2)

	void H265_Intra_64Pix_MultiAdd_X86_sse2(VO_U8 * dest_buf, VO_U8 leftMost, VO_U8 rightMost);
	...
	void H265_IntraPred_DC_X86_sse2( VO_U8 *p_ref, const VO_U8 *top, const VO_U8 *left,VO_S32 pic_stride, VO_S32 log2TrafoSize, VO_S32 cIdx );
...
// 	VO_U32 maxCUHeight = p_slice->p_sps->log2_luma_coding_block_size;
	...
	VO_U32 bNeedXChange = p_slice->dbk_flag &&
		yTbCmp > 0 && (!(yTbCmp<<(32 - p_slice->p_sps->log2_luma_coding_block_size))) ;
...

	{
		...
		if (bNeedXChange) {
			...
			row = !(row & 1);
#endif
			src_top = cIdx ? (cIdx == 1 ? (p_slice->m_puBufForIntraU[row] + xTbY) :
				(p_slice->m_puBufForIntraV[row] + xTbY)) : (p_slice->m_puBufForIntraY[row] + xTbY) ;
		}else{
    ...
    if ( !(available_flag & 0x1) ) {
        if ( (available_flag & 0x2) ) {
			...
			if(p_slice->p_user_op_all->p_user_op == NULL)
				SetMem_X86_mmx(left+nTbS,left[nTbS-1], nTbS);
			else
				SetMem(p_slice->p_user_op_all->p_user_op, p_slice->p_user_op_all->codec_id, left+nTbS,left[nTbS-1], nTbS);
#else
...
#elif defined (ARCH_X86)
			if(p_slice->p_user_op_all->p_user_op == NULL)
				SetMem_X86_mmx(left,left[-1], nTbS<<1);
			else
				SetMem(p_slice->p_user_op_all->p_user_op, p_slice->p_user_op_all->codec_id, left,left[-1], nTbS<<1);
#else
...
#elif defined (ARCH_X86)
			if(p_slice->p_user_op_all->p_user_op == NULL)
				SetMem_X86_mmx(left,top[0], nTbS<<1);
			else
				SetMem(p_slice->p_user_op_all->p_user_op, p_slice->p_user_op_all->codec_id, left,top[0], nTbS<<1);
#else
...
#elif defined (ARCH_X86)
			if(p_slice->p_user_op_all->p_user_op == NULL)
				SetMem_X86_mmx(top,top[nTbS], nTbS);
			else
				SetMem(p_slice->p_user_op_all->p_user_op, p_slice->p_user_op_all->codec_id, top,top[nTbS], nTbS);
#else
...
//             l_avail     = 1;
...
#elif defined (ARCH_X86)
			if(p_slice->p_user_op_all->p_user_op == NULL)
				SetMem_X86_mmx(top,128, nTbS<<1);
			else
				SetMem(p_slice->p_user_op_all->p_user_op, p_slice->p_user_op_all->codec_id, top,128, nTbS<<1);
#else
...
#elif defined (ARCH_X86)
			if(p_slice->p_user_op_all->p_user_op == NULL)
				SetMem_X86_mmx(left,128, nTbS<<1);
			else
				SetMem(p_slice->p_user_op_all->p_user_op, p_slice->p_user_op_all->codec_id, left,128, nTbS<<1);
#else
...
#elif defined (ARCH_X86)
		if(p_slice->p_user_op_all->p_user_op == NULL)
			SetMem_X86_mmx(left,left[ nTbS ], nTbS);
		else
			SetMem(p_slice->p_user_op_all->p_user_op, p_slice->p_user_op_all->codec_id, left,left[ nTbS ], nTbS);
#else
...
#elif defined (ARCH_X86)
		if(p_slice->p_user_op_all->p_user_op == NULL)
			SetMem_X86_mmx(top,left[ -1 ], nTbS);
		else
			SetMem(p_slice->p_user_op_all->p_user_op, p_slice->p_user_op_all->codec_id, top,left[ -1 ], nTbS);
#else
...
#elif defined (ARCH_X86)
		if(p_slice->p_user_op_all->p_user_op == NULL)
			SetMem_X86_mmx(top+nTbS,top[ nTbS-1 ], nTbS);
		else
			SetMem(p_slice->p_user_op_all->p_user_op, p_slice->p_user_op_all->codec_id, top+nTbS,top[ nTbS-1 ], nTbS);
#else
...
#if defined(ARCH_X86)
					H265_Intra_64Pix_MultiAdd_X86_sse2(filtered_top, top[-1], top[63]);
					H265_Intra_64Pix_MultiAdd_X86_sse2(left, left[-1], left[63]);
#else
...
#elif defined(ARCH_X86)
				H265_Intra_Filter_Left_Top_X86_sse2(filtered_left , filtered_top ,left, top, nTbS);
#else
				filtered_left[2*nTbS-1] = left[2*nTbS-1];
				filtered_top[2*nTbS-1]  = top[2*nTbS-1];
    ...
    case INTRA_PLANAR:
#if defined(ARCH_X86)
		H265_IntraPred_Planar_X86_sse2(p_ref, top, left, pic_stride, log2TrafoSize);
#else
    ...
    case INTRA_DC:
#if defined(ARCH_X86)
		H265_IntraPred_DC_X86_sse2( p_ref, top, left, pic_stride, log2TrafoSize, cIdx );
#else
    ...
    default:
		...
		if ( predModeIntra >= 18 ) {
			...
			{
			case 4:
				H265_IntraPred_Angular_4_8_GE18_X86_sse2(p_ref,top,left,pic_stride,cIdx,predModeIntra);
				break;
			case 8:
				H265_IntraPred_Angular_8_8_GE18_X86_sse2(p_ref,top,left,pic_stride,cIdx,predModeIntra);
				break;
			case 16:
				H265_IntraPred_Angular_16_8_GE18_X86_sse2(p_ref,top,left,pic_stride,cIdx,predModeIntra);
				break;
			case 32:
				H265_IntraPred_Angular_32_8_GE18_X86_sse2(p_ref,top,left,pic_stride,cIdx,predModeIntra);
				break;
			default:
				break;
			}
			...
			{
			case 4:
				H265_IntraPred_Angular_4_8_LT18_X86_sse2(p_ref,top,left,pic_stride,cIdx,predModeIntra);
				break;
			case 8:
				H265_IntraPred_Angular_8_8_LT18_X86_sse2(p_ref,top,left,pic_stride,cIdx,predModeIntra);
				break;
			case 16:
				H265_IntraPred_Angular_16_8_LT18_X86_sse2(p_ref,top,left,pic_stride,cIdx,predModeIntra);
				break;
			case 32:
				H265_IntraPred_Angular_32_8_LT18_X86_sse2(p_ref,top,left,pic_stride,cIdx,predModeIntra);
				break;
			default:
				break;
			}
...
static VO_S32 xCheckIdenticalMotion (  const H265_DEC_SLICE * const p_slice , 
...
static VO_VOID GetWpScaling(const H265_DEC_SLICE *const p_slice, VO_S32 iRefIdx0, VO_S32 iRefIdx1, WP_SCALING_PARAM *w0, WP_SCALING_PARAM *w1 )
{
  ...
  VO_U32        yuv       = 0;
  const wpScalingParam *p_wp0, *p_wp1;
  ...
  {
    assert(0);
	p_wp0 = p_slice->m_weightPredTable[REF_PIC_LIST_0][0];
	p_wp1 = p_slice->m_weightPredTable[REF_PIC_LIST_1][0];
  }
...
//     p_wp1 = NULL;
  ...
  {  // Unidir
    ...
    const wpScalingParam*   pwp      = (iRefIdx0 >=0) ? p_wp0 : p_wp1;
...
static VOINLINE VO_S32 xWeightedPredInterLumaBlk(const H265_DEC_SLICE *const p_slice, TComPic *refPic, TComMv *mv, VO_S32 xPb, VO_S32 yPb, VO_S32 width, VO_S32 height, WP_SCALING_PARAM *wp, TComPic *dstPic , TReconTask *const reconTask)
{
...
//#endif
  ...
  {
    asm_mc0_buff = reconTask->pThdInfo->asm_mc0_buff;
  }
  else
#endif
  {
    asm_mc0_buff = p_slice->asm_mc0_buff;
  }
  #endif
  ...
  if (p_slice->nThdNum > 1)
  {
	  h265_wave3DThdInfo * const pThdInfo = reconTask->pThdInfo;

	  if (refPic->errFlag || (!refPic->bFinishDecode && refPic->p_slice->errFlag))
	  {
		  pThdInfo->errFlag = VO_H265_ERR_RERERENCEERROR;
LOGI("POC %ld occur error in recon\n", p_slice->m_iPOC);
	  	return 0;
	  }
#if ENABLE_THD_INFO
	  reconTask->pThdInfo->mc_sync++;
#endif

	  if (!refPic->bFinishDecode)
	  {
		  pthread_mutex_lock(pThdInfo->outputMutex);
		  if (!refPic->bFinishDecode)
  // 	  if (!CheckPicFinished(reconTask->pThdInfo, refPic))
		  {
			  ...
			  max_x = xInt + width + 3;
			  {
				  ...
				  block = GetRefLCU_rcTask(refPic->p_slice, max_y, max_x);
				  ret = !CheckLCUFinished(refPic, block, reconTask);
			  }
			  if (ret)
			  {
				  //p_slice->cur_pic->errFlag = ret;
#if ENABLE_THD_INFO
				  reconTask->pThdInfo->block_count++;
#endif

				  pthread_mutex_unlock(pThdInfo->outputMutex);
				  return ret;
			  }
		  }
		  pthread_mutex_unlock(pThdInfo->outputMutex);
...
#endif
  cur_stride = dstPic->pic_stride[0];//dstPic->getStride();
  p_cur  =  dstPic->pic_buf[0] + xy_off;
  xFrac     = mv->m_iHor & 0x3;
  yFrac     = mv->m_iVer & 0x3;
  ext_left = mc_ext_left[xFrac];
  ext_top =  mc_ext_left[yFrac];

  pic_width = refPic->m_iPicWidth;
  pic_height = refPic->m_iPicHeight;
  	 ...
  	 yInt < ext_top  || yInt > pic_height - height - mc_ext_right[yFrac])
  {
		...
		if (p_slice->nThdNum > 1)
		{
			fill = reconTask->pThdInfo->edge_emu_buff0;
		}
		else
#endif
		{
			fill = (VO_U8 *)p_slice->edge_emu_buff0;
		}
...
#if MC_ASM_ENABLED
   align_pos =	((VO_U32) p_cur)	& 15;
  if (wp[0].offset == 0 && wp[0].shift == 6)
  {  
    arrFun_put_Luma[xFrac][yFrac](p_cur, cur_stride, p_ref, ref_stride, width, height, asm_mc0_buff + align_pos, MAX_MC_CU_SIZE * 2);
  }
	else
	{
    arrFun_avg_nornd_MC_Bi_Luma[xFrac][yFrac](p_cur, cur_stride, p_ref, ref_stride, width, height, asm_mc0_buff + align_pos, MAX_MC_CU_SIZE * 2);//lina
    xWeightedUnidir(p_cur, cur_stride, height, width, asm_mc0_buff + align_pos, MAX_MC_CU_SIZE, wp[0].w, wp[0].round, wp[0].shift, wp[0].offset);
	}
#else
...
static VOINLINE VO_S32 xWeightedPredInterLumaBlkBi(const H265_DEC_SLICE *const p_slice, TComPic *refPic0, TComPic *refPic1, VO_S32 xPb, VO_S32 yPb, TComMv *mv0, TComMv *mv1, VO_S32 width, VO_S32 height, WP_SCALING_PARAM *wp0, WP_SCALING_PARAM *wp1, TComPic *dstPic, TReconTask *const reconTask)
{
...
//#endif
  ...
  VO_S32 xFrac0;//     = mv0->m_iHor & 0x3;   
  VO_S32 xFrac1;//     = mv1->m_iHor & 0x3;
  VO_S32 yFrac0;//     = mv0->m_iVer & 0x3;   
  VO_S32 yFrac1;//     = mv1->m_iVer & 0x3;
  ...
  {
	  h265_wave3DThdInfo * const pThdInfo = reconTask->pThdInfo;
	  if (refPic0->errFlag || (!refPic0->bFinishDecode && refPic0->p_slice->errFlag))
	  {
		  pThdInfo->errFlag = VO_H265_ERR_RERERENCEERROR;
LOGI("POC %ld occur error in recon\n", p_slice->m_iPOC);
	  	return 0;
	  ...
	  reconTask->pThdInfo->mc_sync++;
#endif
	  if (!refPic0->bFinishDecode)
	  {
		  pthread_mutex_lock(pThdInfo->outputMutex);
		  if (!refPic0->bFinishDecode)
			  // 	  if (!CheckPicFinished(reconTask->pThdInfo, refPic0))
		  {
			  VO_S32 max_y, max_x, ret;
			  ...
			  max_x = x0_off + width + 3;
			  {
				  ...
				  block = GetRefLCU_rcTask(refPic0->p_slice, max_y, max_x);
				  ret = !CheckLCUFinished(refPic0, block, reconTask);
			  }
  ...
  x1_off     = xPb; //GetLumaX(refPic1, p_slice->m_uiCUAddr, p_slice->m_uiAbsIdxInLCU + partAddr, refStride1);
  y1_off     = yPb; //GetLumaY(refPic1, p_slice->m_uiCUAddr, p_slice->m_uiAbsIdxInLCU + partAddr, refStride1);
  x1_off += mv1->m_iHor >> 2;
  y1_off += mv1->m_iVer >> 2;
  ...
  {
	  h265_wave3DThdInfo * const pThdInfo = reconTask->pThdInfo;
      if (refPic1->errFlag || (!refPic1->bFinishDecode && refPic1->p_slice->errFlag))
	  {
		  pThdInfo->errFlag = VO_H265_ERR_RERERENCEERROR;
LOGI("POC %ld occur error in recon\n", p_slice->m_iPOC);
	  	  return 0;
	  ...
	  if (!refPic1->bFinishDecode)
	  {
		  pthread_mutex_lock(pThdInfo->outputMutex);
		  if (!refPic1->bFinishDecode)
			  // 	  if (!CheckPicFinished(reconTask->pThdInfo, refPic1))
		  {
			  VO_S32 max_y, max_x, ret;
			  ...
			  max_x = x1_off + width + 3;
			  {
				  ...
				  block = GetRefLCU_rcTask(refPic1->p_slice, max_y, max_x);
				  ret = !CheckLCUFinished(refPic1, block, reconTask);
			  }
  ...
  xFrac0     = mv0->m_iHor & 0x3;   
  xFrac1     = mv1->m_iHor & 0x3;
  yFrac0     = mv0->m_iVer & 0x3;   
  yFrac1     = mv1->m_iVer & 0x3;
  ext0_left = mc_ext_left[xFrac0];
  ext0_top =  mc_ext_left[yFrac0];


  ext1_left = mc_ext_left[xFrac1];
  ext1_top =  mc_ext_left[yFrac1];
  

  pic_width = refPic0->m_iPicWidth;
  pic_height = refPic0->m_iPicHeight;
  	 ...
  	 y1_off < ext1_top  || y1_off > pic_height - height - mc_ext_right[yFrac1])
  {
		...
		if (p_slice->nThdNum > 1)
		{
			fill = reconTask->pThdInfo->edge_emu_buff1;
		}
		else
#endif
		{
			fill = (VO_U8 *)p_slice->edge_emu_buff1;
		}
  ...
  if (wp0[0].w == wp1[0].w && wp0[0].offset == 0 && wp0[0].shift == 7){
    ...
    arrFun_avg_nornd_MC_Bi_Luma[xFrac0][yFrac0](p_cur, cur_stride, ref0, refStride0, width, height, asm_mc0_buff + align_pos, MAX_MC_CU_SIZE * 2);
    arrFun_avg_MC_Bi_Luma[xFrac1][yFrac1](p_cur, cur_stride, ref1, refStride1, width, height, asm_mc0_buff + align_pos, MAX_MC_CU_SIZE * 2);
#else
    ...
    else
    {
		...
		{
			asm_mc0_buff = reconTask->pThdInfo->asm_mc0_buff;
			asm_mc1_buff = reconTask->pThdInfo->asm_mc1_buff;
		}
		else
#endif
		{
			asm_mc0_buff = p_slice->asm_mc0_buff;
			asm_mc1_buff = p_slice->asm_mc1_buff;
		}

        arrFun_avg_nornd_MC_Bi_Luma[xFrac0][yFrac0](p_cur, cur_stride, ref0, refStride0,  width, height, asm_mc0_buff + align_pos, MAX_MC_CU_SIZE * 2);
        arrFun_avg_nornd_MC_Bi_Luma[xFrac1][yFrac1](p_cur, cur_stride, ref1, refStride1,  width, height, asm_mc1_buff + align_pos, MAX_MC_CU_SIZE * 2);
        xWeightedBidir(p_cur, cur_stride, height, width, asm_mc0_buff + align_pos, asm_mc1_buff + align_pos, MAX_MC_CU_SIZE, wp0->w, wp1->w, wp0->round, wp0->shift, wp0->offset);
#else
        MC_InterLumaWeightedBi(ref0, refStride0, ref1, refStride1, p_cur, cur_stride, width, height, xFrac0, yFrac0, xFrac1, yFrac1, &wp0[0], &wp1[0]);
#endif
    }
...
static VOINLINE VO_VOID xWeightedPredInterChromaBlk(const H265_DEC_SLICE *const p_slice, TComPic *refPic,  TComMv *mv,  VO_S32 xPb, VO_S32 yPb,   VO_S32 width, VO_S32 height, WP_SCALING_PARAM *wp, TComPic *dstPic, TReconTask *const reconTask)
{
...
//#endif
  ...
  yInt += mv->m_iVer >> 3;
            ...
            yInt < ext_top  || yInt > pic_height - height / 2 - chroma_ext_right[!!yFrac])
    {
		...
		if (p_slice->nThdNum > 1)
		{
			fillCb = reconTask->pThdInfo->edge_emu_buff0;
			fillCr = reconTask->pThdInfo->edge_emu_buff1;
		}
		else
#endif
		{
			fillCb = (VO_U8 *)p_slice->edge_emu_buff0;
			fillCr = (VO_U8 *)p_slice->edge_emu_buff1;
		}
	...
	if (wp[0].offset == 0 && wp[0].shift == 6)
	{
        arrFun_put_Chroma[!!xFrac][!!yFrac](dstCb, cur_stride, refCb, ref_stride, width >> 1, height >> 1, asm_mc0_buff + align0_pos, MAX_MC_CU_SIZE, xFrac, yFrac);
        arrFun_put_Chroma[!!xFrac][!!yFrac](dstCr, cur_stride, refCr, ref_stride, width >> 1, height >> 1, asm_mc0_buff + align0_pos, MAX_MC_CU_SIZE, xFrac, yFrac);	
	}
	else
	{
        arrFun_avg_nornd_MC_Bi_Chroma[!!xFrac][!!yFrac](dstCb, cur_stride, refCb, ref_stride,  width >> 1, height >> 1, asm_mc0_buff + align0_pos, MAX_MC_CU_SIZE, xFrac, yFrac);
        arrFun_avg_nornd_MC_Bi_Chroma[!!xFrac][!!yFrac](dstCr, cur_stride, refCr, ref_stride,  width >> 1, height >> 1, asm_mc1_buff + align1_pos, MAX_MC_CU_SIZE, xFrac, yFrac);
        xWeightedUnidir(dstCb, cur_stride, height >> 1, width >> 1, asm_mc0_buff + align0_pos, MAX_MC_CU_SIZE / 2, wp[1].w, wp[1].round, wp[1].shift, wp[1].offset);
        xWeightedUnidir(dstCr, cur_stride, height >> 1, width >> 1, asm_mc1_buff + align1_pos, MAX_MC_CU_SIZE / 2, wp[2].w, wp[2].round, wp[2].shift, wp[2].offset);	
	}
#else
...
static VOINLINE VO_VOID xWeightedPredInterChromaBlkBi(const H265_DEC_SLICE *const p_slice, TComPic *refPic0, TComPic *refPic1, VO_S32 xPb, VO_S32 yPb,   TComMv *mv0, TComMv *mv1, VO_S32 width, VO_S32 height, WP_SCALING_PARAM *wp0, WP_SCALING_PARAM *wp1, TComPic *dstPic, TReconTask *const reconTask )
{
...
//#endif
  ...
  y0_off += mv0->m_iVer >> 3;
            ...
            y1_off < ext_top  || y1_off > pic_height - height / 2 - chroma_ext_right[!!yFrac1])
    {
		...
		if (p_slice->nThdNum > 1)
		{
			fillCb = reconTask->pThdInfo->edge_emu_buff0  + (MAX_CU_SIZE / 2 + 7) * (MAX_CU_SIZE / 2 + 7);
			fillCr = reconTask->pThdInfo->edge_emu_buff1  + (MAX_CU_SIZE / 2 + 7) * (MAX_CU_SIZE / 2 + 7);
		}
		else
#endif
		{
			fillCb = (VO_U8 *)p_slice->edge_emu_buff0 + (MAX_CU_SIZE / 2 + 7) * (MAX_CU_SIZE / 2 + 7);
			fillCr = (VO_U8 *)p_slice->edge_emu_buff1 + (MAX_CU_SIZE / 2 + 7) * (MAX_CU_SIZE / 2 + 7);
		}
  ...
  if (wp0[1].w == wp1[1].w && wp0[1].offset == 0 && wp0[1].shift == 7){
    ...
    arrFun_avg_nornd_MC_Bi_Chroma[!!xFrac0][!!yFrac0](dstCb, cur_stride, refCb0, refStride0, width_UV, height_UV, asm_mc0_buff + align0_pos, MAX_MC_CU_SIZE, xFrac0, yFrac0);
    arrFun_avg_MC_Bi_Chroma[!!xFrac1][!!yFrac1](dstCb, cur_stride, refCb1, refStride1, width_UV, height_UV, asm_mc0_buff + align0_pos, MAX_MC_CU_SIZE, xFrac1, yFrac1);
#else
    ...
    else
    {
		...
		arrFun_avg_nornd_MC_Bi_Chroma[!!xFrac0][!!yFrac0](dstCb, cur_stride, refCb0, refStride0, width >> 1, height >> 1, asm_mc0_buff + align0_pos, MAX_MC_CU_SIZE, xFrac0, yFrac0);
		arrFun_avg_nornd_MC_Bi_Chroma[!!xFrac1][!!yFrac1](dstCb, cur_stride, refCb1, refStride1, width >> 1, height >> 1, asm_mc1_buff + align0_pos, MAX_MC_CU_SIZE, xFrac1, yFrac1);
    xWeightedBidir(dstCb, cur_stride, height >> 1, width >> 1, asm_mc0_buff + align0_pos, asm_mc1_buff + align0_pos, MAX_MC_CU_SIZE / 2, wp0[1].w, wp1[1].w, wp0[1].round, wp0[1].shift, wp0[1].offset);
#else       
    MC_InterChromaWeightedBi(refCb0, refStride0, refCb1, refStride1, dstCb, cur_stride, width >> 1, height >> 1, xFrac0, yFrac0, xFrac1, yFrac1, &wp0[1], &wp1[1]);
#endif
    }
  ...
  if (wp0[2].w == wp1[2].w && wp0[2].offset == 0 && wp0[2].shift == 7){
    ...
    arrFun_avg_nornd_MC_Bi_Chroma[!!xFrac0][!!yFrac0](dstCr, cur_stride, refCr0, refStride0, width_UV, height_UV, asm_mc1_buff + align1_pos, MAX_MC_CU_SIZE, xFrac0, yFrac0);
    arrFun_avg_MC_Bi_Chroma[!!xFrac1][!!yFrac1](dstCr, cur_stride, refCr1, refStride1, width_UV, height_UV, asm_mc1_buff + align1_pos, MAX_MC_CU_SIZE, xFrac1, yFrac1);
#else
    ...
    else
    {
        ...
        arrFun_avg_nornd_MC_Bi_Chroma[!!xFrac0][!!yFrac0](dstCr, cur_stride, refCr0, refStride0, width >> 1, height >> 1, asm_mc0_buff + align1_pos, MAX_MC_CU_SIZE, xFrac0, yFrac0);
        arrFun_avg_nornd_MC_Bi_Chroma[!!xFrac1][!!yFrac1](dstCr, cur_stride, refCr1, refStride1, width >> 1, height >> 1, asm_mc1_buff + align1_pos, MAX_MC_CU_SIZE, xFrac1, yFrac1);
        xWeightedBidir(dstCr, cur_stride, height >> 1, width >> 1, asm_mc0_buff + align1_pos, asm_mc1_buff + align1_pos, MAX_MC_CU_SIZE / 2, wp0[2].w, wp1[2].w, wp0[2].round, wp0[2].shift, wp0[2].offset);		
#else
        MC_InterChromaWeightedBi(refCr0, refStride0, refCr1, refStride1, dstCr, cur_stride, width >> 1, height >> 1, xFrac0, yFrac0, xFrac1, yFrac1, &wp0[2], &wp1[2]);
#endif
    }
...
VO_S32 xPredInterUni ( //H265_DEC *p_dec, 
	const H265_DEC_SLICE* const p_slice, 
	const VO_S32 xPb, 
	const VO_S32 yPb, 
	const VO_S32 nPbW, 
	const VO_S32 nPbH, 
	...
	TReconTask *const reconTask)
{
...
#if MC_ASM_ENABLED
#if defined (ARCH_X86)	
    DECLARE_ALIGNED(16, VO_S16, asm_mc0_buff[MAX_MC_CU_SIZE * MAX_MC_CU_SIZE  + 64 + 8 * (MAX_MC_CU_SIZE+7)]);
	VO_S32	 align_pos;
#else
    VO_S16* asm_mc0_buff;
    VO_S32	 align_pos;
	...
	p_ref_pic = p_slice->ref_pic_list[ ( VO_S32 )( eRefPicList ) ][ iRefIdx ];  

	if(p_ref_pic==NULL)
  	    return 0;
	ref_stride = p_ref_pic->pic_stride[ 0 ];
    ...
    {
		h265_wave3DThdInfo * const pThdInfo = reconTask->pThdInfo;
        if (p_ref_pic->errFlag || (!p_ref_pic->bFinishDecode && p_ref_pic->p_slice->errFlag))
		{
			pThdInfo->errFlag = VO_H265_ERR_RERERENCEERROR;
LOGI("POC %ld occur error in recon\n", p_slice->m_iPOC);
		...
		if (!p_ref_pic->bFinishDecode)
		{
			pthread_mutex_lock(pThdInfo->outputMutex);
			if (!p_ref_pic->bFinishDecode)
				// 		if (!CheckPicFinished(reconTask->pThdInfo, p_ref_pic))
			{
				VO_S32 max_y, max_x, ret;
				...
				max_x = xInt + nPbW + 3;
				{
					...
					block = GetRefLCU_rcTask(p_ref_pic->p_slice, max_y, max_x);
					ret = !CheckLCUFinished(p_ref_pic, block, reconTask);
				}
...
#endif
    xFrac = cMv->m_iHor & 0x3;
    yFrac = cMv->m_iVer & 0x3;

    pic_width  = p_ref_pic->m_iPicWidth;
    pic_height = p_ref_pic->m_iPicHeight;

    xy_off     = yPb * ref_stride + xPb;

    cur_stride = p_cur_pic->pic_stride[ 0 ];
    p_cur      = p_cur_pic->pic_buf[ 0 ] + xy_off;
...
#if MC_ASM_ENABLED 
    align_pos =	((VO_U32) p_cur)	& 15;
    arrFun_put_Luma[xFrac][yFrac](p_cur, cur_stride, p_ref, ref_stride, nPbW, nPbH, asm_mc0_buff+align_pos, MAX_MC_CU_SIZE * 2);
#else
...
#if MC_ASM_ENABLED
    align_pos =	((VO_U32) dstCb)	& 15;
    arrFun_put_Chroma[!!xFrac][!!yFrac](dstCb, cur_stride, refCb, ref_stride, nPbW >> 1, nPbH >> 1, asm_mc0_buff+align_pos, MAX_MC_CU_SIZE * 2, xFrac, yFrac);
    align_pos =	((VO_U32) dstCr)	& 15;
    arrFun_put_Chroma[!!xFrac][!!yFrac](dstCr, cur_stride, refCr, ref_stride, nPbW >> 1, nPbH >> 1, asm_mc0_buff+align_pos, MAX_MC_CU_SIZE * 2, xFrac, yFrac);
#else
...
VO_S32 xPredInterBi ( const H265_DEC_SLICE* const p_slice, 
	VO_S32 xPb, 
	VO_S32 yPb, 
	VO_S32 nPbW, 
	VO_S32 nPbH, 
...
//     assert( iRefIdx_value_1 >= 0 );
...

  {
...
#endif
    {
      ...
      VO_S32 xFrac0;//      = cMv0->m_iHor & 0x3,   
      VO_S32 xFrac1;//      = cMv1->m_iHor & 0x3;
      VO_S32 yFrac0;//      = cMv0->m_iVer & 0x3,   
      VO_S32 yFrac1;//      = cMv1->m_iVer & 0x3;
      ...
      {
		  h265_wave3DThdInfo * const pThdInfo = reconTask->pThdInfo;
          if (p_ref_pic[0]->errFlag || (!p_ref_pic[0]->bFinishDecode && p_ref_pic[0]->p_slice->errFlag))
		  {
			  pThdInfo->errFlag = VO_H265_ERR_RERERENCEERROR;
LOGI("POC %ld occur error in recon\n", p_slice->m_iPOC);
	  	    return 0;
		  ...
		  if (!p_ref_pic[0]->bFinishDecode)
		  {
			  pthread_mutex_lock(pThdInfo->outputMutex);
			  if (!p_ref_pic[0]->bFinishDecode)
				  // 		  if (!CheckPicFinished(reconTask->pThdInfo, p_ref_pic[0]))
			  {
				  VO_S32 max_y, max_x, ret;
				  ...
				  max_x = x0_off + nPbW + 3;
				  {
					  ...
					  block = GetRefLCU_rcTask(p_ref_pic[0]->p_slice, max_y, max_x);
					  ret = !CheckLCUFinished(p_ref_pic[0], block, reconTask);
				  }
      ...
      {
		  h265_wave3DThdInfo * const pThdInfo = reconTask->pThdInfo;
          if (p_ref_pic[1]->errFlag || (!p_ref_pic[1]->bFinishDecode && p_ref_pic[1]->p_slice->errFlag))
		  {
			  pThdInfo->errFlag = VO_H265_ERR_RERERENCEERROR;
LOGI("POC %ld occur error in recon\n", p_slice->m_iPOC);
	  	    return 0;
		  ...
		  if (!p_ref_pic[1]->bFinishDecode)
		  {
			  pthread_mutex_lock(pThdInfo->outputMutex);
			  if (!p_ref_pic[1]->bFinishDecode)
				  // 		  if (!CheckPicFinished(reconTask->pThdInfo, p_ref_pic[1]))
			  {
				  VO_S32 max_y, max_x, ret;
				  ...
				  max_x = x1_off + nPbW + 3;
				  {
					  ...
					  block = GetRefLCU_rcTask(p_ref_pic[1]->p_slice, max_y, max_x);
					  ret = !CheckLCUFinished(p_ref_pic[1], block, reconTask);
				  }
      ...
      xFrac0     = cMv0->m_iHor & 0x3;   xFrac1     = cMv1->m_iHor & 0x3;
      yFrac0     = cMv0->m_iVer & 0x3;   yFrac1     = cMv1->m_iVer & 0x3;
	  ext0_left = mc_ext_left[xFrac0];
      ext0_top =  mc_ext_left[yFrac0];
      ext1_left = mc_ext_left[xFrac1];
      ext1_top =  mc_ext_left[yFrac1];

      pic_width = p_ref_pic[0]->m_iPicWidth;
      pic_height = p_ref_pic[0]->m_iPicHeight;
...
#endif
        arrFun_avg_nornd_MC_Bi_Luma[xFrac0][yFrac0](p_cur, cur_stride, ref0, refStride0, nPbW, nPbH, asm_mc0_buff + align_pos, MAX_MC_CU_SIZE * 2);
        arrFun_avg_MC_Bi_Luma[xFrac1][yFrac1](p_cur, cur_stride, ref1, refStride1, nPbW, nPbH, asm_mc0_buff + align_pos, MAX_MC_CU_SIZE * 2);
     }
#else
...
//       ext0_top =  chroma_ext_left[!!yFrac0];
        ...
        y0_off < ext0_top  || y0_off > pic_height - height_UV -chroma_ext_right[!!yFrac0])
      {
...
#endif
		{
			fill = (VO_U8 *)p_slice->edge_emu_buff0;
                ...
                FillMagin(refCb0 - offset_UV_0, 	refStride_UV_0, fill, dst_stride_UV_0, x0_off - ext0_left,
                          y0_off - ext0_top, pic_width, pic_height, dst_stride_UV_0, fill_height_UV_0);

                refCb0 = fill + ext0_left + ext0_top * dst_stride_UV_0;
                    ...
                    y1_off < ext1_top  || y1_off > pic_height - height_UV - chroma_ext_right[!!yFrac1])
            {
                ...
                FillMagin(refCb1 - offset_UV_1, 	refStride_UV_1, fill, dst_stride_UV_1, x1_off - ext1_left,
                          y1_off - ext1_top, pic_width, pic_height, dst_stride_UV_1, fill_height_UV_1);

                refCb1 = fill + ext1_left + ext1_top * dst_stride_UV_1;
...
#if defined (ARCH_X86)		 
	      DECLARE_ALIGNED(16, VO_S16, asm_mc0_buff[MAX_MC_CU_SIZE  * MAX_MC_CU_SIZE / 4  + 64 + 8 * (MAX_MC_CU_SIZE / 2 + 3)]);		  
#else
		  VO_S16* asm_mc0_buff;
#if USE_3D_WAVE_THREAD
		  if (p_slice->nThdNum > 1)
      ...
      MC_InterChromaBi(refCb0, refStride0, refCb1, refStride1, dstCb, cur_stride, width_UV, height_UV, xFrac0, yFrac0, xFrac1, yFrac1);
            ...
            if (flag_fillMagin_UV_0)
            {
				...
				if (p_slice->nThdNum > 1)
				{
					fillCr = reconTask->pThdInfo->edge_emu_buff0;
				}
				else
#endif
				{
					fillCr = (VO_U8 *)p_slice->edge_emu_buff0;
				}
                FillMagin(refCr0 - offset_UV_0, 	refStride_UV_0, fillCr, dst_stride_UV_0, x0_off - ext0_left,
                          y0_off - ext0_top, pic_width, pic_height, dst_stride_UV_0, fill_height_UV_0);
            ...
            if(flag_fillMagin_UV_1)
            {
                ...
                FillMagin(refCr1 - offset_UV_1, 	refStride_UV_1, fillCr, dst_stride_UV_1, x1_off - ext1_left,
                          y1_off - ext1_top, pic_width, pic_height, dst_stride_UV_1, fill_height_UV_1);
...
#endif
                arrFun_avg_nornd_MC_Bi_Chroma[!!xFrac0][!!yFrac0](dstCr, cur_stride, refCr0, refStride0, width_UV, height_UV, asm_mc0_buff + align1_pos, MAX_MC_CU_SIZE, xFrac0, yFrac0);
                arrFun_avg_MC_Bi_Chroma[!!xFrac1][!!yFrac1](dstCr, cur_stride, refCr1, refStride1, width_UV, height_UV, asm_mc0_buff + align1_pos, MAX_MC_CU_SIZE, xFrac1, yFrac1);
            }
#else
      MC_InterChromaBi(refCr0, refStride0, refCr1, refStride1, dstCr, cur_stride, width_UV, height_UV, xFrac0, yFrac0, xFrac1, yFrac1);
#endif
...
LOGI("POC %ld occur error in recon\n", p_slice->m_iPOC);
  ...
  }
  {
    ...
    //if (p_slice->p_pps->weighted_bipred_flag/* && p_slice->slice_type == B_SLICE*/)
    {
      ...
      ret = xWeightedPredInterLumaBlkBi(p_slice, p_ref_pic[0], p_ref_pic[1], xPb, yPb, cMv0, cMv1, nPbW, nPbH, &wp0[0], &wp1[0], p_cur_pic, reconTask);
	  if (ret)
	  {
		  return ret;
	  }
      xWeightedPredInterChromaBlkBi(p_slice, p_ref_pic[0], p_ref_pic[1], xPb, yPb, cMv0, cMv1, nPbW, nPbH, &wp0[0], &wp1[0], p_cur_pic, reconTask);
    }
  }
...
VO_S32 xInterWeightedPred_Uni ( const H265_DEC_SLICE* const p_slice, 
	VO_S32 xPb, 
	VO_S32 yPb, 
	VO_S32 nPbW, 
	VO_S32 nPbH, 
...
LOGI("POC %ld occur error in recon\n", p_slice->m_iPOC);
  ...
  p_ref_pic          = p_slice->ref_pic_list[(VO_S32)( eRefPicList )][iRefIdx];  // Get reference picture.
  ...
  // if (p_slice->p_pps->weighted_pred_flag)
  {
    ...
    else
    {
      GetWpScaling(p_slice, -1, iRefIdx, NULL, &wp_param[0]);
        ...
        ret = xWeightedPredInterLumaBlk(p_slice, p_ref_pic, cMv,  xPb, yPb, nPbW, nPbH, &wp_param[0], p_cur_pic, reconTask);
		if (ret)
		{
			return ret;
		}
        xWeightedPredInterChromaBlk(p_slice, p_ref_pic, cMv,  xPb, yPb, nPbW, nPbH, &wp_param[0], p_cur_pic, reconTask);
	...
	TReconTask * const reconTask ) 
{
  ...
  if ( p_slice->p_pps->weighted_bipred_flag ) {
    ...
    if ( p_PuMvField->inter_dir != 3 ) {
      ...
      ret = xInterWeightedPred_Uni ( p_slice, xPb, yPb,  nPbW, nPbH, (RefPicList)flag_Uni, p_slice->cur_pic, p_PuMvField, reconTask);
    } else {
      ret = xInterWeightedPred_Bi( p_slice, xPb, yPb,  nPbW, nPbH, p_slice->cur_pic, p_PuMvField, reconTask);  
    }

  } else  if ( p_slice->p_pps->weighted_pred_flag && p_slice->slice_type==P_SLICE) {
    ...
    if ( flag_Uni >= 0  ) {
      ret = xPredInterUni( p_slice, xPb, yPb, nPbW, nPbH, (RefPicList)flag_Uni, p_PuMvField, reconTask);
    } else {
      ret = xPredInterBi( p_slice, xPb, yPb,  nPbW, nPbH, p_slice->cur_pic, p_PuMvField, reconTask);      
    }
