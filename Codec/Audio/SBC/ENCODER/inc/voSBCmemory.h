...
#define voSBCMalloc(iSize)				voMalloc(psbc->vopMemOP, iSize)
#define voSBCRealloc(ptr, iSize)		voRealloc(psbc->vopMemOP, ptr, iSize)
#define voSBCFree(pFree)				voFree(psbc->vopMemOP, pFree)
#define voSBCCalloc(Count, iSize)		voCalloc(psbc->vopMemOP, Count, iSize)
#define voSBCMemcmp(ptr1, ptr2, count)	voMemcmp(psbc->vopMemOP, ptr1, ptr2, count)
#define voSBCMemmove(pDest, pSrc, size) voMemmove(psbc->vopMemOP, pDest, pSrc, size)
