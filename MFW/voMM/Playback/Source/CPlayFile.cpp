...
VO_U32 CPlayFile::Create (VO_PTR pSource, VO_U32 nType, VO_S64 nOffset, VO_S64 nLength)
{
	...
	if(m_IFileBasedDRM && m_pFileBaseOP)
		m_pFileOP = m_pFileBaseOP;
	...
	if ( voFF == VO_FILE_FFUNKNOWN)
	{
		...
		filSource.nReserve= (VO_U32)m_IFileBasedDRM;
	...
	m_pSource->SetWorkPath (m_pWorkingPath);
	m_pSource->SetLibOperator (m_pLibOP ? m_pLibOP : &m_libOP);
...
//	m_pSource->SetCallBack ((VO_PTR)m_cbStreaming.pCallBack, m_cbStreaming.pUserData);
	...
	if(m_IFileBasedDRM)
		m_pSource->SetIFileBasedDrm(m_IFileBasedDRM);
	...
	for (i = 0; i < filInfo.Tracks; i++)
	{
		...
		if ((trkInfo.Type == VO_SOURCE_TT_AUDIO || trkInfo.Type == VO_SOURCE_TT_RTSP_AUDIO) && m_nAudioTrack == -1 && (m_nPlayFlag & 0X02) == 2)
		{
			...
			m_pAudioDec->SetWorkPath (m_pWorkingPath);
			m_pAudioDec->SetLibOperator (m_pLibOP ? m_pLibOP : &m_libOP);
			...
			m_pAudioRender->SetWorkPath (m_pWorkingPath);
			m_pAudioRender->SetLibOperator (m_pLibOP ? m_pLibOP : &m_libOP);
		...
		else if ((trkInfo.Type == VO_SOURCE_TT_VIDEO || trkInfo.Type == VO_SOURCE_TT_RTSP_VIDEO) && m_nVideoTrack == -1 && (m_nPlayFlag & 0X01) == 1)
		{
			...
			m_pVideoRender->SetWorkPath (m_pWorkingPath);
			m_pVideoRender->SetLibOperator (m_pLibOP ? m_pLibOP : &m_libOP);
			...
			m_pVideoDec->SetWorkPath (m_pWorkingPath);
			m_pVideoDec->SetLibOperator (m_pLibOP ? m_pLibOP : &m_libOP);
			...
			{
				// modified by Eric, 2011.10.13
				nRC = m_pVideoDec->Init (NULL, 0, &m_fmtVideo);
//				VO_SOURCE_SAMPLE	sample;
//				sample.Buffer = NULL;
//				sample.Size = 0;
//
...
//				nRC = VO_ERR_NONE;
			...
			// if thumbnail, output video frame immediately
			if(m_bForThumbnail)
			{
				int nOutputMode = 1;
			...
			VO_BOOL bVideoUpsizeDown = VO_FALSE;
			nRC = m_pSource->GetTrackParam(m_nVideoTrack, VO_PID_VIDEO_UPSIDEDOWN, &bVideoUpsizeDown);
			if(nRC == VO_ERR_NONE)
				m_pVideoRender->SetParam(VO_PID_VIDEO_UPSIDEDOWN, &bVideoUpsizeDown);
		}
	}
