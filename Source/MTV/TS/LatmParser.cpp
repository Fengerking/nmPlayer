...
#define GOT_MUX_INF   1
#define NO_MUX_CONFIG_IN_CURRENT_FRAME  0xffffff01
//add by qichaoshen @ 2011-11-11
...
int CLATMParser::ReadMUXConfig()
{
	...
	if (use_same_mux)
	{
		if(iFlagGetMuxInfo == GOT_MUX_INF)
		{
			return 0;  //treat it as OK
		}
		else
		{
			//for skip to next frame by bytes;
			return NO_MUX_CONFIG_IN_CURRENT_FRAME;
		}
	}
	...
	for (int prog=0; prog<=numProgram; prog++) 
	{
		...
		for (int lay=0; lay<=numLayer; lay++) {
		    ...
		    //add by qichaoshen 2011-10-17
		    progSIndex[streamCnt] = prog;
            laySIndex[streamCnt] = lay;
			//add by qichaoshen 2011-10-17
			...
			if (!use_same_config) 
			{
				...
				else 
				{
					...
					//0x1fff is the max size of the LATM Frame
					if((ascLen>0x1fff) || (ascLen<0))
					{
						return -15;
					}
					//
	...
	//add by qichaoshen @2011-11-17
	iFlagGetMuxInfo = GOT_MUX_INF;
    //add by qichaoshen @2011-11-17
...
int CLATMParser::GetFramesInfo(int* sizes, uint8*& pStart)
{
    ...
    if(numSubFrames>= MAX_LATM_AAC_FRAME_COUNT)
    {
		return -1;
    }
...
int CLATMParser::PayloadLengthInfo()
{
	...
	} else 
	{
		...
		for (int chunkCnt=0; chunkCnt <= numChunk; chunkCnt++) {
			...
			if(streamIndex > 0)
			{
				//current implement is streamIndex must 0,so streamIndex > 0 is illegal
		        return -1;
			}
			//add by qichaoshen 2011-10-17
