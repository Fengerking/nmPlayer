...
Mutex voeditor::sServiceLock;
sp<IMediaPlayerService> voeditor::sMediaPlayerService;
sp<voeditor::DeathNotifier> voeditor::sDeathNotifier;
SortedVector< wp<voeditor> > voeditor::sObitRecipients;

// establish binder interface to service
const sp<IMediaPlayerService>& voeditor::getMediaPlayerService()
{
    return NULL;
}

void voeditor::addObitRecipient(const wp<voeditor>& recipient)
{
    Mutex::Autolock _l(sServiceLock);
    sObitRecipients.add(recipient);
}

void voeditor::removeObitRecipient(const wp<voeditor>& recipient)
{
    Mutex::Autolock _l(sServiceLock);
...
int voeditor::vomeMsgHandler(void* pUserData, int nID, void* pParam1, void* pParam2)
{
	voeditor* pm = (voeditor*)pUserData;	
	if (pm == NULL) return 0;

	return pm->msgHandler(nID, pParam1, pParam2);
}

void voeditor::vomeSignalProc(void* pparam) {
	JNILOGD("vomeSignalProc");
	voeditor* pp = (voeditor*) pparam;
	if (pp) 
		pp->vomeMonitor();
}

void voeditor::vomeMonitor() {

	do {		

		JNILOGD("vomeMonitor before wait");
		mSignal2.wait(mNotifyLock); 
		JNILOGD("vomeMonitor after wait");
		mEventLock.lock();
		...
		mEventLock.unlock();		
            ...
            JNILOGD("vomeMonitor after notify");
...
voeditor::voeditor()
{
    ...
    JNILOGV("voeditor constructor 2");
...
void voeditor::disconnect()
{}

// always call with lock held
void voeditor::clear_l()
{
	//VOLOGW ();
...
status_t voeditor::setListener(const sp<voeditorListener>& listener)
{
    JNILOGV("setListener");
...
status_t voeditor::setDataSource(const sp<IMediaPlayer>& player)
{
	//VOLOGW ();
...
status_t voeditor::setDataSource(const char *url)
{
	//VOLOGW ("Source: %s", url);

	 JNILOGI("voeditor setDataSource");
...
status_t voeditor::setDataSource(int fd, int64_t offset, int64_t length)
{
	//VOLOGW ("Source: %d", fd);
	return 0;
	/*
...
status_t voeditor::invoke(const Parcel& request, Parcel *reply)
{
	//VOLOGW ();
...
status_t voeditor::setMetadataFilter(const Parcel& filter)
{
	//VOLOGW ();
...
status_t voeditor::getMetadata(bool update_only, bool apply_filter, Parcel *metadata)
{
	//VOLOGW ();
...
status_t voeditor::setVideoSurface(const sp<Surface>& surface)
{
    JNILOGD("voeditor setVideoSurface");
    
    Mutex::Autolock _l(mLock);
    //LOGW("voeditor::setVideoSurface Heaps: %08x, %08x - Flags: %x", surface->heapBase(0), surface->heapBase(1), surface->getFlags());
...
status_t voeditor::prepareAsync_l()
{
...
// code.
status_t voeditor::prepare()
{
    JNILOGD("voeditor prepare");
...
status_t voeditor::prepareAsync()
{
 	//VOLOGW ();
...
status_t voeditor::start() {
      JNILOGD("voeditor start");
...
status_t voeditor::stop()
{
    JNILOGD("voeditor stop"); 
...
status_t voeditor::pause()
{
     JNILOGD("voeditor pause"); 
...
status_t voeditor::getVideoWidth(int *w)
{
	//VOLOGW ();
...
status_t voeditor::getVideoHeight(int *h)
{
	//VOLOGW ();
...
status_t voeditor::getCurrentPosition(int *msec)
{
	//VOLOGW ();
...
status_t voeditor::getDuration_l(int *msec)
{
    JNILOGV("getDuration");
...
status_t voeditor::getDuration(int *msec)
{
    Mutex::Autolock _l(mLock);
    return getDuration_l(msec);
}

status_t voeditor::seekTo_l(int msec)
{
...
status_t voeditor::seekTo(int msec)
{
   mLockThreadId = getThreadId();
    Mutex::Autolock _l(mLock);
...
status_t voeditor::setAudioStreamType(int type)
{
	//VOLOGW ();

    JNILOGV("voeditor::setAudioStreamType");
    Mutex::Autolock _l(mLock);
...
status_t voeditor::setLooping(int loop)
{
	//VOLOGW ();

    JNILOGV("voeditor::setLooping");
    Mutex::Autolock _l(mLock);
    mLoop = (loop != 0);
    if (mPlayer != 0) {
...
status_t voeditor::setVolume(float leftVolume, float rightVolume)
{
	//VOLOGW ();

    JNILOGV2("voeditor::setVolume(%f, %f)", leftVolume, rightVolume);
    Mutex::Autolock _l(mLock);
...
void voeditor::notify(int msg, int ext1, int ext2)
{
...
/*
	...
	*/
    ...
    //sp<voeditorListener> listener = mListener;
...
void voeditor::DeathNotifier::binderDied(const wp<IBinder>& who)
{

    JNILOGW("voeditor server died!");

    // Need to do this with the lock held
    SortedVector< wp<voeditor> > list;
    {
        Mutex::Autolock _l(voeditor::sServiceLock);
        voeditor::sMediaPlayerService.clear();
    ...
    for (size_t iter = 0; iter < count; ++iter) {
        sp<voeditor> player = list[iter].promote();
...
status_t voeditor::initAudioSink(void* param) 
{
...
voeditor::DeathNotifier::~DeathNotifier()
{
    Mutex::Autolock _l(sServiceLock);
...
int voeditor::snoop(short* data, int len, int kind) {
    return 0;
}

int voeditor::msgHandler(int nID, void * pParam1, void * pParam2)
{
	switch (nID) {
...
status_t voeditor::ExportFile(const char *url)
{
	 JNILOGI("ExportFile");
	...
	m_pvoplayer->ExportFile("/sdcard/dump.3gp");
