...
#define INIT_CABAC( n, m, v ) \
    ...
    p_slice->bs.first_byte = p_slice->bs.p_bit_ptr - 4 -  p_slice->bs.bit_pos/8 - p_slice->bs.p_bit_start; //to check, lina       
    vo_init_cabac_decoder(p_cabac,
      p_slice->bs.p_bit_ptr - 4 -  p_slice->bs.bit_pos/8,
      p_slice->bs.p_bit_end - p_slice->bs.p_bit_ptr + 4 +  p_slice->bs.bit_pos/8);
