...
CAudioObject* auNew (struct CAudioObjectDecoder *paudec)
{
    ...
    pTemp = memset(pau, 0, sizeof(CAudioObject));

    pTemp = memset(&pau->m_qstQuantStep, 0, sizeof(pau->m_qstQuantStep)); // May be struct or float/double
...
Void auDeletePlus(CAudioObjectDecoder *paudec, CAudioObject *pau)
{
    auFree(pau->m_rgiPlusBsBuf);

#if defined(BUILD_WMAPROLBRV2)
    reconProcFree(paudec, pau);
    DELETE_PTR(pau->m_prp);
#endif

    freqexFree(paudec, pau);
...
Void auDelete(CAudioObjectDecoder *paudec, CAudioObject* pau)
{
    if (NULL == pau)
        return;

    //V3 LLMB Chao 03-20-02
    prvFreeMCLMSPredictor(paudec, &pau->m_MCLMSPredictor);
...
//    auDeletePcInfoCommon (pau, pau->m_rgpcinfo);
    ...
    if (pau->m_rgpcinfo)
    {
        for (iCh = 0; iCh < pau->m_cChannel; iCh++) 
        {
            ...
            {
                prvFreeLMSPredictor(paudec, ppcinfo->m_rgLMSPredictors + i);
            }
    ...
    prvDeleteBarkResampleMatrix(paudec, pau);
...
//  DELETE_ARRAY (pau->m_rgiLLMContextProb);
    ...
    if (pau->m_iVersion > 2)
        prvMultiXIDCTDelete(paudec, pau);
    ...
    if (pau->m_pDrc != NULL) drcFree(paudec, pau->m_pDrc);
#endif // BUILD_WMAPRO
...
Void auDeletePcInfoCommon (CAudioObjectDecoder *paudec, CAudioObject* pau, PerChannelInfo* rgpcinfo)
{
    //V3 LLM-B. Chao. Add. 03-15-02 in the common PerChannelInfo we have some auMalloced memory. 
    //delete them before delete pau->m_rgpcinfo;
    I16 iCh;
...
//static VO_S32 ROUNDFLOG2(VO_U32 x)
//{
//	return ((voWMAlog2(x) + 0x8000)>>16);
//}
#define ROUNDFLOG2(x)	((voWMAlog2(x) + 0x8000)>>16)
                                  ...
                                  WMAPlayerInfo *pWmaPlayerInfo)
{
    ...
    wmaResult = prvInitOutputSampleFormatParameters(pau, nBytePerSample, dataType);
    ...
    {
//		int cc = ROUNDF(2.49);	//ËÄÉáÎåÈë
//        pau->m_iAdjustSizeShiftFactor =
//          ROUNDF((float)(log((Double)pau->m_iSamplingRate/iOutputSamplingRate)/log(2.0)));
		pau->m_iAdjustSizeShiftFactor = ROUNDFLOG2((pau->m_iSamplingRate<<8) / iOutputSamplingRate);
    ...
    if (pau->m_bUseDynamicIntegerPrec == WMAB_TRUE)
    {
		Int Temp = MAX32BITVAL;
...
WMARESULT prvInitCommonSecondaryFixed(CAudioObjectDecoder *paudec, CAudioObject *pau)
{
    WMARESULT   wmaResult = WMA_OK;
    ...
    wmaResult = prvInitOutputSampleFormatSecondaryFixed(pau);
    ...
    if (pau->m_iVersion <= 2)
    {
		int temp = ROUNDF ( FLOAT_FROM_WBPS( pau->m_BitsPerSample ) * pau->m_cFrameSampleHalf / 8.0F);
        pau->m_cBitPackedFrameSize = LOG2 ((U32)temp) + 2;
    }
    else    //variable
    {  
    ...
    if (pau->m_iVersion == 1) 
		pau->m_fltDctScale = (BP2Type)((double)BP2_FRAC_SCALE/voWMASqrt512 (pau->m_cFrameSampleHalfAdjusted<<8));
...
//   prvInitInverseQuadRootTable(paudec, pau);    
...
WMARESULT prvAllocatePcInfoCommon(CAudioObjectDecoder *paudec, CAudioObject* pau, PerChannelInfo* rgpcinfo)
{
    WMARESULT   wmaResult = WMA_OK;
    I16 iCh;
    ...
    for (iCh = 0; iCh < pau->m_cChannel; iCh++) 
    {
        ...
        {        
                TRACEWMA_EXIT(wmaResult, 
                    prvInitLMSPredictor(paudec, ppcinfo->m_rgLMSPredictors + i, LLMB_CLMSFLT_MAX_ORDER, pMem, &cbMemUsed, cbMemTtl));
...
WMARESULT prvInitPlusAllocate(CAudioObjectDecoder *paudec, CAudioObject *pau)
{
    WMARESULT wmaResult = WMA_OK;
    ...
    if (pau->m_bCx)
    {
        ...
        TRACEWMA_EXIT(wmaResult, chexInit(paudec, pau));
    }
#endif // BUILD_WMAPROLBRV2
    ...
    {
        TRACEWMA_EXIT(wmaResult, freqexInit(paudec, pau));
    }
    ...
    if (pau->m_fBasePlus)
    {
        pau->m_pbp = bpNew(paudec);
        if (NULL == pau->m_pbp)
        {
            wmaResult = TraceResult(WMA_E_OUTOFMEMORY);
            goto exit;
        }
        TRACEWMA_EXIT(wmaResult, bpInit(paudec, pau, pau->m_pbp));
    }
    ...
    if (pau->m_bReconProc)
    {
        ...
        TRACEWMA_EXIT(wmaResult, reconProcInit(paudec, pau));
    }
#endif
...
WMARESULT prvInitCommonAllocate(CAudioObjectDecoder *paudec, CAudioObject *pau, Bool bEncoder)
{
    WMARESULT   wmaResult = WMA_OK;
    I32 iSize, iSizeCBT;
    I64 i64tmp;
...
#if defined(BUILD_WMAPROLBRV1)
    TRACEWMA_EXIT(wmaResult, prvInitPlusAllocate(paudec, pau));
#endif

    TRACEWMA_EXIT(wmaResult, prvInitMCLMSPredictor(paudec, pau, &pau->m_MCLMSPredictor, LLMB_MCLMS_MAX_ORDER_EACHCH));
    ...
    pau->m_rgiPCMInHistory = (I32 *)memset(pau->m_rgiPCMInHistory, 0, iSize);
    ...
    pau->m_rgiDiscardSilence = (I32 *)memset(pau->m_rgiDiscardSilence, 0, iSize);
    ...
    pau->m_rgiInterlacedInput = (CBT *)memset(pau->m_rgiInterlacedInput, 0, iSizeCBT);
    ...
    pau->m_rgiInterlacedInputT = (CBT *)memset(pau->m_rgiInterlacedInputT, 0, iSizeCBT);
    ...
    TRACEWMA_EXIT(wmaResult, prvAllocateBarkResampleMatrix(paudec, pau));
    ...
    TRACEWMA_EXIT(wmaResult, prvMultiXIDCTAllocate(paudec, pau));
    ...
    if (pau->m_iWeightingMode == LPC_MODE && pau->m_rguiWeightFactor==NULL)    
    {
        if( pau->m_fPad2XTransform )
        {
            ...
            i64tmp = (I64)sizeof(CBT)*((I64)(pau->m_cFrameSampleHalf<<pau->m_iAdjustSizeShiftFactor))*(I64)pau->m_cChannel;
            if(i64tmp > I32_MAX)
            {
                wmaResult = TraceResult(WMA_E_INVALIDARG);
                goto exit;
...
WMARESULT prvInitCommonResetOperational(CAudioObject *pau)
{
    ...
    pau->m_rgiPCMInHistory = (I32 *)memset (pau->m_rgiPCMInHistory, 0,
            sizeof(*pau->m_rgiPCMInHistory) * pau->m_cChannel);
    pau->m_rgiCoefReconOrig = (CBT *)memset(pau->m_rgiCoefReconOrig, 0, sizeof(CBT)*pau->m_iCoefReconSize);

    if (pau->m_fWMAProHalfTransform)
    {
        pau->m_rgiCoefReconMLLMOrig = (CBT *)memset (pau->m_rgiCoefReconMLLMOrig, 0, sizeof (CBT) *
            pau->m_cFrameSampleHalf);
    }
    pau->m_iPCMReconStart = 0;
    pau->m_iPCMReconEnd = 0;
    ...
    pau->m_fLeadingSilence = WMAB_FALSE;
    pau->m_rgiDiscardSilence = (I32 *)memset(pau->m_rgiDiscardSilence, 0, sizeof(pau->m_rgiDiscardSilence[0]) * pau->m_cChannel);
    pau->m_rgiInterlacedInput = (CBT *)memset(pau->m_rgiInterlacedInput, 0, sizeof(pau->m_rgiInterlacedInput[0]) * pau->m_cChannel);
    pau->m_rgiInterlacedInputT = (CBT *)memset(pau->m_rgiInterlacedInputT, 0, sizeof(pau->m_rgiInterlacedInputT[0]) * pau->m_cChannel);
    pau->m_iQuantStepSize = (MIN_QUANT + MAX_QUANT - 1) / 2;
                     ...
                     const BITDEPTH iBitDepthSelector)
{
    ...
    case BITDEPTH_24:
        ...
        *ppfnSetSample = prvSetSample16;	//prvSetSample24;
        *ppfnGetSample = prvGetSample16;	//prvGetSample24;
#else
...
{
    WMARESULT   wmaResult = prvInitOutputSampleFormatSetFunctionPtrs(pau);
...
Void auShiftPCMBuffer(CAudioObject *pau, Int iShiftAmt)
{
    ...
    for (iCh = 0; iCh < pau->m_cChannel; iCh++)
    {
        ...
        pctDst = (CoefType *)voWMAMemcpy(pctDst, pctSrc, iCopyAmt*sizeof(CoefType));
    }
...
void prvShiftPCMBuffer( CAudioObject* pau )
{
    ...
    if( pau->m_iPCMReconStart >= pau->m_cFrameSampleHalfAdjusted )
    {
        ...
        for( iCh = 0; iCh < pau->m_cChannel; iCh++ )
        {
            ...
            pcfPCMBuf = (CoefType *)voWMAMemcpy(pcfPCMBuf, pcfPCMBuf+pau->m_cFrameSampleHalfAdjusted, 
...
#else
                     pTemp = voWMAMemcpy( pcfPCMBuf+pau->m_iPCMReconStart, 
                            pcfPCMBuf+pau->m_cFrameSampleHalfAdjusted + pau->m_iPCMReconStart, 
                            (u32MaxSamplesReq-pau->m_iPCMReconStart)*sizeof(CoefType) );
                                         ...
                                         ConfigFrame ecfConfig)
{
    ...
    I16 *piSize;
    SubFrameConfigInfo *psubfrmconfig = NULL, *psubfrmconfigTile = NULL;
    Int iSubframeTile=0;
...
WMARESULT auAdaptToSubFrameConfig (CAudioObject* pau)
{
    ...
    else if (pau->m_bPureLosslessMode == WMAB_FALSE) 
    {
        ...
        wmaResult = prvAdaptTrigToSubframeConfig(pau);
    }
exit:
#ifdef WMAPROFILE
...
WMARESULT prvMultiXIDCTAllocate(CAudioObjectDecoder *paudec, CAudioObject *pau)
{
    return WMA_OK;
}
...
Void prvMultiXIDCTDelete  (CAudioObjectDecoder *paudec, CAudioObject *pau)
{
}
...
WMARESULT prvAllocChannelGrpInfo(CAudioObjectDecoder *paudec, CChannelGroupInfo **prgChannelGrpInfo, 
                                 Int cChannel)
{
    return WMA_OK;
}
Void prvDeleteChannelGrpInfo(CAudioObjectDecoder *paudec, CChannelGroupInfo **prgChannelGrpInfo, 
                             Int cChannel)
{
    return;
}
...
WMARESULT prvAllocateBarkResampleMatrix(CAudioObjectDecoder *paudec, CAudioObject* pau)
{
    return WMA_OK;
}
Void prvDeleteBarkResampleMatrix(CAudioObjectDecoder *paudec, CAudioObject* pau)
{
}
...
I32 prvBitsMove (CAudioObjectDecoder *paudec, U8 *pbSrc, I32 cbSrc, I32 iSrcStart, I32 iSrcEnd, U8 * pbDst, I32 cbDst, I32 iDstStart)
{
...
WMARESULT prvAllocateNDimArr(CAudioObjectDecoder *paudec, U8 ***p, Int nDims, Int *dims, Int *size)
{
    WMARESULT wr = WMA_OK;
    Int i;
    ...
    (Int)(I32)size[0] != size[0] || 
        ...
        size[0]*dims[0] != (long long)size[0]*dims[0])
    {
        wr = WMA_E_INVALIDARG;
        goto exit;
    }
    ...
    *p = (U8 **)auMalloc(size[0]*dims[0]);
    CHECKWMA_ALLOC(wr, *p);
    *p = (U8 **)memset(*p, 0, size[0]*dims[0]);
    if (nDims != 1)
    {
        for (i=0; i < dims[0]; i++)
        {
            TRACEWMA_EXIT(wr, prvAllocateNDimArr(paudec, (U8 ***)&(*p)[i], nDims-1, dims+1, size+1));
        }
    }
...
Void prvDeleteNDimArr(CAudioObjectDecoder *paudec, U8 **p, Int nDims, Int *dims)
{
    Int i;
    ...
    if (nDims != 1 && NULL!=p)
    {
        for (i=0; i < dims[0]; i++)
        {
            prvDeleteNDimArr(paudec, (U8 **)p[i], nDims-1, dims+1);
        }
    }
    DELETE_ARRAY(p);
}
