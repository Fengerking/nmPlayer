...
void ra_depacki_cleanup_substream_hdr_array(ra_depack_internal* pInt)
{
    if (pInt && pInt->pSubStreamHdr)
    {
        ...
        pInt->pSubStreamHdr = (ra_substream_hdr*)HXNULL;
    }
}
    ...
    if (pInt && pBuf && ulLen >= 68 && pHdr)
    {
            ...
            pHdr->usRAFormatRevision == 0)
        {
            ...
            if (pHdr->ucScatterType)
            {
                ...
                pHdr->pulInterleavePattern = (long unsigned int*)ra_depacki_malloc(pInt, ulSize);
            ...
            if (retVal == HXR_OK)
            {
                ...
                if (ulLen >= 4)
                {
                    ...
                    {
                        /* Allocate the buffer */
                        pHdr->pOpaqueData = (BYTE*)ra_depacki_malloc(pInt, pHdr->ulOpaqueDataSize);
                        if (pHdr->pOpaqueData)
                        {
                                ...
                                UINT32              ulFlags)
{
    ...
    if (pInt && pInt->fpAvail)
    {
        ...
        ra_block* pBlock = (ra_block*)ra_depacki_malloc(pInt, sizeof(ra_block));
        if (pBlock)
        {
            /* Zero out the memory */
                                      ...
                                      ra_substream_hdr*   pHdr)
{
    ...
    if (pInt && pHdr && !pHdr->pFragBuffer)
    {
        ...
        pHdr->pFragBuffer = (BYTE*)ra_depacki_malloc(pInt, INITIAL_FRAG_BUFFER_SIZE);
        if (pHdr->pFragBuffer)
        {
            /* Zero out the buffer */
                                        ...
                                        UINT32              ulNewSize)
{
    ...
    if (pInt && pHdr && pHdr->pFragBuffer)
    {
        ...
        BYTE* pNewBuf = (BYTE*)ra_depacki_malloc(pInt, ulNewSize);
        if (pNewBuf)
        {
            /* Copy the old buffer */
...
HX_RESULT ra_depacki_deinterleave_send(ra_depack_internal* pInt, UINT32 ulSubStream)
{
        ...
        pInt->fpAvail)
    {
        ...
        ra_block* pBlock       = (ra_block*)HXNULL;
