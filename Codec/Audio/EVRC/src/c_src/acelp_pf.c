...
#ifdef ASM_OPT
    cor_h_x2_asm(h1, xn, dn);
#else
	voEVRCCodec_cor_h_x(h1, xn, dn);
#endif
					...
					Shortword l_subfr)	
{
	...
	exp_xy = norm_l(s);
	xy = (Shortword)((s << exp_xy)>>16);
/* If (xy < 0) gain = 0  */
	if (xy <= 0)
		return ((Shortword) 0);

	exp_yy = norm_l(s1);
	yy = (Shortword)((s1 << exp_yy)>>16);
