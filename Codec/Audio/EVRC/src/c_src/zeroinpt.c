			   ...
			   )
{
	...
	short *memA, *memA1, *memA2;	/* filter memory */
	short *output;

	memA  = evrc_encobj->zero_memA;
	memA1 = evrc_encobj->zero_memA1;
	memA2 = evrc_encobj->zero_memA2;
	output = evrc_encobj->zir;
...
#ifdef ASM_OPT
		iir_asm(output, input, coef, memA, order, length);
		fir_asm(output, output, wcoef1, memA1, order, length);
		iir_asm(output, output, wcoef2, memA2, order, length);
#else
	...
	else
	{
		...
		for (i = 0; i < order; i++)
			memory[i] = memA[i];
#ifdef  ASM_OPT
		iir_asm(output, output, coef, memory, order, length);
#else
...
#ifdef  ASM_OPT
		fir_asm(output, output, wcoef1, memory, order, length);
#else
		...
		for (i = 0; i < order; i++)
			memory[i] = memA2[i];
#ifdef ASM_OPT
		iir_asm(output, output, wcoef2, memory, order, length);
#else
