...
WMARESULT initReconTileInfo(CAudioObjectDecoder *paudec, CAudioObject *pau,
...
WMARESULT chexInitAnchorReconTileInfo(CAudioObjectDecoder *paudec, CAudioObject *pau, Chex *pcx,
                                      CxAnchorReconTileInfo *pcxarti)
{
    ...
    TRACEWMA_EXIT(wmaResult,
                 prvAllocateNDimArr(paudec, (U8***)&pcxarti->m_param0, 2, dims, size));

    size[1] = sizeof(ChexType);
    TRACEWMA_EXIT(wmaResult,
                 prvAllocateNDimArr(paudec, (U8***)&pcxarti->m_param1, 2, dims, size));
    TRACEWMA_EXIT(wmaResult,
                 prvAllocateNDimArr(paudec, (U8***)&pcxarti->m_param2, 2, dims, size));
...
WMARESULT chexInitReconTileInfo(CAudioObjectDecoder *paudec, 
								CAudioObject *pau,
...
WMARESULT fexInitPerChannelRecon(CAudioObjectDecoder *paudec, CAudioObject *pau, Fex *pfx,
...
WMARESULT fexInitReconTileInfo(CAudioObjectDecoder *paudec, CAudioObject *pau,
                               Fex *pfx, RFxReconTileInfo *prfxrti, Int iPos)
{
    ...
    {
        TRACEWMA_EXIT(wmaResult,
           fexInitPerChannelRecon(paudec, pau, pfx, prfxrti->m_rgpcr + iCh, iCh, iPos));
    }
...
Void huffInfoInit(CAudioObject *pau,
                  ...
                  Int iBins, const Int iExtraBits,
...
WMARESULT chexInit(CAudioObjectDecoder *paudec, CAudioObject *pau)
{
    WMARESULT wmaResult = WMA_OK;
    Chex *pcx = pau->m_pcx;
    //Fex  *pfx = &(pau->m_fex);
...
//		TRACEWMA_EXIT(wmaResult, WMA_E_INVALIDARG);
    ...
    TRACEWMA_EXIT(wmaResult,
                prvAllocateNDimArr(paudec, (U8***)&pcx->m_rgrgScMat, 2, dims, size));

exit:
    return wmaResult;
}

Void freeReconTileInfo(CAudioObjectDecoder *paudec, CAudioObject *pau,
                       ReconProc *prp, ReconTileInfo *prti)
{
    DELETE_ARRAY(prti->m_rgiPower);
}

Void chexFreeAnchorReconTileInfo(CAudioObjectDecoder *paudec, CAudioObject *pau,
                                 Chex *pcx, CxAnchorReconTileInfo *pcxarti)
{
    ...
    prvDeleteNDimArr(paudec, (U8**)pcxarti->m_param0, 2, dims);
    prvDeleteNDimArr(paudec, (U8**)pcxarti->m_param1, 2, dims);
    prvDeleteNDimArr(paudec, (U8**)pcxarti->m_param2, 2, dims);

    DELETE_ARRAY(pcxarti->m_iCodeMono);
}

Void chexFreeReconTileInfo(CAudioObjectDecoder *paudec, CAudioObject *pau,
                           Chex *pcx, ReconTileInfo *prti)
{
...
Void fexFreePerChannelRecon(CAudioObjectDecoder *paudec, CAudioObject *pau,
...
Void fexFreeReconTileInfo(CAudioObjectDecoder *paudec, CAudioObject *pau,
                          Fex *pfx, RFxReconTileInfo *prfxrti, Int iPos)
{
    Int iCh;
    ...
    for (iCh=0; iCh < pfx->m_iAllocCh; iCh++)
        fexFreePerChannelRecon(paudec, pau, pfx, prfxrti->m_rgpcr + iCh, iPos);

    DELETE_ARRAY(prfxrti->m_rgpcr);
}

Void chexFree(CAudioObjectDecoder *paudec, CAudioObject *pau)
{
    Chex *pcx = pau->m_pcx;
    Int dims[2];
    ...
    cxReverbFree(paudec, pcx->m_pcxrf);
    DELETE_PTR(pcx->m_pcxrf);

    dims[0] = pcx->m_cMaxBands;
    dims[1] = NUM_CXMATELEMS;
    prvDeleteNDimArr(paudec, (U8**)pcx->m_rgrgScMat, 2, dims);
                             ...
                             Int iSizeCurr)
{
	...
	//iStart is used to avoid the warning of ignoring return value
    iStart = chexSetBands(pcx->m_rgiBandSizes, pcx->m_iNumBandsInConfig, cBands);
...
//        scale  >= 0 )           // scales may be < 0 for corrupt bitstreams
      if(revRMS >  scale   &&    // if revRMS <= scale, no contraction is needed
        revRMS >  0       &&    // if revRMS is <= 0, the reverb is too small.
        scale  >= 0 )           // scales may be < 0 for corrupt bitstreams
    {
                      ...
                      CoefType *pCoeffDiff)
{
    ...
    pfnSave = (Void*)pau->cxpfnReconMonoDiff;
    ...
    if (WMAB_FALSE == bIsSameBandConfig)
    {
        ...
        pcx->m_rgiBandSizesInterp = (Int *)voWMAMemcpy(pcx->m_rgiBandSizesInterp, pcx->m_rgiBandSizes,
                            ...
                            CoefType *pfTempBuf)
{
...
#endif
        assert(pau->m_bCx);
		//pSrc is used to avoid the warning of ignoring return value
        pSrc = (CoefType *)voWMAMemcpy(pCoeff+prfxrti->m_cFexStart, pSrc+prfxrti->m_cFexStart,
    ...
    pfx->m_pfBaseRfx = (CoefType *)voWMAMemcpy(pfTempBuf, pCoeff, prti->m_iSizeCurr * sizeof(CoefType));
//    pfx->m_pfBaseRfx = pfTempBuf;
#endif

    // set baseband coefs
	// pfx->m_pfRecon is used to avoid the warning of ignoring return value
    if (!pau->m_bIgnoreFreqex)
        pfx->m_pfRecon = (CoefType *)memset(pCoeff + prfxrti->m_cFexStart, 0,
               (prti->m_iSizeCurr - prfxrti->m_cFexStart) * sizeof(CoefType));
...
//        NULL == prtiPrev ||	LT613
    ...
    {
        hr = chexSetCodeAnchor(prtiCurr, prtiPrev->m_pcxartiCode);
    }
...
WMARESULT chexCopyFex(CAudioObject *pau, Int iCh)
{
    ...
    for (i=0; i < pau->m_cChannel; i++)
    {
        ...
        pTemp = voWMAMemcpy(pau->m_rgpcinfo[i].m_rgCoefRecon + pfx->m_cOrigStart,
        ...
        for (iBand=0; iBand < pfx->m_cHoleBands; iBand++)
            pTemp = voWMAMemcpy(pau->m_rgpcinfo[i].m_rgCoefRecon + pfx->m_rgiBandStart[iBand],
...
//        {
            ...
            if (pcxrf && pcxrf->m_rgpHistBuf)
                ...
                pTemp = memset(pcxrf->m_rgpHistBuf[0][0][0], 0,
                       pcxrf->m_nTotalHistSize*sizeof(CoefType));
#else // so ( 0 == CX_SMALL_REVERB )
                pTemp = memset(pcxrf->m_rgpHistBuf[0][0], 0,
...
//        }
        ...
        pTemp = memset(pcx->m_rgiMaxStartBandForFrame, 0,
...
Void chexReset(CAudioObject *pau)
{
    ...
    pTemp = memset(prp->m_rgiTilesInSubframeAdd, 0, 4*sizeof(Int));
    ...
    pTemp = memset(pau->m_rgCoefHistMCLT, 0,
...
WMARESULT reconProcInit(CAudioObjectDecoder *paudec, CAudioObject *pau)
{
    WMARESULT wmaResult = WMA_OK;
    ReconProc *prp = pau->m_prp;
    ...
    if (NULL == prp)
		return WMA_E_INVALIDARG;
//        TRACEWMA_EXIT(wmaResult, WMA_E_INVALIDARG);	LT613
...
#define IMCLT_MAXN 0
    ...
    if (pcx && pau->m_bCx)
    {
        ...
        {
            TRACEWMA_EXIT(wmaResult,
                    chexInitAnchorReconTileInfo(paudec, pau, pcx, prp->m_rgcxarti+i));
        }
    ...
    // extra word is in valid memory.
        ...
        pcx->m_iStartBand = 0;
        TRACEWMA_EXIT(wmaResult, cxReverbInit(paudec, pau, pcx->m_pcxrf));
...

    {
        ...
        for (i=0; i < prp->m_nrfxrti; i++)
        {
            ...
            if (pau->m_bReconFex)
                TRACEWMA_EXIT(wmaResult,
                              fexInitReconTileInfo(paudec, pau, pfx, prfxrti, i));
        }
    }
    ...
    for (i=0; i < prp->m_nTilesInBuf; i++)
    {
        ...
        TRACEWMA_EXIT(wmaResult, initReconTileInfo(paudec, pau, prp, prti));
        if (pau->m_bCx)
        {
            TRACEWMA_EXIT(wmaResult, chexInitReconTileInfo(paudec, pau, pcx, prti));
        }
    }
...
Void reconProcFree(CAudioObjectDecoder *paudec, CAudioObject *pau)
{
    Int i;
    ReconProc *prp = pau->m_prp;
    Chex *pcx = pau->m_pcx;
    ...
    for (i=0; i < prp->m_nTilesInBuf; i++)
    {
        freeReconTileInfo(paudec, pau, prp, &prp->m_rgchextiBuf[i]);
        if (WMAB_TRUE == pau->m_bCx)
            chexFreeReconTileInfo(paudec, pau, pcx, &prp->m_rgchextiBuf[i]); 
   }

    for (i=0; i < prp->m_ncxarti; i++)
    {
        chexFreeAnchorReconTileInfo(paudec, pau, pcx, prp->m_rgcxarti+i);
    }

    for (i=0; i < prp->m_nrfxrti; i++)
    {
        if (WMAB_TRUE == pau->m_bReconFex)
            fexFreeReconTileInfo(paudec, pau, pfx, prp->m_rgrfxrti+i, i);
    }
...
WMARESULT chexShiftBuf(CAudioObject *pau, Int iChSrc)
{
    ...
    {
        ppcinfo->m_rgPCMBuf = (CoefType *)voWMAMemcpy(ppcinfo->m_rgPCMBuf, ppcinfo->m_rgPCMBuf+iOffset,
    ...
    else
    {
		...
		pTemp = (Void *)(ppcinfo->m_rgPCMBuf+prp->m_iPCMPrevFrameSave-iOffset);
        pTemp = voWMAMemcpy(pTemp, ppcinfo->m_rgPCMBuf+prp->m_iPCMPrevFrameSave,
...
WMARESULT chexSubframeRecon(CAudioObjectDecoder *paudec, CAudioObject *pau)
{
    WMARESULT wmaResult = WMA_OK;
    ReconProc *prp = pau->m_prp;
    ...
    {
        prvReplaceSamplesFromFilePostXForm(paudec, pau);
        if (WMAB_TRUE == pau->m_bCx)
            prvCreateMono(pau);
    }
#endif
...
//                              const char *szFile, Int iStart, Int iLen,
                                   ...
                                   CoefType *pDest)
{
    ...
    pDest = (CoefType *)memset(pDest, 0, 2*prti->m_iSizeCurr*sizeof(CoefType));
    iOffset = iStart - (prti->m_iStartPt - prti->m_iSizeCurr/2);
    assert(iOffset >= 0);
    if (iEnd-iStart >= 0)
	{
		CoefType *pTemp = pDest+iOffset;
        pTemp = (CoefType *)voWMAMemcpy(pTemp, ppcinfo->m_rgPCMBuf+(iStart-prp->m_rgiPCMStart[iChSrc]),
                                  ...
                                  CoefType *pDst)
{
    ...
    pDst = (CoefType *)voWMAMemcpy(pDst, ppcinfoShr->m_pDCTBufCurr,
                        ...
                        CoefType *pDCTBuf)
{
    ...
    ppcinfoShr->m_pDCTBufNext = (CoefType *)voWMAMemcpy(ppcinfoShr->m_pDCTBufNext,
                         ...
                         CoefType *pIDCTBuf)
{
    ...
    ppcinfoShr->m_pIDCTBufNext = (CoefType *)voWMAMemcpy(ppcinfoShr->m_pIDCTBufNext, pIDCTBuf,
                         ...
                         ReconTileInfo *prti)
{
    ...
    for (iCh=0; iCh < pau->m_cChannel; iCh++)
    {
        ...
        ppcinfoShr->m_pDCTBufNext = (CoefType *)memset(ppcinfoShr->m_pDCTBufNext, 0,
               prti->m_iSizeNext*sizeof(CoefType));
        ppcinfoShr->m_pIDCTBufNext = (CoefType *)memset(ppcinfoShr->m_pIDCTBufNext, 0,
               prti->m_iSizeNext*sizeof(CoefType));
    }
}
...
)
{
    ...
    voWMAMemcpy(Real, t, n * sizeof(Real[0])); // copy because it gets overwritten
    pImag = t + n;
                        ...
                        CoefType *pHist) // history buffer
{
        ...
        && (prti->m_iSizeCurr <= IMCLT_MAXN))
    {
        ...
        voWMAMemcpy(pHist, pBuf + prti->m_iSizeCurr,
               prti->m_iSizeCurr * sizeof(CoefType));
...
//    Int iStart, iEnd, iStartBand, iCoeff;
    Int iStart, iEnd, iStartBand = 0, iCoeff;
...
WMARESULT chexProcessTile(CAudioObjectDecoder *paudec, 
						  CAudioObject *pau, 
                          ReconTileInfo *prti,
                          ReconTileInfo *prtiPrev)
{
...
#if (CX_DCT_ONLY==1)
    Bool bShiftDCTBuf      = WMAB_FALSE;
    Bool bObtainDSTFromDCT = WMAB_FALSE;
    Bool bUseDSTRecon      = WMAB_FALSE;
#else
 ...
 //   initFP();
    ...
    for (iCh=0; iCh < pau->m_cChannel; iCh++)
    {
        ...
        if (bObtainDSTFromDCT == WMAB_FALSE && bHaveDST == WMAB_FALSE)
        {
            ...
            wmaResult = auPreScaleCoeffsLBR( pau, prti );
            ...
            if (0 == iCh)
                wmaResult = chexCreateMono(pau, prti, !bUseDSTRecon, 0, 1);
            else
                wmaResult = chexCreateMono(pau, prti, !bUseDSTRecon, 1, 0);

            wmaResult = auPostScaleCoeffsLBR( pau, prti );
            ...
            {
#if 1
                prvReplaceSamplesFromFileReconProc(paudec, pau, prti, iCh,
                                                   pau->m_rgCoefXFormMCLT,
                                                   -1, -1);
#else
                prvReplaceSamplesFromFileReconProc(paudec, pau, prti, iCh,
            ...
            {
                voWMAMemcpy(pau->m_rgDCTBuf,
                       pau->m_rgCoefXFormMCLT,
                       prti->m_iSizeCurr*sizeof(CoefType));
                voWMAMemcpy(pau->m_rgCoefXFormDST,
        ...
        else if (bObtainDSTFromDCT == WMAB_FALSE && bHaveDST)
        {
            pau->m_rgCoefXFormMCLT = (CoefType *)voWMAMemcpy(pau->m_rgCoefXFormMCLT,
                   pau->m_rgDCTBuf,
                   prti->m_iSizeCurr*sizeof(CoefType));
#if (CX_DCT_ONLY==0)
            voWMAMemcpy(pau->m_rgCoefXFormMCLT+prti->m_iSizeCurr,
                   pau->m_rgCoefXFormDST,
                   prti->m_iSizeCurr*sizeof(CoefType));
#endif
        }
...
//            if (CHEX_FULL_SPECTRUM(prti))
                pau->m_rgCoefXFormDST = (CoefType *)voWMAMemcpy(pau->m_rgCoefXFormDST,
                       pau->m_rgCoefXFormMCLT+prti->m_iSizeCurr,
                       prti->m_iSizeCurr*sizeof(CoefType));
...
#if (CX_DCT_ONLY==1)
        if (pau->m_bCx && bHaveDST==WMAB_FALSE)
        {
            pau->m_rgDCTBuf = voWMAMemcpy(pau->m_rgDCTBuf,
                   pau->m_rgCoefXFormMCLT,
                   prti->m_iSizeCurr*sizeof(CoefType));
//            if (CHEX_FULL_SPECTRUM(prti))
                bHaveDST = WMAB_TRUE;
...
//                      ppcinfo->m_rgCoefHistMCLT);
		//this is very peculiar, chexIMCLTTile should be return value!
         TRACEWMA_EXIT(wmaResult, chexIMCLTTile(pau, ppcinfo, prti, bUseDSTRecon,
                      pau->m_rgCoefXFormMCLT, ppcinfo->m_rgCoefSampMCLT,
                      ppcinfo->m_rgCoefHistMCLT));
        ...
        else if (iStartCurr != 0 || prtiPrev->m_iSizeCurr != iEndPrev)
        {
			...
			CoefType *pTemp = ppcinfo->m_rgCoefSampMCLTCurr+(iEndPrev-iStartPrev);
            pTemp = (CoefType *)auMemmove(pTemp, pTemp+iGap, (iEndCurr-iStartCurr)*sizeof(CoefType));
        }
    } // for (iCh=...)
...
//        if (iFexCh != iCh && CHEX_FULL_SPECTRUM(prti))
        ...
        if (0 == iCh)
            wmaResult = chexCreateMono(pau, prti, WMAB_TRUE, 0, 1);
        else
            wmaResult = chexCreateMono(pau, prti, WMAB_TRUE, 1, 0);
...
WMARESULT reconProcessUnprocessedTiles(CAudioObjectDecoder *paudec, CAudioObject *pau)
{
    WMARESULT wmaResult = WMA_OK;
    ReconProc *prp = pau->m_prp;
    ...
    if (prp->m_bLastTile)
    {
        ...
        pTemp = memset(pau->m_rgDCTBuf, 0, sizeof(CoefType)*prp->m_iDCTBufSize);
        pTemp = memset(pau->m_rgiCoefReconOrig, 0, sizeof(CBT)*pau->m_iCoefReconSize);
    ...
    for (i=0; i < iTiles; i++)
    {
        ...
        {
            TRACEWMA_EXIT(wmaResult, chexProcessTile(paudec, pau, prti, prtiPrev));
        }
...
Int chexGetSamplesReady(CAudioObject *pau)
{
    ...
    ReconTileInfo *prti=NULL, *prtiPrev;
                   ...
                   Bool fSPDIF)
{
    ...
    if (WMAB_TRUE == pau->m_bEncoder)
    {
		...
		//cSamples is used to avoid ignoring return value.
        cSamples = chexGetSamplesReady(pau);
...
//		printf("ReconSample() success\n");
        ...
        TRACEWMA_EXIT(wmaResult, chexProcessTile((CAudioObjectDecoder *)pHandle, pau, prti, prtiPrev));
                                     ...
                                     Int iTile, Bool bNext, Bool bNoCx)
{
    ...
    prp->m_iTilesInSubframeAdd = 0;
    prp->m_iTilesInSubframeXForm = 0;
    if (((cTiles-1 == iTile) && !pau->m_bEncoder )||((0 == iTile) && pau->m_bEncoder))
    {
#if 0
    ...
    if (pau->m_bReconFex == WMAB_TRUE || 1)
    {
        prp->m_iTilesInSubframeXFormRecon = 0;
        prp->m_iTilesInSubframeCalcFx = 0;
        prp->m_iTilesInSubframeCodeFx = 0;
...
{
    WMARESULT wmaResult = WMA_OK;
    Int i, iCh, iFirstCh=0, iFirstChInGroup;
    PerChannelInfo *ppcinfo;
    //ReconProc *prp = pau->m_prp;
    Chex *pcx = pau->m_pcx;
    Fex *pfx = &pau->m_fex;
...
#if 0
#if defined(_DEBUG) || (!defined(UNDER_CE) && !defined(LINUX))
