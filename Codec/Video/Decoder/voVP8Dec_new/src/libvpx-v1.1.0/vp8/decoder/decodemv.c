...
static void read_kf_modes(VP8D_COMP *pbi, MODE_INFO *mi)
{
	...
	if(vp8_read(bc, pbi->common.kf_ymode_prob[0>>1])) //i==0
	{
		if(vp8_read(bc, pbi->common.kf_ymode_prob[2>>1])) //i==2
		{
			if(vp8_read(bc, pbi->common.kf_ymode_prob[6>>1]))//i==6
				mi->mbmi.mode = TM_PRED;
			else
				mi->mbmi.mode = H_PRED;
		}
		else
		{
			if(vp8_read(bc, pbi->common.kf_ymode_prob[4>>1])) //i==4
				mi->mbmi.mode = V_PRED;
			else
				mi->mbmi.mode = DC_PRED;
		}
	}
	else
	{
...
#ifdef VOOPT
			if(vp8_read(bc, pbi->common.kf_bmode_prob[A] [L][0>>1])) //i =0
			{
				if(vp8_read(bc, pbi->common.kf_bmode_prob[A] [L][2>>1])) //i = 2
				{
					if(vp8_read(bc, pbi->common.kf_bmode_prob[A] [L][4>>1])) //i = 4
					{
						if(vp8_read(bc, pbi->common.kf_bmode_prob[A] [L][6>>1])) //i = 6
						{
							if(vp8_read(bc, pbi->common.kf_bmode_prob[A] [L][12>>1])) //i = 12
							{
								if(vp8_read(bc, pbi->common.kf_bmode_prob[A] [L][14>>1])) //i = 14
								{
									if(vp8_read(bc, pbi->common.kf_bmode_prob[A] [L][16>>1])) //i = 16
									{
										mi->bmi[i].as_mode = B_HU_PRED;
									}
									else
									{
										mi->bmi[i].as_mode = B_HD_PRED;
									}
								}
								else
								{
									mi->bmi[i].as_mode = B_VL_PRED;
								}
							}
							else
							{
								mi->bmi[i].as_mode = B_LD_PRED;
							}
						}
						else
						{
							if(vp8_read(bc, pbi->common.kf_bmode_prob[A] [L][8>>1])) //i = 8
							{
								if(vp8_read(bc, pbi->common.kf_bmode_prob[A] [L][10>>1])) //i = 10
								{
									mi->bmi[i].as_mode = B_VR_PRED;
								}
								else
								{
									mi->bmi[i].as_mode = B_RD_PRED;
								}
							}
							else
							{
								mi->bmi[i].as_mode = B_HE_PRED;
							}
						}
					}
					else
					{
						mi->bmi[i].as_mode = B_VE_PRED;
					}
				}
				else
				{
					mi->bmi[i].as_mode = B_TM_PRED;
				}
			}
			else
			{
				mi->bmi[i].as_mode = B_DC_PRED;
			}
#else
            mi->bmi[i].as_mode =
	...
	//UV
	if(vp8_read(bc, pbi->common.kf_uv_mode_prob[0>>1])) //i=0
	{
		if(vp8_read(bc, pbi->common.kf_uv_mode_prob[2>>1])) // i=2
		{
			if(vp8_read(bc, pbi->common.kf_uv_mode_prob[4>>1])) //i=4
				mi->mbmi.uv_mode = TM_PRED;
			else
				mi->mbmi.uv_mode = H_PRED;
		}
		else
			mi->mbmi.uv_mode = V_PRED;
	}
	else
...
static int read_mvcomponent(vp8_reader *r, const MV_CONTEXT *mvc)
{
	...
	{
#ifdef VOOPT
		if(vp8_read(r,(p + MVPshort)[0>>1]))//i=0
		{
			if(vp8_read(r,(p + MVPshort)[8>>1]))//i=8
			{
				if(vp8_read(r,(p + MVPshort)[12>>1]))//i=12
					x = 7;
				else
					x = 6;
			}
			...
			{
				if(vp8_read(r,(p + MVPshort)[10>>1]))//i=10
					x = 5;
				else
					x = 4;
			}
		...
		{
			if(vp8_read(r,(p + MVPshort)[2>>1]))//i=2
			{
				if(vp8_read(r,(p + MVPshort)[6>>1]))//i=6
					x = 3;
				else
					x = 2;
			}
			...
			{
				if(vp8_read(r,(p + MVPshort)[4>>1]))//i=4
					x = 1;
				else
					x = 0;
			}
...
{
    vp8_reader *const bc = & pbi->bc;
    MV_CONTEXT *const mvc = pbi->common.cfc->mvc;
...
static void read_mb_modes_mv(VP8D_COMP *pbi, MODE_INFO *mi, MB_MODE_INFO *mbmi)
{
    ...
    if (mbmi->ref_frame)    /* inter MB */
    {
...
#ifdef VOOPT
				if (ref_frame_sign_bias[above->mbmi.ref_frame] != ref_frame_sign_bias[mbmi->ref_frame])
				{
					nmv->as_mv.row *= -1;
					nmv->as_mv.col  *= -1;
				}
...
#ifdef VOOPT
				if (ref_frame_sign_bias[left->mbmi.ref_frame] != ref_frame_sign_bias[mbmi->ref_frame])
				{
					this_mv.as_mv.row *= -1;
					this_mv.as_mv.col  *= -1;
				}
...
#ifdef VOOPT
				if (ref_frame_sign_bias[aboveleft->mbmi.ref_frame] != ref_frame_sign_bias[mbmi->ref_frame])
				{
					this_mv.as_mv.row *= -1;
					this_mv.as_mv.col  *= -1;
				}
        ...
        {
			int mb_to_top_edge = pbi->mb.mb_to_top_edge-LEFT_TOP_MARGIN;
			int mb_to_bottom_edge = pbi->mb.mb_to_bottom_edge+RIGHT_BOTTOM_MARGIN;
			int mb_to_left_edge = pbi->mb.mb_to_left_edge -LEFT_TOP_MARGIN;
			int mb_to_right_edge = pbi->mb.mb_to_right_edge + RIGHT_BOTTOM_MARGIN;
            ...
            if( vp8_read(bc, vp8_mode_contexts [cnt[CNT_NEAREST]] [1]) )
            {
...
#ifdef VOOPT
					VO_CLAMP_MV(near_mvs[near_index],mb_to_left_edge,mb_to_right_edge,
                ...
                else
                {
                    mbmi->mode =  NEARMV;
#ifdef VOOPT
					VO_CLAMP_MV(near_mvs[CNT_NEAR],mb_to_left_edge,mb_to_right_edge,
						mb_to_top_edge,mb_to_bottom_edge);
#else
            ...
            else
            {
                mbmi->mode =  NEARESTMV;
#ifdef VOOPT
				VO_CLAMP_MV(near_mvs[CNT_NEAREST],mb_to_left_edge,mb_to_right_edge,
						mb_to_top_edge,mb_to_bottom_edge);
#else
    ...
    else
    {
...
#ifdef VOOPT
		//Y
		if(vp8_read(bc, pbi->common.cfc->ymode_prob[0>>1])) //i==0
		{
			if(vp8_read(bc, pbi->common.cfc->ymode_prob[2>>1])) //i==2
			{
				if(vp8_read(bc, pbi->common.cfc->ymode_prob[6>>1]))//i==6
				{
					int j = 0;
					mbmi->mode = B_PRED;
					mbmi->is_4x4 = 1;
					do
					{
						mi->bmi[j].as_mode = read_bmode(bc, pbi->common.cfc->bmode_prob);
					}
					while (++j < 16);
				}
				else
			...
			{
				if(vp8_read(bc, pbi->common.cfc->ymode_prob[4>>1])) //i==4
					mbmi->mode = H_PRED;
				else
					mbmi->mode = V_PRED;
			}
		...
		//UV
		if(vp8_read(bc, pbi->common.cfc->uv_mode_prob[0>>1])) //i=0
		{
			if(vp8_read(bc, pbi->common.cfc->uv_mode_prob[2>>1])) // i=2
			{
				if(vp8_read(bc, pbi->common.cfc->uv_mode_prob[4>>1])) //i=4
...
#else
        /* MB is intra coded */
        if ((mbmi->mode = read_ymode(bc, pbi->common.cfc->ymode_prob)) == B_PRED)
        {
            int j = 0;
            mbmi->is_4x4 = 1;
            do
		 ...
		 mbmi->uv_mode = read_uv_mode(bc, pbi->common.cfc->uv_mode_prob);
#endif
    }
                               ...
                               MB_MODE_INFO *mbmi)
{
     ...
     */
	int *pSedID = NULL;
	if (pbi->common.pSegID_s != NULL)
	{
		pSedID = pbi->common.pSegID_s + (mi - pbi->common.mi);
	}
	
    if (pbi->mb.update_mb_segmentation_map)
	{
        read_mb_features(&pbi->bc, &mi->mbmi, &pbi->mb);
		pSedID && (*pSedID = mi->mbmi.segment_id);
	}
	...
	{
		pSedID && (mi->mbmi.segment_id = *pSedID);
	}
