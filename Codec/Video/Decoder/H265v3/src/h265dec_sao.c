...
VO_VOID setNDBBorderAvailabilityPerLCU(H265_DEC_SLICE *p_slice, TComSampleAdaptiveOffset* p_SAO, VO_U32 uiAddr, VO_U32 uiContainLastLCULine){
  ...
  pbAvailBorder = p_slice->m_vNDFBlock[uiAddr].isBorderAvailable;
  sliceID = p_slice->m_sliceIDMapLCU[uiAddr];
  ...
  if(!bPicLBoundary)
  {
    pRefID = p_slice->m_sliceIDMapLCU[uiAddr-1];
  ...
  if(p_SAO->uiLCURowStartY==0){
    ...
    if(!bPicTBoundary)
    {    
      pRefID = p_slice->m_sliceIDMapLCU[uiAddr-frameWidthInCU];
      *pbAvail = (VO_BOOL)( (*pbAvail) & ((pRefID == sliceID)?(VO_TRUE):(VO_FALSE)) );
    }
    ...
    {
      pRefID = p_slice->m_sliceIDMapLCU[uiAddr-frameWidthInCU-1];
      *pbAvail = (VO_BOOL)( (*pbAvail) & ((pRefID == sliceID)?(VO_TRUE):(VO_FALSE)) );
    }
...
{
  VO_BOOL *pbBorderAvail = p_slice->m_vNDFBlock[iAddr].isBorderAvailable;
  VO_U8  *right, *bottom;
...
//      
//       bottom = pSAO->m_pTmpU + uiUBufOffset;
//       pRec       = pSAO->pLCURec; 
//       pRec      += uiStride*(uiCurLCURowEndY - uiCurLCURowStartY -1);
...
VO_VOID restoreBoundary(H265_DEC_SLICE *p_slice,TComSampleAdaptiveOffset* pSAO, VO_U32 iAddr, VO_S32 iSaoType, VO_S32 iYCbCr, VO_U8 *left, VO_U8 *top, VO_U32 curBlockWidth, VO_U32 uiUBufOffset, VO_U32 uiLBufOffset){
  ...
  if(uiContainLastLCULine&&!isLastLCURow){
...
VO_VOID processSao(VO_VOID *phSlice,TComSampleAdaptiveOffset* pSAO, VO_S32 iYCbCr, VO_S32 iAddr)
{
  ...
  SaoLcuParam* pCurLcuSAOParam = &pSlice->saoLcuParam[iAddr];
  ...
  if(isUpPart){/*pSAO->uiLCURowStartY==0*/
    if(idxYLCU==0){      
//       pSAO->m_pTmpUTopIndex[iYCbCr] = 0;
      CopyMem(pSlice->p_user_op_all->p_user_op, pSlice->p_user_op_all->codec_id, &pSAO->m_pTmpU[uiUBufOffset], pRec, sizeof(VO_U8)*uiPicWidthTmp);
    }
  }
  else
  {
  ...
  if (iSAOType>=0)
  {
    ...
    pSAO->pSaoOffsetTableLCU = &pSlice->pSaoOffsetTable[(3*idxYLCU+iYCbCr)*pSAO->numCuInWidth+idxXLCU];
    ...
    //}else
      ...
      if(pSAO->uiLCURowStartY==0)
      {
        if (iSAOType == SAO_BO)
        {
          ...
          ppLumaTable = pSlice->m_lumaTableBo;
    ...
    if(pSlice->m_bUseNIF&&iSAOType!=SAO_BO)
    {  
		storeBoundary(pSlice, pSAO, iAddr, iSAOType, iYCbCr, left, top, uiEndX-uiStartX, uiUBufOffset, uiLBufOffset);
    } 
	processSaoBlock(pSAO, iSAOType, (iYCbCr!=0)? 1:0, &pSAO->m_pTmpU[(pSAO->m_iPicWidth+16)*iYCbCr],&pSAO->m_pTmpL[pSAO->m_CurLeftIndex][uiLBufOffset]);
    ...
    {    
	  ...
	  if (isUpPart && !uiIsLastLcuRow)
	  {
		  CopyMem(pSlice->p_user_op_all->p_user_op, pSlice->p_user_op_all->codec_id, (pSAO+1)->m_iUpBuff1All[iYCbCr] + 2*pSAO->uiLCULPelX, pSAO->m_iUpBuff1All[iYCbCr] + 2*pSAO->uiLCULPelX, sizeof(VO_U8)*(2*pSAO->m_uiMaxCUWidth));
	  }
...
VO_VOID setNDBBorderAvailabilityPerPPS(H265_DEC_SLICE *p_slice, VO_VOID *phPPS, TComSampleAdaptiveOffset* pSAO, VO_U32 *puiTileIdxMap){
  ...
  p_slice->m_bUseNIF = VO_TRUE;

  SetMem(p_slice->p_user_op_all->p_user_op, p_slice->p_user_op_all->codec_id, p_slice->m_vNDFBlock, VO_TRUE, frameWidthInCU*frameHeightInCU*sizeof(NDBFBlockInfo)); //1. default value
...
VO_VOID SAOProcessOneLCU(VO_VOID* phSlice, TComSampleAdaptiveOffset* pSAO, VO_S32 iAddr)
{
  H265_DEC_SLICE *pSlice = (H265_DEC_SLICE *)phSlice;  
//   TComSampleAdaptiveOffset* pSAO = pSlice->m_saoParam[LCURowIndex];

  if (pSlice->slice_sao_luma_flag)
  { 
    processSao(phSlice, pSAO,  0, iAddr); 

  }

  if(pSlice->slice_sao_chroma_flag)
  {    
    processSao(phSlice, pSAO,  1, iAddr);
    processSao(phSlice, pSAO,  2, iAddr);
