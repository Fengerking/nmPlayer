...
#ifdef ASM_OPT
	ConIR_asm(temp, exctation, H, subframel);
#else
	ConvolveImpulseR(temp, exctation, H, subframel);
#endif
...
//                cross = L_shr(cross,1);
//            }
            Ltemp = ((long)temp[i] * mresidual[i]) << 1;
            Ltemp >>= shift;
            cross += Ltemp;
            Ltemp = ((long)temp[i] * temp[i]) << 1;
            Ltemp >>= shift;
            ener  += Ltemp;
            if( ener >= 0x40000000L)
            {
	...
	if (cross > 0)
	{
		*idxcb = 0;
		...
		{
		    Ltemp = L_mpy_ls(ener, gcb_mid[i - 1]);
//		    Ltemp = L_shl(Ltemp, 1);
		    Ltemp <<= 1;
			if (cross > Ltemp)
			{
	...
	for (i = 0; i < subframel; i++)
	{
//	    Ltemp = L_mult(*lambda, exctation[i]);
//	    Ltemp = L_shl(Ltemp, 1);
//		exctation[i] = round32(Ltemp);
	    Ltemp = (long)(*lambda) * exctation[i];
		exctation[i] = (short)((Ltemp + 0x2000) >> 14);
	}
