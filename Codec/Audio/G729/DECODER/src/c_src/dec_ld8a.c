...
static Word16 lsp_old[M]={30000, 26000, 21000, 15000, 8000, 0, -8000,-15000,-21000,-26000};
...
void Init_Decod_ld8a(void)
{
	...
	Set_zero(mem_syn, M);
	sharp  = SHARPMIN;   //SHARPMIN  3277    /* Minimum value of pitch sharpening     0.2  Q14 */
	old_T0 = 60;
	gain_code = 0;
	gain_pitch = 0;
	Lsp_decw_reset();
...
void Decod_ld8a(
				Word16  parm[],      /* (i)   : vector of synthesis parameters parm[0] = bad frame indicator (bfi)  */
									 ...
									 )
{
	...
	synth = (Word16 *)gData->outbuf_ptr;
	bad_lsf = (Word16)gData->bad_lsf;
	...
	*Vad = ftyp;
	...
	if(ftyp != 1) {
		...
		Dec_cng(past_ftyp, sid_sav, sh_sid_sav, parm, exc, lsp_old,A_t, &seed, lsfq_mem);
		...
		*------------------------------------------------------------------------*/
		...
		for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
		{
			...
			if(i_subfr == 0)
			{
				...
				if( bad_pitch == 0)
				{
					Dec_lag3(index, PIT_MIN, PIT_MAX, i_subfr, &T0, &T0_frac); //PIT_MIN = 20   PIT_MAX=143
					old_T0 = T0;
				}
			...
			*-------------------------------------------------*/
#ifdef ASM_OPT
		    Pred_lt_3_asm(&exc[i_subfr], T0, T0_frac, L_SUBFR);
#else
	...
	*-----------*/
	if(bfi == 0) {
		L_temp = 0L;
#ifdef  ASM_OPT
        L_temp = sum_mac_v4(L_temp, exc, L_FRAME);
#else
		for(i=0; i<L_FRAME; i++) {
		...
		sid_sav = vo_round((L_temp << sh_sid_sav));
		sh_sid_sav = (16 - sh_sid_sav);
	}
