			   ...
			   )
{
	...
	*------------------------------------------------------------------*/
	...
	coeff[1] = -(g_coeff[2]);                    /* coeff[1] = -2 xn y1 */
	exp_coeff[1] = g_coeff[3] + 1;     

	/* Compute scalar product <y2[],y2[]> */
#ifdef ASM_OPT
	coeff[2] = extract_h(voAMRWBEncDot_product12_asm(y2, y2, L_subfr, &exp));
#else
...
#ifdef ASM_OPT
	coeff[3] = extract_h(L_negate(voAMRWBEncDot_product12_asm(xn, y2, L_subfr, &exp)));
#else
...
#ifdef ASM_OPT
	coeff[4] = extract_h(voAMRWBEncDot_product12_asm(y1, y2, L_subfr, &exp));
#else
	...
	*-----------------------------------------------------------------*/
#ifdef ASM_OPT
	L_tmp = voAMRWBEncDot_product12_asm(code, code, L_subfr, &exp_code);
#else
	...
	Log2(L_tmp, &exp, &frac);
	exp += exp_code;
	L_tmp = voMpy_32_16(exp, frac, -24660);  /* x -3.0103(Q13) -> Q14 */
	...
	*---------------------------------------------------*/
	...
	Log2(L_tmp, &exp, &frac);
	exp -= 11;
	L_tmp = voMpy_32_16(exp, frac, 24660);   /* x 6.0206 in Q12 */
