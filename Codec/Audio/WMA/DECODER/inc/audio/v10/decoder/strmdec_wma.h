...
typedef enum {
	NEED_FRAME_STA, 
	NEED_FRAME_END,
	MAX_DFBS_VALUE = MAX_VOENUM_VALUE
} DumpFrameBitsStatus;
...
//wchen: add for streaming mode
    ...
    Void*           m_pUser;
    struct CAudioObjectDecoder*  m_dwOwner; // not really 'dw' these days
...
typedef enum { 
	...
	ModeGetFlush, 
	ModePktHdr,
	MAX_GetMDM_VALUE = MAX_VOENUM_VALUE 
} GetMoreDataMode;

Void        ibstrmInit (CWMAInputBitStream* pibstrm, struct CAudioObjectDecoder* pOwner);
