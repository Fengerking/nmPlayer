...
/*************************************************************************
*
*  Function:   voAMRNBEnc_gain_adapt_init
*  Purpose:    Allocates state memory and initializes state memory
*
**************************************************************************
*/
int voAMRNBEnc_gain_adapt_init (GainAdaptState **st, VO_MEM_OPERATOR *pMemOP)
{
	GainAdaptState* s;
	...
	*st = NULL;
	/* allocate memory */
	if ((s= (GainAdaptState *)voAMRNBEnc_mem_malloc(pMemOP, sizeof(GainAdaptState), 32)) == NULL){
		return -1;
	}
	voAMRNBEnc_gain_adapt_reset(s);
	*st = s;
...
/*************************************************************************
*
*  Function:   voAMRNBEnc_gain_adapt_reset
*  Purpose:    Initializes state memory to zero
*
**************************************************************************
*/
int voAMRNBEnc_gain_adapt_reset (GainAdaptState *st)
{
	Word16 i;

	if (st == (GainAdaptState *) NULL){
		fprintf(stderr, "voAMRNBEnc_gain_adapt_reset: invalid parameter\n");
		return -1;
	}
...
/*************************************************************************
*
*  Function:   voAMRNBEnc_gain_adapt_exit
*  Purpose:    The memory used for state memory is freed
*
**************************************************************************
*/
void voAMRNBEnc_gain_adapt_exit (GainAdaptState **st, VO_MEM_OPERATOR *pMemOP)
{
	if (st == NULL || *st == NULL)
		return;
	/* deallocate memory */
...
{
	Word32 adapt;      /* adaptdation status; 0, 1, or 2       */
	Word32 result;     /* alpha factor, Q13                    */
	Word32 filt;       /* median-filtered LTP coding gain, Q13 */
	Word32 tmp, i;
	/* basic adaptation */
	...
	*/
	...
	st->ltpg_mem[0] = ltpg;      
	filt = voAMRNBEnc_gmed_n (st->ltpg_mem, 5);  /* function result */
	if (adapt == 0)
	{
		...
		else
		{
			...
			{   /* result       =   0.5 - 0.75257499*filt     */
				...
				filt = shl3 (filt, 2); /* Q15 */
				result = sub3 (16384, mult3 (24660, filt));
			}
		}
	}
	else
