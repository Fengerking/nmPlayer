...
*
*************************************************************************/
static void MR795_gain_code_quant3(
								   ...
								   )
{
	...
	*/
	...
	exp_code = sub3(exp_gcode0, 10);
	/* calculate exp_max[i] = s[i]-1 */
	exp_max[0] = sub3(exp_coeff[0], 13);                        
	exp_max[1] = sub3(exp_coeff[1], 14);                        
	exp_max[2] = add3(exp_coeff[2], add3(15, shl3(exp_code, 1))); 
	exp_max[3] = add3(exp_coeff[3], exp_code);                  
	exp_max[4] = add3(exp_coeff[4], add3(exp_code,1));           
	/*-------------------------------------------------------------------*
	...
	*-------------------------------------------------------------------*/
	...
	{
		if (sub3(exp_max[i], e_max) > 0)
		{
			e_max = exp_max[i];                                
		}
	}
	e_max = add3(e_max, 1);      /* To avoid overflow */
	for (i = 0; i < 5; i++) {
		j = sub3(e_max, exp_max[i]);
	...
	*-------------------------------------------------------------------*/
	...
	for (j = 0; j < 3; j++)
	{
		...
		for (i = 0; i < NB_QUA_CODE; i++)
		{
			...
			L_tmp = Mac_32  (L_tmp0, coeff[2], coeff_lo[2],g2_code_h, g2_code_l);
			L_tmp = Mac_32_16(L_tmp, coeff[3], coeff_lo[3],g_code);
			L_tmp = Mac_32   (L_tmp, coeff[4], coeff_lo[4],g_pit_cod_h, g_pit_cod_l);
	...
	*------------------------------------------------------------------*/
	...
	L_tmp = L_shr(L_tmp, sub3(9, exp_gcode0));
...
*
*************************************************************************/
static Word16 MR795_gain_code_quant_mod(  /* o  : index of quantization.            */
						  Word16 gain_pit,        /* i  : pitch gain,                   Q14 */
						  Word16 exp_gcode0,      /* i  : predicted CB gain (exponent), Q0  */
						  Word16 gcode0,          /* i  : predicted CB gain (norm.),    Q14 */
						  Word16 frac_en[],       /* i  : energy coefficients (4), fraction part, Q15 */
						  Word16 exp_en[],        /* i  : energy coefficients (4),eponent part,                 Q0  */
						  Word16 alpha,           /* i  : gain adaptor factor (>0),     Q15 */
						  Word16 gain_cod_unq, 
						  /* i  : Code gain (unquantized)           */
						  /*      (scaling: Q10 - exp_gcode0)       */
						  Word16 *gain_cod,       /* i/o: Code gain (pre-/quantized),   Q1  */
						  Word16 *qua_ener_MR122, 
						  /* o  : quantized energy error,       Q10 */
						  /*      (for MR122 MA predictor update)   */
						  Word16 *qua_ener       
						  /* o  : quantized energy error,       Q10 */
						  /*      (for other MA predictor update)   */
						  )
{
	...
	*/
	...
	tmp = extract_h (alpha * frac_en[1]);
	/* directly store in 32 bit variable because no further mult. required */
	L_t1 = L_mult (tmp, g2_pitch);                 
	exp_coeff[1] = sub3 (exp_en[1], 15);           

	tmp = extract_h (alpha * frac_en[2]);
	coeff[2] = mult(tmp, gain_pit);               
	exp = sub3 (exp_gcode0, 10);
	exp_coeff[2] = add3 (exp_en[2], exp);          
	/* alpha <= 0.5 -> mult. by 2 to keep precision; compensate in exponent */
	coeff[3] = extract_h ((alpha * frac_en[3]));
	exp = sub3 (shl (exp_gcode0, 1), 7);
	exp_coeff[3] = add3 (exp_en[3], exp);          
	coeff[4] = mult (one_alpha, frac_en[3]);       
	exp_coeff[4] = add3 (exp_coeff[3], 1);         
	L_tmp = L_mult (alpha, frac_en[0]);
	/* sqrt_l returns normalized value and 2*exponent
	-> result = val >> (exp/2)
	exp_coeff holds 2*exponent for c[0]            */
	/* directly store in 32 bit variable because no further mult. required */
	L_t0 = sqrt_l_exp (L_tmp, &exp); /* normalization included in sqrt_l_exp *//* function result */
	exp = add3 (exp, 47);
	exp_coeff[0] = sub3 (exp_en[0], exp);
	/*
	...
	*/
	...
	e_max = add3 (exp_coeff[0], 31);
	for (i = 1; i <= 4; i++)
	{
		if (sub3(exp_coeff[i], e_max) > 0)
		{
			e_max = exp_coeff[i];
		}
	}
	...
	exp = sub3 (e_max, 31);             /* new exponent */
	...
	if ((tmp & 0x1) != 0)
	{
		...
		L_t0 = Mpy_32_16(coeff[0], coeff_lo[0], 23170);                    /* 23170 Q15 = 1/sqrt(2)*/
	}
...
********************************************************************************
*/
#if SAFE_CODE_MR795_gain_quant
#define MR795_gain_code_quant3 vo_MR795_gain_code_quant3
#define MR795_gain_code_quant_mod vo_MR795_gain_code_quant_mod
#endif//SAFE_CODE_MR795_gain_quant
/*************************************************************************
*
...
*
*************************************************************************/
void voAMRNBEnc_MR795_gain_quant(
					  ...
					  )
{
	...
	*/
	gain_pit_index = voAMRNBEnc_q_gain_pitch(MR795, gp_limit, gain_pit,g_pitch_cand, g_pitch_cind); 
	/* function result */
	/*-------------------------------------------------------------------*
	...
	*-------------------------------------------------------------------*/
	gcode0 = extract_l(voAMRNBEnc_Pow2(14, frac_gcode0));          /* Q14 */
	...
	*/
	...
	voAMRNBEnc_calc_unfilt_energies(res, exc, code, *gain_pit, L_subfr,frac_en, exp_en, &ltpg);
	...
	*/
	voAMRNBEnc_gain_adapt(adapt_st, ltpg, *gain_cod, &alpha);
...
#if 0
		gain_cod_index = gain_cod_unq +16;
#else
