...
OMX_PTR				g_hMobiTVInst = NULL;
voCOMXCoreMobiTV *	g_pMobiTVCore = NULL;
...
OMX_API OMX_ERRORTYPE OMX_APIENTRY OMX_Init(void)
{
	...
	if (g_pMobiTVCore != NULL)
		return OMX_ErrorNone;

	g_pMobiTVCore = new voCOMXCoreMobiTV ();
	if (g_pMobiTVCore == NULL)
		return OMX_ErrorInsufficientResources;

	return g_pMobiTVCore->Init (g_hMobiTVInst);
}
															...
															OMX_IN  OMX_U32 nIndex)
{
	...
	OMX_STRING pName = g_pMobiTVCore->GetName (nIndex);
	if (pName == NULL)
													...
													OMX_IN  OMX_CALLBACKTYPE* pCallBacks)
{
	...
	OMX_ERRORTYPE errType = g_pMobiTVCore->LoadComponent (pComp, cComponentName);
...
OMX_API OMX_ERRORTYPE OMX_APIENTRY OMX_FreeHandle(OMX_IN  OMX_HANDLETYPE hComponent)
{
	...
	OMX_ERRORTYPE errType = g_pMobiTVCore->FreeComponent (pComp);
	if (errType != OMX_ErrorNone)
	{
		voOMXMemFree (pComp);
		return errType;
												...
												OMX_INOUT   OMX_U8  **compNames)
{
	...
	return g_pMobiTVCore->GetComponentsOfRole (role, pNumComps, compNames);
}
												...
												OMX_OUT     OMX_U8 **roles)
{
	...
	return g_pMobiTVCore->GetRolesOfComponent (compName, pNumRoles, roles);
}
