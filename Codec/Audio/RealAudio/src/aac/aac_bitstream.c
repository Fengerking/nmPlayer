...
int newBitstream(struct BITSTREAM **ppBitstream, int nBits,
                 void* pUserMem, rm_malloc_func_ptr fpMalloc, VO_MEM_OPERATOR  *pMemOP)
{
    struct BITSTREAM *pBitstream ;

    pBitstream = (struct BITSTREAM *)fpMalloc(pMemOP, pUserMem, sizeof(struct BITSTREAM)) ;
...
void deleteBitstream(struct BITSTREAM *pBitstream, void* pUserMem,
                     rm_free_func_ptr fpFree, VO_MEM_OPERATOR  *pMemOP)
{
    if (pBitstream) fpFree(pMemOP, pUserMem, pBitstream) ;
}
