...
#if 0
void prvSetSample24OUT16INLINE(const PCMSAMPLE iValue,
...
void prvReconMonoPureLLM_I24_OUT16(const CAudioObject *pau, Int *pi, const Int iSegEnd,                            
            I16 **ppcSampleGet, PCMSAMPLE **ppiDst, CBT **ppiData,                          
            const Int cChan)
{
	Int i = *pi;                                                                            
	...
	for (; i < iSegEnd; i++,(*pcSampleGet)++)    {                                          
        iResult = (I32)piData[i]; 
		iResult = checkRange(iResult,I24_MIN,I24_MAX);                          
		prvSetSample24OUT16INLINE(iResult, piDst, pau, 0);

		piDst = prvAdvancePCMPointer16(piDst, pau->m_nBytePerSample, cChan);
	}
	*pi = i;                                                                                
    *ppcSampleGet = pcSampleGet;                                                            
    *ppiDst = piDst;
...
RECONMONO_PURELLM(prvReconMonoPureLLM_I24, ROUND_SATURATE_STORE24_LLM,
				...
				// Instantiate 24-bit out 16-bit loop functions
RECONMONO_PURELLM(prvReconMonoPureLLM_I24_OUT16, ROUND_SATURATE_STORE24_OUT16_LLM,
                I24_MIN, I24_MAX, prvAdvancePCMPointer16)
    ...
    if (i < pau->m_rgiPCMInHistory[iCh])
    {
        ...
        switch (pau->m_iBitDepthSelector)
        {
            case BITDEPTH_24:
				...
				prvReconMonoPureLLM_I24_OUT16(pau, &i, iSegEnd, &pcSampleGet, &piDst, &piData, cChan);
#else
...
Void prvBackupFiltersLLM_VERB(CAudioObject * pau, PerChannelInfo * ppcInfoComms)
{
    ...
    pau->m_MCLMSPredictor.m_rgiFilterBackup = (I16 *)memset(pau->m_MCLMSPredictor.m_rgiFilterBackup, 0, 
    ...
    // clear Backup buffer to solve the problem when order changes at seeking points.
    pau->m_MCLMSPredictor.m_rgiFilterCurrTimeBackup = (I16 *)memset(pau->m_MCLMSPredictor.m_rgiFilterCurrTimeBackup, 0, 
    ...
    for (iCh = 0; iCh < pau->m_cChannel; iCh++) {
        ...
        for (k = 0; k < ppcInfoComm->m_cLMSPredictors; k++) {
            ...
            // clear Backup buffer to solve the problem when order changes at seeking points.
            ppcInfoComm->m_rgLMSPredictors[k].m_rgiFilterBackup = (I16 *)memset(ppcInfoComm->m_rgLMSPredictors[k].m_rgiFilterBackup, 0, 
