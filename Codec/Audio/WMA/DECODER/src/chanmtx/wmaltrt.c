...
{
    pLtRtDownmix = (LtRtDownmix *)memset(pLtRtDownmix, 0, sizeof(LtRtDownmix));
...
void ltrtDownmixFree(CAudioObjectDecoder *paudec, LtRtDownmix *pLtRtDownmix)
{
    if (True == pLtRtDownmix->m_bCanFree)
    {
        bufFiltFree(paudec, &pLtRtDownmix->m_bfSurround);
        bufDelayFree(paudec, &pLtRtDownmix->m_bfLeft);
        bufDelayFree(paudec, &pLtRtDownmix->m_bfRight);
...
WMARESULT ltrtDownmixInit(CAudioObjectDecoder *paudec, LtRtDownmix *pLtRtDownmix, Int nHalfFilterLen, Int samplingRate,
                          DataType maxVal, Int bufSize)
{
    ...
    ltrtDownmixFree(paudec, pLtRtDownmix);
...
#define THRESH (0.92f) // don't worry too much about clipping -- use time constants instead to smooth out gain
    ...
    hr = bufFiltInit(paudec, &pLtRtDownmix->m_bfSurround, overallFilter,
        -nHalfFilterLen, nHalfFilterLen, bufSize);
    if (WMA_FAILED(hr)) goto exit;

    // init delay left
    hr = bufDelayInit(paudec, &pLtRtDownmix->m_bfLeft, nHalfFilterLen);
    if (WMA_FAILED(hr)) goto exit;

    // init delay right
    hr = bufDelayInit(paudec, &pLtRtDownmix->m_bfRight, nHalfFilterLen);
    if (WMA_FAILED(hr)) goto exit;
    ...
    pLtRtDownmix->m_buf = (DataType*)memset(pLtRtDownmix->m_buf, 0, pLtRtDownmix->m_bufSize * 3 * sizeof(DataType));
...
WMARESULT ltrtDownmixInitBuf(CAudioObjectDecoder *paudec, LtRtDownmix *pLtRtDownmix,
                             ...
                             DataType maxVal, Int bufSize, PCMFormat *pcmIn)
{
    ...
    hr = SetMixFromChannelMaskF(paudec, pLtRtDownmix->m_cInChannels, LTRT_SRC_CHANNELS,
    ...
    TRACEWMA_EXIT(hr, ltrtDownmixInit(paudec, pLtRtDownmix, nHalfFilterLen,
...
WMARESULT ltrtDownmix(struct CAudioObjectDecoder *paudec, LtRtDownmix *pLtRtDownmix, Int len, Int *pnOut)
{
    ...
    hr = pLtRtDownmix->m_pfnBufFilt(paudec, &pLtRtDownmix->m_bfSurround, pfSurround, len);
    if (WMA_FAILED(hr)) goto exit;
    nOutAll = len;

    // delay left (in place)
    hr = bufDelay(paudec, &pLtRtDownmix->m_bfLeft, pfLeft, len, &nOut);
    if (WMA_FAILED(hr)) goto exit;
    assert(nOut == nOutAll);

    // delay right (in place)
    hr = bufDelay(paudec, &pLtRtDownmix->m_bfRight, pfRight, len, &nOut);
    if (WMA_FAILED(hr)) goto exit;
    assert(nOut == nOutAll);
...
//------------------------------------------------------------------------------
void ltrtDownmixBuf(CAudioObjectDecoder *paudec, LtRtDownmix *pLtRtDownmix, Int cSamples,
                    LtRtBufType *bufIn, LtRtBufType *bufOut)
{
    ...
    else if (pLtRtDownmix->m_bOptMtx2to4chan)
    {
        ...
        // Do LtRt processing
        wmaResult = ltrtDownmix(paudec, pLtRtDownmix, cSamples, &nOut);
                         ...
                         LtRtBufType *bufIn, LtRtBufType *bufOut)
{
    ...
    while ( i > pLtRtDownmix->m_bufSize)
    {
        ...
        ltrtDownmixBuf(paudec, pLtRtDownmix, pLtRtDownmix->m_bufSize, bufIn, bufOut);
        bufIn += pLtRtDownmix->m_bufSize * pLtRtDownmix->m_cInChannels;
        bufOut += pLtRtDownmix->m_bufSize * LTRT_DST_CHANNELS;
    }
    if (i)
    {
        ltrtDownmixBuf(paudec, pLtRtDownmix, i, bufIn, bufOut);
    }
}
...
// -----------------------------------------------------------------------------
void ltrtDownmixBufPCM(CAudioObjectDecoder *paudec, LtRtDownmix *pLtRtDownmix, Int cSamples, U8 *bufInPCM,
                       U8 *bufOutPCM)
{
    ...
    while ( i > pLtRtDownmix->m_bufSize)
    {
        ...
        ltrtDownmixBuf(paudec, pLtRtDownmix, pLtRtDownmix->m_bufSize,
    ...
    if (i)
    {
        ...
        ltrtDownmixBuf(paudec, pLtRtDownmix, i, pLtRtDownmix->m_pfAuxBuf, pLtRtDownmix->m_pfAuxBuf);
