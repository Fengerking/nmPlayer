...
**************************************************************************
*/
int Post_Process_init (Post_ProcessState **state, VO_MEM_OPERATOR *pMemOP)
{
	Post_ProcessState* s;
	...
	*state = NULL;
	/* allocate memory */
	if ((s= (Post_ProcessState *) voAMRNBDecmem_malloc(pMemOP, sizeof(Post_ProcessState), 32)) == NULL){
		return -1;
	}
...
**************************************************************************
*/
void Post_Process_exit (Post_ProcessState **state, VO_MEM_OPERATOR *pMemOP)
{
...
#ifdef C_OPT
{
	...
	for (i = 0; i < lg; i++)
	{
		...
		/*                     + a[1]*y[i-1] + a[2] * y[i-2];      */
		L_tmp = (st->y1_hi * 15836) ;

		L_tmp += (((st->y1_lo * 15836)>>15));

		L_tmp += (st->y2_hi * -7667);

		L_tmp += (((st->y2_lo * -7667)>>15));

		L_tmp += ((st->x1 * -15398));

		L_tmp += (((st->x0 + x2) *7699));
		L_tmp = L_shl2(L_tmp, 3);
		/* Multiplication by two of output speech with saturation. */
		signal[i] = vo_round(L_shl2(L_tmp, 1));
		st->y2_hi = st->y1_hi;                  
		st->y2_lo = st->y1_lo;                  
		st->y1_hi = L_tmp >> 16;
		st->y1_lo = (L_tmp - (st->y1_hi<<16))>>1;  
	}
	return 0;   
}
#else
{
	...
	for (i = 0; i < lg; i++)
	{
		...
		L_tmp = L_shl (L_tmp, 2);
		/* Multiplication by two of output speech with saturation. */
		signal[i] = vo_round(L_shl(L_tmp, 1));  
		st->y2_hi = st->y1_hi;               
		st->y2_lo = st->y1_lo;               
		voAMRNBDecL_Extract (L_tmp, &st->y1_hi, &st->y1_lo);
	}
	return 0;    
}
#endif