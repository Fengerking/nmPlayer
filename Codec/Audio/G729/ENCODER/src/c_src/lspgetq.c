...
)
{
	...
	for(j = 0; j <M; j++)
	{
		L_acc = (lsp_ele[j] * fg_sum[j])<<1;

		//for(k = 0; k < MA_NP; k++)
		L_acc += (freq_prev[0][j] * fg[0][j])<<1;
		L_acc += (freq_prev[1][j] * fg[1][j])<<1;
		L_acc += (freq_prev[2][j] * fg[2][j])<<1;
		L_acc += (freq_prev[3][j] * fg[3][j])<<1;
...
)
{
  ...
  for ( j = 0 ; j < M ; j++ ) {
    ...
    for ( k = 0 ; k < MA_NP ; k++ )
      L_temp -= (freq_prev[k][j] * fg[k][j] )<<1;

    temp = extract_h(L_temp);
    L_temp = ( temp * fg_sum_inv[j] )<<1;
    lsp_ele[j] = extract_h( ( L_temp << 3 ) );
...
)
{
  ...
  for ( k = MA_NP-1 ; k > 0 ; k-- )
	  memcpy(freq_prev[k],freq_prev[k-1],2*M);
    //Copy(freq_prev[k-1], freq_prev[k], M);

  //Copy(lsp_ele, freq_prev[0], M);
  memcpy(freq_prev[0],lsp_ele,2*M);
