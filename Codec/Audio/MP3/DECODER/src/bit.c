...
void voMP3DecInitBits(Bitstream *bitptr, unsigned char *byte, unsigned int length)
{
	bitptr->byte  = byte;
	bitptr->cache = 0;
	bitptr->bitsleft  = 0;
...
static __inline void voMP3DecReflush(Bitstream *bitptr)
{
...
unsigned int voMP3DecGetBits(Bitstream *bitptr, int nBits)
{
...
unsigned int voMP3DecSkipBits(Bitstream *bitptr, int nBits)
{
	...
	{
		nsBits = nBits;
		voMP3DecGetBits(bitptr, nBits);
	}
	else
	{
		nsBits = bitptr->bitsleft;
		nBits -= bitptr->bitsleft;
		voMP3DecGetBits(bitptr, bitptr->bitsleft);
		nSByte = nBits >> 3;
		nBits = nBits & 7;
...
/*
 * NAME:	voMP3DecBits_Crc()
 * DESCRIPTION:	compute CRC-check word
 */
unsigned int voMP3DecBits_Crc(Bitstream *bitptr, unsigned int len, unsigned int init)
{
	unsigned int crc, data;
	
	for (crc = init; len >= 32; len -= 32) 
	{
		data = voMP3DecGetBits(bitptr, 8);		
	...
	{
	case 3: crc = (crc << 8) ^
				crc_table[((crc >> 8) ^ voMP3DecGetBits(bitptr, 8)) & 0xff];
	case 2: crc = (crc << 8) ^
				crc_table[((crc >> 8) ^ voMP3DecGetBits(bitptr, 8)) & 0xff];
	case 1: crc = (crc << 8) ^
				crc_table[((crc >> 8) ^ voMP3DecGetBits(bitptr, 8)) & 0xff];
	...
	while (len--) 
	{
		data = voMP3DecGetBits(bitptr, 1) ^ (crc >> 15);
