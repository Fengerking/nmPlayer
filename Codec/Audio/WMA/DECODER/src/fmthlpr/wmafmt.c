...
#if !defined(UNDER_CE) && !defined(LINUX)//=!(defined(UNDER_CE) || defined(LINUX))
#include <assert.h>
#endif
...
void WMAFormat2PCMFormat(void *hCodec, WMAFormat* pSrc, PCMFormat* pDst)
{
	CAudioObjectDecoder* paudec = (CAudioObjectDecoder *)hCodec;
//	paudec->SupptHiSmplrt = 1;
	if(paudec->SupptHiSmplrt == 0)
	{
		if(pSrc->nSamplesPerSec <= 48000)
		{			
			pDst->nSamplesPerSec = pSrc->nSamplesPerSec;
//			if(pSrc->nSamplesPerSec == 8000)
//				pDst->nSamplesPerSec = 48000; //for test
		}
		else if(pSrc->nSamplesPerSec % 44100 == 0)
			pDst->nSamplesPerSec = 44100;
		else
			pDst->nSamplesPerSec = 48000;
	}
	else
		pDst->nSamplesPerSec = pSrc->nSamplesPerSec;

	if(paudec->Support24Bit == 0)
		pDst->nValidBitsPerSample = (pSrc->nValidBitsPerSample <= 16) ? pSrc->nValidBitsPerSample : 16;
	else 
	...
	if(paudec->SupptMtChanl == 0)
		pDst->nChannels = (pSrc->nChannels <= 2) ? pSrc->nChannels : 2;
	else
		pDst->nChannels = pSrc->nChannels;

	pDst->nChannelMask = WMAProDefaultChannelMask(pDst->nChannels);
	pDst->cbPCMContainerSize = (pDst->nValidBitsPerSample + 7) / 8; 
