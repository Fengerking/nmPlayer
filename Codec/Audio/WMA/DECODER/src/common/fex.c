...
WMARESULT freqexInit(CAudioObjectDecoder *paudec, CAudioObject *pau)
{
    WMARESULT wmaResult = WMA_OK;
    Fex *pfx = &(pau->m_fex);
...
Void freqexFree(CAudioObjectDecoder *paudec, CAudioObject *pau)
{
    Fex *pfx = &(pau->m_fex);
    Int iCh;
...
{
    if (pau->m_bPlusV1)
        pau->m_ucNonzeroCoeffMask = (U8 *)memset(pau->m_ucNonzeroCoeffMask,
                                  ...
                                  Bool bForceCalc)
{
    ...
    pau->m_pucNonzeroMask = (U8 *)memset(pau->m_pucNonzeroMask, 0, pau->m_cFrameSampleHalf/8);
...
//    catchFP();
    ...
    if( 0 < fScale.iFraction )
        fScale = FASTFLOAT_FROM_FLOAT( (Float)((Float)voWMAlog10( (VO_S32)(FLOAT_FROM_FASTFLOAT( fScale ) * 256)) * 5.0f /16384.0f));
//        fScale = FASTFLOAT_FROM_FLOAT( (Float)(20.0f * log10( FLOAT_FROM_FASTFLOAT( fScale ) ) ));
    else
        fScale.iFraction = 0;
                                               ...
                                               FexGlobalParam *pfxgp )
{
    ...
    for (iCh=0; iCh < pau->m_cChInTile; iCh++)
    {
        ...
        if( pfxgp->m_bArbitraryScaleBandConfig )
            pTemp = voWMAMemcpy( pfx->m_rgiBandSizeM + pcis->m_iFxBandConfig * pfx->m_cMaxBands, 
...
Void freqexSetLastCodedIndex(CAudioObject *pau, Int iChCodeSrc)
{
    ...
    for (iCh=0; iCh < cChannel; iCh++)
    {
            ...
            (pcisSrc->m_uFxChannelDependencyMask & (1<<iChSrc)))
        {
            ...
            iNum++;
			if(ppcinfo->m_iPower != 0)bPower = WMAB_TRUE;
        ...
        for (iCh=0; iCh < pau->m_cChannel; iCh++)
        {
            ...
            for (iBark=0; iBark < pau->m_cValidBarkBand; iBark++)
            {
                if (!prvIsBarkInd(pau, iCh, iBark) || pau->m_bCx)
                {
                    ...
                    pTemp = voWMAMemcpy(pucMask1+iStartByte,
                           pucMask0+iStartByte,
                              ...
                              Bool bGetMax, Bool bMvBands)
{
    ...
    const Int *piBandSizeM = (pcis->m_bFxArbitraryBandConfig == WMAB_TRUE) ?
        pfx->m_piBandSizeM : pfx->m_piFexBandSizesM;
...
INLINE Int freqexRandomMv( const Fex *pfx )
{
    ...
    return rand()*(g_iFexNoiseLen+1-pfx->m_cBandSize)/(RAND_MAX);
	//return rand()*(g_iFexNoiseLen+1-pfx->m_cBandSize)/(RAND_MAX + 1);  --- old source, clean up warning, maybe have not bit-match issue
}
                           ...
                           const QuantStepType fGlobalQ )
{
    ...
    if (iCb==2)
    {
        ...
        for (j = 0; j < pfx->m_rgcScaleBandsPerMvBand[iBand]; j++)
        {
            ...
            // complement the mask
			// iCbStart1 is used to avoid warning of ignoring return value
            iCbStart1 = freqexComplementMask(pfx->m_rgucCwMask, pfx->m_cBandSize);
    ...
    else if (iCb == 4 && !pfx->m_bReconDomain)
    {
        ...
        for (j = 0; j < pfx->m_rgcScaleBandsPerMvBand[iBand]; j++)
        {
            ...
            fMvScale = freqexGetMvBandScaleWithMask( pfx,
                                                     ...
                                                     cLeftShiftBits );
            // reconstruct peaks
            pfRecon = (CoefType *)memset(pfRecon, 0, sizeof(CoefType) * cCoeffsActRem);
            TRACEWMA_EXIT(wmaResult,
                          freqexScaleBand( ppcinfo, pfx,
                                           ppcinfo->m_pffltFexScaleFac,
                                           fMvScale, pfRecon, pfCb,
             ...
             const Int iLen )
{
    ...
    if (WMAB_FALSE==pfx->m_bSplitTileIntoSubtiles)
    {
        ...
        {
        case ClearBuf:
            dst = (CoefType *)memset(dst, 0, iLen*sizeof(CoefType));
            break;
        case CopyBuf:
            dst = (CoefType *)voWMAMemcpy(dst, src, iLen*sizeof(CoefType));
            break;
        case AddToBuf:
            for (i=0; i < iLen; i++)
            {
    ...
    else if (dst != src)
    {
        ...
        {
        case ClearBuf:
            pdst0 = (CoefType *)memset(pdst0, 0, iLen/2*sizeof(CoefType));
            pdst1 = (CoefType *)memset(pdst1, 0, iLen/2*sizeof(CoefType));
            break;
        case CopyBuf:
    ...
    for (iCh=0; iCh < pau->m_cChannel; iCh++)
    {
        ...
        while( i < prp->m_iTilesInSubframeProcFx )
        {
            if(!chexInfoStatNoFx(prti->m_uInfoStat))
            {
                ...
                {
                    if (0 == iType)
                        fprintf(fp, "%d ", ppcr->m_rgiFexMv[j]);
                    else if (1 == iType)
                        fprintf(fp, "%f ", ppcr->m_rgFexExp[j]);
                }
                fprintf(fp, "\n");
                i++;
            }
...
Void bpeakReset( CAudioObject *pau )
{
    if( pau->m_rgcBasePeakCoefs )
        pau->m_rgcBasePeakCoefs = (Int *)memset( pau->m_rgcBasePeakCoefs, 0, pau->m_cOrigChannel * sizeof(Int) );
}
