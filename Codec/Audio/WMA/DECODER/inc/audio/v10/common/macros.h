...
#if defined(SCRAMBLE_GLOBAL_SYMBOL_NAMES)
#include "scramble.h"
#endif
...
#if (defined(BUILD_INTEGER) || defined(UNDER_CE) || defined(LINUX)) && !defined(CX_LOW_POWER)
#define CX_LOW_POWER
#endif
...
#define MAX_VOENUM_VALUE 0x7fffffff
...
#if defined(BUILD_INTEGER) && defined(BUILD_WMAPRO) && !defined(UNDER_CE) && !defined(__arm) && !defined(DISABLE_COEF64BIT) && !defined(LINUX)
#define COEF64BIT
...
// bits-per-sample type
    typedef U32 BPSType;
#   define WBPS_BITS 27
#   define WBPS(x) (I32)((x)*(1<<WBPS_BITS))
...
#else
extern I32	MULT_HI_DWORDN_ARM( I32, I32, I32);
#	define MULT_HI_DWORDN(a,b,N) MULT_HI_DWORDN_ARM(a, b, N)//(I32)((((I64)(a))*((I64)(b)))>>(32-(N)))
#endif
...
static __inline U32 ByteSwap(U32 i){ 
    U8 *p = (U8 *)&i;
    return (p[0]<<24)|(p[1]<<16)|(p[2]<<8)|p[3];
}
#endif
...
static __inline I32 DIVI64BYU32(I64 a, U32 b) {
    return (I32)((a)/(b));
}
#endif
...
#ifndef PLATFORM_SPECIFIC_ROUND
static __inline I32   ROUNDF(float f)
{
    if (f < 0.0f)
        return (I32) (f - 0.5f);
    else
...
//---------------------------------------------------------------------------
static __inline I32   ROUNDD(double f)
{
...
#define ROUND_SATURATE_STORE24_OUT16_LLM(piDst,iResult,mn,mx)     \
    assert(pau->m_nBytePerSample == 3);                           \
    iResult = checkRange(iResult,mn,mx);                          \
    prvSetSample24OUT16INLINE(iResult, piDst, pau, 0);
...
FORCEINLINE static int WMAPrintf(const char* pszFmt, ... )
{
#if (!defined (UNDER_CE) && !defined(LINUX) && !defined(_IOS)) || defined (SDE_WANTS_PRINTFIO)
    int ret;
    va_list vargs;
...
FORCEINLINE static int WMAFprintf(FILE *stream, const char* pszFmt, ... )
{
#if (!defined(UNDER_CE) && !defined(LINUX) && !defined(_XBOX)) || defined (SDE_WANTS_PRINTFIO)
    int ret;
    va_list vargs;
    va_start(vargs, pszFmt);
...
/*FORCEINLINE int WMASprintf(char *buffer, const char* pszFmt, ... )
{
...
#if (!defined (UNDER_CE) && !defined(LINUX)) || defined (SDE_WANTS_PRINTFIO)
    #ifdef _XBOX
        #include <tchar.h>
        #define XFILE void
        #define STDOUT (void *)                
    ...
    #define FFLUSH(a)          EOF
#endif //(!defined (UNDER_CE) && !defined(LINUX)) || defined (SDE_WANTS_PRINTFIO)
//#define DEBUG_ENCODE_FOR_XBOX

#include "wmspecstring.h"

#ifdef SEUIC //special processing for SEUIC platform
#define WMA_OPT_FOR_SPEED               1  //lose quality in favor of speed,
#else
#define WMA_OPT_FOR_SPEED               0  // disable it to get high quality for other platforms
#endif
