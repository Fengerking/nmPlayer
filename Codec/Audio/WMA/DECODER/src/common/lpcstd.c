...
Void prvConvolve_odd(CAudioObjectDecoder* paudec, CAudioObject* pau, 
					 LpType* in1, Int l1, LpType* in2, Int l2, LpType* out, Int *lout);
Void prvConvolve(CAudioObjectDecoder* paudec, CAudioObject* pau, 
				 LpType *in1, Int l1, LpType *in2, Int l2, LpType *out, Int *lout);
Void prvLsp2pq(CAudioObjectDecoder* paudec, CAudioObject* pau, U8 *lsfQ, LpType *p, LpType *q, Int order);
					 ...
					 LpType* in1, Int l1, LpType* in2, Int l2, LpType* out, Int *lout)
{
  ...
  I32   i,j;
//  LpType ret[100];
	LpType *ret = auMalloc(sizeof(LpType)*100);
	ret = voWMAMemset(ret, 0, sizeof(LpType)*100);
...
Void prvLsp2pq(CAudioObjectDecoder* paudec, CAudioObject* pau, U8 *lsfQ, LpType *p, LpType *q, Int order)
{
    ...
    for (i = 0; i < order; i+= 2) {
        ...
        prvConvolve(paudec, pau, short_seq, short_length, long_seq, long_length, 
    ...
    for (i = 3; i < order; i += 2) {
        short_seq[1]= LSF_DECODE(i,lsfQ);   // -2.0F*(Float)cos(2.0*PI*lsp[i])
        prvConvolve_odd(paudec, pau, short_seq, short_length, long_seq, long_length, 
            long_seq, &long_length);
    }
...
Void    auLsp2lpc(CAudioObjectDecoder* paudec, CAudioObject* pau, U8 *lsfQ, LpType *lpc, Int order)
{
  LpType p[LPCORDER+2],q[LPCORDER+2];
  
  prvLsp2pq(paudec, pau, lsfQ,p,q,order);
...
{
 0x78, 0x66, 0x55, 0x55, 0x44, 0x44, 0x44, 0x44, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33,
...
Void prvInitInverseQuadRootTable (CAudioObjectDecoder *paudec, CAudioObject* pau)
{
