...
// #endif
        ...
        {           
            VO_S32 minBlockAddrCurr = p_slice->p_pps->MinTbAddrZs[(VO_S32)((yPb >> 2) * p_slice->p_pps->MinTbAddrZsStride + (xPb>>2))];
            VO_S32 minBlockAddrN = p_slice->p_pps->MinTbAddrZs[(VO_S32)((yNbY >> 2) * p_slice->p_pps->MinTbAddrZsStride + (xNbY>>2))];
            if (/*minBlockAddrN < 0 || */
	...
	VO_S32 iRefIdx )
{
    ...
    PicMvField *col_motion_buffer;
	if(pColPic==NULL)
		return VO_FALSE;
	col_motion_buffer = pColPic->motion_buffer + \
...
// 				VO_S32 max_y = yP + nPSH - 1;
				while(colPic->nProgress_mv < (VO_S32)(p_slice->m_uiCUAddr+avalColBr) && !colPic->errFlag);
					//{printf("fillmvp errflag:%x\n",colPic->errFlag);}
			}
#endif
...
{ 
	...
	iCurrRefPOC         = p_slice->ref_pic_list[(VO_U32)eRefPicList][iRefIdx]->m_iPOC;
	bIsCurrRefLongTerm = p_slice->ref_pic_list[(VO_U32)eRefPicList][iRefIdx]->m_bIsLongTerm;
