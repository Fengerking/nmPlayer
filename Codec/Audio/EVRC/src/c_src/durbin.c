...
)                           
{
	...
	else
	{
		...
		spc[i-1] = negate((pc[i-1] + 0x8000) >> 16);
		for(k=0; k < order-1; k++)
		{
			spc[k] = negate((pc_scratch[k] + 0x8000) >> 16);
		}
    }  /* end else */
	...
	if (*R <= 1)
	{
		...
		if (R[0] != 0)
		{
			EVRC_PRINTF("Ill condition in durbin.\n");
		}
