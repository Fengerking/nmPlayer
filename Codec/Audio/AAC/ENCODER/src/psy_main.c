...
Word16 PsyNew(PSY_KERNEL *hPsy, Word32 nChan, VO_MEM_OPERATOR *pMemOP)
{
  ...
  mdctSpectrum = (Word32 *)mem_malloc(pMemOP, nChan * FRAME_LEN_LONG * sizeof(Word32), 32);
  if(NULL == mdctSpectrum)
	  return 1;

  scratchTNS = (Word32 *)mem_malloc(pMemOP, nChan * FRAME_LEN_LONG * sizeof(Word32), 32);
  if(NULL == scratchTNS)
  {
	  return 1;
  }

  mdctDelayBuffer = (Word16 *)mem_malloc(pMemOP, nChan * BLOCK_SWITCHING_OFFSET * sizeof(Word16), 32);
...
Word16 PsyDelete(PSY_KERNEL  *hPsy, VO_MEM_OPERATOR *pMemOP)
{
  ...
  if(hPsy)
  {
	if(hPsy->psyData[0].mdctDelayBuffer)
		mem_free(pMemOP, hPsy->psyData[0].mdctDelayBuffer);
      
    if(hPsy->psyData[0].mdctSpectrum)
		mem_free(pMemOP, hPsy->psyData[0].mdctSpectrum);
	...
	if(hPsy->pScratchTns)
	{
		mem_free(pMemOP, hPsy->pScratchTns);
...
//      psyData->sfbThresholdnm1[i] = MAX_32;                              
//    }
                                ...
                                const Word16 ch)
{
  ...
  for(w = 0; w < TRANS_FAC; w++) {
    ...
    */
    ...
    /* threshold in quiet */
    data0 = psyData->sfbThreshold.sfbShort[w];
	ntemp = &(hPsyConfShort->sfbThresholdQuiet[0]);
	for (i=hPsyConfShort->sfbCnt; i; i--)
    {
		*data0 = max(*data0, (*ntemp >> normEnergyShift));
	...
	data1 = psyData->sfbEnergy.sfbShort[w];
	for (i=hPsyConfShort->sfbCnt; i; i--) {
      //psyData->sfbSpreadedEnergy.sfbShort[w][i] = psyData->sfbEnergy.sfbShort[w][i];     
	  *data0++ = *data1++;
    }
    SpreadingMax(hPsyConfShort->sfbCnt,
