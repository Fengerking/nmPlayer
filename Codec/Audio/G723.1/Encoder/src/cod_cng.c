...
static void ComputePastAvFilter(CODCNGDEF *CodCng);
...
**
*/
void Init_Cod_Cng(CODCNGDEF *state)
{
    int i;
	state->CurGain = 0;
...
{
    int i;
    Word16 *DataExc = (Word16 *)st->input_buffer;
  ...
  */
        /* Compute LspSid */
        AtoLsp(CodCng->LspSid, CodCng->SidLpc, st->cod_state->PrevLsp);
        Line->LspId = Lsp_Qnt(CodCng->LspSid, st->cod_state->PrevLsp);
        Lsp_Inq(CodCng->LspSid, st->cod_state->PrevLsp, Line->LspId, 0);
  ...
  */
    ...
    Calc_Exc_Rand(CodCng->CurGain, st->cod_state->PrevExc, DataExc,
                                                &CodCng->RandSeed, Line, st);

 /*
  * Interpolate LSPs and update PrevLsp
  */
    Lsp_Int(QntLpc, CodCng->LspSid, st->cod_state->PrevLsp);
    for (i=0; i < LpcOrder ; i++) {
        st->cod_state->PrevLsp[i] = CodCng->LspSid[i];
    }
...
**
*/
void Update_Acf(Word16 *Acf_sf, Word16 *ShAcf_sf, CODCNGDEF *CodCng)
{
...
**
*/
void ComputePastAvFilter(CODCNGDEF *CodCng)
{
    int i, j;
	Word16 *Coeff = CodCng->SidLpc;
    Word16 *ptr_Acf;
