...
VO_U32 oggreader::Load(VO_U32 nSourceOpenFlags, VO_FILE_SOURCE* pFileSource)
{
	...
	const track_info * ptr_vorbis_track = m_header_parser.get_track_info(MD_VORBIS);

	if(ptr_vorbis_track){
		m_ptr_audio_track = new ogg_vorbis_track( VOTT_AUDIO, 1, 0, this, m_pMemOp );		
		m_ptr_audio_track->set_trackinfo( ptr_vorbis_track );
		VO_U32 duration = ((ogg_vorbis_track*)m_ptr_audio_track)->CalcTheDuration();
		m_ptr_audio_track->SetDuration(duration);
	}

	const track_info * ptr_theora_track = m_header_parser.get_track_info(MD_THEORA);
...
//		m_ptr_audio_track->set_trackinfo( ptr_vorbis_track );
	...
	if( !ptr_vorbis_track && !ptr_theora_track)
	{
...
/*
	...
	VO_U64 duration = m_header_parser.get_length();
...
VO_U32 oggreader::MoveTo(VO_S64 llTimeStamp)
{
	...
	m_data_parser.seek_to( file_pos,  llTimeStamp, ((ogg_vorbis_track *)m_ptr_audio_track)->CalcTimeCodecByGranule, m_ptr_audio_track);
...
VO_U32 oggreader::GetFileRealSize(VO_U64 * pSize)
{
	*pSize = m_ullFileSize;
	return VO_ERR_SOURCE_OK;
}
