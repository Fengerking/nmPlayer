...
#endif
      default:
         AMRWBPLUS_FPRINTF(stderr, "error in sampling frequency.  choice of filter are: \n");
#ifdef FILTER_44kHz
         AMRWBPLUS_FPRINTF(stderr, "     11, 22, 44 kHz \n");
#endif
#ifdef FILTER_48kHz
         AMRWBPLUS_FPRINTF(stderr, "     8, 16, 24, 32, 48 kHz \n");
#endif
         exit(EXIT_FAILURE);
         break;
      }
     ...
     switch (samplingRate)
     {
      ...
      case 24000:
         ...
         *invsamplingRate = 5592;   move16(); /*Q15+12*/
         break;
      default:
         AMRWBPLUS_FPRINTF(stderr, "error in sampling freq: without fsratio(isf) only 8, 16 or 24 kHz are allowed\n");
         exit(EXIT_FAILURE);
         break;
      }
   }
...
FILE *fp_chen;
Word8 chen_output_filename[100];
Word32 chen_pos;
#endif
// end
...
// @shanrong added
#ifdef CHEN_TIME_CAL
time_begin  = time(NULL);	//get second
clock_begin = clock();	//get millisecond

strcpy(chen_output_filename, output_filename);
chen_pos = strlen(chen_output_filename);
chen_output_filename[chen_pos-4] = '\0';
strcat(chen_output_filename, "_enctime.dat");
fp_chen = fopen(chen_output_filename, "w");
#endif
// end
  ...
  *--------------------------------------------------------------------------*/
  ...
  if (config_filename != 0 ) 
  {
    ...
    {
      AMRWBPLUS_FPRINTF(stderr, "Must have 16kHz (to switch amr-wb -> wb+ (mode 10@13))\n"
        "or 48kHz (wb+) input\n\n");
      exit(EXIT_FAILURE);
    }
    ...
    {
      AMRWBPLUS_FPRINTF(stderr, "** Warning **\nDo not switch to stereo... \nyou have mono input\n\n");
    }
...
// @shanrong added
#ifdef CHEN_TIME_CAL
time_end  = time(NULL);	//get second
clock_end = clock();	//get millisecond
AMRWBPLUS_FPRINTF_NEED(fp_chen, "\n***decoding time is %ds,\t clock is %dms***\n", (time_end - time_begin), (clock_end - clock_begin));
fclose(fp_chen);
#endif
// end
