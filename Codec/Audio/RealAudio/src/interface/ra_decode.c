/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: ra_decode.c,v 1.1.1.1.2.2 2005/05/04 18:21:41 hubbe Exp $
 * 
				  ...
				  VO_MEM_OPERATOR	  *pMemOP)
{
    ...
    if (fpMalloc && fpFree)
    {
        pRet = (ra_decode*) fpMalloc(pMemOP, pUserMem, sizeof(ra_decode));
...
void
ra_decode_destroy(ra_decode* pFrontEnd, VO_MEM_OPERATOR	  *pMemOP)
{
    rm_free_func_ptr fpFree;
    void* pUserMem;
    ...
    if (pFrontEnd && pFrontEnd->fpFree)
    {
        ...
        fpFree(pMemOP, pUserMem, pFrontEnd);
    }
}
			   ...
			   VO_LIB_OPERATOR	  *pLibOper)
{
    ...
    if (retVal == HXR_OK && pFrontEnd && pFrontEnd->fpInit && pStreamInfo)
    {
        retVal = pFrontEnd->fpInit(pInitParams, ulInitParamsSize, pStreamInfo,
                                   ...
                                   pFrontEnd->fpMalloc, pFrontEnd->fpFree, pMemOP, pLibOper);
    }
                 ...
                 UINT32      ulFlags)
{
    ...
    if (pFrontEnd && pFrontEnd->fpDecode)
    {
        retVal = pFrontEnd->fpDecode(pFrontEnd->pDecode, pData, ulNumBytes,
                                     ...
                                     ulNumSamplesAvail, pNumSamplesOut, ulFlags);
    }
