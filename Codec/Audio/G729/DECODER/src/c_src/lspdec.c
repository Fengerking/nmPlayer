...
)
{
  ...
  if( erase==0 ) {  /* Not frame erasure */
    mode_index = (prm[0] >> NC0_B) & (Word16)1;     // get the L0 bit  (1 bit)
    code0 = prm[0] & (Word16)(NC0 - 1);              // get the L1 bits (7 bits)
    code1 = (prm[1] >> NC1_B) & (Word16)(NC1 - 1);  // get the L2 bits (5 bits)
    code2 = prm[1] & (Word16)(NC1 - 1);              // get the L3 bits (5 bits)
