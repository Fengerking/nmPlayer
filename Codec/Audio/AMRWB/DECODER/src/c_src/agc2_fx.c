		  ...
		  )
{
	...
	{
		temp = sig_out[i] >> 2;
		s = L_add(s, (temp * temp)<<1);   //can't be modified
	}
	if (s == 0)
	{
		return;
	...
	for (i = 0; i < l_trm; i++)
	{
		sig_out[i] = extract_h(L_shl2(((sig_out[i] * g0) <<  1), 2));
	}
	return;
#else
	Word16 i, exp;
