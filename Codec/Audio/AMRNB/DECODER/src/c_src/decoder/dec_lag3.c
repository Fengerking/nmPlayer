...
void Dec_lag3(Word16 index,     /* i : received pitch index                 */
			  ...
			  Word16 T0_prev,   /* i : integer pitch delay of last subframe used in 2nd and 4th subframes */
		...
		if (flag4 == 0) {
			...
			*T0_frac = (index -2) - i;
			...
			else
			{
				if(index < 12)
				{
					i = (mult((index - 5), 10923) - 1);
					*T0 = add(i, tmp_lag);
					i = add(add(i, i), i);
					*T0_frac = sub((index - 9), i);
				}
				else
				{
					i = ( (index- 12) + tmp_lag);
