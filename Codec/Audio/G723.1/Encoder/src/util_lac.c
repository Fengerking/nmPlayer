...
**
*/
void  Rem_Dc(G723EncState *gate)
{
	int    i;
	Word32   Acc0,Acc1 ;
	Word16   *Dpnt = (Word16 *)gate->input_buffer;
	CODSTATDEF *CodStat = (CODSTATDEF*)gate->cod_state;
	//set UseHp = 1
	//if ( UseHp ) {
	if(1){
	for ( i = 0 ; i < Frame ; i ++ ) {
...
{
	G723EncState *gate = (G723EncState *)st;
	Word16 *PrevDat = (Word16 *)gate->cod_state->PrevDat;
	Word16 *DataBuff = (Word16 *)gate->input_buffer;
	int   i;
...
void Line_Pack( LINEDEF *Line, G723EncState *st, Word16 Ftyp )
{
	int     i ;
	int     BitCount ;
    char    *Vout = (char *)st->output_buffer;
