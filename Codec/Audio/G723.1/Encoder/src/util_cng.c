...
void Calc_Exc_Rand(Word16 curGain, Word16 *PrevExc, Word16 *DataExc,
				   Word16 *nRandom, LINEDEF *Line, G723EncState *st)
{
	int i, i_subfr, iblk;
	Word16 temp, temp2;
	Word16 j;
	...
	*/
	...
	for(iblk=0; iblk<SubFrames/2; iblk++) {
		...
		/* decode LTP only */
		Decod_Acbk(curExc, &PrevExc[0], Line->Olp[iblk], Line->Sfs[i_subfr].AcLg, Line->Sfs[i_subfr].AcGn, st);
		Decod_Acbk(&curExc[SubFrLen], &PrevExc[SubFrLen], Line->Olp[iblk],
			Line->Sfs[i_subfr+1].AcLg, Line->Sfs[i_subfr+1].AcGn, st);
