...
VO_TCHAR * g_szChannelName[] = 
		{_T("CCTV-1"), _T("CCTV-2"), _T("CCTV-3"),
		 _T("CCTV-4"), _T("CCTV-5"), _T("CCTV-6"), 
		 _T("CCTV-9"), _T("CCTV-10"), _T("BTV"),
		 _T("ZTV"),  _T("STV"),  _T("BTV-1"),};
...
CEngineCMMBFile::CEngineCMMBFile()
	: m_nPacketSize (0)
	, m_nBitrate (0)
	, m_ppFilesName (NULL)
	, m_hPushThread (NULL)
...
CEngineCMMBFile::~CEngineCMMBFile(void)
{
	Stop();
...
int CEngineCMMBFile::Scan (VO_LIVESRC_SCAN_PARAMEPTERS * pParam, bool nQuickScan)
{
...
// 	m_nBitrate = pScanInfo->nBitrate;
	...
	vostrcpy (m_pChannelInfo[nIndex].szName, g_szChannelName[nIndex % 12]);
	...
	if (hFind == INVALID_HANDLE_VALUE)
		return VO_ERR_LIVESRC_LOADDRIVERFAILED;
	m_nChannelCount++;
	while(true) 
	{
	...
	while(true) 
	{
		...
		{
			voOS_Sleep (200);
			m_fStatus (m_pUserData, VO_LIVESRC_STATUS_SCAN_PROCESS, nIndex * 100 / m_nChannelCount, 0);
		}
...
int CEngineCMMBFile::Start (void)
{
...
int CEngineCMMBFile::SetChannel (int nNewChannelID)
{
...
/*
int CEngineCMMBFile::GetESG (int nChannelID, int * pCount, VO_LIVESRC_ESGINFO ** ppESGInfo)
{
	*pCount = 0;
	*ppESGInfo = NULL;
...
VO_U32 CEngineCMMBFile::PushDataEntry(VO_PTR pParam)
{
	CEngineCMMBFile * pFileSource = (CEngineCMMBFile*) pParam;
	VO_U32 nRC = pFileSource->PushData();
	return nRC;
}

VO_U32 CEngineCMMBFile::PushData()
{
	VO_U32	dwRead = 0;
...
VO_PBYTE CEngineCMMBFile::ParseCMMBBuffer (VO_PBYTE pBuffer, int & nBufSize)
{
	VO_PBYTE pHead = NULL;
...
void CEngineCMMBFile::BitrateControl(void)
{
// 	if (m_nFileType == 10) // CMMB
// 		return;
...
void CEngineCMMBFile::ReleaseData (void)
{
	if (m_nChannelCount == 0)
		return;
...
void CEngineCMMBFile::ReadData (void)
{
	VO_FILE_SOURCE	fileSource;
	memset(&fileSource, 0, sizeof(VO_FILE_SOURCE));
	fileSource.nFlag = VO_FILE_TYPE_NAME;
...
void CEngineCMMBFile::WriteData (void)
{
	VO_FILE_SOURCE	fileSource;
	memset(&fileSource, 0, sizeof(VO_FILE_SOURCE));
	fileSource.nFlag = VO_FILE_TYPE_NAME;
