...
VO_U32 CPlayGraph::Run (void)
{
	...
	JNILOGE2("RUN threadnum v2 : %d", m_nThreadNum);

	if (m_nThreadNum >= 1)
	{
		JNILOGE("audio");
		JNILOGE2("RUN m_pAudioRender : 0x%08x m_hThreadAudio : 0x%08x", m_pAudioRender, m_hThreadAudio);
		if (m_pAudioRender != NULL && m_hThreadAudio == NULL)
		{
			m_bStopAudio = VO_FALSE;

			JNILOGE("RUN PlaybackAudioLoopProc");
	...
	if (m_nThreadNum >= 2)
	{
		if (m_pVideoRender != NULL && m_hThreadVideo == NULL)
		{
			m_bStopVideo = VO_FALSE;
				JNILOGE("RUN PlaybackVideoLoopProc");
...
*/
	...
	VOLOGI("Frame Type : %d", m_pVideoDec->GetFrameType (&m_videoSourceBuffer));
...
VO_U32 CPlayGraph::RenderVideoData (VO_VIDEO_BUFFER * pVideoBuffer, VO_S64 nStart, VO_BOOL bWait)
{
	...
	if (m_pVideoRender != NULL)
	{
		static int nTestVideoAudio = 0;
		nTestVideoAudio++;

		VOLOGE("RenderVideo %d ", nTestVideoAudio);
