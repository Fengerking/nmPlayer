...
const int postSkip1[NUM_MLT_SIZES] = {7, 3, 1};
...
#else
		PreMultiplyRescale(tabidx, mlt, es, &nmltTab[0], &cos4sin4tab[0], &cos4sin4tabOffset[0]);		
		R4FFT(tabidx, mlt);		
		PostMultiplyRescale(tabidx, mlt, es, &nmltTab[0], &cos1sin1tab[0], &postSkip1[0]);
#endif
	} else 
	{
#ifndef ARM
...
#else
		PreMultiply(tabidx, mlt, &nmltTab[0], &cos4sin4tab[0], &cos4sin4tabOffset[0]);		
		R4FFT(tabidx,mlt);		
		PostMultiply(tabidx, mlt, &nmltTab[0], &cos1sin1tab[0], &postSkip1[0]);
#endif
	}
