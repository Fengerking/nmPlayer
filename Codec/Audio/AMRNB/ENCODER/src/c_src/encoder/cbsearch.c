...
*****************************************************************************
*/
int voAMRNBEnc_cbsearch(Word16 x[],    /* i : target vector, Q0                       */
	...
	*/
	...
	{  /* MR122 */
		...
		*--------------------------------------------------------------*/
		...
		voAMRNBEnc_code_10i40_35bits (x, res2, h, code, y, *anap);
		*anap += 10;      	                                   
		...
		*-------------------------------------------------------*/ 
		for (i = T0; i < L_SUBFR; i++)
		{
			temp = (code[i - T0] * pit_sharpTmp)>>15;
	...
	{   /* MR59 */
		*(*anap)++ = voAMRNBEnc_code_2i40_11bits(x, h, T0, pitch_sharp, code, y, &index);
		*(*anap)++ = index;                                              /* sign index */
	}

	else if (mode == MR67)
	{   /* MR67 */
		*(*anap)++ = voAMRNBEnc_code_3i40_14bits(x, h, T0, pitch_sharp, code, y, &index);
		*(*anap)++ = index;                                              /* sign index */ 
	}

	else if ((mode == MR74)|| (mode == MR795))
	{   /* MR74, MR795 */
		*(*anap)++ = voAMRNBEnc_code_4i40_17bits(x, h, T0, pitch_sharp, code, y, &index);
		*(*anap)++ = index;                                             /* sign index */
	}
	else if (mode == MR102)
	{   /* MR102 */
		...
		* - Innovative codebook search (find index and gain)           *
		*--------------------------------------------------------------*/
		voAMRNBEnc_code_8i40_31bits (x, res2, h, code, y, *anap);
		*anap += 7;  
		/*-------------------------------------------------------*
		* - Add the pitch contribution to code[].               *
		*-------------------------------------------------------*/ 
	...
	else if ( mode<= MR515)    /* MR475, MR515 */
	{  
		*(*anap)++ = voAMRNBEnc_code_2i40_9bits(subNr, x, h, T0, pitch_sharp, code, y, &index);
