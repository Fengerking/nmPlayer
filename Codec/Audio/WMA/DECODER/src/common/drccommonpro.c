...
{
  p = (DRCOptions *)memset(p, 0, sizeof(DRCOptions)); // just to be safe
...
{
  p = (DRCData *)memset(p, 0, sizeof(DRCData)); // just to be safe
...
{
	Void *pTemp = NULL;
  p = (DRC *)memset(p, 0, sizeof(DRC)); // just to be safe
  pTemp = memset(&p->m_opt, 0, sizeof(DRCOptions));
...
void drcFree(CAudioObjectDecoder *paudec, DRC *p)
{
  Int i;
...
WMARESULT drcCreateScaleTables(CAudioObjectDecoder *paudec, DRC *p, Int nTable)
{
  WMARESULT wr = WMA_OK;
  Int i;
...
WMARESULT drcInit(CAudioObjectDecoder *paudec, DRC *p, WAVEFORMATEXTENSIBLE *pwfx,
                  DRCData *pd, DRCOptions *po)
{
  ...
  DRCDbType desiredRmsDb[2]={0, 0}, desiredPeakDb[2]={0, 0};
  Bool fNormalizeRms;
	Void *pTemp = NULL;
  ...
  if (po == NULL) drcSetDefaultOptions(&p->m_opt);
    ...
    pTemp = voWMAMemcpy(&p->m_opt, po, sizeof(DRCOptions));
    // overwrite drc curve
...
#undef TIMES_TO_COEFF
  ...
  if (po->m_dataFormatRms == PostProcDrcDataInNormU16) {
    if (po->m_desiredRms.normVal <= 0) {
      ...
      pTemp = voWMAMemcpy(&po->m_desiredRms, &pd->rms, sizeof(PostProcDrcDataFormatVal));
    }
  }
  if (po->m_dataFormatPeak == PostProcDrcDataInNormU16) {
    if (po->m_desiredPeak.normVal <= 0) {
      po->m_dataFormatPeak = pd->dataFormatPeak;
      pTemp = voWMAMemcpy(&po->m_desiredPeak, &pd->peak, sizeof(PostProcDrcDataFormatVal));
    }
  }
  ...
  if (p->m_nScaleTables > 0) {
    ...
    p->m_scaleFactor = (DRCType*)memset(p->m_scaleFactor, 0, p->m_nScaleTables * sizeof(DRCType));

    p->m_fNeedDRCOnNormalize = (Bool*)auMalloc(p->m_nScaleTables*sizeof(Bool));
    CHECK_ALLOC(p->m_fNeedDRCOnNormalize);
    p->m_fNeedDRCOnNormalize = (Bool*)memset(p->m_fNeedDRCOnNormalize, 0, p->m_nScaleTables * sizeof(Bool));

    p->m_intSlopeShiftBits = (I32*)auMalloc(p->m_nScaleTables*sizeof(I32));
    CHECK_ALLOC(p->m_intSlopeShiftBits);
    p->m_intSlopeShiftBits = (I32*)memset(p->m_intSlopeShiftBits, 0, p->m_nScaleTables * sizeof(I32));

    p->m_maxPeakValDec = (DRCType*)auMalloc(p->m_nScaleTables*sizeof(DRCType));
    CHECK_ALLOC(p->m_maxPeakValDec);
    p->m_maxPeakValDec = (DRCType*)memset(p->m_maxPeakValDec, 0, p->m_nScaleTables * sizeof(DRCType));
  }
  ...
  if (p->m_opt.m_fUseTables) {
    if (!p->m_opt.m_fDRCDecode ||
      ...
      p->m_scaleTable = (DRCType**)memset(p->m_scaleTable, 0, p->m_nScaleTables*sizeof(DRCType*));
      p->m_scaleTableBase =
        (DRCType**)auMalloc(p->m_nScaleTables*sizeof(DRCType*));
      CHECK_ALLOC(p->m_scaleTableBase);
      p->m_scaleTableBase = (DRCType**)memset(p->m_scaleTableBase, 0, p->m_nScaleTables*sizeof(DRCType*));
      p->m_scaleSlopeTable =
        (DRCType**)auMalloc(p->m_nScaleTables*sizeof(DRCType*));
      CHECK_ALLOC(p->m_scaleSlopeTable);
      p->m_scaleSlopeTable = (DRCType**)memset(p->m_scaleSlopeTable, 0, p->m_nScaleTables*sizeof(DRCType*));
      p->m_scaleSlopeTableBase =
        (DRCType**)auMalloc(p->m_nScaleTables*sizeof(DRCType*));
      CHECK_ALLOC(p->m_scaleSlopeTableBase);
      p->m_scaleSlopeTableBase = (DRCType**)memset(p->m_scaleSlopeTableBase, 0, p->m_nScaleTables*sizeof(DRCType*));
    }
  }
  ...
  for (i=0; i < p->m_nScaleTables; i++) {
    ...
    if (!p->m_opt.m_fDRCDecode ||
      ...
      wr = drcCreateScaleTables(paudec, p, i);
      if (WMA_FAILED(wr)) goto exit;
    }
  }
