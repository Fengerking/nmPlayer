...
#endif
SourceInfoXMLDumper::SourceInfoXMLDumper( const VO_CHAR * dump_xml , const VO_CHAR * filename , const VO_CHAR *xmlName, ISource2Interface * ptr_obj )
...
void SourceInfoXMLDumper::DumpProgram( _PROGRAM_INFO * ptr_info )
{
	m_ptr_writer->write_document_start( "SourceInfoDump" );
...
void SourceInfoXMLDumper::DumpStream( _STREAM_INFO * ptr_info )
{
...
void SourceInfoXMLDumper::DumpTrack( _TRACK_INFO * ptr_info )
{
...
// #else
	m_ptr_writer->write_element_attribute( "ASTrackID" , ptr_info->uASTrackID );
	m_ptr_writer->write_element_attribute( "FPTrackID" , ptr_info->uFPTrackID );
	m_ptr_writer->write_element_attribute( "OutSideTrackID" , ptr_info->uOutSideTrackID );
...
//#endif
	...
	if( ptr_info->uTrackType == VO_SOURCE2_TT_AUDIO || ptr_info->uTrackType == VO_SOURCE2_TT_AUDIOGROUP)
	{
	...
	else if( ptr_info->uTrackType == VO_SOURCE2_TT_VIDEO || ptr_info->uTrackType == VO_SOURCE2_TT_VIDEOGROUP )
	{
	...
	else if( ptr_info->uTrackType == VO_SOURCE2_TT_SUBTITLE || ptr_info->uTrackType == VO_SOURCE2_TT_SUBTITLEGROUP)
	{
