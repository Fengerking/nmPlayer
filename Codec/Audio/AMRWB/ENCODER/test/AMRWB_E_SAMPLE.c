...
int encode(AMRWBEncoderParams *AMREnc)
{
...
/*#ifdef _WIN32_WCE
	...
	mode = AMREnc->mode;
	srcfile = AMREnc->input_file;
	dstfile = AMREnc->output_file;
...
#ifdef LINUX
	handle = dlopen("/data/local/tmp/libvoAMRWBEnc.so", RTLD_NOW);
	if(handle == 0)
	{
...
#elif _IOS
int beginTest_amrwbenc(char *inFile, char *outFile, int time)  
#else	// _WIN32_WCE
...
#endif//_WIN32_WCE
{
	...
	AMRWBEncoderParams AMREncParams, AMREncParams_Reentant;

	AMREncParams.mode = VOAMRWB_MD1265;
	AMREncParams.input_file = argv[1];
	AMREncParams.output_file = argv[2];
...
#ifdef LINUX
    AMREncParams_Reentant.mode = VOAMRWB_MD1265;
	AMREncParams_Reentant.input_file = argv[3];
	AMREncParams_Reentant.output_file = argv[4];
	//re-entrancy test: Greate independant threads each of which will execute the decoder function

	iret1 = pthread_create( &thread1, NULL, (void*) encode, (void*) &AMREncParams);
	iret2 = pthread_create( &thread2, NULL, (void*) encode, (void*) &AMREncParams_Reentant);
...
#elif _IOS
	AMREncParams.mode = VOAMRWB_MD2385;
	AMREncParams.input_file = inFile;
	AMREncParams.output_file = outFile;
#else
	if (argc != 3)
	{
