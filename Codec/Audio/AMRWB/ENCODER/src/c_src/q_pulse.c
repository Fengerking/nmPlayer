		...
		Word16 N)                             /* (i) number of bits for position */
{
	...
	{
		index = quant_2p_2N1(pos1, pos2, (N - 1));    /* index = quant_2p_2N1(pos1, pos2, (N-1)); */
		/* index += (pos1 & nb_pos) << N; */
	...
	{
		index = quant_2p_2N1(pos1, pos3, N - 1);    /* index = quant_2p_2N1(pos1, pos3, (N-1)); */
		index = vo_L_add(index, (L_deposit_l((Word16) (pos1 & nb_pos)) << N)); 
		/* index += (pos1 & nb_pos) << N; */
		...
		Word16 N)                             /* (i) number of bits for position */
{
	...
	switch (i)
	{
		...
		case 1:
			...
			index = vo_L_shl(quant_1p_N1(posA[0], n_1), tmp);
		...
		case 2:
			...
			index = vo_L_shl(quant_2p_2N1(posA[0], posA[1], n_1), tmp);
		...
		case 3:
			...
			index = vo_L_shl(quant_3p_3N1(posA[0], posA[1], posA[2], n_1), N);
	...
	index = vo_L_add(index, vo_L_shl((L_deposit_l(i) & (3L)), tmp));
		...
		Word16 N)                             /* (i) number of bits for position */
{
	...
	switch (i)
	{
		case 0:
			...
			index = vo_L_shl(1L, tmp);   /* index = 1 << ((5*N)-1); */
			tmp = add1((N << 1), 1);  /* index += quant_3p_3N1(posB[0], posB[1], posB[2], n_1) << ((2*N)+1);*/
			tmp2 = vo_L_shl(quant_3p_3N1(posB[0], posB[1], posB[2], n_1), tmp);
		...
		case 3:
			...
			index = vo_L_shl(quant_3p_3N1(posA[0], posA[1], posA[2], n_1), tmp);
