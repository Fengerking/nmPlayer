...
WMARESULT ibstrmDumpFrmBitsPreGetMoreData(CWMAInputBitStream *pibs, I32 iWinFrameNumber, Int iVersion)
{
    ...
    {
        voWMAMemcpy(pibs->m_rgbInputTailPrev, pibs->m_pbInputBuffer + pibs->m_cbInputBuffer - 8, 8);
    }
    else
    {
...
FORCEINLINE static WMARESULT _ibstrmAttach(CWMAInputBitStream *pibs, U8* pbSrc, I32 cbSrc, 
...
//                iNextPacketNum - iPrevPacketNum == 0 ||
                ...
                || bForcedPacketLoss)
            {
				...
				pibs->m_dwOwner->m_fPacketLoss = WMAB_TRUE;
            ...
            {
                pibs->m_dwOwner->m_fPacketLoss = WMAB_FALSE;
                       ...
                       void* pfnDescramble)
{
    ...
    if ( 2 == p->uVer )
    {
        tmp.pfn = (void*)((unsigned long long)(tmp.pfn) ^ AUDIO_MAGIC_NUM);
    }
...
WMARESULT ibstrmFlushOldBits(CWMAInputBitStream *pibs)
{
	WMARESULT wmaResult = WMA_OK;
    ...
    if (cOldBits > 0)
    {
        wmaResult = ibstrmFlushBits(pibs, cOldBits);
    }
	return wmaResult;
}
