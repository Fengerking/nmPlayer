...
/*
			...
			*/
			...
			if(ptr_buffer->Time < 0)
				ptr_buffer->Time = 0;
			VOLOGR("start_time dg audio  %lld" , ptr_buffer->Time);
...
VO_VOID vo_mpd_streaming::load_DASH_parser(){
    ...
    #elif defined _IOS
	{
		voGetParserAPI (&m_dashhparser_api);
...
VO_VOID vo_mpd_streaming::FulshAudioSample(VO_S64 time)
{
	#ifdef _DASH_SOURCE_
	//m_bNeedFlushBuffer = VO_TRUE;
//	VOLOGR("before Flush BufferFromList size: %d",m_List_buffer.GetCount());
...
VO_U32 vo_mpd_streaming::SetParam(VO_U32 nParamID, VO_PTR pParam)
{
	...
	//	VOLOGRnit (pVologCB->pUserData, pVologCB->fCallBack);
	}
...
VO_VOID vo_mpd_streaming::start_dashstream(){
	...
	if( !is_live())
	{
		...
		{	
			...
			//is_live()  ? pbuffer.nStartTime  = m_manager.get_audiostarttime()  : pbuffer.nStartTime  = m_manager.set_pos(seek_time);
	...
	else 
	{
		...
		int update_number = 14;
		if(is_update())
		{
		//	ini_chunk = curSegmentCounts/2; //45;
			if(m_manager.m_reader.unknown_flag==0)
			    ini_chunk = (curSegmentCounts*2)*3/4; //45;
			else
			  ini_chunk = 0;
		}
		int count_video = 0;
		...
		start_time_tamp = m_manager.m_reader.m_ptr_FI_head->starttime;
		while( !m_bMediaStop )
		{
			if( !ptr_item ||fragment_number > update_number|| live_after_pause)
		//if( !ptr_item ||fragment_number > curSegmentCounts|| live_after_pause)
			{   
				flag = VO_TRUE;
				...
				int gene_chunk = m_manager.GetSegmentCounts()*2;
				if(left_chunk_before == left_chunk)
				{
					...
					if(left_chunk_before<gene_chunk)
					{
						...
						fragment_number = update_number-left_number;
					}
					else if(left_chunk_before >= gene_chunk )
					{
						fragment_number = update_number+1;
						continue;
					}
...
//	sample.Sample.Duration =(ptr_buffer->nEndTime-ptr_buffer->nStartTime)/m_manager.GetTimeScaleMs();;
	...
	while( m_is_pause )
	{
		...
		if( !m_recoverfrompause && is_live()&&is_update())
		{
      ...
      /* VO_S64 count ;
	   VO_S64 test_time_video;
	   VO_S64 test_time_audio;
        if(sample.nTrackID == VO_LIVESRC_OUTPUT_VIDEO)
		{
			m_databuffer_video.AddBuffer(&sample);
		    count= m_databuffer_video.GetBuffCount();
			VO_LIVESRC_SAMPLE sample_test_video;
			memset( &sample_test_video , 0 , sizeof( VO_LIVESRC_SAMPLE ) );
			m_databuffer_video.GetBuffer(&sample_test_video);
			 test_time_video=  m_databuffer_video.GetBuffTime();
		}
		else if(sample.nTrackID == VO_LIVESRC_OUTPUT_AUDIO){
	          m_databuffer_audio.AddBuffer(&sample);
			  VO_LIVESRC_SAMPLE sample_test_audio;
			  memset( &sample_test_audio , 0 , sizeof( VO_LIVESRC_SAMPLE ) );
			  m_databuffer_audio.GetBuffer(&sample_test_audio);
			   test_time_audio =  m_databuffer_audio.GetBuffTime();
	...
	//	VOLOGR( "===AddData:Flag(%x), CodecType (%d),TrackID(%d),TimeStamp(%lld),BufferSize(%d)",sample.Sample.Flag, sample.nCodecType,sample.nTrackID,sample.Sample.Time,sample.Sample.Size );
