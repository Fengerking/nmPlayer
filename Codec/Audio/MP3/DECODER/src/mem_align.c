...
void *voMP3DecMem_malloc(VO_MEM_OPERATOR *pMemop, unsigned int size, unsigned char alignment)
{
	int ret;
	...
	if (!alignment) {
		...
		MemInfo.Flag = 0;
		MemInfo.Size = size + 1;
		ret = pMemop->Alloc(VO_INDEX_DEC_MP3, &MemInfo);
		if(ret != 0)
			return 0;
		mem_ptr = (unsigned char *)MemInfo.VBuffer;

		pMemop->Set(VO_INDEX_DEC_MP3, mem_ptr, 0, size + 1);
		...
		*mem_ptr = (unsigned char)1;
		...
		pMemop->Set(VO_INDEX_DEC_MP3, tmp, 0, size + alignment);
...
void voMP3DecMem_free(VO_MEM_OPERATOR *pMemop, void *mem_ptr)
{
	...
	/* Free the memory */
	pMemop->Free(VO_INDEX_DEC_MP3, ptr);
}
