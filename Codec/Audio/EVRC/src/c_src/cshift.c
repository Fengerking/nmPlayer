				...
				short extra)
{
	...
	bg = guard - (GUARD << 1) - FrameSize + EWL;
#endif
 ...
 */
	...
	length = (short)(((delay << 2) - delay) >> 17);

	offset = dpm + iacshift;
	offset = offset - (short)(delay >> 18);
#if ANSI_EVRC_BOTTOM_GUARD
	if (offset < bg)
    {
		offset = bg;
	...
	g = guard - offset - 2;		/* Maximum allowed search size */
	...
	if (loc < dpm)
	{
		offset = (short)((L_mpy_ls(delay, 24576) + 0x8000) >> 16);
		offset += iacshift + loc;
#if ANSI_EVRC_BOTTOM_GUARD
		if (offset < bg)
        {
	...
	{
		offset = (short)((delay + (delay >> 2) + 0x8000) >> 16);
		offset = iacshift - offset + loc;
#if ANSI_EVRC_BOTTOM_GUARD
		if (offset < bg)
        {
			offset = bg;
	...
	*sfend = loc + extra;
	*maxshift2 = maxshift;
	if (loc < (subframel - (extra >> 1)) && *sfend > subframel)
		*sfend = subframel;
	if (loc < (subframel + (extra >> 1)) && *sfend > (subframel + extra))
		*sfend = add(subframel, extra);
	if (loc >= (subframel + (extra >> 1)))
		*sfend = subframel;
