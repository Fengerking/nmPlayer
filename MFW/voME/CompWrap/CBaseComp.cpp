...
OMX_ERRORTYPE CBaseComp::Start (void)
{
	...
	OMX_STATETYPE	state;
	errType = m_pComponent->GetState (m_pComponent, &state);
...
OMX_ERRORTYPE CBaseComp::ResetPort (int nIndex)
{
...
int CBaseComp::onNotifyEvent (int nMsg, int nV1, int nV2)
{
	...
	if (nMsg == VOCOMP_EVENT_RESETPORT)
	{
		ResetPort (nV1);
	}
...
OMX_ERRORTYPE CBaseComp::SetVideoSize (VO_U32 nWidth, VO_U32 nHeight)
{
	...
	VOLOGI("MAAA SetVideoSize = %d %d " , (int)m_nVideoWidth , (int)m_nVideoHeight);
...
OMX_ERRORTYPE CBaseComp::CreateComponent(void)
{
	OMX_ERRORTYPE		errType = OMX_ErrorNone;
	...
	if (m_pInputPort != NULL)
	{
		...
		else if (m_nCodecType == VO_COMPWRAP_CODEC_AudioDec)
		{
			...
			if (errType != OMX_ErrorNone)
			{
				VOLOGE ("It could not set intpu audio info! Return %08X", errType);
				return errType;
			}
		}
		...
		//	OMX_OTHER_EXTRADATATYPE * pExtData = (OMX_OTHER_EXTRADATATYPE *) new unsigned char[sizeof (OMX_OTHER_EXTRADATATYPE) + nHeadSize];
		//	SetHeader (pExtData, sizeof (OMX_OTHER_EXTRADATATYPE) + nHeadSize);
		//	memcpy (pExtData->data, m_pHeadData, m_nHeadSize);
		//	pExtData->nPortIndex = 0;
		//	pExtData->nDataSize = nHeadSize;

		//	errType = SetParameter (OMX_IndexParamCommonExtraQuantData, pExtData);
		//	delete [](unsigned char *)pExtData;
		//	//if (errType != OMX_ErrorNone)
		//	//	return errType;
		//}
	}
...
OMX_ERRORTYPE CBaseComp::GetParameter (OMX_INDEXTYPE nIndex, OMX_PTR pParam)
{
	if (m_pComponent == NULL)
		return OMX_ErrorInvalidState;
...
OMX_ERRORTYPE CBaseComp::SetParameter (OMX_INDEXTYPE nIndex, OMX_PTR pParam)
{
	if (m_pComponent == NULL)
		return OMX_ErrorInvalidState;
...
OMX_ERRORTYPE CBaseComp::GetConfig (OMX_INDEXTYPE nIndex, OMX_PTR pConfig)
{
	if (m_pComponent == NULL)
		return OMX_ErrorInvalidState;
...
OMX_ERRORTYPE CBaseComp::SetConfig (OMX_INDEXTYPE nIndex, OMX_PTR pConfig)
{
	if (m_pComponent == NULL)
		return OMX_ErrorInvalidState;
...
OMX_ERRORTYPE CBaseComp::EventHandler (OMX_EVENTTYPE eEvent,OMX_U32 nData1, OMX_U32 nData2, OMX_PTR pEventData)
{
	...
	{
		VOLOGI ("OMX_EventPortSettingsChanged, Data1 %d, Data2 %d", (int)nData1, (int)nData2);
		//ResetPort (nData1);
