						  ...
						  Word16 pos_indx[])  /* o : position of 3 pulses (decompressed)   */
{
	...
	ia = mult(MSBs, 1311);
	ia = sub(MSBs, extract_l((L_mult(ia, 25)>> 1)));    
	ib = shl2(sub(ia, extract_l((L_mult(mult(ia, 6554),5)>> 1))), 1);

	ic = shl2((LSBs>> 2), 2);
	ic = sub(LSBs, ic);
	pos_indx[index1] = add(ib, (ic & 1));                        

	ib = shl2(mult(ia, 6554), 1);
	pos_indx[index2] = add(ib, (ic>> 1));

	pos_indx[index3] = add(shl2(mult(MSBs, 1311), 1), (LSBs>> 2));    
...
)
{
	...
	*/
	...
	MSBs0_24 = (add(extract_l((L_mult(MSBs, 25)>> 1)), 12)>> 5);

	ia = mult(MSBs0_24, 6554) & 1;
	ib = sub(MSBs0_24, extract_l((L_mult(mult(MSBs0_24, 6554), 5)>> 1)));

	if(ia == 1)
	{
		ib = 4 - ib;
	}
	pos_indx[3] = ((ib << 1) + (LSBs & 1));                        

	ia = shl2(mult(MSBs0_24, 6554), 1);
