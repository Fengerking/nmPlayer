...
VOMJPEGDECRETURNCODE VO_API Initpicture(Jpeg_DecOBJ * decoder)
{
	UINT16	i, k;
	UINT32	MblockSize = 0;
...
VOMJPEGDECRETURNCODE VO_API	UpdateParameter(Jpeg_DecOBJ * decoder, VO_VIDEO_BUFFER *pOutData, VO_VIDEO_OUTPUTINFO *pOutFormat)
{
	UINT16	i;
	UINT32	yuv_width[3], totalwidth;  
	UINT32	yuv_height[3], totalheight;
	UINT32	yuv_size[3] = {0,0,0}; 
	UINT32	totalsize;
	JPEG_COMP_ITEM	*comptr;
	JPEG_PARA		*jpara;
	IMAGE_FIELD		*field;
	...
	else
	{
		...
		if(decoder->enableNormalChroma==0)
		{
			/************/ //zou add  (change to 420 buffer size)
			yuv_width[0] = (yuv_width[0] + 31) & ~31;
			yuv_height[0] = (yuv_height[0] + 31) & ~31;
			yuv_size[0]   = yuv_width[0] * yuv_height[0];
			 
			for(i = 1; i < jpara->num_component; i++)
			{
				yuv_width[i]  = (yuv_width[0])/2;
				yuv_height[i] = (yuv_height[0])/2;
				yuv_size[i]   = yuv_width[i] * yuv_height[i];	
			}
			if(jpara->num_component == 1)
			{
	...
	if(pOutData)
	{
		if(decoder->enableNormalChroma==1)
		{
...
VO_U32 VO_API voJPEGSUBDecInit(Jpeg_DecOBJ *dec,VO_HANDLE *phCodec)
{
	...
	Jpeg_DecOBJ *decoder;
	JPEGDEC_FRONT* pDecFront = NULL; //zou 315
	...
	{
		pDecFront = (JPEGDEC_FRONT *)mem_malloc(sizeof(JPEGDEC_FRONT), DCACHE_ALIGN_NUM); //315
	...
	pDecFront->pDecCore = (VO_VOID *)decoder; //315
	*phCodec = (VO_HANDLE)pDecFront; //315
...
static VO_U32 VO_API voDecExifProcess(VO_HANDLE hCodec,VO_VIDEO_BUFFER *pOutData, VO_VIDEO_OUTPUTINFO *pOutFormat)
{
	...
	pDecFront = (JPEGDEC_FRONT *)hCodec;  //zou 315
	decoder = (Jpeg_DecOBJ *)pDecFront->pDecCore; //315
...
static VO_U32 VO_API voDecIMGProcess(VO_HANDLE hCodec,VO_CODECBUFFER *pInData,VO_VIDEO_BUFFER *pOutData, VO_VIDEO_OUTPUTINFO *pOutFormat)
{
	...
	pDecFront = (JPEGDEC_FRONT *)hCodec;  //zou 315
	decoder = (Jpeg_DecOBJ *)pDecFront->pDecCore; //315
	//decoder = (Jpeg_DecOBJ *)hCodec; //315
	bitstream = &decoder->instream;	
	...
	{		
		...
		{
			reti = initfstream(decoder, bitstream, (INT8*)pInData->Buffer);
			if(reti == -2)
...
VO_U32 VO_API voJPEGDecInit(VO_HANDLE *phCodec, VO_IMAGE_CODINGTYPE vType, VO_CODEC_INIT_USERDATA * pUserData)
{
	INT32 ret = VO_ERR_NONE;
	Jpeg_DecOBJ *decoder = NULL;
	JPEGDEC_FRONT* pDecFront;
	...
	if(pUserData==NULL)
	{
		ret=voCheckLibInit(&decoder->phCheck, VO_INDEX_DEC_JPEG, 2, g_hJPEGDecInst,NULL);
	}
	else
		if((pUserData->memflag & 0xF0) == 0x10  && pUserData->libOperator != NULL)
			ret=voCheckLibInit(&decoder->phCheck, VO_INDEX_DEC_JPEG, pUserData->memflag|2, g_hJPEGDecInst,pUserData->libOperator);
		else
			ret=voCheckLibInit(&decoder->phCheck, VO_INDEX_DEC_JPEG, pUserData->memflag|2, g_hJPEGDecInst,NULL);
#endif
	return ret;
}
...
// 	VOCODECVIDEOBUFFER *pOutData;
	...
	switch(nID)
	{
    case VO_PID_DEC_JPEG_SET_NORMAL_CHROMA:
        {
		INT32 enableNormalCharoma = *pValue;/*huwei 20090726 normal chroma*/
	...
	case VO_JPEG_BUFFER_LIMITED:
		...
		break;
	case VO_JPEG_FILE_HANDLE:
		//void* infile =  (FILE *)plValue;
		reti = initfstream2(decoder, bitstream, (FILE *)plValue);
	...
	case VO_JPEG_OUTPUT_ZOOMOUT:
		...
		case 2:
			decoder->addstep = 4;
			decoder->idct_block = voIDCT8x8_4x4;
			decoder->idct_block_V6 = voIDCT8x8_4x4_V6;
			break;
		case 4:
			decoder->addstep = 2;
			decoder->idct_block = voIDCT8x8_2x2;
			decoder->idct_block_V6 = voIDCT8x8_2x2_V6;
			break;
		case 8:
			decoder->addstep = 1;
			decoder->idct_block = voIDCT8x8_1x1;
			decoder->idct_block_V6 = voIDCT8x8_1x1_V6;
			break;
		default: /* > 8 */
			decoder->addstep = 1;
			decoder->idct_block = voIDCT8x8_1x1;
...
VO_U32 VO_API voJPEGDecGetParameter(VO_HANDLE hCodec, VO_S32 nID, VO_PTR plValue)
{
	...
	case VO_PID_JPEG_HEIGHT:
		...
		break;
	case VO_JPEG_NUM_COMPONENT:
		*((UINT32*)plValue)=pjpara->num_component;
		break;
	case VO_JPEG_SCANLINE_HEIGHT_Y:
		*((UINT32*)plValue)=pjpara->MCU_heigth;
		break;
	...
	case VO_JPEG_OUTPUT_BUFFER:
		...
		{		
			...
			else
			{
				...
				//if((retc = Initpicture(decoder)) < 0)//518
					//return retc;//
	...
	case VO_PID_MACHINE_MAKE:
		...
		{
			readlen = strlen((INT8*)decoder->exifMark.exiflib.Tiffdir.CameraMake);
	...
	case VO_PID_MACHINE_MODEL:
		...
		{
			readlen = strlen((INT8*)decoder->exifMark.exiflib.Tiffdir.CameraModel);
			if(decoder->memoryOperator.Copy)
			{
	...
	case VO_PID_GPS_MAPDATUM:
		...
		{		
			readlen = strlen((INT8*)decoder->exifMark.exiflib.Gpsdir.GPSMapDatum);
			if(decoder->memoryOperator.Copy)
			{
