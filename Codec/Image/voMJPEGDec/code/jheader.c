					  ...
					  JPEG_QUANT_TABLE* quant_tbl)
{
	...
	marklen = len;
	/*{
		FILE* fp =fopen("E:/errortest.dat","a");
...
//			}
            ...
            qtbl = (JPEG_QUANT_TBL *)voMemMalloc(decoder,sizeof(JPEG_QUANT_TBL), DCACHE_ALIGN_NUM);
			
			if(qtbl == NULL)
				return -1;
		}
		
		quantraw8 = (UINT8*)SAMPLE_ALIGN8(qtbl->quantrawval);
...
//				}
                htbl = (JPEG_HUFF_TBL *)voMemMalloc(decoder,sizeof(JPEG_HUFF_TBL), ALIGN_NUM_8);
					  ...
					  JPEG_PARA *	jpara)
{
...
#ifdef STABILITY
	if( jpara->width< 24 || jpara->heigth< 24 ||jpara->width>10240||jpara->heigth>10240)
	{
		jpara->width =0;
		jpara->heigth =0;
	...
	for(i = 0; i < jpara->num_component; i++)
	{
		...
		/*{
			...
			printf("---------%d %d  %d\n",jpara->compon[i].h_sample,jpara->compon[i].v_sample,n++);
			if(n==21)
				n=n;
		}*/
#ifdef STABILITY
		if(jpara->compon[i].v_sample==0 || jpara->compon[i].h_sample==0)
		{
			jpara->compon[i].v_sample =10;
			jpara->compon[i].h_sample =10;
...
#ifdef STABILITY
		if(jpara->compon[i].quant_tbl_no>3)  //error
		{
			jpara->compon[i].quant_tbl_no = 0;
			return VO_ERR_DEC_MJPEG_INDATAERROE;
		}
#endif
	}
...
static INT32 read_sos(JPEG_STREAM* bitstream, JPEG_PARA * jpara)
{
	...
	if(jpara->Ns != jpara->num_component)
		return VO_ERR_DEC_MJPEG_INDATAERROE;
#endif
	...
	for (i = 0; i < jpara->Ns; i++) 
	{
		...
		//if(index != i+1) //zou  318  419 delete  ID: 4678 
		//{
		//	index =index;
		//	//return VO_ERR_DEC_MJPEG_INDATAERROE;
		//}
...
#ifdef STABILITY
		if(comptr->dc_tbl_no>3 || comptr->ac_tbl_no>3)//error
		{
			comptr->dc_tbl_no = 0;
			comptr->ac_tbl_no =0;
			return VO_ERR_DEC_MJPEG_INDATAERROE;
				  ...
				  UINT32 Marker)
{
	...
	for( ; ; )
	{
		if(Marker == 0)
		{
			readlen = find_nextMark(bitstream, (INT32*)&Marker);
			if(readlen < 0)
				return -1;
			totallen += readlen;
		}
					   ...
					   JPEG_PARA* jpara)
{
	...
	if(jpara->Ns==1)
	{
		if(jpara->cur_com_ptr[0] == NULL) //zou 408
			return -1;
	}
	else if(jpara->Ns == 2)
	{
		if(jpara->cur_com_ptr[0] == NULL || jpara->cur_com_ptr[1] == NULL) //zou 408
			return -1;
	}
	else
	{
		if(jpara->cur_com_ptr[0] == NULL || jpara->cur_com_ptr[1] == NULL ||jpara->cur_com_ptr[2] == NULL) //zou 408
			return -1;
	}
#endif
