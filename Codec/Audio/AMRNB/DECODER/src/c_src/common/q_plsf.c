...
*/
#include "q_plsf.h"
const char voAMRNBDecq_plsf_id[] = "@(#)$Id $" q_plsf_h;
...
**************************************************************************
*/
int Q_plsf_init (Q_plsfState **state, VO_MEM_OPERATOR *pMemOP)
{
	Q_plsfState* s;
	...
	*state = NULL;
	/* allocate memory */
	if ((s= (Q_plsfState *) voAMRNBDecmem_malloc(pMemOP, sizeof(Q_plsfState), 32)) == NULL){
		return -1;
	}
	Q_plsf_reset(s);
	*state = s;
...
**************************************************************************
*/
void Q_plsf_exit (Q_plsfState **state, VO_MEM_OPERATOR *pMemOP)
{
