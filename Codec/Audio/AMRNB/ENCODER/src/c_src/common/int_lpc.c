...
**************************************************************************
*/
void voAMRNBEnc_Int_lpc_1and3 (
					Word16 lsp_old[],  /* i : LSP vector at the 4th subfr. of past frame (M) */
					Word16 lsp_mid[],  /* i : LSP vector at the 2nd subfr. of  present frame (M) */
					Word16 lsp_new[],  /* i : LSP vector at the 4th subfr. of present frame (M)  */
					Word16 Az[]        /* o : interpolated LP parameters in all subfr.(AZ_SIZE)  */
)
{
	Word32   i;
	Word16   lsp[M];
	...
	voAMRNBEnc_Lsp_Az(lsp, Az);           /* Subframe 1 */
	Az += MP1;                 

	voAMRNBEnc_Lsp_Az(lsp_mid, Az);       /* Subframe 2 */
	Az += MP1;                 
	...
	voAMRNBEnc_Lsp_Az(lsp, Az);           /* Subframe 3 */
	Az += MP1;                 

	voAMRNBEnc_Lsp_Az(lsp_new, Az);       /* Subframe 4 */
...
**************************************************************************
*/
void voAMRNBEnc_Int_lpc_1and3_2 (
					  Word16 lsp_old[],  /* i : LSP vector at the 4th subfr. of past frame (M) */
...
)
{
	...
	for (i = 0; i < M; i++) {
		lsp[i] = add3((lsp_new[i]>>2), sub3(lsp_old[i], (lsp_old[i]>>2)));
	}
	...
	for (i = 0; i < M; i++) {
		lsp[i] = add3((lsp_old[i]>> 1), (lsp_new[i]>> 1));
	}

	voAMRNBEnc_Lsp_Az(lsp, Az);        /* Subframe 2 */
	Az += MP1;           

	for (i = 0; i < M; i++) {
		lsp[i] = add3((lsp_old[i]>>2), sub3(lsp_new[i], (lsp_new[i]>> 2)));
	}
...
void voAMRNBEnc_Int_lpc_1to3_2(
					Word16 lsp_old[],  /* input : LSP vector at the 4th SF of past frame    */
					Word16 lsp_new[],  /* input : LSP vector at the 4th SF of present frame */
					Word16 Az[]        /* output: interpolated LP parameters in SFs 1,2,3   */
)
{
	Word32 i,var0,var1,var2;
	Word16 lsp[M];
	for (i = 0; i < M; i++) {
		var0 = sub3(lsp_old[i], (lsp_old[i]>>2));
		var1 = (lsp_new[i]>> 2);
		lsp[i] = add3(var0, var1);
	}
	...
	for (i = 0; i < M; i++) {
		var2 = sub3(lsp_new[i], (lsp_new[i]>>2));
		lsp[i] = add3((lsp_old[i]>>2),var2 );
	}
