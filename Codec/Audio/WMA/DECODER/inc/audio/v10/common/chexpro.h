...
//    return WMAB_FALSE;
//}
INLINE void chexSetSegAnchor(ReconTileInfo *prti,
                             CxAnchorReconTileInfo *pcxarti)
{
    if (prti->m_pcxartiSeg != pcxarti)
    {
...
ChexType qfLogInRange(QuantFlt *pqf, Float fVal)
{
    ...
    if (fVal != 0)
        return qfInRange(pqf, (voWMAlog10((VO_U32)(fVal * 256))<<(CX_BITS-16)));
...
ChexType qfLogInRangeRnd(QuantFlt *pqf, Float fVal)
{
    ...
    if (fVal != 0)
        return checkRange((voWMAlog10((VO_U32)(fVal * 256))<<(CX_BITS-16)), -pqf->m_fMax, pqf->m_fMax);
//        return checkRange(CX_FROM_FLOAT((Float)log10(fVal)), -pqf->m_fMax, pqf->m_fMax);
    else
        return -pqf->m_fMax;
}
