	...
	for (sfbOffs=0; sfbOffs<psyOutChan->sfbCnt; sfbOffs+=psyOutChan->sfbPerGroup){
		for (sfb=0; sfb<psyOutChan->maxSfbPerGroup; sfb++) {
			...
			if (psyOutChan->sfbEnergy[i] > psyOutChan->sfbThreshold[i]) {
				...
				for (j=sfbw; j; j--) {
					accu += formfac_sqrt(ABS(*mdctSpec)); mdctSpec++;
				}
...
//#endif
		*dist = sfbDistBest;                                              
		...
		for (cnt=0; cnt<3; cnt++) {
			...
			if (fixmul(0x66666666,sfbDist) < sfbDistAllowed) {
...
static Word16 calcSingleSpecPe(Word16 scf, Word16 sfbConstPePart, Word16 nLines)
{
	...
	if (ldRatio < 8*3) {
		...
		ldRatio = (ldRatio + 21) >> 1;
	}
	specPe = nLines * ldRatio;
	specPe = (specPe * 0x059a) >> 14;
	
	return extract_l(specPe);
}
                               ...
                               Word16 sfbCnt, Word16 startSfb, Word16 stopSfb)
{
	...
	sfbLast = startSfb;                                                   
	while (sfbLast < stopSfb && scfOld[sfbLast] == VO_SHRT_MIN) {
		
		sfbLast = sfbLast + 1;
	}
	/* search for previous relevant sfb and count diff */
	sfbPrev = startSfb - 1;
	while ((sfbPrev>=0) && scfOld[sfbPrev] == VO_SHRT_MIN) {
	...
	sfbNext = stopSfb;                                                    
	while (sfbNext < sfbCnt && scfOld[sfbNext] == VO_SHRT_MIN) {
                             ...
                             Word16 stopSfb)
{
	...
	for (sfb=startSfb; sfb<stopSfb; sfb++) {
		...
		if (scfOld[sfb] != VO_SHRT_MIN) {
			...
			if (ldRatioOld < 8*3) {
				/* 21 : 2*8*PE_C2, 2*PE_C3 ~ 1*/
				ldRatioOld = (ldRatioOld + 21) >> 1;
			}
			
			if (ldRatioNew < 8*3) {
				/* 21 : 2*8*PE_C2, 2*PE_C3 ~ 1*/
				ldRatioNew = (ldRatioNew + 21) >> 1;
			}
	...
	specPeDiff = (specPeDiff * 0x059a) >> 14;
	
	return extract_l(specPeDiff);
}
                            ...
                            Word16          *sfbNRelevantLines)
{
	...
	for (i=0; i<psyOutChan->sfbCnt; i++) {
		...
		/* maximum of spectrum */
		for (j=sbfwith; j; j-- ) {
			Word32 absSpec = ABS(*mdctSpec); mdctSpec++;
			maxSpec |= absSpec;                                                       
		}
		...
		if ((maxSpec > 0) && (energy > thresh)) {
			...
			//scfInt = mult(((thresholdPart - energyPart) - 20), 0x5555);
			scfInt = ((thresholdPart - energyPart - 20) * 0x5555) >> 15;
