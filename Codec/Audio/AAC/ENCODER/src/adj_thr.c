    ...
    PSY_OUT_CHANNEL *psyOutChan = &psyOutChannel[ch];
	 for(sfbGrp = 0; sfbGrp < psyOutChan->sfbCnt; sfbGrp+= psyOutChan->sfbPerGroup)	 
	  ...
	  for (sfb=0; sfb<psyOutChan->maxSfbPerGroup; sfb++) {
        //thrExp[ch][sfbGrp+sfb] = rsqrt(rsqrt(psyOutChan->sfbThreshold[sfbGrp+sfb],INT_BITS),INT_BITS);
                        ...
                        const Word16        nChannels)
{
  ...
  for (ch=0; ch<nChannels; ch++) {
    ...
    if (nSfb > 0) {
      //Word32 inSfb;
      //shift = norm_l(nSfb);
	  //inSfb = Div_32( 1 << shift, nSfb << shift);
  ...
  for (ch=0; ch<nChannels; ch++) {
    ...
    if (psyOutChan->windowSequence != SHORT_WINDOW) {
      for(sfbGrp = 0;sfbGrp < psyOutChan->sfbCnt;sfbGrp+= psyOutChan->sfbPerGroup){
		 ...
		 for (sfb=0; sfb<psyOutChan->maxSfbPerGroup; sfb++) {
          //psyOutChan->sfbSpreadedEnergy[sfbGrp+sfb] = psyOutChan->sfbSpreadedEnergy[sfbGrp+sfb] >> 1;
			*psfbSpreadEn = *psfbSpreadEn >> 1;
			++psfbSpreadEn;
        }
      }
    }
      ...
      for(sfbGrp = 0;sfbGrp < psyOutChan->sfbCnt;sfbGrp+= psyOutChan->sfbPerGroup){
		psfbSpreadEn = psyOutChan->sfbSpreadedEnergy + sfbGrp;
        for (sfb=0; sfb<psyOutChan->maxSfbPerGroup; sfb++) {
          *psfbSpreadEn = (*psfbSpreadEn >> 1) + (*psfbSpreadEn >> 3);
		  ++psfbSpreadEn;
        }
      }
    }
      ...
      for(sfbGrp = 0;sfbGrp < psyOutChan->sfbCnt;sfbGrp+= psyOutChan->sfbPerGroup){
		...
		for (sfb=0; sfb<psyOutChan->maxSfbPerGroup; sfb++) {
          ...
          if (sfbEn > avgEn && avgEn > 0) {
            Word32 tmpMinSnr;
            shift = norm_l(sfbEn);
			tmpMinSnr = Div_32(L_mpy_ls(avgEn, 0x6666) << shift, sfbEn << shift );
            tmpMinSnr = max(tmpMinSnr, 0x2873);                  
          ...
          if ((sfbEn < (avgEn >> 1)) && (sfbEn > 0)) {
            Word32 tmpMinSnr;
            Word32 minSnrEn = L_mpy_ls(avgEn, *psfbMinSnr) >>  1;                 
            ...
            tmpMinSnr = min(0x6666, tmpMinSnr);

            *psfbMinSnr =
              (min((tmpMinSnr >>  2), mult(*psfbMinSnr, 0x651f)) << 2);
          }
		  psfbMinSnr++;
        }
      }
  ...
  if (nChannels == 2) {
    ...
    for (sfb=0; sfb<psyOutChanM->sfbCnt; sfb++) {
      if (psyOutElement->toolsInfo.msMask[sfb]) {
        ...
        Word32 maxThr = L_mpy_ls(maxSfbEn, psyOutChanM->sfbMinSnr[sfb]) >> 2;
        ...
        if(maxThr >= sfbEnM) {
		  ...
		  psyOutChanM->sfbMinSnr[sfb] = min(max(psyOutChanM->sfbMinSnr[sfb], 
			  round16(Div_32(maxThr<<shift, sfbEnM << shift))), 0x6666);
        }
        ...
        if(maxThr >= sfbEnS) {
          ...
          psyOutChanS->sfbMinSnr[sfb] = min(max(psyOutChanS->sfbMinSnr[sfb], 
			  round16(Div_32(maxThr << shift, sfbEnS << shift))), 0x6666);
        }

         
        if (sfbEnM > psyOutChanM->sfbSpreadedEnergy[sfb])
          psyOutChanS->sfbSpreadedEnergy[sfb] = L_mpy_ls(sfbEnS, 0x7333);
         
        if (sfbEnS > psyOutChanS->sfbSpreadedEnergy[sfb])
          psyOutChanM->sfbSpreadedEnergy[sfb] = L_mpy_ls(sfbEnM, 0x7333);
      }
    }
  }
                       ...
                       const Word16     nChannels)
{
  ...
  *pe = ipe;                                                       
  *constPart = iconstPart;                                                
  *nActiveLines = inActiveLines;  
}
  ...
  if (nChannels==2 &&
    ...
    for (sfb=0; sfb<psyOutChanL->sfbCnt; sfb++) {
      ...
      if (psyOutElement->toolsInfo.msMask[sfb]) {
        ...
        minEn = L_mpy_ls(psyOutChanL->sfbEnergy[sfb], (0x3333 * psyOutChanL->sfbMinSnr[sfb]) >> 15);
        ...
        minEn = L_mpy_ls(psyOutChanR->sfbEnergy[sfb], (0x3333 * psyOutChanR->sfbMinSnr[sfb]) >> 15);
                ...
                Word32         chBitrate)
{
  ...
  msaParam->maxRed = 0x20000000;                             
  /* start adaptation of minSnr for avgEn/sfbEn > startRatio */
  msaParam->startRatio = 0x0ccccccd;                         
  /* maximum minSnr reduction to minSnr^maxRed is reached for
     avgEn/sfbEn >= maxRatio */
  msaParam->maxRatio =  0x0020c49c;                          
