...
)
{
	...
	L_tmp += (pred[0] * past_qua_en[0])<<1;
	L_tmp += (pred[1] * past_qua_en[1])<<1;
	L_tmp += (pred[2] * past_qua_en[2])<<1;
	L_tmp += (pred[3] * past_qua_en[3])<<1;
...
)
{
	...
	L_acc = L_deposit_h(exp - 13);
	L_acc = L_mac(L_acc, frac, 1);

	tmp = (Word16)((L_shl2(L_acc, 13))>>16);       /* tmp:Q13           */
	past_qua_en[0] = (tmp * 24660)>>15;        /* past_qua_en[]:Q10 */
}
