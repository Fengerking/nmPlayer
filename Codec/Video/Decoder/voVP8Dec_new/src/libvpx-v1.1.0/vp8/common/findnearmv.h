...
#define VO_CLAMP_MV(mv,left_edge,right_edge,top_edge,bottom_edge) \
	if ((mv).as_mv.col < left_edge ) \
        (mv).as_mv.col = left_edge ;\
    else if ((mv).as_mv.col > right_edge)\
        (mv).as_mv.col =  right_edge;\
    if ((mv).as_mv.row <  top_edge)\
        (mv).as_mv.row =  top_edge ;\
    else if ((mv).as_mv.row > bottom_edge )\
        (mv).as_mv.row =  bottom_edge ;
