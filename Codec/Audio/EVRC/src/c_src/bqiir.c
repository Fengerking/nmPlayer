...
__voinline void V_copy_SS (Shortword* a,Shortword* b,Shortword n)
{
	/*...execute....*/
	while (n--) *b++ = *a++;
}
__voinline void  V_copy_LL (Longword* a,Longword* b,Shortword n)
{
	/*....execute....*/
	while (n--)  *b++ = *a++;
}
__voinline void  V_copy_SL (Shortword* a,Longword* b,Shortword n)
{
	/*....execute....*/
	while (n--)  *b++ = L_shl2((Longword)(*a++),12);
}
__voinline void  V_copy_LS (Longword* a,Shortword* b,Shortword n)
{
	/*....execute....*/
//shanrong
...
/*----------------------------------------------------------------------*/
void  bqiir(ERVC_ENC_OBJ *encobj, Shortword* buf)
{
#if (FUNC_BQIIR_OPT)
	...
	bq_xsave = encobj->bq_xsave;
	bq_ysave = encobj->bq_ysave;

	/*....execute....*/
    counter = 40;
    dst_s_p = bq_x + BQ_N_SAVE;
    src_s_p = buf;
    while(counter-- != 0)
    {
        *dst_s_p++ = *src_s_p++;
	...
	while (i-- != 0)
	{
        ...
        *dst_l_p++ = *src_l_p++;
		...
		while (j-- != 0)
		{
			...
			sum -= L_mpy_ls(*tmpPy++,*wpP++);
			sum -= L_mpy_ls(*tmpPy++,*wpP++);
			*ypP++ = sum << 2;
		}
        dst_s_p = xsP;
        src_s_p = bq_x + BQ_N_BUFFER-BQ_N_SAVE;
        dst_l_p = ysP;
        src_l_p = bq_y + BQ_N_BUFFER - BQ_N_SAVE;
        ...
        *dst_l_p++ = *src_l_p++;
        ...
        while (counter-- != 0)
        {
            Longword chen_temp_l;
            chen_temp_l = *src_l_p++;
            if (chen_temp_l > (Longword)0x07ffffffL)
            {
                *dst_s_p++ = (Shortword)0x7fff;
            }
            else if (chen_temp_l < (Longword)0xf8000000L)
            {
                *dst_s_p++ = (Shortword)((MIN_32 + 0x8000) >> 16);
            }
            else
            {
                *dst_s_p++ = (chen_temp_l < (Longword)0x07fff800) ? (Shortword)((chen_temp_l + 0x800) >> 12) : (Shortword)0x7fff;
            }
        }
	}
    counter = BQ_N_DATA;
    dst_s_p = buf;
    src_l_p = bq_y+BQ_N_SAVE;
    while (counter-- != 0)
    {
        Longword chen_temp_l;
        chen_temp_l = *src_l_p++;
