 ...
 * \returns error code
 */
static VO_S32 IniCtbAddrTS( H265_DEC* const p_dec, 
    H265_DEC_SPS *p_sps, 
	H265_DEC_PPS *p_pps )
{
	...
	VO_U32 *p_col_width  = ( VO_U32 * )AlignMalloc(p_dec->user_op_all.p_user_op, p_dec->user_op_all.codec_id, ( num_tile_columns_minus1 + 1 ) * sizeof( VO_U32 ) , CACHE_LINE);
	VO_U32 *p_row_height = ( VO_U32 * )AlignMalloc(p_dec->user_op_all.p_user_op, p_dec->user_op_all.codec_id, ( num_tile_rows_minus1 + 1 ) * sizeof( VO_U32 ) , CACHE_LINE);
	VO_U32 *p_col_bd     = ( VO_U32 * )AlignMalloc(p_dec->user_op_all.p_user_op, p_dec->user_op_all.codec_id, ( num_tile_columns_minus1 + 1 + 1 ) * sizeof( VO_U32 ) , CACHE_LINE);
	VO_U32 *p_row_bd     = ( VO_U32 * )AlignMalloc(p_dec->user_op_all.p_user_op, p_dec->user_op_all.codec_id, ( num_tile_rows_minus1 + 1 + 1) * sizeof( VO_U32 ) , CACHE_LINE);
    ...
    */
	...
	p_pps->CtbAddrRsToTs = ( VO_U32 * )AlignMalloc(p_dec->user_op_all.p_user_op, p_dec->user_op_all.codec_id, ( PicSizeInCtbsY ) * sizeof( *p_pps->CtbAddrRsToTs ) , CACHE_LINE);
    ...
    */
	...
	p_pps->CtbAddrTsToRs = ( VO_U32 * )AlignMalloc(p_dec->user_op_all.p_user_op, p_dec->user_op_all.codec_id, ( PicSizeInCtbsY ) * sizeof( *p_pps->CtbAddrTsToRs ) , CACHE_LINE);
    ...
    */
    ...
    p_pps->TileId        = ( VO_U32 * )AlignMalloc(p_dec->user_op_all.p_user_op, p_dec->user_op_all.codec_id, ( PicSizeInCtbsY ) * sizeof( *p_pps->TileId ) ,CACHE_LINE);
    ...
    */
    ...
    p_pps->ExtendMinTbAddrZs = ( VO_S32 * ) AlignMalloc(p_dec->user_op_all.p_user_op, p_dec->user_op_all.codec_id, p_pps->MinTbAddrZsStride * (PicHeightInMinTb + 2) * sizeof( *p_pps->ExtendMinTbAddrZs ) , CACHE_LINE);
    p_pps->MinTbAddrZs = p_pps->ExtendMinTbAddrZs + p_pps->MinTbAddrZsStride + 1;
    
	if ( !p_col_width || !p_row_height || \
		 !p_col_bd || !p_row_bd || \
		 !p_pps->CtbAddrRsToTs || !p_pps->CtbAddrTsToRs || !p_pps->TileId ||
  ...
  {
    if ( p_pps->CtbAddrRsToTs[ i ] >=PicSizeInCtbsY)
      VOH265ERROR(VO_H265_ERR_PPSERROR);
...
// 	}
    ...
    for ( j = 0; j < PicHeightInMinTb; j++ ) {
        p_pps->MinTbAddrZs[ (VO_S32)(j * p_pps->MinTbAddrZsStride - 1) ] = -1;
        for ( i = 0 ; i < PicWidthInMinTb; i++ ) {
            VO_S32 k;
            VO_S32 tb_x = i >> log2_diff_ctb_min_tb_size;
            VO_S32 tb_y = j >> log2_diff_ctb_min_tb_size;
            ...
            p_pps->MinTbAddrZs[ (VO_S32)(j * p_pps->MinTbAddrZsStride  + i) ] = val;
        }

        for ( i =  p_sps->pic_width_in_luma_samples >> 2; i <= PicWidthInMinTb; i++ ) {
            p_pps->MinTbAddrZs[ (VO_S32)(j * p_pps->MinTbAddrZsStride  + i) ] = -1;
        }
    }
   ...
   // p_slice->currEntryNum = 0;
    for ( j = 0, k = 0; j  <=  num_tile_rows_minus1; j++ ) {
        for ( i = 0; i  <=  num_tile_columns_minus1; i++, k++ ) {
            ...
            for ( y = p_row_bd[ j ]; y < p_row_bd[ j + 1 ]; y++ )
              for ( x = p_col_bd[ i ]; x < p_col_bd[ i + 1 ]; x++ ){
                    if (y * PicWidthInCtbsY + x>=PicSizeInCtbsY)
                        VOH265ERROR(VO_H265_ERR_PPSERROR);
                    p_pps->TileId[ p_pps->CtbAddrRsToTs[ y * PicWidthInCtbsY + x ] ] = k;
              }
        }
...
IniCtbAddrTSEnd:
	if ( p_col_width )
		AlignFree(p_dec->user_op_all.p_user_op, p_dec->user_op_all.codec_id, p_col_width );
	if ( p_row_height )
		AlignFree(p_dec->user_op_all.p_user_op, p_dec->user_op_all.codec_id, p_row_height) ;
	if ( p_col_bd )
		AlignFree(p_dec->user_op_all.p_user_op, p_dec->user_op_all.codec_id, p_col_bd );
	if ( p_row_bd )
		AlignFree(p_dec->user_op_all.p_user_op, p_dec->user_op_all.codec_id, p_row_bd );
	...
	VO_S32 idx )
{
    ...
    if (delta_rps<1)
    {
      VOH265ERROR(VO_H265_ERR_RPSERROR);
		...
		if ( code ) 
			delta_rps = -delta_rps;
    if (p_rps_ref->m_numberOfPictures>MAX_NUM_REF_PICS)
    {
      VOH265ERROR(VO_H265_ERR_RPSERROR);
    }
		///////////////////////////////////////////////////////////////////YU_TBD
...
// 					VOH265ERROR(VO_H265_ERR_RPSERROR);
// 				}
				deltaPOC = delta_rps + ((j < p_rps_ref->m_numberOfPictures)? p_rps_ref->delta_poc[j] : 0);
        if (deltaPOC==0)
        {
          VOH265ERROR(VO_H265_ERR_RPSERROR);
        }
    ...
    if(p_rps->num_negative_pics>MAX_NUM_REF_PICS)
      VOH265ERROR(VO_H265_ERR_RPSERROR);
		p_rps->num_positive_pics = READ_UEV( p_bs, "num_positive_pics" );
    if(p_rps->num_positive_pics>MAX_NUM_REF_PICS-p_rps->num_negative_pics)
      VOH265ERROR(VO_H265_ERR_RPSERROR);
...
VO_S32 ParseSliceParamID(H265_DEC* p_dec, H265_DEC_SLICE *p_slice, BIT_STREAM *p_bs)
{
  ...
  if(code>63)
    VOH265ERROR(VO_H265_ERR_SLICEHEADERERROR);
  p_pps = p_slice->p_pps = &p_dec->pps[ code ];
#if 1 //YU_TBD: move temp
	if ( p_slice->slice_pic_parameter_set_id != code ) {
		//@Harry: TBD, to support multi-pps we need active pps here
        if(p_pps->Valid==VO_FALSE) 
          VOH265ERROR(VO_H265_ERR_PPSIsNULL);
...
// LOGI("ConfigDecoder done!\n");
    ...
    if (p_slice->p_sps->scaling_list_enabled_flag)
    {
      ...
      {
        ret = alloc_ScalingList(p_dec,p_slice->pSliceScalingListEntity);
		if (ret)
		{
			return ret;
		}
// LOGI("alloc_ScalingList done!\n");
...
#endif
	///////////////////////////////////////////////////////////
  p_sps = &p_dec->sps[ p_pps->pps_seq_parameter_set_id ];   
  if(p_pps->Valid==VO_FALSE)
    VOH265ERROR(VO_H265_ERR_PPSIsNULL);
  if(p_sps->Valid==VO_FALSE) 
    VOH265ERROR(VO_H265_ERR_SPSIsNULL);
...
// 	VO_S32 ret;
	...
	if ( !p_slice->first_slice_segment_in_pic_flag ) {
        ...
        if(p_slice->slice_end_address!=0&&p_slice->slice_segment_address!=p_slice->slice_end_address)
			VOH265ERROR(VO_H265_ERR_SLICEHEADERERROR);
		if(p_slice->slice_segment_address < p_slice->slice_end_address || p_slice->slice_segment_address>=p_slice->PicSizeInCtbsY)
			VOH265ERROR(VO_H265_ERR_SLICEHEADERERROR);
	...
	if ( p_slice->dependent_slice_segment_flag == 0 ) {
		...
		p_slice->SliceMinTbAddrZs = p_pps->MinTbAddrZs[ (VO_S32)(slice_y * (p_pps->MinTbAddrZsStride) + slice_x) ];
	}
    ...
    if(p_slice->slice_type>2)
      VOH265ERROR(VO_H265_ERR_SLICEHEADERERROR);
		...
		if ( p_slice->nal_unit_type ==  NAL_UNIT_CODED_SLICE_IDR || \
			...
			p_slice->p_rps = p_rps;
     /* if(p_rps->Valid==VO_FALSE)   //lina_error
        VOH265ERROR(VO_H265_ERR_SLICEHEADERERROR);*/
			...
			if ( p_sps->long_term_ref_pics_present_flag ) {
				...
				if ( p_sps->num_long_term_ref_pic_sps > 0 ) {
					...
					if(num_long_term_sps>p_sps->num_long_term_ref_pic_sps||offset+num_long_term_sps>MAX_NUM_REF_PICS)
						VOH265ERROR(VO_H265_ERR_SLICEHEADERERROR);
					//numOfLtrp += num_long_term_sps;
					//p_rps->num_long_term_pics = numOfLtrp;
				}
				p_rps->num_long_term_pics = READ_UEV( p_bs, "num_long_term_pics");
        if(offset+p_sps->num_long_term_ref_pic_sps>MAX_NUM_REF_PICS||p_rps->num_long_term_pics+offset+p_sps->num_long_term_ref_pic_sps>MAX_NUM_REF_PICS)
          VOH265ERROR(VO_H265_ERR_SLICEHEADERERROR);
				...
				for ( i = 0; i < num_long_term_sps + p_rps->num_long_term_pics ; i++ ) {
					VO_S32 poc_lsb_lt = 0;
      ...
      if((p_rps->num_negative_pics+p_rps->num_positive_pics+p_rps->num_long_term_pics)==0)
        VOH265ERROR(VO_H265_ERR_SLICEHEADERERROR);
          ...
          if(p_slice->collocated_ref_idx>p_slice->num_ref_idx[REF_PIC_LIST_0]||p_slice->collocated_ref_idx>p_slice->num_ref_idx[REF_PIC_LIST_1])
            VOH265ERROR(VO_H265_ERR_SLICEHEADERERROR);
				} else {
      ...
      if(p_slice->max_num_merge_cand<1||p_slice->max_num_merge_cand>5)
        VOH265ERROR(VO_H265_ERR_SLICEHEADERERROR);
		}else{
      p_slice->cabac_init_flag = VO_FALSE;
    }
    ...
    if(p_slice->slice_qp>51||p_slice->slice_qp<0)
      VOH265ERROR(VO_H265_ERR_SLICEHEADERERROR);
	...
	if ( p_pps->deblocking_filter_control_present_flag ) 
	{
	  ...
	  if ( deblocking_filter_override_flag )
	  {
		  ...
		  if ( !p_slice->slice_deblocking_filter_disabled_flag  ) 
		  {
              ...
              if(p_slice->beta_offset_div2>6||(VO_S32)p_slice->beta_offset_div2<-6)
                  p_slice->beta_offset_div2 = p_pps->pps_beta_offset_div2;
			  p_slice->tc_offset_div2 = READ_SEV( p_bs, "tc_offset_div2" );
              if(p_slice->tc_offset_div2>6||(VO_S32)p_slice->tc_offset_div2<-6)
    ...
    if(num_entry_point_offsets>=MAX_ENTRY_SIZE)
      VOH265ERROR(VO_H265_ERR_SLICEHEADERERROR);
      ...
      if(offset_len>32)
		...
		for ( idx = 0; idx < num_entry_point_offsets; idx++ ) {
			...
			p_slice->entries[idx].entry_point_offset =  entry_point_offset + 1;
		}
		p_slice->num_entry_point_offsets = num_entry_point_offsets;
	} else {
...
VO_S32 ParsePPS( H265_DEC* const p_dec, H265_DEC_SPS* sps_array ,
	...
	BIT_STREAM *p_bs )
{
 ...
 /* if(p_dec->sps->Valid==VO_FALSE)   //lina_error
    VOH265ERROR(VO_H265_ERR_SPSIsNULL);*/
  ...
  if(code>63)
	...
	p_pps = pps_array + code ;
		...
		if ( p_dec->nThdNum > 1 ) {
			if ( p_pps->Valid ) {
				...
				//reset pps id for all slice
				p_dec->slice.slice_pic_parameter_set_id = (VO_U32)(-1);
				for (i = 0; i < p_dec->nDecodingFrame; ++i)
				{
					H265_DEC_SLICE *pSlice = p_dec->slicePool+i;
    ...
    p_pps->Valid = VO_FALSE;// default value

	p_pps->pps_seq_parameter_set_id = READ_UEV( p_bs, "pps_seq_parameter_set_id" );
    if(p_pps->pps_seq_parameter_set_id>15)
        VOH265ERROR(VO_H265_ERR_PPSERROR);
	p_sps = sps_array+p_pps->pps_seq_parameter_set_id;
    if(p_sps->Valid == VO_FALSE)
		VOH265ERROR(VO_H265_ERR_SPSIsNULL);
  ...
  if(p_pps->num_ref_idx_l1_default_active>15)
	...
	if ( p_pps->tiles_enabled_flag ) {
		VO_U32 log2_ctb_size     = p_sps->log2_luma_coding_block_size;
		VO_U32 pic_width_cbts_y  = ( ( p_sps->pic_width_in_luma_samples + ( 1 << log2_ctb_size ) - 1 ) >> log2_ctb_size );
	    VO_U32 pic_height_cbts_y = ( ( p_sps->pic_height_in_luma_samples + ( 1 << log2_ctb_size ) - 1 ) >> log2_ctb_size );
		p_pps->num_tile_columns_minus1 = READ_UEV( p_bs, "num_tile_columns_minus1" );	
    if(p_pps->num_tile_columns_minus1>=pic_height_cbts_y) //lina_error
      VOH265ERROR(VO_H265_ERR_PPSERROR);
		p_pps->num_tile_rows_minus1 = READ_UEV( p_bs, "num_tile_rows_minus1" );	
    if(p_pps->num_tile_rows_minus1>=pic_width_cbts_y)     //lina_error
      VOH265ERROR(VO_H265_ERR_PPSERROR);
		p_pps->uniform_spacing_flag = READ_FLAG( p_bs, "uniform_spacing_flag" );
    if (p_pps->num_tile_rows_minus1+p_pps->num_tile_columns_minus1==0
		||(p_pps->num_tile_rows_minus1+1)*(p_pps->num_tile_columns_minus1)>=MAX_ENTRY_SIZE)  //lina_error
    {
      VOH265ERROR(VO_H265_ERR_PPSERROR);
    }
		...
		if ( !p_pps->uniform_spacing_flag ) {
            ...
            p_pps->column_width = (VO_U32*)AlignMalloc( p_dec->user_op_all.p_user_op, p_dec->user_op_all.codec_id,  sizeof( VO_U32 ) * p_pps->num_tile_columns_minus1 , CACHE_LINE);
			if ( !p_pps->column_width )
                return VO_ERR_OUTOF_MEMORY;

            p_pps->row_height = ( VO_U32* )AlignMalloc(p_dec->user_op_all.p_user_op, p_dec->user_op_all.codec_id,  sizeof( VO_U32 ) * p_pps->num_tile_rows_minus1, CACHE_LINE);        
			if ( !p_pps->row_height )
                return VO_ERR_OUTOF_MEMORY;
	...
	{
	  p_pps->num_tile_columns_minus1 = 0;
	  p_pps->num_tile_rows_minus1 = 0;
	  p_pps->uniform_spacing_flag = 0;
	  p_pps->loop_filter_across_tiles_enabled_flag = 0;
	}
	...
	if ( p_pps->deblocking_filter_control_present_flag ) {
		...
		if ( !p_pps->pps_deblocking_filter_disabled_flag ) {
			p_pps->pps_beta_offset_div2 = Clip3(-6, 6, READ_SEV( p_bs, "pps_beta_offset_div2" ));
...
//         VOH265ERROR(VO_H265_ERR_PPSERROR);
			p_pps->pps_tc_offset_div2 = Clip3(-6, 6, READ_SEV( p_bs, "pps_tc_offset_div2" ));
// 			p_pps->pps_tc_offset_div2 = READ_SEV( p_bs, "pps_tc_offset_div2" );
//       if(p_pps->pps_tc_offset_div2>6||p_pps->pps_beta_offset_div2<-6)
//         VOH265ERROR(VO_H265_ERR_PPSERROR);
		}
	}
    ...
    if ( p_pps->pps_scaling_list_data_present_flag ) {
        ret = alloc_ScalingList(p_dec, &p_pps->ppsScalingListEntity );
		if (ret)
		{
			return ret;
		}
        ret = scaling_list_data(&p_pps->ppsScalingListEntity, p_bs );
		if(ret)
			return ret;
    }
  ...
  //if(p_pps->log2_parallel_merge_level_minus2>p_sps->log2_luma_coding_block_size-2)   //lina_error
  //  VOH265ERROR(VO_H265_ERR_PPSERROR);
	...
	//YU_TBD
	//rbsp_trailing_bits( );
	ret = IniCtbAddrTS( p_dec, sps_array + p_pps->pps_seq_parameter_set_id , p_pps );
    if ( ret != VO_ERR_NONE )
        return ret;
	...
	BIT_STREAM *p_bs )
{
  ...
  if(sps_max_sub_layers_minus1>6)
    VOH265ERROR(VO_H265_ERR_SPSERROR);
...
#if USE_3D_WAVE_THREAD
  //Harry: temp resolution for more than one SPS
	if ( p_sps->Valid ) {
		if ( p_dec->nThdNum > 1 ) {
			//wait for other thread finish decoding
			h265_wave3DThdPool * const pWave3DThdPool = p_dec->pWave3DThdPool;
			...
			//reset sps id for all slice
			for (i = 0; i < p_dec->nDecodingFrame; ++i)
			{
				H265_DEC_SLICE *pSlice = p_dec->slicePool+i;
				pSlice->slice_seq_parameter_set_id = (VO_U32)(-1); //None sps set
				pSlice->slice_pic_parameter_set_id = (VO_U32)(-1); //None sps set
				pSlice->init_done = VO_FALSE;
			}
		}
		p_dec->slice.slice_seq_parameter_set_id = (VO_U32)(-1);
		p_dec->slice.slice_pic_parameter_set_id = (VO_U32)(-1);
		p_dec->slice.init_done = VO_FALSE;
	}
#endif
	p_sps->Valid = VO_FALSE;
  ...
  if(p_sps->pic_width_in_luma_samples<16 || p_sps->pic_width_in_luma_samples>2560)
    VOH265ERROR(VO_H265_ERR_SPSERROR);
	p_sps->pic_height_in_luma_samples = READ_UEV(p_bs, "pic_height_in_luma_samples" );
  if(p_sps->pic_height_in_luma_samples<16 || p_sps->pic_height_in_luma_samples>2560)
    VOH265ERROR(VO_H265_ERR_SPSERROR);
  ...
  if(p_sps->log2_max_pic_order_cnt_lsb>16)
    ...
    if (p_sps->sps_max_dec_pic_buffering_minus1[i] >= MAX_NUM_REF_PICS) //Harry add
    {
      VOH265ERROR(VO_H265_ERR_SPSERROR);
    }
		p_sps->sps_max_num_reorder_pics[ i ] = READ_UEV( p_bs, "sps_max_num_reorder_pics" );
    if (p_sps->sps_max_num_reorder_pics[i] > p_sps->sps_max_dec_pic_buffering_minus1[i]) //Harry add
    {
      VOH265ERROR(VO_H265_ERR_SPSERROR);
    }
  ...
  if(p_sps->pic_width_in_luma_samples%(1<<p_sps->log2_min_luma_coding_block_size)!=0)
    VOH265ERROR(VO_H265_ERR_SPSERROR);
  if(p_sps->pic_height_in_luma_samples%(1<<p_sps->log2_min_luma_coding_block_size)!=0)
    VOH265ERROR(VO_H265_ERR_SPSERROR);
  ...
  if(p_sps->log2_luma_coding_block_size>6||p_sps->log2_luma_coding_block_size<4)
    VOH265ERROR(VO_H265_ERR_SPSERROR);
  ...
  if(p_sps->log2_max_transform_block_size>5||p_sps->log2_max_transform_block_size>p_sps->log2_luma_coding_block_size)
    VOH265ERROR(VO_H265_ERR_SPSERROR);
	...
	if ( p_sps->scaling_list_enabled_flag ) {
		...
		if ( p_sps->sps_scaling_list_data_present_flag ) {
            ret = alloc_ScalingList(p_dec, &p_sps->spsScalingListEntity );//YU_TBD
			if (ret)
			{
				return ret;
			}
            ret = scaling_list_data(&p_sps->spsScalingListEntity, p_bs);
			if(ret)
				return ret;
		} else {
  ...
  if(p_sps->num_short_term_ref_pic_sets>64)
    VOH265ERROR(VO_H265_ERR_SPSERROR);
	for ( i = 0; i < p_sps->num_short_term_ref_pic_sets; i++ ) {
		ret = ParseShortTermRefPicSet( p_sps, rps_array, rps_array + i, p_bs, i );
		if(ret)
			return ret;
	}
    ...
    if(p_sps->num_long_term_ref_pic_sps>32)
      VOH265ERROR(VO_H265_ERR_SPSERROR);
...
VO_S32 ParseVPS( H265_DEC* const p_dec, H265_DEC_VPS *p_vps,
	BIT_STREAM *p_bs )
{
	VO_U32 i, j;
	TComPTL  vps_ptl;
...
#if USE_3D_WAVE_THREAD
  //Harry: temp resolution for more than one VPS
  if ( p_dec->nThdNum> 1 ) {
    if ( p_vps->Valid ) {
      //wait for other thread finish decoding
		h265_wave3DThdPool * const pWave3DThdPool = p_dec->pWave3DThdPool;
  ...
  if(vps_max_sub_layers_minus1>6)
    VOH265ERROR(VO_H265_ERR_VPSERROR);
    ...
    if(p_vps->vps_num_layer_sets_minus1>1023)
      VOH265ERROR(VO_H265_ERR_VPSERROR);
    ...
    if(vps_num_hrd_parameters>1023)
      VOH265ERROR(VO_H265_ERR_VPSERROR);
  ...
  //if(p_vps->m_numHrdParameters>1024)  //lina_error
  //  VOH265ERROR(VO_H265_ERR_VPSERROR);
