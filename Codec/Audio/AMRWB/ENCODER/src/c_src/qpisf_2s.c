...
#ifdef ARMv6
		tmp_ind[0] = Sub_VQ1_asm(&isf_stage2[0], dico21_isf, 3, SIZE_BK21, &min_err); 
		temp = min_err;
		tmp_ind[1] = Sub_VQ1_asm(&isf_stage2[3], dico22_isf, 3, SIZE_BK22, &min_err); 
		temp = vo_L_add(temp, min_err);
		tmp_ind[2] = Sub_VQ1_asm(&isf_stage2[6], dico23_isf, 3, SIZE_BK23, &min_err); 
#else
...
#ifdef ARMv6
		tmp_ind[0] = Sub_VQ1_asm(&isf_stage2[0], dico24_isf, 3, SIZE_BK24, &min_err);
		temp = min_err; 
		tmp_ind[1] = Sub_VQ_asm(&isf_stage2[3], dico25_isf, 4, SIZE_BK25, &min_err);
		temp = vo_L_add(temp, min_err);
#else
...
{
	Word16 temp, *p_dico;
	Word32 *dist_min,dist_min1[N_SURV_MAX], dist;
	Word32 nLen1 = 0, nLen2;
	...
	*index = 3;
	...
	do{
		...
		if(dist<dist_min[0])
		{
			...
			index[3] = index[2];
			dist_min[2] = dist_min[1];
			index[2] = index[1];
			dist_min[1] = dist_min[0];
			index[1] = index[0];
			dist_min[0] = dist;
			index[0] = (Word16)nLen1;
			goto nextIter;
		...
		if(dist<dist_min[2])
		{
			...
			index[3] = index[2];
			dist_min[2] = dist;
			index[2] = (Word16)nLen1;
			goto nextIter;
		}
		if(dist<dist_min[3])
		{
			dist_min[3] = dist;
			index[3] = (Word16)nLen1;
			goto nextIter;
		}

nextIter:
		nLen1+=1;
	}while(nLen1<dico_size);
