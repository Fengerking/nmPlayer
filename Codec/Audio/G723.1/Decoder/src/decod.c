...
int G723_decoder(G723DncState *st)
{
	...
	Word16  *Dpnt ;
    Word16  *DataBuff = st->output_buffer;
	LINEDEF  Line ;
	PFDEF    Pf[SubFrames] ;
	Word16   Ftyp;
	DECCNGDEF *DecCng = (DECCNGDEF*)st->Dec_Cng;
    DECSTATDEF *DecStat = (DECSTATDEF *)st->Dec_Stat; 

	Info = st->input_buffer[0] & (Word16)0x0003 ;
	switch(Info) {
	...
	if(Ftyp != 1) {
		...
		*/
		...
		if ( DecStat->Ecount == (Word16) 0 ) {
			...
			if(1)
				for ( i = 0 ; i < SubFrames ; i ++ )
					Pf[i] = Comp_Lpf( Temp, Line.Olp[i>>1], (Word16) i, st) ;
	...
	*/
	...
	for ( i = 0 ; i < SubFrames ; i ++ ) {
		...
		* (Text: Section 3.7)
		*/
		Synt( Dpnt, &QntLpc[i*LpcOrder], st) ;
		...
		if(1)
		{
			...
			Senr = Spf( Dpnt, &QntLpc[i*LpcOrder], st) ;
