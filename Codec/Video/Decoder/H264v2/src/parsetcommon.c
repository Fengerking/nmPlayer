...
pic_parameter_set_rbsp_t *AllocPPS (H264DEC_G *pDecGlobal)
 {
   ...
   if ((p=voMalloc(pDecGlobal,sizeof (pic_parameter_set_rbsp_t))) == NULL)
...
seq_parameter_set_rbsp_t *AllocSPS (H264DEC_G *pDecGlobal)
 {
   ...
   if ((p=voMalloc(pDecGlobal,sizeof (seq_parameter_set_rbsp_t))) == NULL)
 ...
 void FreePPS (H264DEC_G *pDecGlobal, pic_parameter_set_rbsp_t *pps)
 {
   ...
   if (pps->slice_group_id != NULL) 
     voFree (pDecGlobal,pps->slice_group_id);
   voFree (pDecGlobal,pps);
 }
