...
WMARESULT plusGetIndexH(CAudioObjectDecoder *paudec,
...
WMARESULT freqexDecInit(CAudioObjectDecoder *paudec)
{
    ...
    pfx->m_rgGlobalParam = (FexGlobalParam *)memset(pfx->m_rgGlobalParam, 0, 2*sizeof(FexGlobalParam));
    ...
    pfx->m_rgiBandSizeMGlobalParam = (Int *)memset(pfx->m_rgiBandSizeMGlobalParam, 0, 2*pfx->m_cMaxBands*sizeof(Int));
...
WMARESULT freqexDecodeTile(CAudioObjectDecoder *paudec)
{
    ...
    for (iCh=0; iCh < pau->m_cChInTile; iCh++)
    {
        ...
        else if (2 < pfx->m_iVersion)
        {
            ...
            Temp = freqexFindStartPosForOverlay(pau, iChSrc);
        }
