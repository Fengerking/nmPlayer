...
{
    Word16 *DataExc = st->output_buffer;
	...
	if(Ftyp == 2) {
		...
		*/
		...
		Lsp_Inq( DecCng->LspSid, st->Dec_Stat->PrevLsp, Line->LspId, 0) ;
	}
	else {
		/*
		* non SID Frame
	...
	Calc_Exc_Rand(DecCng->CurGain, st->Dec_Stat->PrevExc, DataExc,
		&DecCng->RandSeed, Line, st);

	/* Interpolate the Lsp vectors */
	Lsp_Int( QntLpc, DecCng->LspSid, st->Dec_Stat->PrevLsp ) ;

	/* Copy the LSP vector for the next frame */
	for ( i = 0 ; i < LpcOrder ; i ++ )
		st->Dec_Stat->PrevLsp[i] = DecCng->LspSid[i] ;
