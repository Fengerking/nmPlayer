...
**************************************************************************
*/
int ec_gain_code_init (ec_gain_codeState **state, VO_MEM_OPERATOR *pMemOP)
{
	ec_gain_codeState* s;
	...
	*state = NULL;
	/* allocate memory */
	if ((s= (ec_gain_codeState *) voAMRNBDecmem_malloc(pMemOP, sizeof(ec_gain_codeState), 32)) == NULL){
		return -1;
	}
...
**************************************************************************
*/
void ec_gain_code_exit (ec_gain_codeState **state, VO_MEM_OPERATOR *pMemOP)
{
...
**************************************************************************
*/
static const Word16 cdown[7] ={32767, 32112, 32112, 32112,32112, 32112, 22937};
void ec_gain_code (
...
**************************************************************************
*/
int ec_gain_pitch_init (ec_gain_pitchState **state, VO_MEM_OPERATOR *pMemOP)
{
	...
	*state = NULL;
	/* allocate memory */
	if ((s= (ec_gain_pitchState *)voAMRNBDecmem_malloc(pMemOP, sizeof(ec_gain_pitchState), 32)) == NULL){
		return -1;
	}
...
**************************************************************************
*/
void ec_gain_pitch_exit (ec_gain_pitchState **state, VO_MEM_OPERATOR *pMemOP)
{
	if (state == NULL || *state == NULL)
		return;
	/* deallocate memory */
...
{
	static const Word16 pdown[7] ={32767, 32112, 32112, 26214,9830, 6553, 6553};
	Word16 tmp;
