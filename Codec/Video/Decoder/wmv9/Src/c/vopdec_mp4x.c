...
#include "codehead.h"
#include "dectable_mp4x.h"
#include "voWmvPort.h"
#if !(defined(WMV9_SIMPLE_ONLY) || defined(WMV789_ONLY))

#ifdef _TI_C55X_
#   define BLOCK_SQUARE_SIZE_2         0   // Fake, must be fixed
#   define BLOCK_SQUARE_SIZE_2MINUS2   0   // Fake, must be fixed
//#   include <string.h>
#   include <limits.h>
#endif
...
#   define BOE 
    ...
    I32_WMV decodeVideoPacketHeader(tWMVDecInternalMember *pWMVDec, I32_WMV* piCurrentQP) {return TRUE_WMV;};
#else
// -------- MP4S - MP43
...
tWMVDecodeStatus DecodeIMBAcPred_MP4X (
    ...
    Bool_WMV                bLeftMB, 
    Bool_WMV                bTopMB, 
    Bool_WMV                bLeftTopMB, I32_WMV dummy)
{
    ...
    if (pWMVDec->m_bDCTTable_MB){
        ...
        pIntraDCTTableInfo_Dec = pWMVDec->m_ppIntraDCTTableInfo_Dec[0];
    }
...
#ifdef _WMV9AP_
    setCodedBlockPattern (pmbmd, U_BLOCK, iCBPC >> 1);
    setCodedBlockPattern (pmbmd, V_BLOCK, iCBPC & 1);

    {
        U32_WMV iBlk;

        for ( iBlk = (U32_WMV) Y_BLOCK1; iBlk <= (U32_WMV) Y_BLOCK4; iBlk++)
            setCodedBlockPattern (pmbmd, (BlockNum)iBlk, (iCBPY >>  (4 - iBlk)) & 1);
    }
...
{
    if (pvctSrc->x < -1 * iMvRange)                   //* 2 to get length of [-range, range]
        pvctSrc->x += 2 * iMvRange;
    else if (pvctSrc->x >= iMvRange)
        pvctSrc->x  -= 2 * iMvRange;

    if (pvctSrc->y < -1 * iMvRange)
        pvctSrc->y += 2 * iMvRange;
    else if (pvctSrc->y >= iMvRange)
        pvctSrc->y  -= 2 * iMvRange;
}

Void_WMV getDiffMV (tWMVDecInternalMember *pWMVDec, CVector* pvctDiffMV)     //get half pel
{
    I32_WMV iResidual;
    I32_WMV lSymbol = BS_getMaxBits ( pWMVDec->m_pbitstrmIn, MAXCODEWORDLNGMV,gDecodeCodeTableMV);
    ...
    if (iVLC != 0)
        ...
        iResidual = 0;
    pvctDiffMV->x  = (I8_WMV) deScaleMV (iVLC, iResidual, pWMVDec->iScaleFactor);
    ...
    if (iVLC != 0)
        ...
        iResidual = 0;
    pvctDiffMV->y  = (I8_WMV) deScaleMV (iVLC, iResidual, pWMVDec->iScaleFactor);
}
...
tWMVDecodeStatus DecodeInverseInterBlockQuantizeEscCode_MMX_MPEG4(tWMVDecInternalMember *pWMVDec, CDCTTableInfo_Dec** ppInterDCTTableInfo_Dec, U8_WMV *pZigzagInv, I32_WMV iXformMode, DQuantDecParam *pDQ            )
{
...
{
    CDCTTableInfo_Dec* InterDCTTableInfo_Dec = *pInterDCTTableInfo_Dec;
    Huffman_WMV* hufDCTACDec = InterDCTTableInfo_Dec->hufDCTACDec;
    I8_WMV* rgLevelAtIndx = InterDCTTableInfo_Dec->pcLevelAtIndx;
    U8_WMV* rgRunAtIndx = InterDCTTableInfo_Dec->puiRunAtIndx;
//  Change iStartIndxOfLastRun to iStartIndxOfLastRunMinus1 and <= to <
...
#if !defined(_WIN32) && !defined(_Embedded_x86)&&!defined(_APOGEE_) && !defined(_MAC) && !defined(__arm) && !defined(_MAC_OS) && !defined(_IOS)
...
//        pWMVDec->m_volmd.uiQuantPrecision = 5;
    ...
    pWMVDec->m_bResyncDisable = FALSE_WMV;
    ...
    BS_flushMPEG4 (pWMVDec->m_pbitstrmIn, 0);
