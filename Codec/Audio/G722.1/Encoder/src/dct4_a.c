...
void dct_type_iv_a (Word16 *input,Word16 *output,Word16 dct_length)
{
	...
	for (set_count_log=0;set_count_log<=temp;set_count_log++)
	{
		...
		for (sets_left=set_count;sets_left>0;sets_left--)
		{
			...
			do 
			{
				...
				acca            = (acca >> 1);
				out_val_low     = (Word16)(acca);

				acca            = (in_val_low - in_val_high);
				acca            = (acca >> 1);
				out_val_high    = (Word16)(acca);
	...
	for (pairs_left=temp; pairs_left > 0; pairs_left--)
	{
		for ( k=0; k<CORE_SIZE; k++ )
		{
			...
			{
				sum += (pair_ptr[i] * dct_core_a[i][k])<<1;
			}
	...
	for (set_count_log = temp; set_count_log >= 0;    set_count_log--)
	{
		...
		for (sets_left = set_count; sets_left > 0;sets_left--)
		{
			...
			do 
			{
				...
				neg_msin_even = -msin_even;
				sum += (neg_msin_even * in_high_even)<<1;
				out_low_even = voRound(sum);

				sum = (msin_even * in_low_even)<<1;
				sum += (cos_even * in_high_even)<<1;
				out_high_even= voRound(sum);

				
				sum = (cos_odd * in_low_odd)<<1;
				sum +=(msin_odd * in_high_odd)<<1;
				out_low_odd= voRound(sum);

				
				sum = (msin_odd * in_low_odd) <<1;
				neg_cos_odd = -cos_odd;
				sum += (neg_cos_odd * in_high_odd)<<1;
				out_high_odd= voRound(sum);
