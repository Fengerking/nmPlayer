...
*
**************************************************************************/
Word16 voAMRWBDecDtx_dec_init(dtx_decState ** st, Word16 isf_init[], VO_MEM_OPERATOR *pMemOP)
{
	dtx_decState *s;
	...
	*st = NULL;
	/* allocate memory */
	if ((s = (dtx_decState *) voAMRWBDec_mem_malloc(pMemOP, sizeof(dtx_decState), 32)) == NULL)
	{
		return -1;
	}
	voAMRWBDecDtx_dec_reset(s, isf_init);
	*st = s;
	return 0;
}
...
*
**************************************************************************/
Word16 voAMRWBDecDtx_dec_reset(dtx_decState * st, Word16 isf_init[])
{
	Word16 i;
	if (st == (dtx_decState *) NULL)
	{
...
*
**************************************************************************/
void voAMRWBDecDtx_dec_exit(dtx_decState ** st, VO_MEM_OPERATOR *pMemOP)
{
	if (st == NULL || *st == NULL)
		return;
	/* deallocate memory */
	voAMRWBDec_mem_free(pMemOP, *st);
	*st = NULL;
	return;
}
...
void voAMRWBDecDtx_dec_activity_update(
							 dtx_decState * st,
							 Word16 isf[],
							 Word16 exc[])
{
	...
	voAMRWBDecLog2(L_frame_en, &log_en_e, &log_en_m);
...
Word16 voAMRWBDecRx_dtx_handler(
					  dtx_decState * st,                    /* i/o : State struct     */
					  ...
					  )
{
	...
	if ((frame_type == RX_SID_FIRST)||(frame_type == RX_SID_UPDATE)||(frame_type == RX_SID_BAD)||
		...
		((frame_type == RX_NO_DATA)||(frame_type == RX_SPEECH_BAD)||(frame_type == RX_SPEECH_LOST))))
	{
		...
		if ((st->dtxGlobalState == DTX_MUTE) &&((frame_type == RX_SID_BAD)||
			(frame_type == RX_SID_FIRST)||(frame_type == RX_SPEECH_LOST)||(frame_type == RX_NO_DATA)))
		{
			newState = DTX_MUTE;
		}
	...
	* after handover this might delay the bwd CNI analysis in the new decoder slightly. */
	...
	st->decAnaElapsedCount = st->decAnaElapsedCount + 1;
	st->dtxHangoverAdded = 0; 

	if ((frame_type == RX_SID_FIRST)||(frame_type == RX_SID_UPDATE)|| (frame_type == RX_SID_BAD)||(frame_type == RX_NO_DATA))
	{
		encState = DTX; 
	} else
	{
