...
Longword voEVRCCodecInv_sqrt(				/* (o) Q30 : output value   (range: 0<=val<1)           */
...
void cor_h_vec123(
						 ...
						 Shortword cory[]	/* (o) result of correlation (NB_POS elements) */
);
static void cor_h_vec40(
						 Shortword h[],		/* (i) scaled impulse response */
						 Shortword vec[],	/* (i) vector to correlate with h[] */
						 Shortword sign[],	/* (i) sign vector */
						 Shortword rrixix[][NB_POS],	/* (i) correlation of h[x] with h[x] */
...
{
#if (FUNC_CODE_8I55_35BITS_OPT)
	Shortword sign[L_SUBFR], vec[L_SUBFR];
	Shortword ip[NB_PULSE], codvec[NB_PULSE + 1];
	...
	Shortword ix, iy, index, track;
	Shortword psk, ps, alpk, alp, pos;
	int snorm, snorm1, snorm2, s_short, ps1_short, ps2_short;
	int i, k, j, k1;
	...
	ps1_short = ((ps1 << snorm1) >> 16);
	ps2_short = ((ps2 << snorm2) >> 16);

	/* Compute s = sqrt(ps1*ps2) */

	snorm1 += snorm2;

	s = ps1_short * ps2_short << 1;
	snorm = norm_l(s);
	s_short = sqroot((s << snorm));
	snorm += snorm1;

	if ((snorm) & 0x0001)		/*if snorm is odd, multiply by sqrt(2)/2 */
	{
		s_short = (short)((long)s_short * 23170 >> 15);
	}
	...
	for(i = 0; i < L_SUBFR; i++)
	{
		s = ((ps2_short * cn[i] << 1) >> snorm2) + ((s_short * dn[i] << 1) >> snorm);
		if (s >= 0)
		{
		...
		s = (s > 0)? s : -s;
		if (s > (Word32)0x007fffffL)
		{
			cn[i] = (Shortword)0x7fff;
		}
		else
		{
			cn[i] = (Shortword)(s >> 8);
	...
	*------------------------------------------------------------*/
	...
	for (k = 0; k < NB_POS; k++)
	{
		...
		cor = 0x00004000L;		/* 0.5 (for rounding) */
		ptr_h1 = h;
		ptr_h2 = ptr_hf;
	...
	*------------------------------------------------------------*/
	...
	for (i = 4; i < L_SUBFR; i += STEP)
	{
		psign=sign;
		if (psign[i] < 0)
			psign = vec;
		for (j = 0; j < L_SUBFR; j += STEP)
		{
			*p0 =(*p0) * psign[j] >> 15;
			p0++;
		}
	...
	*-------------------------------------------------------------------*/
	...
	for (k = 0; k < NB_ITER; k++)
	{
        ...
        ps = 0;
		alp = 0;
		search_ixiy(ipos[k], ipos[k + 1], &ps, &alp, &ix, &iy, dn, cn, rrixix[ipos[k]], rrixix[ipos[k+1]], rrixiy);
		ip[0] = ix;
		ip[1] = iy;
		//i2, i3
        if(sign[ix] < 0)
		...
		for(i = 0; i < L_SUBFR; i++)
			vec[i] = (*p0++) + (*p1++);
		if(ipos[k + 2] == 4)
        cor_h_vec40(h, vec,sign, rrixix, cor_x, cor_y);
		else
		cor_h_vec123(h, vec, ipos[k + 2], sign, rrixix, cor_x, cor_y);

		search_ixiy(ipos[k + 2], ipos[k + 3], &ps, &alp, &ix, &iy, dn, cn, cor_x, cor_y, rrixiy);
		ip[2] = ix;
		ip[3] = iy;
		//i4, i5
        if(sign[ix] < 0)
		...
		cor_h_vec123(h, vec, ipos[k + 4], sign, rrixix, cor_x, cor_y);

		search_ixiy(ipos[k + 4], ipos[k + 5], &ps, &alp, &ix, &iy, dn, cn, cor_x, cor_y, rrixiy);
		ip[4] = ix;
		ip[5] = iy;	
		//i6, i7
        if(sign[ix] < 0)
		...
		cor_h_vec123(h, vec, ipos[k + 6], sign, rrixix, cor_x, cor_y);

		search_ixiy(ipos[k + 6], ipos[k + 7], &ps, &alp, &ix, &iy, dn, cn, cor_x, cor_y, rrixiy);
		ip[6] = ix;
		ip[7] = iy;

		/* memorise new codevector if it's better than the last one. */
		ps = ps * ps >> 15;
		s = (alpk * ps << 1) - (psk * alp << 1);
		if (s > 0)
		{
			psk = ps;
			alpk = alp;
		...
		for (j = 2; j < NB_PULSE; j += 2)
		{
			...
			*--------------------------------------------------*/
			...
			if(ipos[k+j]==4)
				...
				cor_h_vec123(h, vec, ipos[k + j], sign, rrixix, cor_x, cor_y);
#else
			...
			*--------------------------------------------------*/
			...
			search_ixiy(ipos[k + j], ipos[k + j + 1], &ps, &alp, &ix, &iy, dn, cn, cor_x, cor_y, rrixiy);
	...
	*-------------------------------------------------------------------*/
	...
	for (k = 0; k < NB_PULSE; k++)
	{
		...
		index = (i * Q15_1_5) >> 15;	/* index = pos/5       */
		/* track = pos%5 */
		track = i - index * 5;
		...
		*-------------------------------------------------------*/
		...
		else
		{
			if (((index ^ indx[track]) & 16) == 0)
			{
				...
				if(indx[track] <= index)
				{
					indx[track] = ((indx[track] & 16) << 3)
						+ (indx[track] & 15) * NB_POS
						+ (index & 15);
				}
				else
				{
					indx[track] = ((index & 16) << 3)
						+ (index & 15) * NB_POS
						+ (indx[track] & 15);
				}
			}
	...
	*------------------------------------------------------------*/
	...
	cor = L_shr(cor, 12);
	k = extract_h(L_shl(voEVRCCodecInv_sqrt(cor), 8));
	...
	*-------------------------------------------------------------------*/
		...
		*-------------------------------------------------------*/
		...
		else
		{
			if (((index ^ indx[track]) & 16) == 0)
			{
				...
				{
					indx[track] = shl((Shortword)(indx[track] & 16), 3)
						+ shr(extract_l(L_mult((Shortword)(indx[track] & 15), NB_POS)), 1)
						+ (index & 15);
				}
				else
				{
					indx[track] = shl((Shortword)(index & 16), 3)
						+ shr(extract_l(L_mult((Shortword)(index & 15), NB_POS)), 1)
						+ (indx[track] & 15);
				}
			}
			else
			{
				...
				if (sub((Shortword)(indx[track] & 15), (Shortword)(index & 15)) <= 0)
				{
					indx[track] = shl((Shortword)(index & 16), 3)
						+ shr(extract_l(L_mult((Shortword)(index & 15), NB_POS)), 1)
						+ (indx[track] & 15);
	...
	indx[3] = shl(codvec[8], 9)
		+ shl((Shortword)(i & 16), 4)
		+ shl((Shortword)(j & 16), 3)
		+ shr(extract_l(L_mult((Shortword)(i & 15), NB_POS)), 1)
		+ (j & 15);
...
void cor_h_vec123(
						 ...
						 Shortword rrixix[][NB_POS],	/* (i) correlation of h[x] with h[x] */
						 Shortword corx[],	/* (o) result of correlation (NB_POS elements) */
						 Shortword cory[]   /* (o) result of correlation y*/
)
{
	int i, j, pos;
	short *p0, *p1, *p2, *p3;
	int   s, s1;
	short s_temp, s1_temp;
	...
	for (i = 0; i < NB_POS; i++, pos += STEP)
	{
		...
		s1 <<= 1;
		s_temp = (s + 0x8000) >> 16;
		s1_temp = (s1 + 0x8000) >> 16;

		corx[i] = (s_temp * sign[pos] >> 15) + (*p0++);
		cory[i] = (s1_temp * sign[pos + 1] >> 15) + (*p3++);
	}
	return;
}
#endif
...
{
	int i, j, pos;
	Shortword *p0, *p1, *p2, *p3, *p4;
	Longword s, s1;
	Shortword s_temp, s1_temp;
	...
	for (i = 0; i < NB_POS; i++, pos += STEP)
	{
		...
		s  <<= 1;
		s1 <<= 1;
		s_temp = (Shortword)((s + 0x8000) >> 16);
		s1_temp = (Shortword)((s1 + 0x8000) >> 16);
		//s_temp = (s < 0x7fff8000L) ? ((s + 0x00008000) >> 16) : 0x7fff;
		corx[i] = (s_temp * sign[pos] >> 15) + (*p0++);
		cory[i] = (s1_temp * sign[pos - 4] >> 15) + (*p4++);
	}
	return;
}
