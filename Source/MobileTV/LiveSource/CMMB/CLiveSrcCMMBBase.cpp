...
CLiveSrcCMMBBase::CLiveSrcCMMBBase(void)
: m_bCancelScan (false)
, m_bStop (true)
...
//, m_fSendDataEx (NULL)
{
	...
	m_pAudioBuffer = new VO_BYTE[1024*256];
...
VO_U32 CLiveSrcCMMBBase::doInitParser()
{
...
VO_U32 CLiveSrcCMMBBase::doUninitParser()
{
	if (m_pParser)
	{
		m_pParser->Close();
...
int CLiveSrcCMMBBase::ReadLine(VO_PBYTE pData, int nDataLen, VO_PTR hFile)
{
...
VO_U32 CLiveSrcCMMBBase::Open(VO_PTR pUserData, VOSRCSTATUS fStatus, VOSRCSENDDATA fSendData)
{
	CLiveSrcBase::Open(pUserData, fStatus, fSendData);
...
VO_U32 CLiveSrcCMMBBase::doOpenDevice()
{
	if (m_pCAS == NULL)
	{
...
VO_U32 CLiveSrcCMMBBase::doCloseDevice()
{
	if (m_pCAS != NULL)
	{
		m_pCAS->TerminateCAS();
...
VO_U32 CLiveSrcCMMBBase::doStartRecieve()
{
	return VO_ERR_LIVESRC_OK;
}

VO_U32 CLiveSrcCMMBBase::doStopRecieve()
{
	return VO_ERR_LIVESRC_OK;
}


int CLiveSrcCMMBBase::PreScan()
{
	return VO_ERR_LIVESRC_OK;
}

int CLiveSrcCMMBBase::PostScan()
{
	return VO_ERR_LIVESRC_OK;
}

int CLiveSrcCMMBBase::DeviceScan()
{
	return VO_ERR_LIVESRC_FAIL;
}

VO_U32 CLiveSrcCMMBBase::Scan(VO_LIVESRC_SCAN_PARAMEPTERS* pParam)
{
...
VO_U32 CLiveSrcCMMBBase::Cancel(VO_U32 nID)
{
	m_bCancelScan =  true;
...
int	CLiveSrcCMMBBase::ReceiveESGInfo()
{
...
int CLiveSrcCMMBBase::UpdateSchedule()
{
...
VO_U32 CLiveSrcCMMBBase::SetChannel(VO_S32 nNewChannelID)
{
...
VO_U32 CLiveSrcCMMBBase::EBQuery()
{
...
/*
void CLiveSrcCMMBBase::SendData(int packetType, unsigned char * pData, int nDataLen, VO_U64 nTimeStamp, int nCodecType, int nStreamId, bool bKeyFrame)
{
	if (!m_bStop)
	{
		if (m_bPause)
...
VO_BOOL CLiveSrcCMMBBase::doOnParsedData(VO_PARSER_OUTPUT_BUFFER * pData)
{
	if (pData == NULL) 
		return VO_FALSE;
...
// 				return VO_TRUE;
			...
			{
				VO_BYTE *	pNewData = m_pAudioBuffer;
				int		nSampleRate = 6;
	...
	case VO_PARSER_OT_CMMB_PACKET_DATA :
		{
			...
			data_service.pData		= pDataBuffer->data;
			data_service.nDataLen	= pDataBuffer->data_len;
			data_service.nDataType	= pDataBuffer->data_type;
			m_fSendData(m_pUserData, VO_LIVESRC_OUTPUT_DATABLOCK, &data_service);
			return VO_TRUE;
		}
		break;
...
int CLiveSrcCMMBBase::DecryptData(int nCAT, void * pDataPtr)
{
	if (m_pCAS == NULL)
		return -1;
...
int CLiveSrcCMMBBase::SetDeviceChannel(int nNewChannelID)
{
	return VO_ERR_LIVESRC_FAIL;
}
...
VO_U32 CLiveSrcCMMBBase::GetChannel(VO_U32 * pCount, VO_LIVESRC_CHANNELINFO ** ppChannelInfo)
{
...
VO_U32 CLiveSrcCMMBBase::GetESG (VO_S32 nChannelID, VO_LIVESRC_ESG_INFO** pESGInfo)
{
	if (m_nChannelCount <= 0)
		return VO_ERR_LIVESRC_NO_CHANNEL;
...
void CLiveSrcCMMBBase::ReleaseData (void)
{
	m_nContServiceCount = 0;
...
void CLiveSrcCMMBBase::InitData(void)
{
	ReleaseData();

	m_pContService = new VOCONTINUESERVICEINFO[MAX_SERVICE_COUNT];
	m_pServiceInfo = new VOSERVICEINFO[MAX_SERVICE_COUNT];
...
int CLiveSrcCMMBBase::FindChannelIndex(int nChannelID)
{
	int nIndex = 0;
...
int CLiveSrcCMMBBase::FindServiceIndex(int nChannelID)
{
	int nIndex = 0;
	for (nIndex = 0; nIndex < m_nContServiceCount; nIndex ++)
...
int CLiveSrcCMMBBase::AddNewService(VOCONTINUESERVICEINFO * pContService)
{
...
int CLiveSrcCMMBBase::FindServiceInfoIndex(int nChannelID)
{
	int nIndex = 0;
...
int CLiveSrcCMMBBase::AddNewServiceInfo(VOSERVICEINFO * pServiceInfo)
...
int CLiveSrcCMMBBase::UpdateChannel()
{
...
int CLiveSrcCMMBBase::AddNewSchedule(VOSCHEDULEINFO * pSchedule)
{
...
VO_U32 CLiveSrcCMMBBase::GetFrequencyValue(int id)
{
	static VO_U32 freq[]=
	{
		0,   0,   0,   0, //  0 - 3
...
void CLiveSrcCMMBBase::ReadData (void)
{
...
void CLiveSrcCMMBBase::WriteData (void)
{
	VO_FILE_SOURCE	fileSource;
	memset(&fileSource, 0, sizeof(VO_FILE_SOURCE));
	fileSource.nFlag = VO_FILE_TYPE_NAME;
