...
)
{
    ...
    for (i = 0; i < nc; i++)
    {
        ...
        hi = (Word16)(f1[i] >> 16);
        lo = (Word16)((f1[i] & 0xffff) >> 1);
        t0 = Mpy_32_16(hi, lo, isp[m - 1]);
        f1[i] = L_add(f1[i], t0);       //can't be modified
        //f1[i] += t0;
    ...
    for (i = 1, j = m-1; i < nc; i++, j--)
    {
        ...
        t0 = L_add(f1[i], f2[i]);          /* f1[i] + f2[i]             */  //can'n be modified
        tmax |= L_abs(t0);
        a[i] = (Word16)((t0 + ((Word32)1 << 11)) >> 12);
        
        /* a[j] = 0.5*(f1[i] - f2[i]); */

        t0 = f1[i] - f2[i];          /* f1[i] - f2[i]             */
        tmax |= L_abs(t0);
        a[j] = (Word16)((t0 + ((Word32)1 << 11)) >> 12);
    }
      ...
      for (i = 1, j = (m - 1); i < nc; i++, j--)
        {
          ...
          t0 = f1[i] + f2[i];          /* f1[i] + f2[i]             */
          a[i] = (Word16)((t0 + ((Word32)1 << (q_sug - 1))) >> q_sug); /* from Q23 to Q12 and * 0.5 */

          /* a[j] = 0.5*(f1[i] - f2[i]); */

          t0 = f1[i] - f2[i];          /* f1[i] - f2[i]             */
          a[j] = (Word16)((t0 + ((Word32)1 << (q_sug - 1))) >> q_sug); /* from Q23 to Q12 and * 0.5 */
        }
      a[0] >>= q;
    }
    else
...
//    L_Extract(f1[nc], &hi, &lo);
    hi = (Word16)(f1[nc] >> 16);
    lo = (Word16)((f1[nc] & 0xffff) >> 1);
    t0 = Mpy_32_16(hi, lo, isp[m - 1]);
    t0 = L_add(f1[nc], t0);    //can't be modified
    a[nc] = (Word16)((t0 + ((Word32)1 << (q_sug - 1))) >> q_sug);    /* from Q23 to Q12 and * 0.5 */
    /* a[m] = isp[m-1]; */
    a[m] = (isp[m - 1] + (1 << (2 + q))) >> (3 + q);           /* from Q15 to Q12          */
    return;
#else
    Word16 i, j, hi, lo;
    Word32 f1[NC16k + 1], f2[NC16k];
...
static void Get_isp_pol(Word16 * isp, Word32 * f, Word16 n)
{
    ...
    f[0] = 0x800000;                           //L_mult(4096, 1024);               /* f[0] = 1.0;        in Q23  */
    f[1] = (isp[0] *(-256))<<1;             /* f[1] = -2.0*isp[0] in Q23  */
...
static void Get_isp_pol_16kHz(Word16 * isp, Word32 * f, Word16 n)
{
    ...
    f[0] = 0x200000;                /* f[0] = 1.0;        in Q23  */
    f[1] = (isp[0] * (-64))<<1;              /* f[1] = -2.0*isp[0] in Q23  */
