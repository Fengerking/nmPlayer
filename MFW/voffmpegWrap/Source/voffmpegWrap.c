...
VO_U32 voffmpegVideoDecInit(VO_HANDLE * phDec,VO_VIDEO_CODINGTYPE vType, VO_CODEC_INIT_USERDATA * pUserData)
{
	...
	VO_FFMPEG_CONTEXT * pffmpegContext = malloc (sizeof (VO_FFMPEG_CONTEXT));
...
VO_U32 voffmpegVideoDecSetInputData(VO_HANDLE hDec, VO_CODECBUFFER * pInput)
{
	...
	VO_FFMPEG_CONTEXT * pffmpeg = hDec;
	...
	case VO_PID_COMMON_HEADDATA:
		{
			...
			pffmpeg->pHeadData = malloc (1024 * 1024);
...
VO_S32 voGetH264DecAPI (VO_VIDEO_DECAPI * pDecHandle, VO_U32 uFlag)
{
	VOLOGI ("Flag %d", uFlag);
...
VO_S32 voGetMPEG4DecAPI (VO_VIDEO_DECAPI * pDecHandle, VO_U32 uFlag)
{
	VOLOGI ("Flag %d", uFlag);
...
VO_U32 voffmpegVideoDecInitFFMPEG (VO_FFMPEG_CONTEXT * pffmpegContext)
{
	...
	if (pffmpegContext->nCodecType == VO_VIDEO_CodingH264)
		pffmpegContext->hCodec = pffmpeg->sFunc.fFindDec (AV_CODEC_ID_H264);
	else if (pffmpegContext->nCodecType == VO_VIDEO_CodingMPEG4)
		pffmpegContext->hCodec = pffmpeg->sFunc.fFindDec (AV_CODEC_ID_MPEG4);
	else
		pffmpegContext->hCodec = pffmpeg->sFunc.fFindDec (AV_CODEC_ID_H264);
...
VO_U32 voffmpegLoadLib (VO_FFMPEG_CONTEXT * pffmpegContext)
{
	...
	{  
		...
		if (strstr (szPackageName, "/system/bin/mediaserver") == NULL)
			sprintf(szLib, "/data/data/%s/lib/libavcodec.so", szPackageName);
	...
	pCxt->sFunc.hDll = dlopen (szLib, RTLD_NOW);
	if (pCxt->sFunc.hDll == NULL)
	{
		VOLOGE ("Load file %s failed. Error %s", szLib, dlerror ());
		return -1;
	}
	
	pCxt->sFunc.fMalloc = dlsym (pCxt->sFunc.hDll, "av_malloc");
	VOLOGI ("av_malloc %p", pCxt->sFunc.fMalloc);

	pCxt->sFunc.fFree = dlsym (pCxt->sFunc.hDll, "av_free");
	VOLOGI ("av_free %p", pCxt->sFunc.fFree);

	pCxt->sFunc.fRegAll = dlsym (pCxt->sFunc.hDll, "avcodec_register_all");
	VOLOGI ("avcodec_register_all %p", pCxt->sFunc.fRegAll);

	pCxt->sFunc.fFindDec = dlsym (pCxt->sFunc.hDll, "avcodec_find_decoder");
	VOLOGI ("avcodec_find_decoder %p", pCxt->sFunc.fFindDec);
	
	pCxt->sFunc.fAllocContext3 = dlsym (pCxt->sFunc.hDll, "avcodec_alloc_context3");
	VOLOGI ("avcodec_alloc_context3 %p", pCxt->sFunc.fAllocContext3);

	pCxt->sFunc.fAllocFrame = dlsym (pCxt->sFunc.hDll, "avcodec_alloc_frame");
	VOLOGI ("avcodec_alloc_frame %p", pCxt->sFunc.fAllocFrame);
	
	pCxt->sFunc.fInitPacket = dlsym (pCxt->sFunc.hDll, "av_init_packet");
	VOLOGI ("av_init_packet %p", pCxt->sFunc.fInitPacket);
	
	pCxt->sFunc.fOpen2 = dlsym (pCxt->sFunc.hDll, "avcodec_open2");
	VOLOGI ("avcodec_open2 %p", pCxt->sFunc.fOpen2);
	
	pCxt->sFunc.fDecVideo2 = dlsym (pCxt->sFunc.hDll, "avcodec_decode_video2");
	VOLOGI ("avcodec_decode_video2 %p", pCxt->sFunc.fDecVideo2);

	pCxt->sFunc.fClose = dlsym (pCxt->sFunc.hDll, "avcodec_close");
