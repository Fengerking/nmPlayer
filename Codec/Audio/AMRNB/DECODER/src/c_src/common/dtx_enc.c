...
*/
#include "dtx_enc.h"
const char voAMRNBDecdtx_enc_id[] = "@(#)$Id $" dtx_enc_h;
...
int voAMRNBDecdtx_enc_init (dtx_encState **st)
{
	...
	if (st == (dtx_encState **) NULL){
		fprintf(stderr, "voAMRNBDecdtx_enc_init: invalid parameter\n");
		return -1;
	}
	...
	*st = NULL;
	...
	if ((s= (dtx_encState *) malloc(sizeof(dtx_encState))) == NULL){
		fprintf(stderr, "voAMRNBDecdtx_enc_init: can not malloc state structure\n");
		return -1;
	}

	voAMRNBDecdtx_enc_reset(s);
	*st = s;
...
int voAMRNBDecdtx_enc_reset (dtx_encState *st)
{
	...
	if (st == (dtx_encState *) NULL){
		fprintf(stderr, "voAMRNBDecdtx_enc_reset: invalid parameter\n");
		return -1;
	}
	...
	{
		voAMRNBDecCopy(voAMRNBDeclsp_init_data, &st->lsp_hist[i * M], M);
	}

	/* Reset energy history */
	voAMRNBDecSetZero(st->log_en_hist, M);
			...
			)
{
	...
	if ((computeSidFlag != 0))
	{
		/* compute new SID frame if safe i.e don't compute immediately after a talk spurt  */
		log_en = 0;                                           
		for (i = 0; i < M; i++)
		{
			L_lsp[i] = 0;                                    
		...
		for (j = 0; j < M; j++)
		{
			lsp[j] = extract_l((L_lsp[j]>> 3));   /* divide by 8 */
		}
		...
		//Q_plsf_3(qSt, MRDTX, lsp, lsp_q, st->lsp_index, &st->init_lsf_vq_index);
	}
...
)
{
	...
	//test ();
	if ((st->hist_ptr- DTX_HIST_SIZE) == 0)
	{
		st->hist_ptr = 0;                                     //move16 ();
	}

	/* copy lsp vector into buffer */
	voAMRNBDecCopy(lsp_new, &st->lsp_hist[st->hist_ptr * M], M);
	...
	for (i=0; i < L_FRAME; i++)
	{
		...
		L_frame_en += (speech[i]*speech[i])<<1;
	}
