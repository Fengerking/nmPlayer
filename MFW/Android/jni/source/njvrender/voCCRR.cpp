...
#if 0
	if (m_nOutRight - m_nOutLeft > 320 || m_nOutBottom - m_nOutTop > 480)
		return -1;
#endif
...
/*
int voCCRR::ProcessRGB565 (int format, unsigned char * source, unsigned char * target, int angle)
{
	if (format != PIXEL_FORMAT_YCbCr_420_P || m_hCCRR == NULL)
		return -1;
	...
	m_inBuffer.Buffer[0] = source;
	m_inBuffer.Buffer[1] = source + m_nInWidth * m_nInHeight;
	m_inBuffer.Buffer[2] = source + m_nInWidth * m_nInHeight * 5 / 4;

	m_inBuffer.Stride[0] = m_nInWidth;
	m_inBuffer.Stride[1] = m_nInWidth / 2;
	m_inBuffer.Stride[2] = m_nInWidth / 2;

	m_outBuffer.Buffer[0] = target + m_nOutWidth * 2 * m_nOutTop + m_nOutLeft * 2;
	m_outBuffer.Stride[0] = m_nOutWidth * 2;

//	LOGW ("CCRR Buffer %d, %d, %d %d, %d", m_inBuffer.Buffer[0], m_inBuffer.Buffer[1], m_inBuffer.Buffer[2], m_outBuffer.Buffer[0], angle);

//	LOGW ("CCRR Stride %d, %d, %d %d", m_inBuffer.Stride[0], m_inBuffer.Stride[1], m_inBuffer.Stride[2], m_outBuffer.Stride[0]);

	VO_U32 nRC = m_ccrrFunc.Process (m_hCCRR, &m_inBuffer, &m_outBuffer, 0, VO_TRUE);
...
#if 0
	if (nOutWidth > 320 || nOutHeight > 480)
		return -1;
#endif

//	if (m_nRotate != VO_RT_DISABLE)
//	{
//		if (nOutHeight > nOutWidth)
//			m_nRotate = VO_RT_90R;
//	}
...
{
        char s[256];
	strcpy(s, vo_application_lib_location); strcat(s, "libvoMMCCRRS.so");
        //m_hModule = dlopen (VO_APPLICATION_LIB_LOCATION "libvoMMCCRRS.so", RTLD_NOW);
